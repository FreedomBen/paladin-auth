...
- [x] Keyboard Shortcuts window (`GtkShortcutsWindow` wired to the primary
  menu's new "Keyboard Shortcuts" entry and the GNOME-canonical `Ctrl+?`
  accelerator, listing all four window accelerators).
  - [x] Pin the new (label, action, action_name, accelerator) quadruple
  ...
  - [x] Extend `format_app_primary_menu_entries` from six to seven
  ...
  - [x] Add `AppMsg::OpenKeyboardShortcuts`. Map the action name to it
  ...
  - [x] Construct the `GtkShortcutsWindow` from a single
  ...
  - [x] Pinned tests in `tests/startup_probes.rs` mirror the existing
...
- [x] `.deb`, `.rpm`, Flatpak, and AppImage artifacts for `paladin-auth-gtk`,
  ...
  - [x] Add the packaging dry-run job to CI: produces `.deb`,
    ...
    payload.)
- [x] **Next-code column (DESIGN §7).** Lands on top of the
...
- [x] **`DestroyDialog` (Milestone 10; DESIGN §4.3 / §7).** Depends
  ...
  - [x] **Action wiring.** Install the
    ...
    item to the primary menu as the last entry (after Quit) so it
    reads as the destructive group; the primary-menu entry's
    sensitivity follows `AppState::allows_mutating_menu`
    (Unlocked-only) while the shared `app.delete-vault` action's
    enabled flag follows the new `AppState::allows_destroy_action`
    (enabled in `Unlocked` / `Locked` / `StartupError`-with-path,
    disabled in `Missing` / `UnlockedBusy`). Add a flat footer
    link `Delete vault…` to both `UnlockComponent` and
    `StartupErrorView`; each emits a typed component `Output`
    (`UnlockDialogOutput::DeleteVaultLinkClicked` /
    `StartupErrorOutput::DeleteVaultLinkClicked`) that `AppModel`
    routes to `AppMsg::OpenDestroyDialog` — independent of the
    menu action's Unlocked-gated sensitivity so the link still
    works on the `Locked` / `StartupError` surfaces. Tests in
  ...
  - [x] **Pure-logic tests.** Add `tests/destroy_dialog_logic.rs`
  ...
  - [x] **Smoke test.** Extend `tests/gtk_smoke.rs` to cover the
    happy-path destroy flow
    ...
    runner cannot inject GTK pointer / key events, so the
    *interactive* leg (open the menu item, type `yes`, activate the
    destructive response, observe the `Vault deleted.` toast) is
