...
| File                              | Purpose                                                                                                  |
| --------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `org.tamx.Paladin.Gui.yml`        | The flatpak-builder manifest Flathub consumes. Filename matches the app-id basename per Flathub convention. |
| `flathub.json`                    | Per-app build options Flathub honors (initial scope: `only-arches: ["x86_64"]`).                          |
| `README.md`                       | This file.                                                                                               |

`packaging/flatpak/paladin-gtk.yml` is the **local** packaging
...
2. Create a branch named `org.tamx.Paladin.Gui` in your fork.
3. Add `org.tamx.Paladin.Gui.yml` and `flathub.json` from this
   directory to the root of that branch (Flathub's "new-submission"
   layout). Verify the files match in-tree byte-for-byte —
   `crates/paladin-gtk/tests/packaging_flathub_submission_logic.rs`
...
5. Open a pull request against `flathub/flathub` with the title
   `New app: org.tamx.Paladin.Gui` and link to the GitHub Release
...
sed -i "s/tag: v.*/tag: ${TAG}/" packaging/flathub/org.tamx.Paladin.Gui.yml
sed -i "s/# commit: .*/commit: ${COMMIT}/" packaging/flathub/org.tamx.Paladin.Gui.yml
...
`flathub/org.tamx.Paladin.Gui` with the stamped manifest and the
...
Flathub's trust anchor, not against the paladin project's
...
`crates/paladin-gtk/tests/packaging_flathub_submission_logic.rs::flathub_manifest_uses_locked_offline_cargo_build`)
