...
#
# paladin-gtk postremove scriptlet (.deb and .rpm).
#
# Refresh the same two system-owned freedesktop caches the
# postinstall scriptlet rebuilds, so an `apt remove` / `dnf
# remove` of paladin-gtk leaves the launcher in a clean state:
# the stale "Paladin" entry disappears from the app drawer
# immediately rather than persisting until the next unrelated
# package transaction triggers a refresh.
#
# Security posture matches paladin-gtk-postinstall.sh — see the
