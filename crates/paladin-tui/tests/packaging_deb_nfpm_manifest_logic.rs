...
//! `nfpm` `.deb` manifest contract tests for `paladin-tui`.
//!
//! Per `docs/DESIGN.md` §11.3 ("Per-front-end packages") and
//! `docs/IMPLEMENTATION_PLAN_03_TUI.md` §"Packaging (per §11)":
//!
//! * Installs `/usr/bin/paladin-tui` and the gzipped man page at
//!   `/usr/share/man/man1/paladin-tui.1.gz`.
//! * Depends only on `libc6` — the TUI is terminal-only and never
//!   links GTK / libadwaita. Matches the headless-friendly footprint
//!   §11.3 promises and is the Debian analogue of the `glibc`
//!   dependency declared in `packaging/rpm/paladin-tui.yaml`.
...
//!   `$XDG_DATA_HOME/paladin/` and is created on first use, so install
...
//!   from `${PALADIN_VERSION}` at packaging time.
//!
...
const DEB_MANIFEST_RELPATH: &str = "packaging/deb/paladin-tui.yaml";
...
const REQUIRED_INSTALL_SOURCES: &[&str] =
    &["target/release/paladin-tui", "target/man/paladin-tui.1.gz"];
...
fn deb_manifest_exists_at_expected_path() {
    ...
    assert!(
        ...
        "expected nfpm .deb manifest at {} — DESIGN.md §11.3 requires \
         `packaging/deb/paladin-tui.yaml` for the TUI",
        path.display(),
    );
}
...
#[test]
fn deb_manifest_declares_package_name_paladin_tui() {
    let manifest = read_deb_manifest();
    let name =
        top_level_scalar(&manifest, "name").expect("deb nfpm manifest has a top-level `name:` key");
    assert_eq!(
        name, "paladin-tui",
        "deb package name must be `paladin-tui` so the published artifact matches the binary \
...
#[test]
fn deb_manifest_version_is_interpolated_from_paladin_version_env() {
    let manifest = read_deb_manifest();
    let version = top_level_scalar(&manifest, "version")
        .expect("deb nfpm manifest has a top-level `version:` key");
    assert_eq!(
        version, "${PALADIN_VERSION}",
        "deb `version:` must inherit from ${{PALADIN_VERSION}} so the TUI artifact ships under \
...
fn deb_manifest_declares_libc6_runtime_dep_only() {
    ...
    // The TUI is otherwise statically linked per DESIGN §11.3 — any
    // dep beyond the baseline is a regression that masks a runtime
    // surprise on minimal Debian / Ubuntu installs. In particular,
    // the TUI MUST NOT pull in GTK / libadwaita — it's terminal-only.
    assert_eq!(
        depends.len(),
    ...
    for dep in &depends {
        assert!(
            !dep.contains("libgtk") && !dep.contains("libadwaita"),
            "deb `depends:` for paladin-tui must NOT contain GTK / libadwaita — the TUI is \
             terminal-only; got {dep:?}",
        );
    }
}
...
fn deb_manifest_has_no_maintainer_scripts_section() {
    let manifest = read_deb_manifest();
    // No `scripts:` key — the TUI never runs preinst, postinst,
    // prerm, or postrm hooks. Vault files live under
    // `$XDG_DATA_HOME/paladin/` and are created by `paladin init` or
    // the TUI's create-vault screen.
    assert!(
...
fn deb_manifest_binary_install_uses_executable_mode() {
    // /usr/bin/paladin-tui must be world-executable. nfpm defaults
    // file mode to 0644, so the manifest MUST set `mode: 0755`
    // explicitly on the binary entry — a missing `mode:` line on the
    // binary entry would land a non-executable file in /usr/bin and
    // break every `paladin-tui` invocation.
    let manifest = read_deb_manifest();
    let lines: Vec<&str> = manifest.lines().collect();
    let mut binary_block_start: Option<usize> = None;
    for (idx, raw) in lines.iter().enumerate() {
        let trimmed = strip_trailing_comment(raw).trim();
        if trimmed == "dst: /usr/bin/paladin-tui" || trimmed == "dst: \"/usr/bin/paladin-tui\"" {
            binary_block_start = Some(idx);
            break;
        }
    }
    let start = binary_block_start
        .expect("deb nfpm manifest is missing the /usr/bin/paladin-tui dst entry");
    ...
    assert!(
        found_mode_0755,
        "deb nfpm manifest must set `mode: 0755` on the /usr/bin/paladin-tui entry so the \
