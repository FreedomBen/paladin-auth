...
//! `nfpm` `.rpm` manifest contract tests for `paladin-tui`.
//!
...
//!   `$XDG_DATA_HOME/paladin/` and is created on first use, so
//!   install and removal touch nothing global.
//! * Inherits `version` from `${PALADIN_VERSION}` at packaging time.
//!
//! Helpers are intentionally module-local — mirrors the parser
//! pattern used by `crates/paladin-cli/tests/packaging_rpm_nfpm_manifest_logic.rs`
//! and `crates/paladin-gtk/tests/packaging_rpm_nfpm_manifest_logic.rs`.

use std::fs;
use std::path::PathBuf;

const RPM_MANIFEST_RELPATH: &str = "packaging/rpm/paladin-tui.yaml";
...
fn rpm_manifest_exists_at_expected_path() {
    ...
    assert!(
        ...
        "expected nfpm .rpm manifest at {} — DESIGN.md §11.3 requires \
         `packaging/rpm/paladin-tui.yaml` for the TUI",
        path.display(),
    );
}
...
#[test]
fn rpm_manifest_declares_package_name_paladin_tui() {
    let manifest = read_rpm_manifest();
    let name =
        top_level_scalar(&manifest, "name").expect("rpm nfpm manifest has a top-level `name:` key");
    assert_eq!(
        name, "paladin-tui",
        "rpm package name must be `paladin-tui` so the published artifact matches the binary \
...
#[test]
fn rpm_manifest_version_is_interpolated_from_paladin_version_env() {
    let manifest = read_rpm_manifest();
    let version = top_level_scalar(&manifest, "version")
        .expect("rpm nfpm manifest has a top-level `version:` key");
    assert_eq!(
        version, "${PALADIN_VERSION}",
        "rpm `version:` must inherit from ${{PALADIN_VERSION}} so the TUI artifact ships under \
...
fn rpm_manifest_does_not_declare_gtk_or_libadwaita() {
    // Catch a copy-paste from packaging/rpm/paladin-gtk.yaml. The TUI
    ...
    for dep in &depends {
        assert!(
            !dep.contains("gtk"),
            "rpm `depends:` must not declare GTK — paladin-tui is terminal-only; got {dep:?}",
        );
        assert!(
            !dep.contains("libadwaita") && !dep.contains("adwaita"),
            "rpm `depends:` must not declare libadwaita — paladin-tui is terminal-only; got {dep:?}",
        );
    }
}
