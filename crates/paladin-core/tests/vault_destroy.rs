...
use paladin_core::{destroy_vault, DestroyReport, ErrorKind, PaladinError};
...
fn make_vault(dir: &std::path::Path) -> std::path::PathBuf {
    ...
    fs::write(&path, b"paladin vault bytes").expect("write placeholder vault");
    path
}
...
fn destroy_missing_vault_returns_vault_missing() {
    ...
    assert!(matches!(err, PaladinError::VaultMissing));
}
...
fn destroy_primary_unlink_failure_is_plain_io_error() {
    ...
    assert_eq!(err.kind(), ErrorKind::IoError);
    match err {
        PaladinError::IoError { operation, .. } => assert_eq!(operation, "unlink_vault_file"),
...
fn destroy_is_idempotent_across_reruns() {
    ...
    // `vault_missing`, so a `paladin destroy || true` script pattern is
    // safe.
    let dir = test_tempdir();
    let path = make_vault(dir.path());
