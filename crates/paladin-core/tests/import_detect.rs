...
use paladin_core::{detect, ImportFormat};

// ---------- Paladin magic ----------

#[test]
fn paladin_plaintext_magic_returns_paladin() {
    ...
    assert_eq!(detect(&bytes), ImportFormat::Paladin);
}

#[test]
fn paladin_encrypted_magic_returns_paladin() {
    let mut bytes = b"PALADIN\0".to_vec();
    bytes.push(1); // format_ver
...
#[test]
fn paladin_magic_takes_precedence_over_image_magic() {
    // Even if subsequent bytes resemble an image, magic wins (it
    // never can in practice — `PALADIN\0` is not an image magic — but
    // the rule is shape-only and Paladin is checked first).
    let mut bytes = b"PALADIN\0".to_vec();
...
#[test]
fn paladin_magic_only_when_full_eight_bytes_present() {
    // Truncated to 7 bytes — no full magic match.
    let bytes = b"PALADIN".to_vec();
...
#[test]
fn paladin_magic_with_wrong_byte_is_unknown() {
    // Differ in last byte of magic.
    let bytes = b"PALADIN\x01extra".to_vec();
    assert_eq!(detect(&bytes), ImportFormat::Unknown);
}
