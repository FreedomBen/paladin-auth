...
//! optional explicit format, calls `classify_paladin_auth_import_precheck`
//! before any Paladin Auth bundle passphrase prompt, prompts only for
//! encrypted-Paladin Auth sources, applies a user-selected on-conflict
...
//! `paladin_auth_core::ImportReport` merge totals
...
//! [`paladin_auth_core::format_validation_warning`] so this renderer only
...
//!
//! Encrypted-Paladin Auth passphrase sub-phase rendering lands alongside
...
pub fn render(frame: &mut Frame<'_>, modal: &ImportModal, no_color: bool) {
    let modal_height = modal_height_for(modal);
    let modal_area = centered_rect(frame.area(), MODAL_WIDTH, modal_height);
    frame.render_widget(Clear, modal_area);

    let block = theme::titled_block(" Import accounts ", no_color, Padding::symmetric(1, 0));
...
/// carried [`paladin_auth_core::ImportReport`], so each of the four
...
/// through [`paladin_auth_core::format_validation_warning`] by the reducer,
...
/// ASCII output of [`paladin_auth_core::format_validation_warning`]: words
...
/// emitted by [`paladin_auth_core::format_validation_warning`].
...
fn format_selector_line(selector: ImportFormatSelector) -> Line<'static> {
    ...
    let paladin_auth = segment_label(
        "Paladin Auth",
        selector == ImportFormatSelector::PaladinAuth,
    );
    let qr = segment_label("QR", selector == ImportFormatSelector::Qr);
    Line::from(format!(
        "{:<LABEL_COL_WIDTH$}{auto}  {otpauth}  {aegis}  {paladin_auth}  {qr}",
        "Format:"
    ))
}
