...
//! Shared color palette and style helpers for `paladin-auth-tui`.
//!
...
/// frontend's `progress_urgency` bands in `paladin-auth-gtk`):
/// `> 15 s` → [`CODE_CALM`], `6..=15 s` → [`WARN`], `<= 5 s` →
/// [`URGENT`]. A zero `period` falls through to [`CODE_CALM`]
/// defensively — `paladin_auth_core::validation` rejects a zero period
