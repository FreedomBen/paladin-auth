...
//!
//! Under the `paladin-auth-tui/test-hooks` cargo feature the
//! `PALADIN_AUTH_CLIPBOARD_DRYRUN` env-var bypass short-circuits each
...
//! [`paladin-auth-cli`'s `clipboard` adapter](../../../paladin-auth-cli/src/clipboard.rs)
...
/// to [`paladin_auth_core::import::qr_image_bytes`], which re-validates the
...
impl ClipboardSession {
    ...
    /// Read the current clipboard text contents.
    ///
    /// Test-hooks build: honors `PALADIN_AUTH_CLIPBOARD_DRYRUN` —
    ...
    pub fn read_text(&mut self) -> Result<String, ()> {
        use std::ffi::OsStr;
        if let Some(v) = std::env::var_os("PALADIN_AUTH_CLIPBOARD_DRYRUN") {
            if v == OsStr::new("1") {
                return Ok(FAKE
                    .lock()
...
// ---------------------------------------------------------------------------
// Test-build fake-clipboard storage (gated on `paladin-auth-tui/test-hooks`).
//
// Honors `PALADIN_AUTH_CLIPBOARD_DRYRUN` so reducer / effect tests can
// exercise copy → schedule → only-if-unchanged auto-clear without a
// system clipboard server:
//
//   * `PALADIN_AUTH_CLIPBOARD_DRYRUN=1` → bypass arboard, route reads and
//     writes through an in-process fake addressable through
//     [`seed_test_clipboard`] / [`read_test_clipboard`].
//   * `PALADIN_AUTH_CLIPBOARD_DRYRUN=fail` → bypass arboard, return
...
/// serializes any test that touches `PALADIN_AUTH_CLIPBOARD_DRYRUN` or the
...
/// `PALADIN_AUTH_CLIPBOARD_DRYRUN=1`-gated [`ClipboardSession::read_text`]
...
/// a `PALADIN_AUTH_CLIPBOARD_DRYRUN=1`-gated
...
/// `PALADIN_AUTH_CLIPBOARD_DRYRUN=1`-gated
...
/// `ImageReadError::NoImage` branch under `PALADIN_AUTH_CLIPBOARD_DRYRUN=1`.
///
/// Available only under `cfg(feature = "test-hooks")`.
#[cfg(feature = "test-hooks")]
