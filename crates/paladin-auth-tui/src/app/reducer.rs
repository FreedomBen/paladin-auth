...
use paladin_auth_core::{
    classify_paladin_auth_import_precheck, format_plaintext_export_warning,
    format_validation_warning, hotp_reveal_deadline, summary_display_label, validate_account_edit,
    validate_icon_hint_slug, validate_label, AccountEdit, AccountId, AccountKindInput,
    AccountSummary, Algorithm, ClipboardClearPolicy, ClipboardClearToken, Code, IconHintInput,
    IdlePolicy, PaladinAuthError, PaladinAuthImportPrecheck, SettingPatch, Store, Vault,
};
...
/// `idle_deadline` and [`paladin_auth_core::IdlePolicy::is_expired`]
...
/// fall out of [`paladin_auth_core::IdlePolicy::should_arm`] rather than a
...
/// [`paladin_auth_core::IdlePolicy::is_expired`] so the TUI shares
...
/// (`paladin_auth_core::HOTP_REVEAL_SECS` measured on a monotonic
/// clock)."* The boundary is `now >= deadline`, mirroring
/// [`paladin_auth_core::IdlePolicy::is_expired`].
...
/// [`paladin_auth_core::ClipboardClearPolicy::should_clear`] decision live
...
/// executor never calls [`paladin_auth_core::Vault::save`] on the Export
...
/// [`paladin_auth_core::Vault::save`] on the QR export path, the live
...
fn reduce_qr_export_result(
    mut state: AppState,
    result: Result<std::path::PathBuf, PaladinAuthError>,
) -> (AppState, Vec<Effect>) {
...
/// [`paladin_auth_core::ClipboardClearPolicy::schedule`] to obtain a
...
/// [`paladin_auth_core::ClipboardClearPolicy`] identically so the
...
/// [`paladin_auth_core::hotp_reveal_deadline`]. Any prior reveal slot is
/// dropped in place — its `SecretString` zeroizes on drop per the
/// "Sensitive UI buffers" guarantee.
///
/// On `Err(PaladinAuthError::SaveDurabilityUnconfirmed)` with a
...
fn reduce_hotp_advance_result(
    ...
    } = &mut state
    {
        match result {
            ...
            Err(PaladinAuthError::SaveDurabilityUnconfirmed) => {
...
/// [`paladin_auth_core::AccountSummary`] carried in the Ok payload.
///
...
///   partial [`paladin_auth_core::DestroyReport`].
///
...
fn reduce_destroy_vault_result(
    mut state: AppState,
    result: Result<paladin_auth_core::DestroyReport, PaladinAuthError>,
) -> (AppState, Vec<Effect>) {
    ...
    match result {
        ...
        Err(PaladinAuthError::VaultMissing) => destroy_committed(
            path.clone(),
            std::mem::replace(prior.as_mut(), placeholder_state()),
            VAULT_ALREADY_GONE,
        ),
...
/// [`paladin_auth_core::format_validation_warning`] text) lands with the
...
/// discarded so the carried [`paladin_auth_core::ValidatedAccount`] (with
...
fn reduce_add_result(
    ...
    match result {
        Ok(success) => {
            ...
            // the keyboard UI). Any
            // `paladin_auth_core::ValidationWarning`s collected by
...
/// [`AddMode::Qr`] and the carried [`paladin_auth_core::ImportReport`]
/// seeds the post-success counts panel: `imported` / `skipped` totals
/// flow through verbatim, and each [`paladin_auth_core::ImportWarning`] is
/// rendered through [`paladin_auth_core::format_validation_warning`] up
...
/// rendered through `paladin_auth_core::format_validation_warning()` in the
...
/// carried [`paladin_auth_core::ImportReport`] / [`PaladinAuthError`] drops
...
/// from the carried [`paladin_auth_core::ImportReport`] — the four merge
/// totals (`imported` / `skipped` / `replaced` / `appended`) flow
/// through verbatim, and each [`paladin_auth_core::ImportWarning`] is
/// rendered through [`paladin_auth_core::format_validation_warning`] up
/// front so the view layer only needs to display the already-formatted
/// strings. Any prior inline error from a failed retry is cleared so
...
/// back inside [`paladin_auth_core::Vault::mutate_and_save`] — the executor
...
/// [`paladin_auth_core::ImportReport`] / [`PaladinAuthError`] drops without
/// mutating state.
...
/// to [`paladin_auth_core::IdlePolicy::next_deadline`]).
fn reduce_unlock_result(
    state: AppState,
    open: Result<(Vault, Store), PaladinAuthError>,
    opened_at: Instant,
) -> (AppState, Vec<Effect>) {
    match state {
        AppState::Unlock {
            ...
            Err(PaladinAuthError::DecryptFailed) => (
                AppState::Unlock {
                    path,
                    error: Some(render_error_message(&PaladinAuthError::DecryptFailed)),
                    passphrase,
                },
                Vec::new(),
            ),
...
/// empty account list — the same vault `paladin-auth init` would produce.
    ...
    match state {
        AppState::CreateVault { path, mut step, .. } => match result {
            ...
            Err(err) => {
                ...
                // names the parent directory paladin-auth tried to mkdir.
...
/// [`paladin_auth_core::VaultSettings`] into the modal's pending fields at
...
/// runs at submit time via `paladin_auth_core::validate_manual`, so typing
...
/// [`paladin_auth_core::DIGITS_MIN`]..=[`paladin_auth_core::DIGITS_MAX`]
...
/// [`paladin_auth_core::AccountKindInput`] selector between `Totp` and
...
/// [`paladin_auth_core::TOTP_PERIOD_MIN`]..=[`paladin_auth_core::TOTP_PERIOD_MAX`])
...
fn step_period_or_counter(add: &mut AddModal, up: bool) {
    match add.kind {
        AccountKindInput::Totp => {
            if up {
                if add.period_secs < paladin_auth_core::TOTP_PERIOD_MAX {
                    add.period_secs += 1;
                }
            } else if add.period_secs > paladin_auth_core::TOTP_PERIOD_MIN {
                add.period_secs -= 1;
            }
        }
...
/// against [`paladin_auth_core::QR_RGBA_MAX_BYTES`], decode any encoded
/// `otpauth://` URIs via `paladin_auth_core::import::qr_image_bytes`, and
/// import them via `Vault::import_accounts` with
/// [`paladin_auth_core::ImportConflict::Skip`] per
...
/// [`paladin_auth_core::VaultSettings`] and emits a single
...
/// live [`paladin_auth_core::VaultSettings`] and return one
...
/// through [`paladin_auth_core::validate_label`] and either emits
...
fn classify_edit_submit(
    ...
    if projection.label.is_none() && projection.issuer.is_none() && projection.icon_hint.is_none() {
        edit.error = Some(render_error_message(&PaladinAuthError::ValidationError {
            field: "edit",
    ...
    let Some(prior_account) = vault.iter().find(|a| a.id() == edit.account_id) else {
        edit.error = Some(render_error_message(&PaladinAuthError::InvalidState {
...
/// explicit `otpauth` / `aegis` / `paladin-auth` / `qr`), and an
/// on-conflict selector.
///
/// The modal runs in one of two phases keyed off
/// [`ImportModal::paladin_auth_passphrase`]:
///
/// - **Path-entry phase** (`paladin_auth_passphrase.is_none()`, the
...
///   [`paladin_auth_core::classify_paladin_auth_import_precheck`] over the
///   trimmed `source_path` + forced format selector:
///   - [`PaladinAuthImportPrecheck::NoPrompt`] emits
///     [`Effect::Import`] with `paladin_auth_passphrase: None`. This
///     covers missing files, non-Paladin Auth payloads, and forced
///     non-Paladin Auth formats — `import::from_file` owns the per-format
///     failure surfaces from there.
///   - [`PaladinAuthImportPrecheck::Reject`] surfaces the carried
///     [`PaladinAuthError`] inline through `import.error` and emits no
///     effect; the modal stays open in path-entry phase so the user
///     can retry / cancel.
///   - [`PaladinAuthImportPrecheck::PromptForPassphrase`] transitions
///     the modal to the passphrase-entry phase by seeding
///     `import.paladin_auth_passphrase = Some(PassphraseBuffer::new())`.
///     No effect is emitted; the next `Enter` submits with the
///     buffered passphrase.
/// - **Passphrase-entry phase** (`paladin_auth_passphrase.is_some()`).
...
///   emits [`Effect::Import`] with `paladin_auth_passphrase: Some(_)`.
...
fn route_import_modal_input(
    ...
    let has_passphrase_phase = import.paladin_auth_passphrase.is_some();
    match key.code {
        KeyCode::Char(c)
            if !key
                .modifiers
                .intersects(KeyModifiers::CONTROL | KeyModifiers::ALT) =>
        {
            if let Some(buf) = import.paladin_auth_passphrase.as_mut() {
                buf.push(c);
            } else {
        ...
        KeyCode::Enter if has_passphrase_phase => {
            ...
            // `paladin_auth_passphrase` away from `Some`.
            let buf = import
            ...
            vec![Effect::Import {
                ...
                paladin_auth_passphrase: Some(secret),
            }]
        }
        KeyCode::Enter => {
            let source_path = std::path::PathBuf::from(import.path_text.trim());
            let forced_format = import.format.forced();
            match classify_paladin_auth_import_precheck(&source_path, forced_format) {
                ...
                PaladinAuthImportPrecheck::Reject(err) => {
                    import.error = Some(render_error_message(&err));
                    Vec::new()
                }
...
/// [`PaladinAuthError::ValidationError`] with `field = "path"` /
/// `reason = "output_exists"` lands in
/// [`ExportModal::error`](crate::app::state::ExportModal::error) so
/// the wording matches `paladin-auth-cli/src/commands/export.rs`'s
...
/// surfaces a rendered
/// [`PaladinAuthError::InvalidPassphrase`] with
/// `reason = "confirmation_mismatch"` inline on
/// [`ExportModal::error`](crate::app::state::ExportModal::error) so
/// the wording matches the CLI's `prompt_new_passphrase`
/// (`paladin-auth-cli/src/prompt.rs`, docs/DESIGN.md §5) and the GTK
...
/// [`PaladinAuthError::InvalidPassphrase`] with `reason = "zero_length"`
/// inline on
...
/// [`paladin_auth_core::format_plaintext_export_warning`] lands verbatim
/// on [`ExportModal::error`](crate::app::state::ExportModal::error) so
/// the wording matches the CLI's stderr advisory
/// (`paladin-auth-cli/src/commands/export.rs`, docs/DESIGN.md §4.6 / §6) and
...
fn route_export_modal_input(
    ...
    if matches!(key.code, KeyCode::Enter) {
        ...
        if matches!(target.try_exists(), Ok(true)) {
            export.error = Some(render_error_message(&PaladinAuthError::ValidationError {
                field: "path",
        ...
        {
            export.error = Some(render_error_message(&PaladinAuthError::InvalidPassphrase {
...
fn submit_qr_save_sub_flow(
    ...
    if trimmed.is_empty() {
        sub.error = Some(render_error_message(&PaladinAuthError::ValidationError {
            field: "path",
...
fn hotp_advance_effect(
    ...
    let id = selected?;
    let account = vault.iter().find(|a| a.id() == id)?;
    if account.kind() != paladin_auth_core::AccountKindSummary::Hotp {
        return None;
    }
...
fn copy_code_effect(
    ...
    let visible = match account.kind() {
        paladin_auth_core::AccountKindSummary::Totp => true,
        paladin_auth_core::AccountKindSummary::Hotp => {
            hotp_reveal.is_some_and(|r| r.account_id == id)
        }
    };
    if !visible {
        return None;
...
fn copy_next_code_outcome(
    ...
    match account.kind() {
        paladin_auth_core::AccountKindSummary::Totp => {
            CopyNextCodeOutcome::Effect(Effect::CopyNextCode {
                path: path.to_path_buf(),
                account_id: id,
            })
        }
        paladin_auth_core::AccountKindSummary::Hotp => CopyNextCodeOutcome::Reject(
            StatusLine::Error(crate::app::state::NO_NEXT_CODE_FOR_HOTP.to_string()),
        ),
    }
}
...
/// [`paladin_auth_core::select_after_filter`] with the case-insensitive
/// issuer/label substring filter from
/// [`paladin_auth_core::account_matches_search`]). The prev selection
...
/// the live [`paladin_auth_core::VaultSettings`].
