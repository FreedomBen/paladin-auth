...
//! and `paladin-auth-core` / OS resources.
//!
...
use paladin_auth_core::{
    ...
    PaladinAuthError, QrRenderOptions, SettingPatch, Store, ValidatedAccount, Vault, VaultInit,
    VaultLock,
};
...
pub fn execute(
    ...
    match effect {
        ...
        Effect::ClearClipboard { value } => {
            ...
            // `PALADIN_AUTH_CLIPBOARD_DRYRUN=fail` under `test-hooks`) is a
        ...
        Effect::CopyCode { path, account_id } => {
            ...
            let code_str: zeroize::Zeroizing<String> = match account.kind() {
                ...
                paladin_auth_core::AccountKindSummary::Hotp => match hotp_reveal {
        ...
        Effect::CopyNextCode { path, account_id } => {
            ...
            if account.kind() != paladin_auth_core::AccountKindSummary::Totp {
                return EffectOutcome::Continue;
            }
            ...
            } = state
            {
                if *state_path == path {
                    let result = vault.mutate_and_save(store, |v| {
                        let account =
                            v.remove(account_id).ok_or(PaladinAuthError::InvalidState {
                                operation: "remove",
...
///
/// Builds the `paladin_auth_core::VaultInit` from the front-end
...
) -> Result<(Vault, Store), PaladinAuthError> {
...
///
/// 1. Build a [`paladin_auth_core::AccountInput`] from the carried form
///    fields, mapping the empty issuer string to `None` and parsing
///    the icon-hint token via
///    [`paladin_auth_core::parse_icon_hint_token`].
/// 2. Call [`paladin_auth_core::validate_manual`] over the input with one
///    `SystemTime::now()` sample (the submit time used for the
///    account's `created_at` / `updated_at`).
/// 3. Call [`paladin_auth_core::Vault::find_duplicate`] over the validated
///    candidate. A collision returns an existing account's
///    [`paladin_auth_core::AccountSummary`] alongside the validated
///    pending account so the reducer can render the
///    `duplicate_account` rejection and stash pending state.
/// 4. (Subsequent slice) wrap [`paladin_auth_core::Vault::add`] in
///    [`paladin_auth_core::Vault::mutate_and_save`] on the non-duplicate
///    path.
///
...
fn execute_add(
    ...
    algorithm: paladin_auth_core::Algorithm,
    digits: u8,
    kind: paladin_auth_core::AccountKindInput,
    period_secs: u32,
    counter: u64,
    icon_hint_text: &str,
    state: &mut AppState,
    ...
    let input = AccountInput {
        ...
        period_secs: match kind {
            paladin_auth_core::AccountKindInput::Totp => Some(period_secs),
            paladin_auth_core::AccountKindInput::Hotp => None,
        },
        counter: match kind {
            paladin_auth_core::AccountKindInput::Hotp => Some(counter),
            paladin_auth_core::AccountKindInput::Totp => None,
        },
        icon_hint,
    };
...
/// 2. Call [`paladin_auth_core::Vault::find_duplicate`] over the parsed
///    candidate. A collision returns an existing account's
///    [`paladin_auth_core::AccountSummary`] alongside the parsed pending
...
/// 3. (Subsequent slice) wrap [`paladin_auth_core::Vault::add`] in
///    [`paladin_auth_core::Vault::mutate_and_save`] on the non-duplicate
///    path.
///
/// The path check protects against a stale effect emitted before an
...
/// [`Vault::mutate_and_save`](paladin_auth_core::Vault::mutate_and_save) so
/// the rollback semantics from `paladin-auth-core` cover the batch: a
...
/// 2. Pre-flight: re-run
///    [`paladin_auth_core::Vault::find_duplicate_after_edit`] against the
...
///    [`paladin_auth_core::Vault::mutate_and_save`] — the executor sends
...
///    `PALADIN_AUTH_FAULT_INJECT` sink the Add / Rename executor tests
///    already use).
/// 3. Apply via [`paladin_auth_core::Vault::edit_account_metadata`] inside
///    [`paladin_auth_core::Vault::mutate_and_save`] so a pre-commit
...
fn execute_edit_account_metadata(
    ...
    let projected = match mutate_result {
        Ok(()) => match vault.get(account_id) {
            ...
            None => Err(EditFailure::Save(PaladinAuthError::InvalidState {
...
/// executor wraps [`paladin_auth_core::Vault::add`] in
/// [`paladin_auth_core::Vault::mutate_and_save`] so the new account is
/// committed atomically. `Vault::add` assigns a fresh
/// [`paladin_auth_core::AccountId`] so the inserted account is distinct
...
///    [`paladin_auth_core::import::qr_image_bytes`] — the reducer renders
///    each with its own user-facing wording.
/// 2. Hand the RGBA buffer to
///    [`paladin_auth_core::import::qr_image_bytes`], which re-validates
...
///    [`paladin_auth_core::parse_otpauth`]. Errors map to
...
///    [`paladin_auth_core::Vault::import_accounts`] wrapped in
///    [`paladin_auth_core::Vault::mutate_and_save`] with the same
...
///
/// 1. Build a [`paladin_auth_core::ImportOptions`] from the carried
///    `format` (`None` runs auto-detect via
///    [`paladin_auth_core::detect`] inside the facade, [`Some`] forces the
///    matching format and lets the facade sanity-check the input
///    shape) and `paladin_auth_passphrase` (consumed only when the facade
///    dispatches to [`ImportFormat::PaladinAuth`]).
/// 2. Call [`paladin_auth_core::import::from_file`] over `source_path` with
...
///    [`paladin_auth_core::ValidatedAccount`] batch through
///    [`paladin_auth_core::Vault::import_accounts`] wrapped in
///    [`paladin_auth_core::Vault::mutate_and_save`] with the same
...
fn execute_import(
    ...
    paladin_auth_passphrase: Option<secrecy::SecretString>,
    state: &mut AppState,
    sender: &Sender<AppEvent>,
) -> EffectOutcome {
    let AppState::Unlocked {
...
///    [`paladin_auth_core::export::otpauth_list`] (a JSON array of
///    `otpauth://` URIs); [`ExportFormat::Encrypted`] routes through
///    [`paladin_auth_core::export::encrypted`] with the user-supplied
///    twice-confirmed bundle passphrase.
/// 2. Hand the bytes to [`paladin_auth_core::write_secret_file_atomic`],
...
fn execute_export(
    ...
    let bytes_result: Result<Vec<u8>, PaladinAuthError> = match format {
...
///    [`paladin_auth_core::Vault::export_qr_png`] (a `Zeroizing<Vec<u8>>`
///    of PNG bytes); [`QrSaveFormat::Svg`] routes through
///    [`paladin_auth_core::Vault::export_qr_svg`] (a `Zeroizing<String>`
///    of SVG markup).
/// 2. Hand the bytes to [`paladin_auth_core::write_secret_file_atomic`],
///    which writes through a tmpfile + rename and enforces mode
///    `0600` on the final file.
...
/// [`paladin_auth_core::Vault::export_qr_png`] and
/// [`paladin_auth_core::Vault::export_qr_svg`] take `&self`. The executor
...
/// [`paladin_auth_core::Vault::set_passphrase`]. The outcome is sent back
...
/// [`paladin_auth_core::Vault::change_passphrase`]; core handles the
...
///
/// Routes through [`paladin_auth_core::Vault::remove_passphrase`]; the
