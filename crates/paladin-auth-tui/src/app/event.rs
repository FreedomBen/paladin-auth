...
use paladin_auth_core::{
    ...
    DestroyReport, ImportConflict, ImportFormat, ImportReport, PaladinAuthError, SettingPatch,
    Store, ValidatedAccount, ValidationWarning, Vault,
};
...
pub enum AppEvent {
    ...
    /// [`paladin_auth_core::IdlePolicy::next_deadline`] to refresh the
...
pub enum EffectResult {
    ...
    /// or a [`PaladinAuthError`]. `decrypt_failed` surfaces inline on the
    ...
    /// account list — the same vault `paladin-auth init` would produce),
    /// or a [`PaladinAuthError`] surfaced from
    /// [`paladin_auth_core::EncryptionOptions::new`],
    /// [`paladin_auth_core::Store::create`], or
    /// [`paladin_auth_core::Vault::save`].
    ///
    ...
    /// by `account_id`.
    ///
    /// On `Err(PaladinAuthError::SaveDurabilityUnconfirmed)`, if the
    ...
    /// feeds it into [`paladin_auth_core::hotp_reveal_deadline`] to compute
    ...
    HotpAdvance {
        ...
        result: Result<Code, PaladinAuthError>,
    ...
    /// routes through
    /// [`paladin_auth_core::ClipboardClearPolicy::schedule`] to seed
    ...
    /// feeds it into [`paladin_auth_core::ClipboardClearPolicy::schedule`]
    ...
    CopyCode {
        ...
        /// [`paladin_auth_core::ClipboardClearPolicy::schedule`].
        completed_at: Instant,
    },
    ...
    /// [`paladin_auth_core::ClipboardClearPolicy`] identically to a
    ...
    CopyNextCode {
        ...
        /// [`paladin_auth_core::ClipboardClearPolicy::schedule`] just like
    ...
    /// `Err(PaladinAuthError::DuplicateAccount { .. })` from the
    ...
    Remove {
        ...
        result: Result<String, PaladinAuthError>,
    },
    ...
    /// [`paladin_auth_core::validate_manual`] over the carried Manual-mode
    ...
    /// calls [`paladin_auth_core::Vault::find_duplicate`]; a collision
    ...
    /// and any [`ImportWarning`](paladin_auth_core::ImportWarning) messages
    ...
    /// Results delivered while not on `Unlocked`, while a different
    /// modal is open, or after the Add modal closed are discarded so
    /// the carried `ImportReport` / [`PaladinAuthError`] drops without
    /// mutating state.
    QrImport {
        /// The QR-import outcome. `Ok` carries the
        /// [`ImportReport`](paladin_auth_core::ImportReport) for the counts
    ...
    /// the carried [`PaladinAuthError`] drops without mutating state.
    Export {
    ...
    /// discarded — the carried [`PaladinAuthError`] drops without
    /// mutating state.
    QrExport {
        ...
        result: Result<PathBuf, PaladinAuthError>,
    },
    ...
    /// [`paladin_auth_core::Vault::is_encrypted`] alongside other view-only
    /// projections.
    ///
    /// Results delivered while not on
    /// [`crate::app::state::AppState::Unlocked`], while a different
    ...
    ///
    /// On `Err(PaladinAuthError::VaultMissing)` the on-disk vault
    ...
    /// [`paladin_auth_core::DestroyReport`] so the user can decide whether to
    ...
    /// discarded so the carried payload drops without mutating state.
    DestroyVault(Result<DestroyReport, PaladinAuthError>),
}
...
/// [`paladin_auth_core::validate_manual`]. The reducer renders the
...
///   [`paladin_auth_core::Vault::find_duplicate_after_edit`] re-check
...
///   spec.
/// - [`EditFailure::Save`] — [`paladin_auth_core::Vault::mutate_and_save`]
...
/// - [`QrImportFailure::Import`] — wraps a [`PaladinAuthError`] returned
///   by `paladin_auth_core::import::qr_image_bytes` (oversized buffer
...
pub enum QrImportFailure {
    ...
    /// [`PaladinAuthError`] (oversized RGBA buffer, zero decoded QRs,
    /// non-otpauth payload, save-not-committed, etc.).
    Import(PaladinAuthError),
}

/// Successful outcome of an [`Effect::Import`] attempt.
///
/// Carries the [`ImportReport`] returned by
/// [`paladin_auth_core::Vault::import_accounts`] so the reducer can populate
...
pub struct ImportSuccess {
    ...
    /// [`paladin_auth_core::Vault::import_accounts`].
    pub report: ImportReport,
}

/// Failure outcome of an [`Effect::Import`] attempt.
///
/// Wraps the [`PaladinAuthError`] returned by
/// [`paladin_auth_core::import::from_file`] (importer / facade /
/// format-specific errors) or by
/// [`paladin_auth_core::Vault::import_accounts`] /
/// [`paladin_auth_core::Vault::mutate_and_save`] (merge / save / durability
...
pub enum Effect {
    ...
    /// [`paladin_auth_core::VaultInit`] from `init` (encrypted: derive
    /// [`paladin_auth_core::EncryptionOptions::new`] with defaults),
    /// calls [`paladin_auth_core::Store::create`], then
    /// [`paladin_auth_core::Vault::save`], and surfaces the outcome
    ...
    /// [`paladin_auth_core::ClipboardClearPolicy::should_clear`], and
    ...
    /// `paladin_auth_tui::clipboard::write_text`, samples
    ...
    /// [`paladin_auth_core::Vault::totp_next_code`], samples the wall-clock
    /// for `seconds_until_valid = period - (now_unix % period)`,
    /// writes through `paladin_auth_tui::clipboard::write_text`, samples
    /// `Instant::now()` after the write, and posts back
    /// `EffectResult::CopyNextCode` so the reducer can route the
    /// success path through [`paladin_auth_core::ClipboardClearPolicy::schedule`]
    /// and publish the
    ...
    /// draft that passes [`paladin_auth_core::validate_label`] — empty /
    ...
    /// [`paladin_auth_core::Vault::edit_account_metadata`] inside
    /// [`paladin_auth_core::Vault::mutate_and_save`] and posts the outcome
    /// back through an
    ...
    /// [`paladin_auth_core::VaultSettings`] at Confirm time and emits this
    ...
    /// `paladin_auth_core::validate_manual(input, submit_time)`. … Each
    ...
    /// builds a `paladin_auth_core::AccountInput` from the carried fields,
    ...
    Add {
        ...
        /// runs it through
        /// [`paladin_auth_core::parse_icon_hint_token`] before building
        /// the `AccountInput`.
    ...
    /// string is passed to `paladin_auth_core::parse_otpauth(uri,
    ...
    /// the executor calls
    /// [`paladin_auth_core::parse_otpauth`] over the carried URI bytes,
    ...
    AddFromUri {
        ...
        /// [`paladin_auth_core::parse_otpauth`] which trims and enforces
        /// the §4.4 URI grammar.
        uri: SecretString,
    },
    ...
    /// [`paladin_auth_core::AccountId`] so the new account is distinct from
    ...
    /// `paladin_auth_core::import::qr_image_bytes(width, height, rgba_bytes,
    ...
    /// `paladin_auth_core::QR_RGBA_MAX_BYTES` before allocating or copying
    ...
    ///
    /// Per `docs/IMPLEMENTATION_PLAN_03_TUI.md` "Modals (per §6)" > Import:
    /// *"The selected `paladin_auth_core::import::from_file` call returns
    ...
    /// [`paladin_auth_core::classify_paladin_auth_import_precheck`] gate has
    /// returned `NoPrompt` (or after the encrypted-Paladin Auth passphrase
    /// prompt resolved). The executor calls
    /// [`paladin_auth_core::import::from_file`] over `source_path` with the
    /// carried [`paladin_auth_core::ImportOptions`] and one
    /// `SystemTime::now()` sample as `import_time`; on success it
    /// commits the resulting [`paladin_auth_core::ValidatedAccount`] batch
    /// through [`paladin_auth_core::Vault::import_accounts`] wrapped in
    /// [`paladin_auth_core::Vault::mutate_and_save`] so the merge and save
    /// are atomic. The outcome rides on [`EffectResult::Import`].
    ///
    /// `format` mirrors [`paladin_auth_core::ImportOptions::format`]:
    /// [`None`] auto-detects via [`paladin_auth_core::detect`] inside the
    /// facade, [`Some`] forces the matching format and lets the facade
    /// sanity-check the input shape.
    ///
    /// `paladin_auth_passphrase` is consumed only when the facade dispatches
    /// to [`ImportFormat::PaladinAuth`]; it lands in subsequent slices once
    ...
    Import {
        ...
        /// [`paladin_auth_core::ImportOptions::format`]). [`None`] means
        /// auto-detect via [`paladin_auth_core::detect`].
        format: Option<ImportFormat>,
        /// Per-batch merge policy passed to
        /// [`paladin_auth_core::Vault::import_accounts`].
        conflict: ImportConflict,
        /// Bundle passphrase for encrypted-Paladin Auth imports; consumed
        /// only when the facade dispatches to
        /// [`ImportFormat::PaladinAuth`]. The auto-detect first slice
        /// passes [`None`].
        paladin_auth_passphrase: Option<SecretString>,
    },
    /// Write the live vault to `target_path` as a plaintext
    /// `otpauth://` JSON list or an encrypted Paladin Auth bundle.
    ///
    /// Per `docs/IMPLEMENTATION_PLAN_03_TUI.md` "Modals (per §6)" > Export:
    /// *"format selector (plaintext `otpauth://` JSON list or encrypted
    /// Paladin Auth bundle) and a destination path field. ... Writes go
    /// through `paladin_auth_core::write_secret_file_atomic`. ... Export
    ...
    /// [`paladin_auth_core::export::otpauth_list`] (plaintext) or
    /// [`paladin_auth_core::export::encrypted`] (encrypted) and hands them
    /// off to [`paladin_auth_core::write_secret_file_atomic`].
    ///
    ...
    Export {
        ...
        /// routes through [`paladin_auth_core::export::otpauth_list`];
    ...
    /// `target_path` through
    /// [`paladin_auth_core::write_secret_file_atomic`] (mode `0600`).
    ///
    ...
    /// [`paladin_auth_core::Vault::export_qr_svg`] (both `&self`,
    /// neither mutates the vault), writes the bytes through
    /// [`paladin_auth_core::write_secret_file_atomic`], then surfaces
    /// the outcome through
    ...
    QrExport {
        ...
        /// routes through [`paladin_auth_core::Vault::export_qr_png`];
    ...
    /// [`paladin_auth_core::Vault::set_passphrase`]"*. The executor wraps
    /// the carried `new_passphrase` in
    /// [`paladin_auth_core::EncryptionOptions::new`] (default §4.4 Argon2
    /// params) and calls
    /// [`paladin_auth_core::Vault::set_passphrase`]; the core handles its
    ...
    /// [`paladin_auth_core::EncryptionOptions::new`] (default §4.4 Argon2
    /// params) and calls
    /// [`paladin_auth_core::Vault::change_passphrase`]; the core handles
    ...
    ///
    /// The executor calls
    /// [`paladin_auth_core::Vault::remove_passphrase`]; the core handles
    /// its own pre-commit rollback per DESIGN §4.5 and the outcome
    ...
    /// `yes`). This is the **only** `paladin-auth-tui` effect that does not
    /// route through [`paladin_auth_core::Vault::mutate_and_save`] — destroy
    ...
    ///
    /// The executor calls [`paladin_auth_core::destroy_vault`] on its worker
    ...
    DestroyVault {
        ...
        /// [`paladin_auth_core::destroy_vault`]; the sibling `.bak` is
