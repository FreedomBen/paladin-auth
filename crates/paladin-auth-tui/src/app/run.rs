...
//!   `crates/paladin-auth-tui/tests/run_tests.rs` pass fake spawners that
...
//!   `crates/paladin-auth-tui/tests/terminal_tests.rs`; the tests in
...
/// `main` must return, writing a single-line `paladin-auth-tui: <err>`
...
pub fn exit_code_from_run_result<W: Write>(
    ...
    match result {
        ...
        Err(err) => {
            let _ = writeln!(stderr, "paladin-auth-tui: {err}");
            ExitCode::FAILURE
        }
    }
}
...
/// same `paladin-auth-tui: <err>` stderr path as a terminal-setup
...
/// so a draw failure surfaces on the same `paladin-auth-tui: <err>`
