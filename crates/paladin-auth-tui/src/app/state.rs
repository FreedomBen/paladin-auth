...
use paladin_auth_core::{
    ...
    ImportFormat, PaladinAuthError, Store, ValidatedAccount, Vault, VaultLock, VaultStatus,
...
/// [`paladin_auth_core::ClipboardClearPolicy::should_clear`] when the wake
...
pub struct PendingClipboardClear {
    ...
    /// [`paladin_auth_core::ClipboardClearPolicy::schedule`]. A later
...
/// [`paladin_auth_core::HOTP_REVEAL_SECS`] elapses or `n` advances the
/// counter again.
///
...
pub struct HotpReveal {
    ...
    /// [`paladin_auth_core::hotp_reveal_deadline`]; on each Tick the
...
/// [`paladin_auth_core::PaladinAuthError::VaultMissing`] — the on-disk vault
...
/// [`paladin_auth_core::format_destroy_warning`] so the wording matches the
...
pub struct DestroyModal {
    ...
    /// path). Threaded into [`paladin_auth_core::format_destroy_warning`]
    ...
    /// [`paladin_auth_core::format_destroy_warning`]. Rendered verbatim as
    /// the modal body.
    pub warning: String,
...
impl DestroyModal {
    ...
    /// grammar (`paladin-auth destroy` prompts for `yes`).
    #[must_use]
...
///
/// Maps onto the four [`AccountEdit.icon_hint`](paladin_auth_core::AccountEdit)
...
///   routed through [`paladin_auth_core::validate_icon_hint_slug`].
#[derive(Debug, Default, Clone, Copy, PartialEq, Eq)]
...
/// modal opens.
///
/// The reducer projects buffers onto [`paladin_auth_core::AccountEdit`] at
...
/// [`paladin_auth_core::Account`] the modal actually edits.
...
pub struct EditPrior {
    ...
    /// sees the same text they would on a CLI `paladin-auth list` row.
...
/// `None` on the corresponding [`paladin_auth_core::AccountEdit`] field
...
pub struct EditModal {
    ...
    /// [`paladin_auth_core::AccountEdit`] fields per the WYSIWYS rules.
    pub prior: EditPrior,
...
/// [`paladin_auth_core::VaultSettings`] when the modal opens; subsequent
...
pub struct SettingsModal {
    ...
    /// [`paladin_auth_core::VaultSettings::auto_lock_enabled`] at modal
    ...
    /// [`paladin_auth_core::VaultSettings::clipboard_clear_enabled`] at
...
/// already know from `paladin-auth add` with no flags (DESIGN §13).
#[derive(Debug, Default, Clone, Copy, PartialEq, Eq)]
pub enum AddMode {
    ...
    /// [`paladin_auth_core::parse_otpauth`] on submit.
    Uri,
...
pub struct AddModal {
    ...
    /// passed to [`paladin_auth_core::validate_manual`] on submit.
    pub label: String,
    /// Manual-mode issuer buffer. Edited modal-locally; trimmed and
    /// passed to [`paladin_auth_core::validate_manual`] on submit (empty
    ...
    /// [`paladin_auth_core::DIGITS_DEFAULT`] (6); valid values are
    ...
    /// [`paladin_auth_core::TOTP_PERIOD_DEFAULT`] (30); only consulted
    ...
    /// [`paladin_auth_core::parse_icon_hint_token`] on submit; an empty
    /// buffer resolves to [`paladin_auth_core::IconHintInput::Default`]
    ...
    /// [`paladin_auth_core::validate_manual`], wiping the local buffer in
    ...
    /// string is passed to [`paladin_auth_core::parse_otpauth`] … The URI
    ...
    /// are rendered through `paladin_auth_core::format_validation_warning()`
    ...
    /// [`paladin_auth_core::ImportReport`], rendering each
    /// [`paladin_auth_core::ImportWarning`] through
    /// [`paladin_auth_core::format_validation_warning`] up front so the
...
/// by `Vault::import_accounts` plus any [`paladin_auth_core::ImportWarning`]
/// rendered through [`paladin_auth_core::format_validation_warning`] so the
...
/// [`paladin_auth_core::ImportConflict::Skip`] per the plan, so `replaced`
...
pub struct CountsPanel {
    ...
    /// — mirrors [`paladin_auth_core::ImportReport::imported`].
    pub imported: usize,
    /// Number of source rows skipped under
    /// [`paladin_auth_core::ImportConflict::Skip`] — mirrors
    /// [`paladin_auth_core::ImportReport::skipped`].
    pub skipped: usize,
    /// Number of source rows that replaced an existing account under
    /// [`paladin_auth_core::ImportConflict::Replace`] — mirrors
    /// [`paladin_auth_core::ImportReport::replaced`]. Always `0` for the
    /// QR-add path (which forces `Skip`).
    pub replaced: usize,
    /// Number of source rows appended as new accounts under
    /// [`paladin_auth_core::ImportConflict::Append`] — mirrors
    /// [`paladin_auth_core::ImportReport::appended`]. Always `0` for the
    /// QR-add path (which forces `Skip`).
    pub appended: usize,
    /// Non-fatal validation warnings already rendered through
    /// [`paladin_auth_core::format_validation_warning`]. Order mirrors
    /// the original [`paladin_auth_core::ImportReport::warnings`] order
...
/// `paladin-auth` / `qr`)"*. [`Auto`](Self::Auto) defers to
/// [`paladin_auth_core::detect`] on the loaded bytes — the resulting
/// `forced` value is [`None`] so [`paladin_auth_core::ImportOptions::format`]
/// stays unset and the core facade owns format dispatch. The four
/// explicit variants force the matching [`paladin_auth_core::ImportFormat`]
...
pub enum ImportFormatSelector {
    /// Run [`paladin_auth_core::detect`] on the loaded bytes; the facade
    ...
    Aegis,
    /// Force [`ImportFormat::PaladinAuth`].
    PaladinAuth,
...
impl ImportFormatSelector {
    /// Translate to the [`paladin_auth_core::ImportOptions::format`]
...
/// explicit `otpauth` / `aegis` / `paladin-auth` / `qr`), and an on-conflict
/// selector (`skip` / `replace` / `append`)."* The submit path threads
/// these through [`paladin_auth_core::ImportOptions`] +
/// [`paladin_auth_core::import::from_file`] and then through
/// [`paladin_auth_core::Vault::import_accounts`] inside
/// [`paladin_auth_core::Vault::mutate_and_save`]; the per-slice effect
...
/// [`paladin_auth_core::format_validation_warning`]. The encrypted-Paladin Auth
/// passphrase buffer lives in [`Self::paladin_auth_passphrase`] and is
/// populated by the reducer's
/// [`paladin_auth_core::classify_paladin_auth_import_precheck`] gate on Enter.
#[derive(Debug)]
pub struct ImportModal {
    /// Source-path text buffer; trimmed and passed to
    /// [`paladin_auth_core::import::from_file`] on submit.
    pub path_text: String,
    /// Source-format selector. [`ImportFormatSelector::Auto`] dispatches
    /// through [`paladin_auth_core::detect`]; the explicit variants force
    /// the matching [`paladin_auth_core::ImportFormat`].
    pub format: ImportFormatSelector,
    /// On-conflict merge policy threaded into
    /// [`paladin_auth_core::Vault::import_accounts`]. Defaults to
    ...
    pub error: Option<String>,
    /// Encrypted-Paladin Auth passphrase sub-phase buffer.
    ///
    /// `None` is the initial path-entry phase: `Char` / `Backspace`
    /// edit [`Self::path_text`] and `Enter` runs the
    /// [`paladin_auth_core::classify_paladin_auth_import_precheck`] gate.
    /// `Some(buffer)` is the passphrase-entry phase, entered when the
    /// precheck returns [`paladin_auth_core::PaladinAuthImportPrecheck::PromptForPassphrase`]
    ...
    /// auto-lock."*
    pub paladin_auth_passphrase: Option<PassphraseBuffer>,
    ...
    /// [`paladin_auth_core::ImportReport`], rendering each
    /// [`paladin_auth_core::ImportWarning`] through
    /// [`paladin_auth_core::format_validation_warning`] up front so the view
...
/// Paladin Auth bundle)"*. [`Plaintext`](Self::Plaintext) routes the submit
/// path through [`paladin_auth_core::export::otpauth_list`] (a JSON array of
/// `otpauth://` URIs); [`Encrypted`](Self::Encrypted) routes through
/// [`paladin_auth_core::export::encrypted`], which produces a Paladin Auth bundle
...
/// Per `docs/IMPLEMENTATION_PLAN_03_TUI.md` "Modals (per §6)" > Export:
/// *"format selector (plaintext `otpauth://` JSON list or encrypted
/// Paladin Auth bundle) and a destination path field. Overwriting an
...
/// [`paladin_auth_core::export::otpauth_list`] /
/// [`paladin_auth_core::export::encrypted`] and then through
/// [`paladin_auth_core::write_secret_file_atomic`]; the per-slice effect
/// wiring lands alongside the executor.
///
...
pub struct ExportModal {
    ...
    /// [`paladin_auth_core::write_secret_file_atomic`] on submit.
    pub path_text: String,
    /// Output-format selector. [`ExportFormat::Plaintext`] routes
    /// through [`paladin_auth_core::export::otpauth_list`];
    ...
    /// surfaces as
    /// [`PaladinAuthError::InvalidPassphrase`](paladin_auth_core::PaladinAuthError::InvalidPassphrase)
...
/// [`Vault::is_encrypted()`](paladin_auth_core::Vault::is_encrypted): `set`
...
pub enum PassphraseSubFlow {
    ...
    /// through [`paladin_auth_core::Vault::set_passphrase`] with the
    /// user-supplied twice-confirmed new passphrase.
    #[default]
    Set,
    /// Encrypted → encrypted re-key transition. Routes the submit
    /// path through [`paladin_auth_core::Vault::change_passphrase`] with the
    ...
    /// through [`paladin_auth_core::Vault::remove_passphrase`] after the
    /// user acknowledges
    /// [`paladin_auth_core::format_plaintext_storage_warning()`] inline.
    Remove,
}
...
/// [`paladin_auth_core::Vault::is_encrypted`] gate; that gating slice lands
...
pub struct PassphraseModal {
    ...
    /// [`paladin_auth_core::Vault::remove_passphrase`].
...
/// [`paladin_auth_core::Vault::export_qr_png`]; [`Svg`](Self::Svg) routes
/// through [`paladin_auth_core::Vault::export_qr_svg`]. Both write the
/// resulting bytes through [`paladin_auth_core::write_secret_file_atomic`]
/// (mode `0600`).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum QrSaveFormat {
    /// PNG bytes from [`paladin_auth_core::Vault::export_qr_png`].
    Png,
    /// SVG markup from [`paladin_auth_core::Vault::export_qr_svg`].
    Svg,
}
...
pub struct QrSaveSubFlow {
    ...
    /// Destination-path text buffer; trimmed and passed to
    /// [`paladin_auth_core::write_secret_file_atomic`] on Confirm.
    pub path_text: String,
...
/// `paladin_auth_core::Vault::export_qr_ansi(id)` (cached in
...
pub struct QrExportModal {
    ...
    /// [`paladin_auth_core::Vault::export_qr_ansi`]. Populated when the
...
pub enum Modal {
    ...
    Edit(EditModal),
    /// Import an existing vault export (Paladin Auth, Aegis, Google
    /// Authenticator).
    Import(ImportModal),
...
pub enum AppState {
    ...
    /// a `paladin-auth init` would produce.
    CreateVault {
    ...
    Unlocked {
        ...
        /// [`paladin_auth_core::IdlePolicy::should_arm`] holds for the
        ...
        /// [`paladin_auth_core::hotp_reveal_deadline`] has not yet
        ...
        /// initial seed follows [`paladin_auth_core::select_after_filter`]
    ...
    Destroy {
        ...
        /// on submit and into
        /// [`paladin_auth_core::format_destroy_warning`] at open time.
        path: PathBuf,
...
impl AppState {
    ...
    /// [`paladin_auth_core::VaultStatus::Missing`] branch and by tests
...
///
/// Mirrors `paladin_auth_core`'s `destroy_vault` backup probe: the suffix
...
/// Map a `paladin_auth_core::inspect` result onto the corresponding initial
/// [`AppState`].
///
...
///
/// Mirrors [`paladin_auth_core::select_after_filter`] for `prev = None` and
...
pub fn initial_selection(vault: &Vault) -> Option<AccountId> {
    vault.iter().next().map(paladin_auth_core::Account::id)
}

/// Compute the auto-lock idle deadline for the given vault at `now`.
///
/// Thin wrapper around [`paladin_auth_core::IdlePolicy::next_deadline`]
...
/// post-primary [`paladin_auth_core::PaladinAuthError::DestroyIoError`] cases,
/// reflects the partial [`paladin_auth_core::DestroyReport`] so it is
...
#[must_use]
pub fn render_destroy_error(error: &PaladinAuthError, vault_path: &Path) -> String {
    let bak_path = {
    ...
    match error {
        PaladinAuthError::DestroyIoError { operation, .. } => match *operation {
            "unlink_backup_file" => {
                format!(
        ...
        PaladinAuthError::IoError { operation, .. } => match *operation {
...
/// [`paladin_auth_core::format_create_vault_dir_error`] so the wizard's
/// inline error names the directory paladin-auth tried to `mkdir -p`; every
...
/// `PaladinAuthError::IoError` does not carry the attempted path.
#[must_use]
pub fn render_create_vault_error_message(
    error: &PaladinAuthError,
...
/// [`AddModal::error`] when [`paladin_auth_core::Vault::find_duplicate`]
...
/// equivalent in the core's `PaladinAuthError` vocabulary because they
...
/// 2. Call [`paladin_auth_core::inspect`].
...
pub fn build_initial_state(vault: Option<PathBuf>) -> AppState {
    build_initial_state_with_resolver(vault, paladin_auth_core::default_vault_path)
}
...
/// [`paladin_auth_core::default_vault_path`] as the resolver. Tests use this
...
where
    F: FnOnce() -> paladin_auth_core::Result<PathBuf>,
{
