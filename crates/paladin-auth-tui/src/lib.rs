...
//! `paladin-auth-tui` library surface.
//!
...
/// Return the live clap `Command` for the `paladin-auth-tui` binary.
///
/// Consumed by `xtask::man` (via `clap_mangen`) so `paladin-auth-tui.1`
...
/// Run the `paladin-auth-tui` binary.
///
...
///    vault path through `paladin_auth_core::default_vault_path` when the
///    override is `None`).
/// 2. Invoke `make_terminal` to construct the ratatui [`Terminal`].
///    If this fails, the composer short-circuits with
///    [`ExitCode::FAILURE`] and the same `paladin-auth-tui: <err>` stderr
