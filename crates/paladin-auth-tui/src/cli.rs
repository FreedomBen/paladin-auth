...
//! Clap argument tree for the `paladin-auth-tui` binary.
//!
//! See `docs/DESIGN.md` §6 and `docs/IMPLEMENTATION_PLAN_03_TUI.md` "Global flags".
//! `paladin-auth-tui` has no JSON mode: `--json` is rejected at parse time
...
/// Top-level arguments accepted by `paladin-auth-tui`.
#[derive(Debug, Parser)]
#[command(
    name = "paladin-auth-tui",
    version,
    about = "Paladin Auth TUI: Rust OTP authenticator (TOTP + HOTP)"
)]
