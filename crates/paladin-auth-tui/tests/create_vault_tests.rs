...
use paladin_auth_core::{EncryptionOptions, PaladinAuthError, Store, Vault, VaultInit};
use secrecy::SecretString;

use paladin_auth_tui::app::event::{AppEvent, CreateVaultInit, Effect, EffectResult};
use paladin_auth_tui::app::reducer::reduce;
use paladin_auth_tui::app::state::{
    AppState, CreateVaultMode, CreateVaultStep, PassphraseFieldFocus,
};
...
fn create_actual_vault(dir: &tempfile::TempDir, encrypted: bool) -> (Vault, Store) {
    ...
    Store::open(
        &path,
        if encrypted {
            paladin_auth_core::VaultLock::Encrypted(p("hunter2"))
        } else {
            paladin_auth_core::VaultLock::Plaintext
        },
    )
    .unwrap()
}
