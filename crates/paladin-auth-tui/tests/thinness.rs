...
//
// Source-level "thinness contract" guard for `paladin-auth-tui`
...
// here — they belong in `paladin-auth-core`. This test scans the production
// TUI source tree and crate manifest for direct references to denied
// crates and fails with a `git diff`-able, human-readable message
// pointing at the offending file and pattern so the logic can be moved
// into `paladin-auth-core`.
//
// This mirrors the defense-in-depth pattern used by
// `paladin-auth-core/tests/no_network.rs` and `paladin-auth-cli/tests/thinness.rs`.
...
/// `paladin-auth-core`; the TUI consumes its public API instead. Update
...
/// not collide with `PascalCase` paladin-auth-core types like `Algorithm::Sha1`
/// or `Argon2Params` that are re-exported through `paladin_auth_core::*`.
...
#[test]
fn paladin_auth_tui_source_tree_does_not_reference_forbidden_crates() {
    ...
    assert!(
        hits.is_empty(),
        "paladin-auth-tui/src/ contains forbidden crate references — move the \
         underlying logic into paladin-auth-core and import via `paladin_auth_core::*`:\n  {}",
        hits.join("\n  "),
    );
}

#[test]
fn paladin_auth_tui_manifest_does_not_declare_forbidden_dependency() {
    ...
    // are allowed to pull paladin-auth-core test features, and
    ...
    assert!(
        hits.is_empty(),
        "paladin-auth-tui/Cargo.toml declares forbidden direct dependencies — \
         move the underlying logic into paladin-auth-core: {hits:?}",
    );
}
