...
//!
//! `paladin_auth_tui::cli::should_disable_color` already pinned the
...
use paladin_auth_core::{Store, Vault, VaultInit, VaultLock};
use paladin_auth_tui::app::state::{AppState, Focus, StatusLine};
use paladin_auth_tui::view;
...
/// `paladin_auth_core::Store::create` does not reject it via the §4.3
...
///
/// The Paladin Auth block draws a 1-cell border + 1-cell horizontal
...
fn build_render_closure_propagates_no_color_into_draw_frame() {
    ...
    use paladin_auth_tui::app::build_render_closure;
