...
//! Effect-executor tests for `paladin-auth-tui`.
//!
...
//! and `paladin-auth-core` / OS resources. Each [`Effect`] dispatches to the
...
use paladin_auth_core::{
    ...
    PaladinAuthError, SettingPatch, Store, Vault, VaultInit, VaultLock,
};

use paladin_auth_tui::app::effect::{execute, EffectOutcome};
use paladin_auth_tui::app::event::{
    AddFailure, AddSuccess, AppEvent, EditFailure, Effect, EffectResult, ImportFailure,
    ImportSuccess,
};
use paladin_auth_tui::app::state::{AppState, ExportFormat, Focus};
...
/// Process-wide lock around the `PALADIN_AUTH_FAULT_INJECT` env var so the
/// `cargo test` thread pool does not let fault-injecting modules (e.g.
/// `import_save_not_committed`) bleed `PALADIN_AUTH_FAULT_INJECT=pre_commit`
...
mod env_lock {
    ...
    pub const ENV: &str = "PALADIN_AUTH_FAULT_INJECT";
}
...
fn execute_quit_returns_quit_and_sends_no_event() {
    ...
    let outcome = execute(
        ...
        &mut state,
        &tx,
        &mut paladin_auth_tui::clipboard::ClipboardSession::new(),
    );
...
fn execute_unlock_with_wrong_passphrase_sends_decrypt_failed() {
    ...
    match evt {
        AppEvent::EffectResult(EffectResult::Unlock {
            result: Err(PaladinAuthError::DecryptFailed),
            ..
        }) => {}
        other => {
...
fn execute_unlock_against_create_vault_sends_vault_missing() {
    ...
    match evt {
        AppEvent::EffectResult(EffectResult::Unlock {
            result: Err(PaladinAuthError::VaultMissing),
            ..
        }) => {}
...
fn execute_unlock_against_plaintext_vault_sends_wrong_vault_lock() {
    ...
    match evt {
        AppEvent::EffectResult(EffectResult::Unlock {
            result: Err(PaladinAuthError::WrongVaultLock { .. }),
            ..
        }) => {}
        other => {
...
fn execute_rename_with_unknown_account_id_sends_account_not_found_err() {
    ...
    match evt {
        AppEvent::EffectResult(EffectResult::Rename {
            account_id,
            result:
                Err(PaladinAuthError::InvalidState {
                    operation: "rename",
...
/// text-output idiom (see paladin-auth-cli's `display_label`).
#[test]
...
fn execute_apply_settings_with_out_of_range_patch_returns_validation_error() {
    ...
    match evt {
        AppEvent::EffectResult(EffectResult::Settings { result: Err(err) }) => {
            assert!(
                matches!(err, PaladinAuthError::ValidationError { field, .. } if field == "auto_lock.timeout_secs"),
...
///
/// 1. Build the `AccountInput` and call `paladin_auth_core::validate_manual`.
...
/// `(secret, issuer, label)` triple in the vault must:
///
/// 1. Call [`paladin_auth_core::parse_otpauth`] over the carried URI bytes.
/// 2. Call `Vault::find_duplicate(&validated)`.
/// 3. Emit `EffectResult::Add { Err(AddFailure::Duplicate { existing, pending }) }`
///    carrying the existing account's `AccountSummary` and the
...
/// passed to `paladin_auth_core::parse_otpauth(uri, submit_time)`, and on
...
// `paladin_auth_core::ImportOptions { format: None, .. }`, call
// `paladin_auth_core::import::from_file`, and commit the resulting
...
fn execute_import_with_missing_source_file_emits_io_error_failure_and_leaves_vault_untouched() {
    ...
    match evt {
        AppEvent::EffectResult(EffectResult::Import {
            ...
            PaladinAuthError::IoError { operation, .. } => assert_eq!(
...
// "Explicit format overrides (`otpauth` / `aegis` / `paladin-auth` / `qr`)
//  route through `paladin_auth_core::import::from_file`."
// ---------------------------------------------------------------------------
...
fn execute_import_with_forced_format_mismatch_returns_unsupported_import_format_without_mutation() {
    ...
    match evt {
        AppEvent::EffectResult(EffectResult::Import {
            ...
            PaladinAuthError::UnsupportedImportFormat { format } => assert_eq!(
                format, "aegis",
...
mod import_save_not_committed {
    ...
    //! the `PALADIN_AUTH_FAULT_INJECT=pre_commit` hook is compiled into
    //! `paladin-auth-core::storage::fault`. The process-wide env var
    ...
    fn execute_import_with_save_not_committed_failure_rolls_back_live_vault_to_pre_attempt_snapshot(
        ...
        let (vault, store) = Store::open(&path, VaultLock::Plaintext).expect("reopen vault");
        let initial_count = vault.iter().count();
        let initial_ids: Vec<AccountId> =
            vault.iter().map(paladin_auth_core::Account::id).collect();
        let mut state = AppState::Unlocked {
            path: path.clone(),
            vault,
            store,
...
// `paladin_auth_core::export::otpauth_list` and persists via
// `paladin_auth_core::write_secret_file_atomic`.
//
// Per `docs/IMPLEMENTATION_PLAN_03_TUI.md` "Tests" > "Export modal": *"Plaintext
// format selector routes to `paladin_auth_core::export::otpauth_list`"* and
// *"Output is written through `paladin_auth_core::write_secret_file_atomic`
...
fn execute_export_with_plaintext_format_routes_through_otpauth_list_and_writes_via_write_secret_file_atomic(
    ...
    let initial_accounts: Vec<AccountId> =
        vault.iter().map(paladin_auth_core::Account::id).collect();
    ...
    let after_accounts: Vec<AccountId> = vault.iter().map(paladin_auth_core::Account::id).collect();
    assert_eq!(
    ...
    let (reopened, _store) = Store::open(&path, VaultLock::Plaintext).expect("reopen plaintext");
    let reopened_accounts: Vec<AccountId> = reopened
        .iter()
...
// `paladin_auth_core::export::encrypted` and persists via
// `paladin_auth_core::write_secret_file_atomic`.
//
// Per `docs/IMPLEMENTATION_PLAN_03_TUI.md` "Tests" > "Export modal":
// *"Encrypted format selector routes to `paladin_auth_core::export::encrypted`."*
...
// emits (`PALAUTH\0`, format_ver=1, mode=1) and (b) a successful
// round-trip through `import::paladin_auth` with the same passphrase that
// recovers the original account set. That combination is impossible
// to satisfy from `otpauth_list` or any other writer.
// ---------------------------------------------------------------------------

#[test]
// Grew past the 100-line clippy threshold after the rename widened the
// `paladin_auth_core::import::paladin_auth` paths exercised below.
...
fn execute_export_with_encrypted_format_routes_through_export_encrypted_and_writes_via_write_secret_file_atomic(
    ...
    let target_path = tmp.path().join("export.paladin-auth");
    ...
    let written = std::fs::read(&target_path).expect("read written export");
    assert!(
        written.starts_with(b"PALAUTH\0"),
        "encrypted export must begin with the §4.3 PALAUTH magic; got first 16 = {:?}",
    ...
    // Round-trip-routing axis: `import::paladin_auth` with the same passphrase
    ...
    let imported = paladin_auth_core::import::paladin_auth(
        &written,
        SecretString::from(bundle_passphrase.to_string()),
    )
    .expect("encrypted export must round-trip through import::paladin_auth");
...
//
// The executor:
//   1. Reads the live clipboard image via `paladin_auth_tui::clipboard::read_image`.
//      `ImageReadError::NoImage` → `QrImportFailure::NoClipboardImage`;
//      `ImageReadError::DecodeFailure` → `QrImportFailure::ImageDecodeFailure`.
//   2. Calls `paladin_auth_core::import::qr_image_bytes(width, height, &rgba, now)`
...
//
// Tests use the `paladin-auth-tui/test-hooks` clipboard DRYRUN seam to feed
...
mod add_from_clipboard_qr {
    ...
    use paladin_auth_tui::app::event::{QrImportFailure, QrImportSuccess};
    use paladin_auth_tui::clipboard::{
    ...
    /// `(width, height, rgba)`. Mirrors `paladin-auth-core`'s
    ...
    /// Run `body` with `PALADIN_AUTH_CLIPBOARD_DRYRUN=mode` and the
    ...
    fn with_dryrun<R>(mode: &str, body: impl FnOnce() -> R) -> R {
        ...
        std::env::set_var("PALADIN_AUTH_CLIPBOARD_DRYRUN", mode);
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(body));
        std::env::remove_var("PALADIN_AUTH_CLIPBOARD_DRYRUN");
    ...
    fn execute_add_from_clipboard_qr_with_decode_failure_sends_image_decode_failure() {
        ...
        // to init). Modeled in tests through `PALADIN_AUTH_CLIPBOARD_DRYRUN=fail`.
        with_dryrun("fail", || {
            let tmp = secure_tempdir();
            let path = tmp.path().join("vault.bin");
            create_plaintext_vault(&path);
    ...
    fn execute_add_from_clipboard_qr_with_image_containing_no_qrs_sends_no_entries_to_import_failure(
        ...
        // `qr_image_bytes` rejects with `PaladinAuthError::NoEntriesToImport`
...
//   3. Writes via `paladin_auth_tui::clipboard::write_text` and samples
...
//
// Tests use the `paladin-auth-tui/test-hooks` clipboard DRYRUN seam so
...
mod copy_code {
    ...
    use paladin_auth_core::{hotp_reveal_deadline, IconHintInput};
    use paladin_auth_tui::app::state::HotpReveal;
    use paladin_auth_tui::clipboard::{
        read_test_clipboard, seed_test_clipboard, test_clipboard_lock,
    };
    ...
    /// Clipboard write failure (`PALADIN_AUTH_CLIPBOARD_DRYRUN=fail`) maps
...
// this purpose; mirrors `paladin-auth-core`'s `tests/export_qr.rs`).
// ---------------------------------------------------------------------------
...
mod qr_export {
    ...
    use image::Luma;
    use paladin_auth_core::QrRenderOptions;
    use paladin_auth_tui::app::state::QrSaveFormat;
    ...
    /// Mirrors `paladin-auth-core`'s `tests/export_qr.rs::decode_png_to_payload`
    ...
    /// executor surfaces `PaladinAuthError::IoError` with the
    ...
    fn execute_qr_export_png_with_missing_parent_dir_returns_io_error() {
        ...
        match rx.try_recv() {
            Ok(AppEvent::EffectResult(EffectResult::QrExport { result })) => match result {
                Err(PaladinAuthError::IoError { operation, .. }) => assert_eq!(
