...
//! `nfpm` `.deb` manifest contract tests for `paladin-auth-tui`.
//!
//! Per `docs/DESIGN.md` §11.3 ("Per-front-end packages") and
//! `docs/IMPLEMENTATION_PLAN_03_TUI.md` §"Packaging (per §11)":
//!
//! * Installs `/usr/bin/paladin-auth-tui` and the gzipped man page at
...
//!   dependency declared in `packaging/rpm/paladin-auth-tui.yaml`.
...
const DEB_MANIFEST_RELPATH: &str = "packaging/deb/paladin-auth-tui.yaml";

const REQUIRED_DEB_DEPENDS: &[&str] = &["libc6"];

const REQUIRED_INSTALL_DESTINATIONS: &[&str] = &[
    "/usr/bin/paladin-auth-tui",
    "/usr/share/man/man1/paladin-auth-tui.1.gz",
];

const REQUIRED_INSTALL_SOURCES: &[&str] = &[
    "target/release/paladin-auth-tui",
    "target/man/paladin-auth-tui.1.gz",
];
...
fn deb_manifest_exists_at_expected_path() {
    ...
    assert!(
        ...
        "expected nfpm .deb manifest at {} — DESIGN.md §11.3 requires \
         `packaging/deb/paladin-auth-tui.yaml` for the TUI",
        path.display(),
    );
}
...
#[test]
fn deb_manifest_declares_package_name_paladin_auth_tui() {
    let manifest = read_deb_manifest();
    let name =
        top_level_scalar(&manifest, "name").expect("deb nfpm manifest has a top-level `name:` key");
    assert_eq!(
        name, "paladin-auth-tui",
        "deb package name must be `paladin-auth-tui` so the published artifact matches the binary \
...
fn deb_manifest_version_is_interpolated_from_paladin_auth_version_env() {
    ...
    assert_eq!(
        ...
        "deb `version:` must inherit from ${{PALADIN_AUTH_VERSION}} so the TUI artifact ships under \
...
fn deb_manifest_declares_libc6_runtime_dep_only() {
    ...
    for dep in &depends {
        assert!(
            ...
            "deb `depends:` for paladin-auth-tui must NOT contain GTK / libadwaita — the TUI is \
...
fn deb_manifest_has_no_maintainer_scripts_section() {
    ...
    // `$XDG_DATA_HOME/paladin-auth/` and are created by `paladin-auth init` or
...
fn deb_manifest_binary_install_uses_executable_mode() {
    // /usr/bin/paladin-auth-tui must be world-executable. nfpm defaults
    ...
    // break every `paladin-auth-tui` invocation.
    let manifest = read_deb_manifest();
    let lines: Vec<&str> = manifest.lines().collect();
    let mut binary_block_start: Option<usize> = None;
    for (idx, raw) in lines.iter().enumerate() {
        let trimmed = strip_trailing_comment(raw).trim();
        if trimmed == "dst: /usr/bin/paladin-auth-tui"
            || trimmed == "dst: \"/usr/bin/paladin-auth-tui\""
        {
    ...
    let start = binary_block_start
        .expect("deb nfpm manifest is missing the /usr/bin/paladin-auth-tui dst entry");
    ...
    assert!(
        found_mode_0755,
        "deb nfpm manifest must set `mode: 0755` on the /usr/bin/paladin-auth-tui entry so the \
