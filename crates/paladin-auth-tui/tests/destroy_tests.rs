...
//! drives the real `paladin_auth_core::destroy_vault` (no mocks) against
...
use paladin_auth_core::{DestroyReport, PaladinAuthError, Store, VaultInit};
...
fn destroy_vault_result(result: Result<DestroyReport, PaladinAuthError>) -> AppEvent {
...
fn chord_warning_body_matches_core_format_destroy_warning() {
    ...
    assert_eq!(
        modal.warning,
        paladin_auth_core::format_destroy_warning(&path, false)
    );
}
...
fn chord_backup_present_probe_reflects_on_disk_bak() {
    ...
    assert_eq!(
        modal.warning,
        paladin_auth_core::format_destroy_warning(&path, true)
    );
}
...
fn result_vault_missing_transitions_with_already_gone_note() {
    ...
    let (next, effects) = reduce(
        state,
        destroy_vault_result(Err(PaladinAuthError::VaultMissing)),
    );
    assert!(effects.is_empty());
    let AppState::CreateVault { error, .. } = next else {
...
// ---------------------------------------------------------------------------
// Executor — drives the real paladin_auth_core::destroy_vault.
// ---------------------------------------------------------------------------

fn run_destroy(path: &Path) -> (EffectOutcome, Result<DestroyReport, PaladinAuthError>) {
    let (tx, rx) = mpsc::channel::<AppEvent>();
    let mut clipboard = paladin_auth_tui::clipboard::ClipboardSession::new();
...
fn execute_destroy_vault_missing_reports_vault_missing_idempotently() {
    ...
    let (_, result) = run_destroy(&path);
    assert!(
        matches!(result, Err(PaladinAuthError::VaultMissing)),
...
fn execute_destroy_vault_partial_failure_when_bak_is_a_directory() {
    ...
    let (_, result) = run_destroy(&path);
    match result {
        Err(PaladinAuthError::DestroyIoError {
            operation,
            primary_deleted,
            backup_deleted,
            ..
...
fn execute_destroy_vault_symlinked_primary_is_rejected() {
    ...
    assert!(
        matches!(
            result,
            Err(PaladinAuthError::IoError { operation, .. }) if operation == "vault_file_is_symlink"
        ),
