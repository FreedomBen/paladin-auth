...
//! Reducer / state-machine + global-arg tests for `paladin-auth-tui`.
...
use paladin_auth_core::{
    ...
    PaladinAuthError, PermissionSubject, SettingPatch, Store, ValidationWarning, Vault, VaultInit,
    VaultLock, VaultStatus,
};
use paladin_auth_tui::app::event::{
...
use paladin_auth_tui::app::reducer::reduce;
...
use paladin_auth_tui::cli::{should_disable_color, GlobalArgs};
use paladin_auth_tui::prompt::PassphraseBuffer;
...
fn global_args_vault_flag_selects_inspected_path() {
    let args = GlobalArgs::try_parse_from(["paladin-auth-tui", "--vault", "/tmp/v.bin"])
...
fn global_args_default_leaves_vault_unset() {
    let args = GlobalArgs::try_parse_from(["paladin-auth-tui"]).expect("no args should parse");
    assert!(args.vault.is_none());
}

#[test]
fn global_args_no_color_flag_disables_styling() {
    let args = GlobalArgs::try_parse_from(["paladin-auth-tui", "--no-color"])
...
fn global_args_json_flag_is_rejected_at_parse_time() {
    ...
    let err = GlobalArgs::try_parse_from(["paladin-auth-tui", "--json"])
...
fn create_vault_inspect_yields_create_vault_state() {
    let path = PathBuf::from("/tmp/paladin-auth-test-nonexistent.bin");
...
fn encrypted_vault_inspect_yields_unlock_state_with_no_inline_error() {
    let path = PathBuf::from("/tmp/paladin-auth-test-encrypted.bin");
...
fn plaintext_vault_inspect_returns_none_signaling_caller_to_open() {
    let path = PathBuf::from("/tmp/paladin-auth-test-plain.bin");
...
fn create_vault_inspect_does_not_create_or_mutate_files() {
    ...
    // `paladin_auth_core::inspect` path on a non-existent file inside a
    ...
    let path = tmp.path().join("paladin-auth-test-nonexistent.bin");
    assert!(
        !path.exists(),
        "test fixture must start with no vault file at {path:?}"
    );

    let inspect = paladin_auth_core::inspect(&path);
    assert!(
...
fn inspect_error_yields_startup_error_with_rendered_message_and_no_file_mutation() {
    ...
    fs::write(&path, b"not a paladin-auth vault").unwrap();
...
fn render_error_message_uses_format_unsafe_permissions_verbatim() {
    ...
    let err = PaladinAuthError::UnsafePermissions {
        path: PathBuf::from("/tmp/paladin-auth-loose.bin"),
        subject: PermissionSubject::VaultDir,
        actual_mode: "0755".to_string(),
        expected_mode: "0700".to_string(),
    };
    let expected = paladin_auth_core::format_unsafe_permissions(&err)
...
fn render_error_message_falls_back_to_display_for_non_unsafe_permissions_error() {
    ...
    let path = tmp.path().join("garbage.bin");
    fs::write(&path, b"not a paladin-auth vault").unwrap();
    let err = paladin_auth_core::inspect(&path).unwrap_err();
    // Sanity: this is not an unsafe_permissions error.
    assert!(paladin_auth_core::format_unsafe_permissions(&err).is_none());
...
fn render_create_vault_error_message_uses_friendly_helper_for_create_vault_dir() {
    ...
    // directory paladin-auth tried to mkdir. The vault path the user passed
    ...
    let vault_path = PathBuf::from("/home/u/.local/share/paladin-auth/vault.bin");
    let rendered =
        paladin_auth_tui::app::state::render_create_vault_error_message(&err, &vault_path);
    assert_eq!(
        rendered,
...
fn render_create_vault_error_message_falls_back_for_other_errors() {
    ...
    // visually consistent.
    let err = PaladinAuthError::VaultExists;
...
fn build_initial_state_resolver_failure_yields_startup_error_with_no_path_and_no_file_mutation() {
    ...
    // mutate files."
    //
    // `paladin_auth_core::default_vault_path` only fails when
...
fn unlock_result(result: Result<(Vault, Store), PaladinAuthError>) -> AppEvent {
...
fn effect_result_unlock_decrypt_failed_stays_on_unlock_with_inline_error() {
    ...
    let expected = render_error_message(&PaladinAuthError::DecryptFailed);
    ...
    let (state, effects) = reduce(
        ...
        unlock_result(Err(PaladinAuthError::DecryptFailed)),
    );
    assert!(effects.is_empty());
    match state {
        AppState::Unlock {
...
fn effect_result_unlock_non_decrypt_error_transitions_to_startup_error() {
    ...
    fs::write(&garbage, b"not a paladin-auth vault").unwrap();
    let err = paladin_auth_core::inspect(&garbage).unwrap_err();
    assert!(
        !matches!(err, PaladinAuthError::DecryptFailed),
...
//  "idle_deadline is set via paladin_auth_core::policy::auto_lock::
...
// `paladin_auth_core::IdlePolicy::next_deadline`. The plaintext-no-op
...
fn add_modal_default_manual_digits_is_six() {
    ...
    let add = AddModal::default();
    assert_eq!(add.digits, paladin_auth_core::DIGITS_DEFAULT);
...
fn add_modal_default_manual_period_secs_is_thirty() {
    ...
    let add = AddModal::default();
    assert_eq!(add.period_secs, paladin_auth_core::TOTP_PERIOD_DEFAULT);
...
fn char_in_add_modal_manual_mode_secret_focus_appends_to_manual_secret() {
    ...
    // via `paladin_auth_core::validate_manual` — typing accepts any char.
    let tmp = secure_tempdir();
    let state = fresh_unlocked_with_add_modal(&tmp);
    let (state, _) = reduce(state, key(KeyCode::Tab));
    let (state, _) = reduce(state, key(KeyCode::Tab));
...
fn add_modal_focused_on_digits(tmp: &tempfile::TempDir) -> AppState {
    ...
    assert_eq!(
        add_modal_ref(&state).digits,
        paladin_auth_core::DIGITS_DEFAULT
    );
    state
}
...
fn arrows_on_kind_focus_preserve_period_secs_and_counter() {
    ...
    assert_eq!(
        add.period_secs,
        paladin_auth_core::TOTP_PERIOD_DEFAULT,
...
fn up_on_period_focus_with_totp_increments_period_secs_by_one() {
    ...
    let add = add_modal_ref(&after);
    assert_eq!(add.period_secs, paladin_auth_core::TOTP_PERIOD_DEFAULT + 1);
...
fn up_at_period_max_clamps_to_max() {
    ...
    for _ in paladin_auth_core::TOTP_PERIOD_DEFAULT..paladin_auth_core::TOTP_PERIOD_MAX {
        let (next, _) = reduce(state, key(KeyCode::Up));
        state = next;
    }
    assert_eq!(
        add_modal_ref(&state).period_secs,
        paladin_auth_core::TOTP_PERIOD_MAX
    );
    let (after, effects) = reduce(state, key(KeyCode::Up));
    assert!(effects.is_empty());
    assert_eq!(
...
fn down_at_period_min_clamps_to_min() {
    ...
    let tmp = secure_tempdir();
    let mut state = add_modal_focused_on_period_or_counter(&tmp);
    for _ in paladin_auth_core::TOTP_PERIOD_MIN..paladin_auth_core::TOTP_PERIOD_DEFAULT {
        let (next, _) = reduce(state, key(KeyCode::Down));
        state = next;
    }
    assert_eq!(
        add_modal_ref(&state).period_secs,
        paladin_auth_core::TOTP_PERIOD_MIN
    );
    let (after, effects) = reduce(state, key(KeyCode::Down));
    assert!(effects.is_empty());
    assert_eq!(
...
//  entries route through `paladin_auth_core::validate_manual(input,
...
fn enter_in_add_modal_manual_mode_emits_effect_add_with_seeded_defaults() {
    ...
    match &effects[0] {
        Effect::Add {
            ...
            assert_eq!(*digits, paladin_auth_core::DIGITS_DEFAULT);
            assert_eq!(*kind, AccountKindInput::Totp);
            assert_eq!(*period_secs, paladin_auth_core::TOTP_PERIOD_DEFAULT);
            assert_eq!(*counter, 0);
...
fn enter_in_add_modal_qr_mode_emits_add_from_clipboard_qr_effect() {
    ...
    // and call `paladin_auth_core::import::qr_image_bytes` per
...
// semantics are owned by `Vault::mutate_and_save` in `paladin-auth-core`.
// Deliveries that arrive after the modal closed, with a stale
// account_id, or onto a non-Unlocked state are discarded.
// ---------------------------------------------------------------------------

fn remove_result(account_id: AccountId, result: Result<String, PaladinAuthError>) -> AppEvent {
...
fn effect_result_remove_save_not_committed_keeps_modal_open_with_inline_error() {
    ...
    let labels: Vec<&str> = match &state {
        AppState::Unlocked { vault, .. } => vault
            .iter()
            .map(paladin_auth_core::Account::label)
            .collect(),
        other => panic!("expected Unlocked, got {other:?}"),
    };
    assert_eq!(
...
// `Vault::mutate_and_save` in `paladin-auth-core` so the reducer side
...
fn rename_result(account_id: AccountId, result: Result<(), PaladinAuthError>) -> AppEvent {
...
fn effect_result_rename_save_durability_unconfirmed_keeps_modal_open_with_inline_error() {
    ...
    let (state, effects) = reduce(
        unlocked,
        rename_result(id, Err(PaladinAuthError::SaveDurabilityUnconfirmed)),
    );
    assert!(effects.is_empty());
    let rename = expect_rename_modal(&state);
    let surfaced = rename
...
fn up_on_auto_lock_timeout_spinner_increments_by_min_step() {
    ...
    assert_eq!(
        ...
        60 + paladin_auth_core::AUTO_LOCK_SECS_MIN,
...
fn down_on_auto_lock_timeout_spinner_decrements_by_min_step() {
    ...
    assert_eq!(
        modal.auto_lock_timeout_secs,
        120 - paladin_auth_core::AUTO_LOCK_SECS_MIN
    );
}
...
fn up_on_clipboard_clear_spinner_increments_by_min_step() {
    ...
    assert_eq!(
        ...
        30 + paladin_auth_core::CLIPBOARD_CLEAR_SECS_MIN,
...
fn down_on_clipboard_clear_spinner_decrements_by_min_step() {
    ...
    assert_eq!(
        modal.clipboard_clear_secs,
        60 - paladin_auth_core::CLIPBOARD_CLEAR_SECS_MIN
    );
}
...
fn up_at_auto_lock_max_clamps_to_max() {
    ...
    let (modal, _) = reduce_arrow_on_settings_with(
        ...
        SettingsFocus::AutoLockTimeoutSecs,
        KeyCode::Up,
        paladin_auth_core::AUTO_LOCK_SECS_MAX,
        30,
    );
    assert_eq!(
        modal.auto_lock_timeout_secs,
...
fn up_near_auto_lock_max_clamps_to_max_when_step_would_overshoot() {
    ...
    let tmp = secure_tempdir();
    let start = paladin_auth_core::AUTO_LOCK_SECS_MAX - 10;
    let (modal, _) = reduce_arrow_on_settings_with(
        &tmp,
        SettingsFocus::AutoLockTimeoutSecs,
        KeyCode::Up,
...
fn down_at_auto_lock_min_clamps_to_min() {
    ...
    let (modal, _) = reduce_arrow_on_settings_with(
        ...
        SettingsFocus::AutoLockTimeoutSecs,
        KeyCode::Down,
        paladin_auth_core::AUTO_LOCK_SECS_MIN,
        30,
    );
    assert_eq!(
        modal.auto_lock_timeout_secs,
...
fn down_near_auto_lock_min_clamps_to_min_when_step_would_undershoot() {
    let tmp = secure_tempdir();
    let start = paladin_auth_core::AUTO_LOCK_SECS_MIN + 10;
    let (modal, _) = reduce_arrow_on_settings_with(
        &tmp,
        SettingsFocus::AutoLockTimeoutSecs,
        KeyCode::Down,
...
fn up_at_clipboard_max_clamps_to_max() {
    ...
    let (modal, _) = reduce_arrow_on_settings_with(
        ...
        KeyCode::Up,
        60,
        paladin_auth_core::CLIPBOARD_CLEAR_SECS_MAX,
    );
    assert_eq!(
        modal.clipboard_clear_secs,
        paladin_auth_core::CLIPBOARD_CLEAR_SECS_MAX
...
fn down_at_clipboard_min_clamps_to_min() {
    ...
    let (modal, _) = reduce_arrow_on_settings_with(
        ...
        KeyCode::Down,
        60,
        paladin_auth_core::CLIPBOARD_CLEAR_SECS_MIN,
    );
    assert_eq!(
        modal.clipboard_clear_secs,
        paladin_auth_core::CLIPBOARD_CLEAR_SECS_MIN
...
fn settings_result(result: Result<(), PaladinAuthError>) -> AppEvent {
...
fn settings_modal_enter_with_changes_emits_apply_settings_effect_with_diff_patches() {
    ...
    let state = with_settings_modal_mut(state, |s| {
        ...
        s.clipboard_clear_secs = before
            .3
            .saturating_add(paladin_auth_core::CLIPBOARD_CLEAR_SECS_MIN);
    });
    let (state, effects) = reduce(state, key(KeyCode::Enter));
        ...
        Effect::ApplySettings { path: p, patches } => {
            ...
            assert!(matches!(
                ...
                SettingPatch::ClipboardClearSecs(v)
                    if v == before.3.saturating_add(paladin_auth_core::CLIPBOARD_CLEAR_SECS_MIN)
            ));
        }
...
fn settings_modal_enter_with_single_field_change_emits_one_patch() {
    ...
    let before = settings_snapshot(&state);
    let state = with_settings_modal_mut(state, |s| {
        s.auto_lock_timeout_secs = before
            .1
            .saturating_add(paladin_auth_core::AUTO_LOCK_SECS_MIN);
    });
    let (_state, effects) = reduce(state, key(KeyCode::Enter));
    match effects.as_slice() {
        [Effect::ApplySettings { path: p, patches }] => {
            ...
            assert!(matches!(
                ...
                SettingPatch::AutoLockTimeoutSecs(v)
                    if v == before.1.saturating_add(paladin_auth_core::AUTO_LOCK_SECS_MIN)
            ));
        }
...
fn effect_result_settings_save_durability_unconfirmed_keeps_modal_open_with_inline_error() {
    ...
    let (state, effects) = reduce(
        unlocked,
        settings_result(Err(PaladinAuthError::SaveDurabilityUnconfirmed)),
    );
    assert!(effects.is_empty());
    match &state {
        AppState::Unlocked {
...
fn effect_result_settings_validation_error_keeps_modal_open_with_inline_error() {
    ...
    let tmp = secure_tempdir();
    let (_path, _id, state) = fresh_unlocked_with_settings_modal_and_path(&tmp);
    let err = PaladinAuthError::ValidationError {
...
// `paladin_auth_core::select_after_filter` (docs/DESIGN.md §6 / §7): preserve
...
fn emit_unlock_effect_preserves_prior_inline_error_on_unlock_screen() {
    ...
    let prior = render_error_message(&PaladinAuthError::DecryptFailed);
    let mut buf = PassphraseBuffer::new();
...
fn make_duplicate_validated(label: &str) -> paladin_auth_core::ValidatedAccount {
    let input = AccountInput {
        label: label.to_string(),
        issuer: None,
        secret: SecretString::from("JBSWY3DPEHPK3PXP".to_string()),
...
fn make_existing_summary_for_test(label: &str) -> paladin_auth_core::AccountSummary {
    paladin_auth_core::AccountSummary {
        id: AccountId::new(),
        issuer: None,
        label: label.to_string(),
        kind: paladin_auth_core::AccountKindSummary::Totp,
        algorithm: Algorithm::Sha1,
        digits: 6,
        period: Some(30),
        counter: None,
...
// entered string is passed to `paladin_auth_core::parse_otpauth(uri,
...
fn effect_result_add_ok_sets_status_line_confirmation_with_display_label() {
    ...
    let existing_summary = vault
        ...
        .summary();
    let expected_label =
        paladin_auth_tui::app::state::format_account_display_label(&existing_summary);
...
fn effect_result_add_ok_includes_validation_warning_text_in_confirmation() {
    ...
    // `paladin_auth_core::format_validation_warning()` and do not block
    ...
    // wording stays aligned with the CLI's `paladin-auth: warning: <text>`
    // stderr advisory.
    let tmp = secure_tempdir();
    let (path, (mut vault, store)) = open_plaintext_pair(&tmp);
    let existing_id = add_totp_account(&mut vault, &store, "github");
...
//
// Slice covered: the reducer's response to
// `EffectResult::Add { Err(AddFailure::Save(PaladinAuthError::*)) }` for
...
// `PaladinAuthError` (oversized RGBA buffer, zero decoded QRs, non-otpauth
...
fn effect_result_qr_import_oversized_rgba_buffer_sets_inline_error_via_render_error_message() {
    ...
    // PaladinAuthError through the public API by passing dimensions whose
    // `width * height * 4` exceeds `paladin_auth_core::QR_RGBA_MAX_BYTES`,
    ...
    let core_err = paladin_auth_core::import::qr_image_bytes(
        oversized_side,
        oversized_side,
        &[],
        SystemTime::now(),
...
fn effect_result_qr_import_no_qrs_decoded_sets_inline_error_via_render_error_message() {
    // `qr_image_bytes` returns `PaladinAuthError::NoEntriesToImport` when
    // the decoded payload list is empty (no QRs found in the image).
    let tmp = secure_tempdir();
    let state = fresh_unlocked_with_qr_add_modal(&tmp);
    let core_err = PaladinAuthError::NoEntriesToImport;
    let expected = render_error_message(&core_err);
    let (state, _effects) = reduce(
        state,
        qr_import_result(Err(QrImportFailure::Import(core_err))),
...
fn effect_result_qr_import_invalid_qr_payload_sets_inline_error_via_render_error_message() {
    ...
    // arbitrary `PaladinAuthError` values returned by the importer route
    ...
    let tmp = secure_tempdir();
    let state = fresh_unlocked_with_qr_add_modal(&tmp);
    let core_err = PaladinAuthError::UnsupportedImportFormat {
...
fn format_qr_import_failure_import_variant_delegates_to_render_error_message() {
    // Reuses `render_error_message` so any `PaladinAuthError` returned by
...
// is `ZeroizeOnDrop` per `paladin_auth_core::domain::secret`) between the
...
//  "Format auto-detect routes through `paladin_auth_core::import::from_file`.")
//
...
fn enter_in_import_modal_with_default_state_emits_import_effect_with_auto_format_and_skip_conflict()
{
    ...
    match &effects[0] {
        Effect::Import {
            ...
            assert_eq!(
                *conflict,
                paladin_auth_core::ImportConflict::Skip,
                "default ImportConflict is Skip per CLI parity"
            );
            assert!(
                paladin_auth_passphrase.is_none(),
                "auto-detect first slice never carries a Paladin Auth-bundle passphrase"
            );
        }
...
// `paladin_auth_core::import::from_file` with the matching forced
// `ImportFormat`.
//
// (docs/IMPLEMENTATION_PLAN_03_TUI.md > Tests > "Import modal" > "Explicit
//  format overrides (`otpauth` / `aegis` / `paladin-auth` / `qr`) route
//  through `paladin_auth_core::import::from_file`.")
// ---------------------------------------------------------------------------
...
#[test]
fn enter_in_import_modal_with_paladin_auth_selector_emits_import_effect_with_some_paladin_auth() {
    assert_eq!(
        import_format_after_enter_with_selector(ImportFormatSelector::PaladinAuth),
        Some(ImportFormat::PaladinAuth),
        "ImportFormatSelector::PaladinAuth must force ImportFormat::PaladinAuth so the core facade routes through `dispatch_paladin_auth_bytes` (passphrase prompt lives in a later precheck slice)"
    );
}
...
fn import_conflict_after_enter_with_policy(
    policy: paladin_auth_core::ImportConflict,
) -> paladin_auth_core::ImportConflict {
    let tmp = secure_tempdir();
    let unlocked = fresh_plaintext_unlocked(&tmp);
    let (mut state, _) = reduce(unlocked, key(KeyCode::Char('i')));
    if let AppState::Unlocked {
...
fn enter_in_import_modal_with_replace_conflict_emits_import_effect_with_replace() {
    assert_eq!(
        import_conflict_after_enter_with_policy(paladin_auth_core::ImportConflict::Replace),
        paladin_auth_core::ImportConflict::Replace,
...
fn enter_in_import_modal_with_append_conflict_emits_import_effect_with_append() {
    assert_eq!(
        import_conflict_after_enter_with_policy(paladin_auth_core::ImportConflict::Append),
        paladin_auth_core::ImportConflict::Append,
...
#[test]
fn enter_in_import_modal_with_paladin_auth_selector_on_missing_file_emits_import_with_none_passphrase(
) {
    // The forced `ImportFormatSelector::PaladinAuth` path now runs the
    // `classify_paladin_auth_import_precheck` gate on Enter. With the
    // default empty `path_text` the source path resolves to `""` —
    // the precheck cannot read the file and returns
    // `PaladinAuthImportPrecheck::NoPrompt`, so `Effect::Import` is still
    // emitted with `paladin_auth_passphrase: None`. The core facade then
    ...
    {
        import.format = ImportFormatSelector::PaladinAuth;
    } else {
        panic!("expected Import modal open after pressing `i`");
    }
    let (_state, effects) = reduce(state, key(KeyCode::Enter));
    match &effects[0] {
        Effect::Import {
            ...
            assert!(
                paladin_auth_passphrase.is_none(),
                "forced Paladin Auth override over a missing source path must precheck-NoPrompt and submit with `paladin_auth_passphrase: None`"
            );
        }
        other => panic!("expected Effect::Import, got {other:?}"),
    }
...
// ---------------------------------------------------------------------------
// Import modal — `paladin_auth_core::classify_paladin_auth_import_precheck` routing.
// Covers `docs/IMPLEMENTATION_PLAN_03_TUI.md` "Import modal" bullets:
//
//   - "Pre-prompt Paladin Auth decision routes through
//     `paladin_auth_core::classify_paladin_auth_import_precheck`, prompting only
...
//     facade."
//   - "Coverage spans encrypted Paladin Auth, plaintext Paladin Auth,
//     malformed/unsupported Paladin Auth headers, missing files,
//     non-Paladin Auth content, and forced-format mismatches through the
//     shared helper."
// ---------------------------------------------------------------------------

/// Write an encrypted Paladin Auth bundle to `dir/name`, save it, and drop
/// the open `(Vault, Store)` so the file's bytes are flushed to disk
/// and reflect a real encrypted-mode Paladin Auth header.
fn write_encrypted_paladin_auth_bundle(
    dir: &std::path::Path,
    name: &str,
    passphrase: &str,
) -> std::path::PathBuf {
...
/// Write a plaintext Paladin Auth bundle (`PALAUTH\0` magic + plaintext
/// mode byte) to `dir/name`. Used to exercise the precheck's
/// `unsupported_plaintext_vault` rejection.
fn write_plaintext_paladin_auth_bundle(dir: &std::path::Path, name: &str) -> std::path::PathBuf {
    let path = dir.join(name);
    let (vault, store) =
...
#[test]
fn enter_in_import_modal_with_encrypted_paladin_auth_path_transitions_to_passphrase_phase_without_emitting_effect(
) {
    let tmp = secure_tempdir();
    let bundle = write_encrypted_paladin_auth_bundle(tmp.path(), "bundle.bin", "hunter2");
    ...
    match &state {
        AppState::Unlocked {
            ...
            assert!(
                import.paladin_auth_passphrase.is_some(),
                "encrypted-Paladin Auth precheck must seed `paladin_auth_passphrase` with an empty passphrase buffer"
            );
            assert!(
...
#[test]
fn enter_in_import_modal_with_plaintext_paladin_auth_path_surfaces_unsupported_plaintext_vault_inline(
) {
    let tmp = secure_tempdir();
    let bundle = write_plaintext_paladin_auth_bundle(tmp.path(), "plain-import.bin");
    let (state, effects) = drive_import_modal_enter(&tmp, bundle.to_str().unwrap(), None);
    ...
    match &state {
        AppState::Unlocked {
            ...
            assert!(
                import.paladin_auth_passphrase.is_none(),
                "Reject must not transition to passphrase phase"
            );
            let expected = render_error_message(&PaladinAuthError::UnsupportedPlaintextVault);
            assert_eq!(
...
#[test]
fn enter_in_import_modal_with_unsupported_paladin_auth_format_version_surfaces_inline() {
    let tmp = secure_tempdir();
    ...
    match &state {
        AppState::Unlocked {
            ...
            let expected = render_error_message(&PaladinAuthError::UnsupportedFormatVersion {
                format_ver: 99,
            });
            assert_eq!(
                import.error.as_deref(),
...
#[test]
fn enter_in_import_modal_with_invalid_paladin_auth_header_surfaces_inline() {
    let tmp = secure_tempdir();
    let bundle = tmp.path().join("trunc.bin");
    // PALAUTH magic only — header is truncated.
    std::fs::write(&bundle, b"PALAUTH\0").expect("write truncated bundle");
    ...
    match &state {
        AppState::Unlocked {
            ...
            let expected = render_error_message(&PaladinAuthError::InvalidHeader);
            assert_eq!(
                import.error.as_deref(),
                Some(expected.as_str()),
...
#[test]
fn enter_in_import_modal_with_non_paladin_auth_file_proceeds_to_import_effect_with_none_passphrase()
{
    let tmp = secure_tempdir();
    ...
    assert_eq!(
        effects.len(),
        1,
        "NoPrompt over a non-Paladin Auth source must emit one Effect::Import; got {effects:?}"
    );
    match &effects[0] {
        Effect::Import {
            paladin_auth_passphrase,
            ...
            assert!(
                paladin_auth_passphrase.is_none(),
                "NoPrompt path must submit with `paladin_auth_passphrase: None`"
            );
        }
        other => panic!("expected Effect::Import, got {other:?}"),
    }
...
fn enter_in_import_modal_with_missing_source_file_proceeds_to_import_effect_with_none_passphrase() {
    ...
    match &effects[0] {
        Effect::Import {
            ...
            assert!(
                paladin_auth_passphrase.is_none(),
                "missing-file NoPrompt path must submit with `paladin_auth_passphrase: None`"
            );
        }
        other => panic!("expected Effect::Import, got {other:?}"),
    }
...
#[test]
fn enter_in_import_modal_with_forced_otpauth_format_over_encrypted_paladin_auth_emits_import_with_none_passphrase(
) {
    // Forced non-Paladin Auth format ⇒ precheck returns NoPrompt without
    // probing the bytes, even though the file is in fact an
    // encrypted Paladin Auth bundle. `from_file` will surface the
    ...
    match &effects[0] {
        Effect::Import {
            ...
            assert!(paladin_auth_passphrase.is_none());
        }
        other => panic!("expected Effect::Import, got {other:?}"),
    }
    match &state {
        AppState::Unlocked {
            ...
            assert!(
                import.paladin_auth_passphrase.is_none(),
                "forced non-Paladin Auth format must skip the passphrase phase"
            );
        }
        other => panic!("expected Import modal open, got {other:?}"),
    }
...
#[test]
fn enter_in_import_modal_with_forced_aegis_format_over_encrypted_paladin_auth_emits_import_with_none_passphrase(
) {
    let tmp = secure_tempdir();
    let bundle = write_encrypted_paladin_auth_bundle(tmp.path(), "bundle.bin", "hunter2");
...
#[test]
fn enter_in_import_modal_with_forced_qr_format_over_encrypted_paladin_auth_emits_import_with_none_passphrase(
) {
    let tmp = secure_tempdir();
    let bundle = write_encrypted_paladin_auth_bundle(tmp.path(), "bundle.bin", "hunter2");
    let (_state, effects) = drive_import_modal_enter(
...
#[test]
fn enter_in_import_modal_with_forced_paladin_auth_format_over_encrypted_paladin_auth_transitions_to_passphrase_phase(
) {
    let tmp = secure_tempdir();
    let bundle = write_encrypted_paladin_auth_bundle(tmp.path(), "bundle.bin", "hunter2");
    let (state, effects) = drive_import_modal_enter(
        &tmp,
        bundle.to_str().unwrap(),
        Some(ImportFormatSelector::PaladinAuth),
    );

    assert!(
        effects.is_empty(),
        "forced Paladin Auth + encrypted header must precheck-PromptForPassphrase and emit no effect; got {effects:?}"
    );
    match &state {
        AppState::Unlocked {
            modal: Some(Modal::Import(import)),
            ...
            assert!(
                import.paladin_auth_passphrase.is_some(),
                "forced Paladin Auth + encrypted header must seed the passphrase buffer"
            );
        }
        other => panic!("expected Import modal open in passphrase phase, got {other:?}"),
    }
...
#[test]
fn enter_in_import_modal_with_forced_paladin_auth_format_over_plaintext_paladin_auth_surfaces_unsupported_plaintext_vault(
) {
    let tmp = secure_tempdir();
    let bundle = write_plaintext_paladin_auth_bundle(tmp.path(), "plain-import.bin");
    let (state, effects) = drive_import_modal_enter(
    ...
    assert!(
        effects.is_empty(),
        "forced Paladin Auth + plaintext bundle must precheck-Reject and emit no effect; got {effects:?}"
    );
    match &state {
        AppState::Unlocked {
            modal: Some(Modal::Import(import)),
            ...
            assert!(import.paladin_auth_passphrase.is_none());
            let expected = render_error_message(&PaladinAuthError::UnsupportedPlaintextVault);
...
fn enter_in_import_modal_passphrase_phase_emits_import_effect_with_typed_passphrase() {
    ...
    match &effects[0] {
        Effect::Import {
            ...
            assert_eq!(*conflict, paladin_auth_core::ImportConflict::Skip);
            let secret = paladin_auth_passphrase
                .as_ref()
                .expect("passphrase-phase submit must carry Some(SecretString)");
            assert_eq!(
                secret.expose_secret(),
                "hunter2",
                "the typed passphrase must reach `Effect::Import::paladin_auth_passphrase`"
            );
        }
        other => panic!("expected Effect::Import, got {other:?}"),
    }
...
fn import_modal_ref(state: &AppState) -> &paladin_auth_tui::app::state::ImportModal {
    match state {
        AppState::Unlocked {
...
fn effect_result_import_err_unsupported_import_format_renders_inline() {
    let tmp = secure_tempdir();
    let state = fresh_unlocked_with_import_modal(&tmp);
    let err = PaladinAuthError::UnsupportedImportFormat {
        format: "unknown".to_string(),
    };
    let expected = render_error_message(&err);
...
fn effect_result_import_err_unsupported_plaintext_vault_renders_inline() {
    let tmp = secure_tempdir();
    let state = fresh_unlocked_with_import_modal(&tmp);
    let err = PaladinAuthError::UnsupportedPlaintextVault;
    let expected = render_error_message(&err);
...
fn effect_result_import_err_unsupported_encrypted_aegis_renders_inline() {
    let tmp = secure_tempdir();
    let state = fresh_unlocked_with_import_modal(&tmp);
    let err = PaladinAuthError::UnsupportedEncryptedAegis;
    let expected = render_error_message(&err);
    let (state, _effects) = reduce(state, import_result(Err(ImportFailure(err))));
    let import = import_modal_ref(&state);
    assert_eq!(import.error.as_deref(), Some(expected.as_str()));
...
fn effect_result_import_err_unsupported_aegis_entry_type_renders_inline() {
    let tmp = secure_tempdir();
    let state = fresh_unlocked_with_import_modal(&tmp);
    let err = PaladinAuthError::UnsupportedAegisEntryType {
        source_index: 2,
...
fn effect_result_import_err_no_entries_to_import_renders_inline() {
    let tmp = secure_tempdir();
    let state = fresh_unlocked_with_import_modal(&tmp);
    let err = PaladinAuthError::NoEntriesToImport;
    let expected = render_error_message(&err);
    let (state, _effects) = reduce(state, import_result(Err(ImportFailure(err))));
    let import = import_modal_ref(&state);
    assert_eq!(import.error.as_deref(), Some(expected.as_str()));
...
fn effect_result_import_err_invalid_payload_renders_inline() {
    let tmp = secure_tempdir();
    let state = fresh_unlocked_with_import_modal(&tmp);
    let err = PaladinAuthError::InvalidPayload {
...
fn effect_result_import_err_unsupported_format_version_renders_inline() {
    let tmp = secure_tempdir();
    let state = fresh_unlocked_with_import_modal(&tmp);
    let err = PaladinAuthError::UnsupportedFormatVersion { format_ver: 99 };
    let expected = render_error_message(&err);
    let (state, _effects) = reduce(state, import_result(Err(ImportFailure(err))));
    let import = import_modal_ref(&state);
    assert_eq!(import.error.as_deref(), Some(expected.as_str()));
...
fn effect_result_import_err_kdf_params_out_of_bounds_renders_inline() {
    let tmp = secure_tempdir();
    let state = fresh_unlocked_with_import_modal(&tmp);
    let err = PaladinAuthError::KdfParamsOutOfBounds {
        m_kib: 1,
        t: 0,
        p: 0,
    };
...
//
// Slice covered: the reducer's response to
// `EffectResult::Import { Err(ImportFailure(PaladinAuthError::*)) }` for
// both the rolled-back `save_not_committed` path and the committed-
...
fn effect_result_import_save_not_committed_keeps_modal_open_with_inline_error() {
    ...
    let unlocked = AppState::Unlocked {
        ...
        modal: Some(Modal::Import(
            paladin_auth_tui::app::state::ImportModal::default(),
        )),
        selected: None,
        pending_chord_leader: None,
        viewport_height: 0,
...
// ---------------------------------------------------------------------------
// Import modal — `paladin_auth_passphrase` zeroize axes (submit, cancel, modal
// close, auto-lock).
//
// Covers `docs/IMPLEMENTATION_PLAN_03_TUI.md` "Sensitive UI buffers":
//
//   - "Encrypted Paladin Auth import passphrase buffer zeroizes on submit,
//     cancel, modal close, and auto-lock."
//
...
#[test]
fn import_modal_esc_with_typed_paladin_auth_passphrase_closes_modal_and_drops_buffer() {
    // Cancel axis. The user has typed passphrase chars into the
    // encrypted-Paladin Auth-import passphrase field and then dismissed
    ...
    match state {
        AppState::Unlocked {
            ...
            assert!(
                modal.is_none(),
                "Esc must drop the Import modal so its paladin_auth_passphrase buffer zeroizes; got {modal:?}",
            );
            assert!(
                status_line.is_none(),
                "cancel path must not publish a status-line confirmation; got {status_line:?}",
...
#[test]
fn effect_result_import_ok_keeps_modal_open_with_drained_paladin_auth_passphrase_buffer() {
    ...
    // `paladin_auth_passphrase` slot must remain `Some(empty)` so no
    ...
    match &state {
        AppState::Unlocked {
            ...
            let buf = import
                .paladin_auth_passphrase
                .as_ref()
                .expect("Ok arm must not clear paladin_auth_passphrase");
            assert!(
                buf.is_empty(),
                "post-Ok paladin_auth_passphrase must remain drained — no resurrected bytes",
            );
            assert!(
    ...
    match state {
        AppState::Unlocked { modal, .. } => assert!(
            modal.is_none(),
            "Esc after Ok must close the modal so the (already drained) paladin_auth_passphrase buffer drops",
        ),
        other => panic!("expected Unlocked, got {other:?}"),
    }
}
...
#[test]
fn tick_past_idle_deadline_with_open_import_modal_typed_paladin_auth_passphrase_locks_and_drops_buffer(
) {
    // Auto-lock axis. `maybe_auto_lock` transitions `Unlocked →
    // Locked` when `Tick.monotonic` is past `idle_deadline`. The
    // `Locked` state carries only `path` (and any pending clipboard
    ...
    // its `paladin_auth_passphrase` buffer. The buffer's
    ...
    let import = ImportModal {
        paladin_auth_passphrase: Some(buf),
        ..ImportModal::default()
    };
    match &import.paladin_auth_passphrase {
        Some(b) => assert!(
            !b.is_empty(),
            "harness precondition: paladin_auth_passphrase must hold typed bytes",
        ),
        None => panic!("harness precondition: paladin_auth_passphrase must be Some"),
    }
    ...
    match next {
        ...
        other => panic!(
            "expected Locked (Import modal and its paladin_auth_passphrase buffer must be gone), got {other:?}",
        ),
    }
}
...
// (`paladin-auth-cli/src/commands/export.rs`, docs/DESIGN.md §5) and the GTK
// `overwrite_gate_needs_reset` flow keeps the user-facing reason
// stable across all three front-ends — the typed PaladinAuthError flows
...
fn enter_in_export_modal_with_existing_destination_refuses_without_emitting_export_effect() {
    ...
    let expected = render_error_message(&PaladinAuthError::ValidationError {
        field: "path",
...
// `prompt_new_passphrase` (`paladin-auth-cli/src/prompt.rs`, docs/DESIGN.md §5)
...
// PaladinAuthError flows through `render_error_message` so the wording
...
fn enter_in_encrypted_export_modal_with_mismatched_passphrases_refuses_with_confirmation_mismatch()
{
    ...
    let dest = tmp.path().join("bundle.paladin-auth");
    assert!(
        !dest.exists(),
    ...
    let expected = render_error_message(&PaladinAuthError::InvalidPassphrase {
...
// modal stays open, and `paladin_auth_core::format_plaintext_export_warning()`
// lands verbatim on `ExportModal::error` so the wording matches the
// CLI's stderr advisory (`paladin-auth-cli/src/commands/export.rs`,
    ...
    match state {
        AppState::Unlocked {
            ...
            assert_eq!(
                export.error.as_deref(),
                Some(expected.as_str()),
                "plaintext gate must surface paladin_auth_core::format_plaintext_export_warning() verbatim inline",
            );
            assert!(
                status_line.is_none(),
...
fn vault_label_snapshot(state: &AppState) -> Vec<String> {
    match state {
        AppState::Unlocked { vault, .. } => vault
            .iter()
            .map(|a| paladin_auth_core::Account::label(a).to_string())
            .collect(),
        other => panic!("expected Unlocked for label snapshot, got {other:?}"),
    }
}

fn export_result(result: Result<(), PaladinAuthError>) -> AppEvent {
...
fn effect_result_export_err_io_error_surfaces_inline_and_keeps_modal_open() {
    // §5 `io_error` from `paladin_auth_core::write_secret_file_atomic`
...
fn export_modal_esc_with_typed_passphrases_closes_modal_and_drops_passphrase_buffers() {
    ...
    // and `import_modal_esc_with_typed_paladin_auth_passphrase_...`.
    let tmp = secure_tempdir();
    let (vault_path, (vault, store)) = open_plaintext_pair(&tmp);
    let dest = tmp.path().join("bundle.paladin-auth");
...
// `paladin-auth-core` (DESIGN §4.5): the in-memory mode/key reverts on
// `save_not_committed` and is replaced on `save_durability_unconfirmed`.
// The end-to-end rollback semantics live in the `paladin-auth-core` plan; on
...
fn passphrase_result(result: Result<(), PaladinAuthError>) -> AppEvent {
...
fn unlocked_with_encrypted_vault_and_open_passphrase_modal(
    tmp: &tempfile::TempDir,
    sub_flow: paladin_auth_tui::app::state::PassphraseSubFlow,
) -> (PathBuf, AppState) {
    let path = tmp.path().join("vault.bin");
...
fn effect_result_passphrase_save_not_committed_keeps_modal_open_with_inline_error_and_preserves_is_encrypted(
    ...
    let (_path, state_with_modal) = unlocked_with_encrypted_vault_and_open_passphrase_modal(
        &tmp,
        paladin_auth_tui::app::state::PassphraseSubFlow::Change,
    );
...
fn passphrase_modal_esc_with_typed_buffers_closes_modal_and_drops_buffers() {
    ...
    let state_with_modal = AppState::Unlocked {
        ...
        hotp_reveal: None,
        modal: Some(Modal::Passphrase(PassphraseModal {
            sub_flow: paladin_auth_tui::app::state::PassphraseSubFlow::Change,
...
fn enter_in_passphrase_modal_set_subflow_with_matching_passphrases_emits_effect_passphrase_set_and_zeroizes_buffers(
    ...
    let state_with_modal = AppState::Unlocked {
        ...
        hotp_reveal: None,
        modal: Some(Modal::Passphrase(PassphraseModal {
            sub_flow: paladin_auth_tui::app::state::PassphraseSubFlow::Set,
            new_passphrase: passphrase_buffer_with("hunter2"),
            confirm_passphrase: passphrase_buffer_with("hunter2"),
            error: None,
        })),
...
fn enter_in_passphrase_modal_remove_subflow_emits_effect_passphrase_remove() {
    ...
    let state_with_modal = AppState::Unlocked {
        ...
        hotp_reveal: None,
        modal: Some(Modal::Passphrase(PassphraseModal {
            sub_flow: paladin_auth_tui::app::state::PassphraseSubFlow::Remove,
            new_passphrase: PassphraseBuffer::new(),
            confirm_passphrase: PassphraseBuffer::new(),
            error: None,
        })),
...
fn enter_in_passphrase_modal_set_with_mismatched_new_and_confirm_surfaces_confirmation_mismatch_inline_no_effect(
    ...
    let expected_inline = render_error_message(&PaladinAuthError::InvalidPassphrase {
        reason: "confirmation_mismatch",
    });
        ...
        AppState::Unlocked { status_line, .. } => {
            assert_eq!(
                ...
                Some(StatusLine::Error(
                    paladin_auth_tui::app::state::NO_NEXT_CODE_FOR_HOTP.to_string(),
                )),
...
fn pressing_shift_c_with_no_selection_sets_no_account_selected_status_line() {
    ...
    match state {
        AppState::Unlocked { status_line, .. } => assert_eq!(
            status_line,
            Some(StatusLine::Error(
                paladin_auth_tui::app::state::NO_ACCOUNT_SELECTED.to_string(),
            )),
...
fn effect_result_copy_next_code_err_sets_status_line_clipboard_write_failed() {
    ...
    match state {
        AppState::Unlocked { status_line, .. } => assert_eq!(
            status_line,
            Some(StatusLine::Error(
                paladin_auth_tui::app::state::CLIPBOARD_WRITE_FAILED.to_string(),
            )),
...
#[test]
fn qr_export_modal_warning_text_matches_paladin_auth_core_verbatim() {
    ...
    let core_warning = paladin_auth_core::format_plaintext_qr_export_warning();
    assert!(
...
fn qr_export_modal_open_and_close_does_not_advance_hotp_counter() {
    ...
    let initial_counter = vault
        .iter()
        .find(|a| a.id() == hotp_id)
        .map(paladin_auth_core::Account::counter)
        .expect("hotp counter");
    let unlocked = AppState::Unlocked {
        path,
        vault,
...
fn effect_result_qr_export_err_save_not_committed_surfaces_inline_and_keeps_modal_open() {
    // save_not_committed (PALADIN_AUTH_FAULT_INJECT=pre_commit family)
...
fn effect_result_qr_export_err_save_durability_unconfirmed_surfaces_inline_and_keeps_modal_open() {
    // save_durability_unconfirmed (PALADIN_AUTH_FAULT_INJECT=post_commit)
    // surfaces inline; the modal stays open.
    let (state, _id, _path) = qr_unlocked_qr_view();
    let (state, _) = reduce(state, key(KeyCode::Enter));
    let state = type_chars(state, "/tmp/whatever.png");
...
/// helpful message.
fn expect_qr_save_sub_flow(state: &AppState) -> &paladin_auth_tui::app::state::QrSaveSubFlow {
    match state {
        AppState::Unlocked {
            modal: Some(Modal::QrExport(qr)),
            ..
...
fn edit_result(
    ...
    result: Result<paladin_auth_core::AccountSummary, EditFailure>,
) -> AppEvent {
