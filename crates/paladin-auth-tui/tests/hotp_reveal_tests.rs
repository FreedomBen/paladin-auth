...
//! HOTP reveal-window lifecycle tests for `paladin-auth-tui`.
//!
...
//! when the `paladin_auth_core::policy::hotp_reveal::deadline(now)` deadline
...
use paladin_auth_core::{
    ...
    Argon2Params, Code, EncryptionOptions, IconHintInput, PaladinAuthError, Store, Vault,
    VaultInit,
};
use paladin_auth_tui::app::event::{AppEvent, Effect, EffectResult};
use paladin_auth_tui::app::reducer::reduce;
use paladin_auth_tui::app::state::{render_error_message, AppState, Focus, HotpReveal, StatusLine};
use paladin_auth_tui::view::render as view_render;
...
/// Drive `state` through [`paladin_auth_tui::view::render`] using a
...
fn auto_lock_takes_precedence_over_reveal_expiry_when_both_fire() {
    ...
    // variant change.
    use paladin_auth_core::Argon2Params;
    let tmp = secure_tempdir();
    let path = tmp.path().join("vault.bin");
    let (mut vault, store) = create_encrypted_pair(&path, "pp");
    let _ = Argon2Params {
...
fn effect_result_hotp_advance_err_save_not_committed_leaves_reveal_unchanged() {
    ...
    let event = AppEvent::EffectResult(EffectResult::HotpAdvance {
        account_id: hotp_id,
        result: Err(PaladinAuthError::SaveNotCommitted {
            committed: false,
            backup_path: None,
        }),
        staged_code: None,
...
fn effect_result_hotp_advance_err_invalid_state_leaves_reveal_unchanged() {
    ...
    let event = AppEvent::EffectResult(EffectResult::HotpAdvance {
        account_id: hotp_id,
        result: Err(PaladinAuthError::InvalidState {
...
fn effect_result_hotp_advance_err_with_no_prior_reveal_does_not_open_one() {
    ...
    let event = AppEvent::EffectResult(EffectResult::HotpAdvance {
        account_id: hotp_id,
        result: Err(PaladinAuthError::SaveDurabilityUnconfirmed),
        staged_code: None,
        completed_at: Instant::now(),
    });
...
fn effect_result_hotp_advance_err_save_durability_unconfirmed_with_staged_code_opens_reveal() {
    ...
    let completed_at = Instant::now();
    let expected_status = render_error_message(&PaladinAuthError::SaveDurabilityUnconfirmed);
    let event = AppEvent::EffectResult(EffectResult::HotpAdvance {
        account_id: hotp_id,
        result: Err(PaladinAuthError::SaveDurabilityUnconfirmed),
