...
//!
//! [`paladin_auth_tui::app::run::run_event_loop`] is the inner testable
//! composer that owns the `mpsc<AppEvent>` channel + producer
//! spawning + the `dispatch` call. Production callers pass
//! [`paladin_auth_tui::app::input::spawn`] / [`paladin_auth_tui::app::ticker::spawn`];
//! these tests pass fake spawners so the sender-clone + dispatch
//! completion contract can be exercised without a TTY.
//!
//! [`paladin_auth_tui::app::run::run_with_terminal_guard`] layers
//! [`paladin_auth_tui::terminal::TerminalGuard`] above `run_event_loop` so
...
//! [`paladin_auth_tui::terminal::TerminalBackend`] so the setup / teardown
...
use paladin_auth_tui::app::render::draw_frame;
use paladin_auth_tui::app::run::{
...
use paladin_auth_tui::app::state::AppState;
use paladin_auth_tui::terminal::TerminalBackend;
...
fn exit_code_from_run_result_io_error_returns_exit_code_failure_and_writes_advisory() {
    ...
    // and writes a single-line `paladin-auth-tui: <err>\n` advisory to the
    // stderr sink. The wording matches the CLI's `paladin-auth: <err>`
    ...
    assert_eq!(
        written, "paladin-auth-tui: simulated terminal setup failure\n",
...
// production `paladin-auth-tui::run` reads the sink after
...
// reaches the user on the same `paladin-auth-tui: <err>` stderr path as a
...
fn merge_render_failure_into_run_result_surfaces_render_failure_when_loop_exited_cleanly() {
    ...
    // with FAILURE and a `paladin-auth-tui: <render-err>` advisory rather
...
// checklist) — top-level composer the `paladin_auth_tui::run()` binary entry uses
...
// `paladin-auth-tui: <err>` stderr advisory as a setup failure.
// ---------------------------------------------------------------------------
...
fn run_with_components_returns_success_after_dispatch_quits_cleanly() {
    ...
    let vault_path = tmp
        .path()
        .join("paladin-auth-tui-run-with-components-success.bin");
    let args = paladin_auth_tui::cli::GlobalArgs {
    ...
    let exit_code = paladin_auth_tui::run_with_components(
        args,
        false,
        RecordingBackend(log.clone()),
...
fn run_with_components_returns_failure_with_stderr_advisory_on_terminal_construction_failure() {
    ...
    let exit_code = paladin_auth_tui::run_with_components::<TestBackend, _, _, _, _, _>(
        args,
        false,
        RecordingBackend(log.clone()),
    ...
    assert_eq!(
        stderr_str, "paladin-auth-tui: synthetic terminal construction failure\n",
