...
//! ratatui rendering snapshots for `paladin-auth-tui`.
//!
...
use paladin_auth_core::{
    ...
    IconHintInput, PaladinAuthError, PermissionSubject, Store, ValidationWarning, Vault, VaultInit,
    VaultLock,
};
use paladin_auth_tui::app::event::{AppEvent, EffectResult, QrImportFailure};
...
fn snapshot_create_vault_screen() {
    let state = AppState::create_vault_initial(PathBuf::from("/var/lib/paladin-auth/vault.bin"));
...
fn snapshot_create_vault_choose_mode_plaintext_selected() {
    let state = AppState::CreateVault {
        path: PathBuf::from("/var/lib/paladin-auth/vault.bin"),
...
fn snapshot_create_vault_create_error_unsafe_permissions() {
    let err = PaladinAuthError::UnsafePermissions {
        path: PathBuf::from("/var/lib/paladin-auth"),
...
fn snapshot_unlock_screen_with_wrong_passphrase_error() {
    ...
    // `render_error_message(&PaladinAuthError::DecryptFailed)`, which
...
/// `crates/paladin-auth-tui/tests/effect_tests.rs` — duplicated locally
...
fn snapshot_list_view_status_line_after_export() {
    ...
    let (mut vault, store) = Store::open(&path, VaultLock::Plaintext).expect("reopen vault");
    let id = push_totp_account(&mut vault, &store, Some("GitHub"), "ben@example.com");
    let display = "~/exports/paladin-auth-export.json";
...
fn snapshot_add_modal_save_not_committed() {
    ...
    // &PaladinAuthError::SaveNotCommitted { .. })), .. })` so the snapshot
    ...
    let (vault, store) = Store::open(&path, VaultLock::Plaintext).expect("reopen vault");
    let modal = AddModal {
        error: Some(render_error_message(&PaladinAuthError::SaveNotCommitted {
            committed: false,
            backup_path: None,
        })),
        ..AddModal::default()
...
fn snapshot_add_modal_save_durability_unconfirmed() {
    ...
    // typed error here is `PaladinAuthError::SaveDurabilityUnconfirmed`,
...
fn snapshot_add_modal_qr_image_decode_failure() {
    ...
    // documented in `crates/paladin-auth-tui/src/app/event.rs`. The
...
fn snapshot_add_modal_qr_no_qrs_decoded() {
    ...
    //     PaladinAuthError::NoEntriesToImport))), .. })` so the snapshot
    // pins the inline-error row populated when
    // `paladin_auth_core::import::qr_image_bytes` decodes the clipboard
    ...
    let rendered = render_to_text(&state, snapshot_now(), 80, 20);
    let expected = format_qr_import_failure(&QrImportFailure::Import(
        PaladinAuthError::NoEntriesToImport,
    ));
    assert!(
        rendered.contains(&expected),
...
fn snapshot_add_modal_qr_oversized_rgba_buffer() {
    ...
    // holding `Modal::Add(AddModal { mode: AddMode::Qr, error:
    // Some(format_qr_import_failure(&QrImportFailure::Import(
    //     paladin_auth_core::import::qr_image_bytes(5000, 5000, &[], _)
...
fn snapshot_add_modal_qr_invalid_qr_payload() {
    ...
    // holding `Modal::Add(AddModal { mode: AddMode::Qr, error:
    // Some(format_qr_import_failure(&QrImportFailure::Import(
    //     PaladinAuthError::ValidationError { field: "qr_image", reason:
    ...
    // §4.6 / §5 (see `crates/paladin-auth-core/src/import/qr.rs:87`).
    // The reducer's Err arm (`reduce_qr_import_result` in
    // `src/app/reducer.rs`) routes the failure through
    // `format_qr_import_failure`, whose `Import(err)` arm delegates
    // to `render_error_message` and binds the wording to the core
    ...
    create_plaintext_vault(&path);
    let (vault, store) = Store::open(&path, VaultLock::Plaintext).expect("reopen vault");
    let core_err = PaladinAuthError::ValidationError {
        field: "qr_image",
...
fn snapshot_add_modal_qr_counts_panel() {
    ...
    // seeds from `paladin_auth_core::ImportReport` per `docs/DESIGN.md` §6's
...
fn snapshot_add_modal_qr_counts_panel_with_validation_warnings() {
    ...
    // `paladin_auth_core::ImportWarning` through
    // `paladin_auth_core::format_validation_warning()` per `docs/DESIGN.md` §6's
    // "The modal reports imported/skipped/replaced/appended/warning
    // counts plus validation-warning messages rendered through
    // `paladin_auth_core::format_validation_warning()` in a post-success
...
fn snapshot_add_modal_duplicate_account() {
    ...
    // row populated when `paladin_auth_core::Vault::find_duplicate` matches
...
fn snapshot_remove_modal_save_not_committed() {
    ...
    // render_error_message(&PaladinAuthError::SaveNotCommitted { .. })) })`
...
fn snapshot_import_modal_default() {
    ...
    // no inline error, no encrypted-Paladin Auth passphrase sub-phase, no
...
fn snapshot_import_modal_save_not_committed() {
    ...
    // &PaladinAuthError::SaveNotCommitted { .. })), .. })` so the
...
/// with the supplied `PaladinAuthError` pre-rendered into
...
/// diff.
fn render_import_modal_with_inline_error(
...
fn snapshot_import_modal_unsupported_plaintext_vault() {
    let rendered = render_import_modal_with_inline_error(
        ...
        "Paladin Auth import bundle is plaintext",
    );
    insta::assert_snapshot!(rendered);
}
...
fn snapshot_import_modal_counts_panel() {
    ...
    // `paladin_auth_core::ImportReport` per `docs/DESIGN.md` §6's "The modal
...
fn snapshot_export_modal_default() {
    ...
    // or an encrypted Paladin Auth bundle (passphrase prompted twice and
...
fn snapshot_export_modal_refused_overwrite_gate() {
    ...
    // &PaladinAuthError::ValidationError { field: "path",
    ...
    // `PaladinAuthError::ValidationError` `Display` impl
    ...
    let modal = ExportModal {
        error: Some(render_error_message(&PaladinAuthError::ValidationError {
            field: "path",
...
fn snapshot_export_modal_confirmation_mismatch() {
    ...
    // error: Some(render_error_message(&PaladinAuthError::InvalidPassphrase
    ...
    // Mismatch surfaces a rendered `PaladinAuthError::InvalidPassphrase`
...
fn snapshot_export_modal_zero_length() {
    ...
    // a rendered `PaladinAuthError::InvalidPassphrase` with
...
fn snapshot_export_modal_plaintext_export_warning() {
    ...
    // `paladin_auth_core::format_plaintext_export_warning()` verbatim and
    // the user must confirm before the write proceeds."*. Routing the
    // wording through `paladin_auth_core::format_plaintext_export_warning`
    // binds the snapshot to the core helper rather than a hand-typed
    // string so wording stays in lockstep with the CLI's stderr
    // advisory (`paladin-auth-cli/src/commands/export.rs`, docs/DESIGN.md §4.6 /
...
fn snapshot_export_modal_io_error() {
    ...
    // Some(render_error_message(&PaladinAuthError::IoError { ... })), .. })`
    // open so the snapshot pins the inline-error row populated when
    // `paladin_auth_core::write_secret_file_atomic` fails anywhere along
    ...
    let rendered = render_to_text(&state, snapshot_now(), 80, 20);
    let expected = render_error_message(&PaladinAuthError::IoError {
        operation: "write_secret_file_atomic",
        source: std::io::Error::new(std::io::ErrorKind::PermissionDenied, "synthetic-denied"),
    });
    assert!(
...
fn snapshot_export_modal_save_not_committed() {
    ...
    // open so the snapshot pins the inline-error row populated when
    // `paladin_auth_core::write_secret_file_atomic` fails before the
    ...
    let rendered = render_to_text(&state, snapshot_now(), 80, 20);
    let expected = render_error_message(&PaladinAuthError::SaveNotCommitted {
        committed: false,
        backup_path: None,
    });
    assert!(
...
fn snapshot_export_modal_save_durability_unconfirmed() {
    ...
    // `view::render` against an `Unlocked` state with
    // `Modal::Export(ExportModal { error:
    // Some(render_error_message(&PaladinAuthError::SaveDurabilityUnconfirmed)), .. })`
    // open so the snapshot pins the inline-error row populated when
    // `paladin_auth_core::write_secret_file_atomic`'s primary rename
    ...
    let rendered = render_to_text(&state, snapshot_now(), 80, 20);
    let expected = render_error_message(&PaladinAuthError::SaveDurabilityUnconfirmed);
    assert!(
        rendered.contains(&expected),
...
fn snapshot_passphrase_modal_remove_default() {
    ...
    // `paladin_auth_core::format_plaintext_storage_warning()`."*
    let tmp = secure_test_tempdir();
    let path = tmp.path().join("vault.bin");
    create_plaintext_vault(&path);
    let (vault, store) = Store::open(&path, VaultLock::Plaintext).expect("reopen vault");
...
fn snapshot_passphrase_modal_confirmation_mismatch() {
    ...
    // `crates/paladin-auth-tui/src/app/reducer.rs` surfaces the mismatch
    // through `PaladinAuthError::InvalidPassphrase { reason:
...
fn snapshot_passphrase_modal_change_save_durability_unconfirmed() {
    ...
    // above; typed against `PaladinAuthError::SaveDurabilityUnconfirmed`
...
fn snapshot_passphrase_modal_remove_save_not_committed() {
    ...
    // from [`paladin_auth_core::format_plaintext_storage_warning`] — stays
...
fn snapshot_startup_error_unsafe_permissions() {
    ...
    // wording from `paladin_auth_core::format_unsafe_permissions`; any future
    // wording change in core is then surfaced as a diff in this snapshot.
    let path = PathBuf::from("/var/lib/paladin-auth/vault.bin");
    let err = PaladinAuthError::UnsafePermissions {
        path: path.clone(),
        subject: PermissionSubject::VaultDir,
        actual_mode: "0755".to_string(),
...
//
// The modal is 72x30 (centered) per `crates/paladin-auth-tui/src/view/qr.rs`, so
...
// fit the modal's full inner body (the half-block QR grid, the save
// actions, and the informational warning footer). Tracks
...
/// so the drive sequence (`Q` → Enter → Tab → typing → Enter)
...
/// account at counter `0`. Used by `qr_export_modal_hotp` so the QR
...
#[test]
fn snapshot_qr_export_modal_totp() {
    // Plan: "QR view for a TOTP account, captured on open." Drive `Q`
    // so the modal opens directly with `staged_ansi` populated from
    // `Vault::export_qr_ansi(id)` and the warning footer beneath the
    // save actions.
    let (state, _id, _path, _guard) = qr_unlocked_with_one_totp_snapshot();
    let state = qr_drive(state, &[KeyCode::Char('Q')]);
    insta::assert_snapshot!(
        "qr_export_modal_totp",
        render_to_text(&state, snapshot_now(), 80, 32)
    );
}

#[test]
fn snapshot_qr_export_modal_hotp() {
    // Plan: "QR view for a HOTP account." Same drive sequence as the
...
fn snapshot_qr_export_modal_save_destination_prompt() {
    // Plan: "QR view + save sub-flow on EnterPath with a typed path."
    // Drive the modal open, Enter on `Save as PNG…` to open the sub-
    // flow, then type a fixed path so the destination field is non-empty.
    let (state, _id, _path, _guard) = qr_unlocked_with_one_totp_snapshot();
    let state = qr_drive(state, &[KeyCode::Char('Q'), KeyCode::Enter]);
...
fn snapshot_qr_export_modal_save_overwrite_gate() {
    // Plan: "QR view + save sub-flow on OverwriteGate." Drive the
    // reducer to open the QR view (so `staged_ansi` is populated by
    // the real open path), then patch the modal's `save_sub_flow`
    // slot directly into the overwrite-gate shape with a deterministic
    ...
    let state = patch_qr_modal(state, |qr| {
        qr.focus = paladin_auth_tui::app::state::QrExportFocus::SavePngButton;
        qr.save_sub_flow = Some(paladin_auth_tui::app::state::QrSaveSubFlow {
            format: paladin_auth_tui::app::state::QrSaveFormat::Png,
...
where
    F: FnOnce(&mut paladin_auth_tui::app::state::QrExportModal),
{
    match state {
        AppState::Unlocked {
            path,
...
fn snapshot_qr_export_modal_save_succeeded() {
    // Plan: "QR view with last_save_path set + no active sub-flow."
...
fn snapshot_qr_export_modal_save_failed_pre_commit() {
    // Plan: "QR view + save sub-flow showing save_not_committed inline
