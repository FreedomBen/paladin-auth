...
//! Clipboard auto-clear reducer tests for `paladin-auth-tui`.
//!
...
//! the `PALADIN_AUTH_CLIPBOARD_DRYRUN=1` adapter hook ride with later
...
use paladin_auth_tui::app::event::{AppEvent, Effect, EffectResult};
...
// `paladin_auth_core::policy::clipboard_clear::ClipboardClearPolicy::schedule(completed_at, vault.settings())`
...
// ---------------------------------------------------------------------------
// Executor (`paladin_auth_tui::app::effect::execute`) — only-if-unchanged
...
// `read_test_clipboard`, gated on `PALADIN_AUTH_CLIPBOARD_DRYRUN=1` so the
...
mod executor_only_if_unchanged {
    ...
    fn dryrun_adapter_round_trip_writes_and_reads_in_process_fake() {
        with_dryrun("1", || {
            seed_test_clipboard("");
            assert!(
                paladin_auth_tui::clipboard::ClipboardSession::new()
                    .write_text("abc123")
                    .is_ok(),
                "PALADIN_AUTH_CLIPBOARD_DRYRUN=1 must accept writes via the fake"
            );
            assert_eq!(
                read_test_clipboard(),
                "abc123",
            ...
            assert_eq!(
                paladin_auth_tui::clipboard::ClipboardSession::new().read_text(),
                Ok("abc123".to_string()),
    ...
    fn dryrun_adapter_fail_mode_returns_err_for_both_read_and_write() {
        with_dryrun("fail", || {
            assert_eq!(
                paladin_auth_tui::clipboard::ClipboardSession::new().write_text("ignored"),
                Err(()),
                "PALADIN_AUTH_CLIPBOARD_DRYRUN=fail must surface a write error"
            );
            assert_eq!(
                paladin_auth_tui::clipboard::ClipboardSession::new().read_text(),
                Err(()),
                "PALADIN_AUTH_CLIPBOARD_DRYRUN=fail must surface a read error"
            );
        });
    }
...
// `paladin-auth-tui`'s public surface. Errors collapse to two variants the
...
//
// Under `paladin-auth-tui/test-hooks` the same env-var protocol used for
// `read_text` / `write_text` covers images: `PALADIN_AUTH_CLIPBOARD_DRYRUN=1`
...
mod adapter_read_image {
    ...
    /// Run `body` with `PALADIN_AUTH_CLIPBOARD_DRYRUN=mode`, the process-wide
    ...
    fn dryrun_image_round_trip_returns_seeded_dimensions_and_bytes() {
        with_dryrun_image("1", || {
            ...
            match paladin_auth_tui::clipboard::ClipboardSession::new().read_image() {
                Ok(ClipboardImage {
                    width,
                    height,
                    rgba: out,
    ...
    fn dryrun_image_returns_no_image_when_clipboard_unseeded() {
        with_dryrun_image("1", || {
            clear_test_clipboard_image();
            assert_eq!(
                paladin_auth_tui::clipboard::ClipboardSession::new().read_image(),
