...
//! `nfpm` `.rpm` manifest contract tests for `paladin-auth-tui`.
//!
...
//! * Depends only on `glibc` — like the CLI, the TUI binary is
//!   otherwise statically linked where possible. No GTK / libadwaita.
//! * No maintainer scripts: the vault lives under
//!   `$XDG_DATA_HOME/paladin-auth/` and is created on first use, so
//!   install and removal touch nothing global.
//! * Inherits `version` from `${PALADIN_AUTH_VERSION}` at packaging time.
//!
//! Helpers are intentionally module-local — mirrors the parser
//! pattern used by `crates/paladin-auth-cli/tests/packaging_rpm_nfpm_manifest_logic.rs`
//! and `crates/paladin-auth-gtk/tests/packaging_rpm_nfpm_manifest_logic.rs`.

use std::fs;
use std::path::PathBuf;

const RPM_MANIFEST_RELPATH: &str = "packaging/rpm/paladin-auth-tui.yaml";
...
fn rpm_manifest_exists_at_expected_path() {
    ...
    assert!(
        ...
        "expected nfpm .rpm manifest at {} — DESIGN.md §11.3 requires \
         `packaging/rpm/paladin-auth-tui.yaml` for the TUI",
        path.display(),
    );
}
...
#[test]
fn rpm_manifest_declares_package_name_paladin_auth_tui() {
    let manifest = read_rpm_manifest();
    let name =
        top_level_scalar(&manifest, "name").expect("rpm nfpm manifest has a top-level `name:` key");
    assert_eq!(
        name, "paladin-auth-tui",
        "rpm package name must be `paladin-auth-tui` so the published artifact matches the binary \
...
fn rpm_manifest_version_is_interpolated_from_paladin_auth_version_env() {
    ...
    assert_eq!(
        ...
        "rpm `version:` must inherit from ${{PALADIN_AUTH_VERSION}} so the TUI artifact ships under \
...
#[test]
fn rpm_manifest_does_not_declare_gtk_or_libadwaita() {
    // Catch a copy-paste from packaging/rpm/paladin-auth-gtk.yaml. The TUI
    // binary never links GTK and must not pull libgtk-4-1 / libadwaita
    // into a minimal Fedora install.
    let manifest = read_rpm_manifest();
    let depends = top_level_sequence_scalars(&manifest, "depends");
    for dep in &depends {
        assert!(
            !dep.contains("gtk"),
            "rpm `depends:` must not declare GTK — paladin-auth-tui is terminal-only; got {dep:?}",
        );
        assert!(
            !dep.contains("libadwaita") && !dep.contains("adwaita"),
            "rpm `depends:` must not declare libadwaita — paladin-auth-tui is terminal-only; got {dep:?}",
        );
    }
}
