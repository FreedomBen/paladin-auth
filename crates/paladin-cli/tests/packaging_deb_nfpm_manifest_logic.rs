...
//! `nfpm` `.deb` manifest contract tests for the `paladin` CLI.
//!
//! Per `docs/DESIGN.md` §11.3 ("Per-front-end packages") and
//! `docs/IMPLEMENTATION_PLAN_02_CLI.md` §"Packaging (per §11)":
//!
//! * Installs `/usr/bin/paladin` and the gzipped man page at
...
//!   dependency declared in `packaging/rpm/paladin.yaml`.
...
const DEB_MANIFEST_RELPATH: &str = "packaging/deb/paladin.yaml";

const REQUIRED_DEB_DEPENDS: &[&str] = &["libc6"];

const REQUIRED_INSTALL_DESTINATIONS: &[&str] =
    &["/usr/bin/paladin", "/usr/share/man/man1/paladin.1.gz"];

const REQUIRED_INSTALL_SOURCES: &[&str] = &["target/release/paladin", "target/man/paladin.1.gz"];
...
fn deb_manifest_exists_at_expected_path() {
    ...
    assert!(
        ...
        "expected nfpm .deb manifest at {} — DESIGN.md §11.3 requires \
         `packaging/deb/paladin.yaml` for the CLI",
        path.display(),
    );
}
...
#[test]
fn deb_manifest_declares_package_name_paladin() {
    let manifest = read_deb_manifest();
    let name =
        top_level_scalar(&manifest, "name").expect("deb nfpm manifest has a top-level `name:` key");
    assert_eq!(
        name, "paladin",
        "deb package name must be `paladin` so the published artifact matches the binary; \
         got {name:?}",
    );
}
...
fn deb_manifest_version_is_interpolated_from_paladin_version_env() {
    ...
    assert_eq!(
        ...
        "deb `version:` must inherit from ${{PALADIN_VERSION}} so the CLI artifact ships under \
...
fn deb_manifest_has_no_maintainer_scripts_section() {
    ...
    // `$XDG_DATA_HOME/paladin/` and are created by `paladin init`.
    assert!(
...
fn deb_manifest_binary_install_uses_executable_mode() {
    // /usr/bin/paladin must be world-executable. nfpm defaults file
    ...
    // every `paladin` invocation.
    let manifest = read_deb_manifest();
    let lines: Vec<&str> = manifest.lines().collect();
    let mut binary_block_start: Option<usize> = None;
    for (idx, raw) in lines.iter().enumerate() {
        let trimmed = strip_trailing_comment(raw).trim();
        if trimmed == "dst: /usr/bin/paladin" || trimmed == "dst: \"/usr/bin/paladin\"" {
            binary_block_start = Some(idx);
            break;
        }
    }
    let start =
        binary_block_start.expect("deb nfpm manifest is missing the /usr/bin/paladin dst entry");
    ...
    assert!(
        found_mode_0755,
        "deb nfpm manifest must set `mode: 0755` on the /usr/bin/paladin entry so the \
