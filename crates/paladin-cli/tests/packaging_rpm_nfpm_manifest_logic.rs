...
//! `nfpm` `.rpm` manifest contract tests for the `paladin` CLI.
//!
...
const RPM_MANIFEST_RELPATH: &str = "packaging/rpm/paladin.yaml";
...
fn rpm_manifest_exists_at_expected_path() {
    ...
    assert!(
        ...
        "expected nfpm .rpm manifest at {} — DESIGN.md §11.3 requires \
         `packaging/rpm/paladin.yaml` for the CLI",
        path.display(),
    );
}
...
#[test]
fn rpm_manifest_declares_package_name_paladin() {
    let manifest = read_rpm_manifest();
    let name =
        top_level_scalar(&manifest, "name").expect("rpm nfpm manifest has a top-level `name:` key");
    assert_eq!(
        name, "paladin",
        "rpm package name must be `paladin` so the published artifact matches the binary; \
         got {name:?}",
    );
}
...
fn rpm_manifest_version_is_interpolated_from_paladin_version_env() {
    ...
    assert_eq!(
        ...
        "rpm `version:` must inherit from ${{PALADIN_VERSION}} so the CLI artifact ships under \
