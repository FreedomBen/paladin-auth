...
/// `-h`. Examples (with `argv[0]` = `paladin`):
///
/// - `["paladin", "--help"]`              → `"paladin"`
/// - `["paladin", "--vault", "/v", "-h"]` → `"paladin"`
/// - `["paladin", "add", "--help"]`       → `"paladin add"`
/// - `["paladin", "add", "--bogus", "-h"]`→ `"paladin add"` (subcommand
...
mod tests {
    ...
    #[test]
    fn resolve_for_top_level_help_returns_paladin() {
        let cmd = Cli::command();
        assert_eq!(resolve_command_path(["paladin", "--help"], &cmd), "paladin");
        assert_eq!(resolve_command_path(["paladin", "-h"], &cmd), "paladin");
    }
    ...
    fn resolve_skips_flag_values_before_subcommand() {
        let cmd = Cli::command();
        assert_eq!(
            resolve_command_path(["paladin", "--vault", "/v", "add", "--help"], &cmd),
            "paladin add"
        );
    }
    ...
    fn resolve_walks_into_subcommand_tree_for_passphrase_set() {
        let cmd = Cli::command();
        assert_eq!(
            resolve_command_path(["paladin", "passphrase", "set", "--help"], &cmd),
    ...
    fn resolve_handles_clap_help_command_form() {
        let cmd = Cli::command();
        assert_eq!(
            resolve_command_path(["paladin", "help", "show"], &cmd),
            "paladin show"
        );
    }
    ...
    fn render_json_envelope_is_valid_json_with_command_and_text_fields() {
        let mut buf = Vec::new();
        render_json("paladin add", "Usage: paladin add ...", &mut buf).unwrap();
        let s = String::from_utf8(buf).unwrap();
        let v: serde_json::Value = serde_json::from_str(s.trim()).unwrap();
        assert_eq!(v["help"]["command"], serde_json::json!("paladin add"));
        assert_eq!(
            v["help"]["text"],
            serde_json::json!("Usage: paladin add ...")
        );
    }
}
