...
mod tests {
    ...
    fn argv_pre_scan_finds_json_flag() {
        assert!(argv_has_json_flag(["paladin", "--json", "list"]));
        assert!(argv_has_json_flag(["paladin", "show", "x", "--json"]));
        assert!(argv_has_json_flag(["paladin", "--vault", "/v", "--json"]));
    }

    #[test]
    fn argv_pre_scan_ignores_argv0_and_substring_matches() {
        assert!(!argv_has_json_flag(["--json", "list"]));
        assert!(!argv_has_json_flag(["paladin", "list"]));
        assert!(!argv_has_json_flag(["paladin", "--json=true"]));
        assert!(!argv_has_json_flag(["paladin", "--jsonish"]));
    }
