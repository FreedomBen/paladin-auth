...
use paladin_core::{Argon2Params, EncryptionOptions, PaladinError};
...
/// the module header.
pub fn parse_argon2_params(args: &KdfArgs) -> Result<Argon2Params, PaladinError> {
...
) -> Result<EncryptionOptions, PaladinError> {
    let params = parse_argon2_params(args)?;
    EncryptionOptions::with_params(passphrase, params)
}

fn parse_optional_u32(raw: Option<&str>, field: &'static str) -> Result<Option<u32>, PaladinError> {
    match raw {
        None => Ok(None),
        Some(s) => s
            .parse::<u32>()
...
fn validation_err(field: &'static str, reason: &'static str) -> PaladinError {
...
mod tests {
    ...
    fn assert_validation(err: &PaladinError, field: &str, reason: &str) {
        match err {
            PaladinError::ValidationError {
                field: f,
                reason: r,
    ...
    fn memory_mib_above_ceiling_rejects_with_kdf_out_of_bounds() {
        ...
        assert_eq!(err.kind(), ErrorKind::KdfParamsOutOfBounds);
        match err {
            PaladinError::KdfParamsOutOfBounds { m_kib, .. } => {
    ...
    fn time_below_floor_rejects_with_kdf_out_of_bounds() {
        ...
        assert_eq!(err.kind(), ErrorKind::KdfParamsOutOfBounds);
        match err {
            PaladinError::KdfParamsOutOfBounds { t, .. } => assert_eq!(t, 0),
            other => panic!("unexpected variant: {other:?}"),
        }
    }
    ...
    fn time_above_ceiling_rejects_with_kdf_out_of_bounds() {
        ...
        assert_eq!(err.kind(), ErrorKind::KdfParamsOutOfBounds);
        match err {
            PaladinError::KdfParamsOutOfBounds { t, .. } => assert_eq!(t, 11),
            other => panic!("unexpected variant: {other:?}"),
        }
    }
    ...
    fn parallelism_below_floor_rejects_with_kdf_out_of_bounds() {
        ...
        assert_eq!(err.kind(), ErrorKind::KdfParamsOutOfBounds);
        match err {
            PaladinError::KdfParamsOutOfBounds { p, .. } => assert_eq!(p, 0),
            other => panic!("unexpected variant: {other:?}"),
        }
    }
    ...
    fn parallelism_above_ceiling_rejects_with_kdf_out_of_bounds() {
        ...
        assert_eq!(err.kind(), ErrorKind::KdfParamsOutOfBounds);
        match err {
            PaladinError::KdfParamsOutOfBounds { p, .. } => assert_eq!(p, 5),
            other => panic!("unexpected variant: {other:?}"),
        }
    }
    ...
    fn encryption_options_empty_passphrase_after_valid_kdf_rejects() {
        ...
        match err {
            PaladinError::InvalidPassphrase { reason } => assert_eq!(reason, "zero_length"),
            other => panic!("unexpected variant: {other:?}"),
        }
    }
}
