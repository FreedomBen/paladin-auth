...
//! Build script for `paladin-auth-gtk`.
//!
//! Two responsibilities:
//!
//! 1. Pack `data/paladin-auth-gtk.gresource.xml` into a binary
//!    `paladin-auth-gtk.gresource` under `OUT_DIR` via
...
//!    `/org/tamx/PaladinAuth/Gui/style.css` through `gtk::CssProvider`.
//!
//! 2. Compile `data/org.tamx.PaladinAuth.Gui.gschema.xml` into
//!    `OUT_DIR/schemas/gschemas.compiled` via the system
//!    `glib-compile-schemas` and export the directory path as
//!    `PALADIN_AUTH_GTK_SCHEMA_DIR` so `paladin_auth_gtk::gsettings` can
...
fn main() {
    ...
    // which is `crates/paladin-auth-gtk/`) lets the manifest pull in
    // the repo-root `LICENSE` (AGPL-3.0-or-later body shipped
    // under `/org/tamx/PaladinAuth/Gui/LICENSE`) without a duplicate
    ...
    glib_build_tools::compile_resources(
        &["data", "../.."],
        "data/paladin-auth-gtk.gresource.xml",
        "paladin-auth-gtk.gresource",
    );

    compile_gsettings_schema();
}

/// Compile `data/org.tamx.PaladinAuth.Gui.gschema.xml` into
...
/// Exports `PALADIN_AUTH_GTK_SCHEMA_DIR` so `paladin_auth_gtk::gsettings`
...
fn compile_gsettings_schema() {
    println!("cargo:rerun-if-changed=data/org.tamx.PaladinAuth.Gui.gschema.xml");
    ...
    let src = PathBuf::from("data/org.tamx.PaladinAuth.Gui.gschema.xml");
    let dst = schema_dir.join("org.tamx.PaladinAuth.Gui.gschema.xml");
    ...
    println!(
        "cargo:rustc-env=PALADIN_AUTH_GTK_SCHEMA_DIR={}",
        schema_dir.display(),
    );
}
