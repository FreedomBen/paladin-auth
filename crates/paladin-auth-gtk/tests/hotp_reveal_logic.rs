...
//! Pure-logic HOTP reveal-window tests for `paladin-auth-gtk`.
//!
...
use paladin_auth_core::{
    ...
    AccountKindSummary, AccountSummary, Algorithm, Code, IconHintInput, PaladinAuthError, Store,
    Vault, VaultInit, VaultLock, HOTP_REVEAL_SECS,
};

use paladin_auth_gtk::account_row::{CodeDisplay, CounterText};
use paladin_auth_gtk::hotp_reveal::{
...
fn save_not_committed_pre_rename() -> PaladinAuthError {
...
// ---------------------------------------------------------------------------
// `deadline` routes through `paladin_auth_core::hotp_reveal_deadline`
// ---------------------------------------------------------------------------

#[test]
fn deadline_routes_through_paladin_auth_core_policy() {
    let now = Instant::now();
    let got = deadline(now);
    let from_core = hotp_reveal_deadline(now);
    assert_eq!(got, from_core, "must match paladin_auth_core policy");
    assert_eq!(
        got,
        now + Duration::from_secs(HOTP_REVEAL_SECS),
        "uses paladin_auth_core::HOTP_REVEAL_SECS"
    );
}
...
fn io_error_failure_retains_prior_and_drops_staged() {
    ...
    let decision = apply_advance_outcome(AdvanceOutcome {
        account_id: account,
        result: Err(PaladinAuthError::IoError {
