...
//! Pure-logic search tests for `paladin-auth-gtk`.
//!
//! Tracks the §"Tests > Pure-logic unit tests > `tests/search_logic.rs`"
//! checklist in `docs/IMPLEMENTATION_PLAN_04_GTK.md`:
//!
//! * Filtering routes through `paladin_auth_core::account_matches_search`
...
use paladin_auth_gtk::search::{filtered_account_ids, select_after_search};
...
fn cli_id_prefix_form_is_not_honored() {
    ...
    // `issuer:label` match key, parity with `paladin_auth_tui::search`.
    assert!(
