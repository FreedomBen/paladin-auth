...
//! `paladin_auth_core::default_vault_path()`), call `paladin_auth_core::inspect`,
//! and — for `VaultStatus::Plaintext` — `paladin_auth_core::Store::open`
...
//! stdout line `paladin-auth-gtk` emits under `--exit-after-startup`. The
...
use paladin_auth_gtk::app::model::{run_startup_probes, startup_state_marker, StartupOutcome};
use paladin_auth_gtk::app::state::AppState;
use paladin_auth_gtk::startup_error::StartupErrorSource;
...
/// probe reopens it. `paladin_auth_core` enforces `0700` on the vault
...
fn prepare_plaintext_vault() -> (tempfile::TempDir, PathBuf) {
    ...
    std::fs::set_permissions(tempdir.path(), std::fs::Permissions::from_mode(0o700))
        .expect("chmod tempdir to 0700 so paladin_auth_core::Store::create accepts it");
    ...
    let path = tempdir.path().join("vault.bin");
    {
        ...
        // pipeline (see `paladin_auth_core::Store::create` docs).
        let (vault, store) =
            paladin_auth_core::Store::create(&path, paladin_auth_core::VaultInit::Plaintext)
...
fn run_startup_probes_routes_corrupted_file_to_startup_error() {
    let tempdir = tempfile::tempdir().expect("create tempdir");
    let path = tempdir.path().join("vault.bin");
    // A non-magic byte run forces `paladin_auth_core::inspect` into the
    // invalid-header branch, which routes to StartupError(Inspect).
    std::fs::write(&path, b"not a paladin-auth vault").expect("write corrupted vault");
...
fn startup_state_marker_unlocked_includes_path() {
    ...
    assert_eq!(
        ...
        "paladin-auth-gtk: startup_state=Unlocked path=/tmp/example/vault.bin",
    );
}
...
fn startup_state_marker_locked_and_missing_include_path() {
    ...
    assert_eq!(
        ...
        "paladin-auth-gtk: startup_state=Locked path=/tmp/example/vault.bin",
    );
    assert_eq!(
        startup_state_marker(&AppState::Missing { path: path.clone() }),
        "paladin-auth-gtk: startup_state=Missing path=/tmp/example/vault.bin",
    );
}
...
fn startup_state_marker_startup_error_renders_path_or_placeholder() {
    ...
    let with_path_marker = startup_state_marker(&AppState::StartupError {
        ...
        error: paladin_auth_gtk::startup_error::StartupError {
            source: StartupErrorSource::Inspect,
            kind: paladin_auth_core::ErrorKind::InvalidHeader,
            rendered: String::new(),
        },
    });
    assert_eq!(
        with_path_marker,
        "paladin-auth-gtk: startup_state=StartupError path=/tmp/example/vault.bin",
    );
    ...
    let without_path_marker = startup_state_marker(&AppState::StartupError {
        ...
        error: paladin_auth_gtk::startup_error::StartupError {
            ...
            kind: paladin_auth_core::ErrorKind::IoError,
            rendered: String::new(),
        },
    });
    assert_eq!(
        without_path_marker,
        "paladin-auth-gtk: startup_state=StartupError path=(unresolved)",
    );
}
...
fn format_app_window_default_size_returns_1280_by_960() {
    ...
    use paladin_auth_gtk::app::model::format_app_window_default_size;
...
fn format_app_add_button_icon_name_returns_list_add_symbolic() {
    ...
    // source of truth.
    use paladin_auth_gtk::app::model::format_app_add_button_icon_name;
...
fn format_app_add_button_tooltip_returns_add_account() {
    ...
    //
    // Distinct from `paladin_auth_gtk::add_account::format_add_dialog_title`
    ...
    use paladin_auth_gtk::app::model::format_app_add_button_tooltip;
...
#[test]
fn format_app_window_title_returns_paladin_auth() {
    // The `AppModel`'s `adw::ApplicationWindow::set_title` attribute
    // is populated from this helper. The wording (`"Paladin Auth"`) names
    ...
    use paladin_auth_gtk::app::model::format_app_window_title;
...
fn format_app_search_button_icon_name_returns_system_search_symbolic() {
    ...
    use paladin_auth_gtk::app::model::format_app_search_button_icon_name;
...
fn format_app_search_button_tooltip_returns_search_accounts() {
    ...
    use paladin_auth_gtk::app::model::format_app_search_button_tooltip;
...
fn format_app_search_button_visible_returns_true_when_a_vault_is_open() {
    ...
    use paladin_auth_gtk::app::model::format_app_search_button_visible;
    use paladin_auth_gtk::app::state::AppState;
    use paladin_auth_gtk::startup_error::{StartupError, StartupErrorSource};
...
fn format_app_search_button_visible_matches_format_app_add_button_visible() {
    ...
    use paladin_auth_core::ErrorKind;
    use paladin_auth_gtk::app::model::{
...
fn format_app_search_toggle_msg_active_emits_set_search_mode_enabled_true() {
    ...
    // inline.
    use paladin_auth_gtk::account_list::AccountListMsg;
    use paladin_auth_gtk::app::model::format_app_search_toggle_msg;
...
fn format_app_menu_button_icon_name_returns_open_menu_symbolic() {
    ...
    // affordances against a single source of truth.
    use paladin_auth_gtk::app::model::format_app_menu_button_icon_name;
...
fn format_app_menu_button_tooltip_returns_main_menu() {
    ...
    // Passphrase…, Preferences, About Paladin Auth, Quit) and matches
    ...
    use paladin_auth_gtk::app::model::format_app_menu_button_tooltip;
...
fn format_app_menu_import_label_returns_import_with_ellipsis() {
    ...
    // Preferences, About Paladin Auth, Quit) which will land in
    ...
    use paladin_auth_gtk::app::model::format_app_menu_import_label;
...
fn format_app_menu_export_label_returns_export_with_ellipsis() {
    ...
    use paladin_auth_gtk::app::model::format_app_menu_export_label;
...
fn format_app_menu_passphrase_label_returns_passphrase_with_ellipsis() {
    ...
    use paladin_auth_gtk::app::model::format_app_menu_passphrase_label;
...
fn format_app_menu_preferences_label_returns_preferences_without_ellipsis() {
    ...
    use paladin_auth_gtk::app::model::format_app_menu_preferences_label;
...
#[test]
fn format_app_menu_about_label_returns_about_paladin_auth() {
    // The `AppModel`'s primary `gio::Menu` "About Paladin Auth" entry's
    // label is populated from this helper. The wording
    // (`"About Paladin Auth"`) names the surface the entry opens
    ...
    // identity before opening the dialog. The trailing "Paladin Auth"
    ...
    use paladin_auth_gtk::app::model::format_app_menu_about_label;
...
fn format_app_menu_about_label_carries_application_name() {
    ...
    use paladin_auth_gtk::app::model::{format_app_menu_about_label, format_app_window_title};
...
fn format_app_menu_quit_label_returns_quit() {
    ...
    use paladin_auth_gtk::app::model::format_app_menu_quit_label;
...
fn format_app_menu_import_action_returns_app_import() {
    ...
    // registration).
    use paladin_auth_gtk::app::model::format_app_menu_import_action;
...
fn format_app_menu_export_action_returns_app_export() {
    ...
    use paladin_auth_gtk::app::model::format_app_menu_export_action;
...
fn format_app_menu_passphrase_action_returns_app_passphrase() {
    ...
    // contract side; together they pin both halves (visible
    // label + action target) against a single source of truth.
    use paladin_auth_gtk::app::model::format_app_menu_passphrase_action;
...
fn format_app_menu_preferences_action_returns_app_preferences() {
    ...
    // contract side; together they pin both halves (visible
    // label + action target) against a single source of truth.
    use paladin_auth_gtk::app::model::format_app_menu_preferences_action;
...
fn format_app_menu_about_action_returns_app_about() {
    // The `AppModel`'s primary `gio::Menu` "About Paladin Auth"
    ...
    // `"about"` rather than `"about_paladin_auth"` so the action name
    ...
    // contract side; together they pin both halves (visible
    // label + action target) against a single source of truth.
    use paladin_auth_gtk::app::model::format_app_menu_about_action;
...
fn format_app_menu_quit_action_returns_app_quit() {
    ...
    // helpers.
    use paladin_auth_gtk::app::model::format_app_menu_quit_action;
...
fn format_app_action_group_name_returns_app() {
    ...
    use paladin_auth_gtk::app::model::format_app_action_group_name;
...
fn format_app_menu_import_action_name_returns_import() {
    ...
    // truth.
    use paladin_auth_gtk::app::model::format_app_menu_import_action_name;
...
fn format_app_menu_export_action_name_returns_export() {
    ...
    use paladin_auth_gtk::app::model::format_app_menu_export_action_name;
...
fn format_app_menu_passphrase_action_name_returns_passphrase() {
    ...
    // visible-label side; together they pin all three halves of
    // the menu-entry contract against a single source of truth.
    use paladin_auth_gtk::app::model::format_app_menu_passphrase_action_name;
...
fn format_app_menu_preferences_action_name_returns_preferences() {
    ...
    use paladin_auth_gtk::app::model::format_app_menu_preferences_action_name;
...
fn format_app_menu_preferences_accelerator_returns_control_comma() {
    ...
    use paladin_auth_gtk::app::model::format_app_menu_preferences_accelerator;
...
fn format_app_menu_about_action_name_returns_about() {
    ...
    // name is `"about"` rather than `"about_paladin_auth"` so the
    ...
    use paladin_auth_gtk::app::model::format_app_menu_about_action_name;
...
fn format_app_menu_quit_action_name_returns_quit() {
    ...
    use paladin_auth_gtk::app::model::format_app_menu_quit_action_name;
...
fn format_app_menu_quit_accelerator_returns_control_q() {
    ...
    // against a single source of truth.
    use paladin_auth_gtk::app::model::format_app_menu_quit_accelerator;
...
fn format_app_menu_keyboard_shortcuts_label_returns_keyboard_shortcuts() {
    ...
    use paladin_auth_gtk::app::model::format_app_menu_keyboard_shortcuts_label;
...
fn format_app_menu_keyboard_shortcuts_action_returns_app_shortcuts() {
    ...
    use paladin_auth_gtk::app::model::format_app_menu_keyboard_shortcuts_action;
...
fn format_app_menu_keyboard_shortcuts_action_name_returns_shortcuts() {
    ...
    use paladin_auth_gtk::app::model::format_app_menu_keyboard_shortcuts_action_name;
...
fn format_app_menu_keyboard_shortcuts_accelerator_returns_control_question() {
    ...
    use paladin_auth_gtk::app::model::format_app_menu_keyboard_shortcuts_accelerator;
...
fn format_app_add_button_action_returns_app_add() {
    ...
    // against a single source of truth.
    use paladin_auth_gtk::app::model::format_app_add_button_action;
...
fn format_app_add_button_action_name_returns_add() {
    ...
    use paladin_auth_gtk::app::model::format_app_add_button_action_name;
...
fn format_app_add_button_accelerator_returns_control_shift_n() {
    ...
    // source of truth.
    use paladin_auth_gtk::app::model::format_app_add_button_accelerator;
...
fn format_app_copy_next_code_accelerator_returns_control_shift_c() {
    ...
    use paladin_auth_gtk::app::model::format_app_copy_next_code_accelerator;
...
fn format_app_copy_next_code_action_returns_app_copy_next_code() {
    ...
    use paladin_auth_gtk::app::model::format_app_copy_next_code_action;
...
fn format_app_copy_next_code_action_name_returns_copy_next_code() {
    ...
    use paladin_auth_gtk::app::model::format_app_copy_next_code_action_name;
...
fn format_app_window_accelerator_bindings_targets_are_distinct() {
    ...
    // disjoint.
    use paladin_auth_gtk::app::model::format_app_window_accelerator_bindings;
...
fn wire_app_window_accelerators_signature_takes_application_reference() {
    ...
    let _: fn(&gtk4::Application) = paladin_auth_gtk::app::model::wire_app_window_accelerators;
}
...
fn format_app_primary_menu_entries_returns_eight_entries_in_pinned_order() {
    ...
    // Preferences, Keyboard Shortcuts, About Paladin Auth, Quit, and the
...
fn format_app_primary_menu_entries_uses_app_group_prefix_throughout() {
    ...
    use paladin_auth_gtk::app::model::{
        format_app_action_group_name, format_app_primary_menu_entries,
    };
...
fn build_app_primary_menu_model_appends_every_format_app_primary_menu_entries_pair() {
    ...
    use libadwaita::prelude::*;
    use paladin_auth_gtk::app::model::{
        build_app_primary_menu_model, format_app_primary_menu_entries,
    };
...
fn build_app_about_dialog_threads_every_format_app_about_dialog_helper_through_a_setter() {
    ...
    // application menu's "About Paladin Auth" entry presents an
...
fn format_app_add_button_visible_returns_true_when_a_vault_is_open() {
    ...
    use paladin_auth_core::ErrorKind;
    use paladin_auth_gtk::app::model::format_app_add_button_visible;
    use paladin_auth_gtk::app::state::AppState;
    use paladin_auth_gtk::startup_error::{StartupError, StartupErrorSource};
...
fn format_app_add_button_visible_is_a_relaxation_of_format_app_add_button_sensitive() {
    ...
    use paladin_auth_core::ErrorKind;
    use paladin_auth_gtk::app::model::{
        format_app_add_button_sensitive, format_app_add_button_visible,
    };
    use paladin_auth_gtk::app::state::AppState;
    use paladin_auth_gtk::startup_error::{StartupError, StartupErrorSource};
...
fn format_app_menu_delete_vault_accelerator_returns_control_shift_delete() {
    ...
    use paladin_auth_gtk::app::model::format_app_menu_delete_vault_accelerator;
...
fn dispatch_app_window_action_covers_every_bundled_action_name() {
    ...
    use paladin_auth_gtk::app::model::{
        dispatch_app_window_action, format_app_window_action_names,
    };
...
fn dispatch_app_window_action_returns_none_for_unknown_action_names() {
    ...
    use paladin_auth_gtk::app::model::dispatch_app_window_action;
...
fn app_msg_carries_open_about_dialog_variant() {
    // Per §"libadwaita usage" and §"Component tree": the
    // application menu's "About Paladin Auth" entry mounts an
    ...
    use paladin_auth_gtk::app::model::AppMsg;
...
fn wire_app_window_action_activations_signature_takes_group_and_input_sender() {
    ...
    use paladin_auth_gtk::app::model::{wire_app_window_action_activations, AppMsg};
...
fn wire_app_window_action_group_signature_takes_application_window_and_action_group() {
    ...
    let _: fn(&libadwaita::ApplicationWindow, &libadwaita::gio::SimpleActionGroup) =
        paladin_auth_gtk::app::model::wire_app_window_action_group;
}
...
fn wire_app_menu_button_menu_model_signature_takes_menu_button_reference() {
    ...
    // Export, Passphrase, Preferences, About Paladin Auth, Quit).
    ...
    let _: fn(&gtk4::MenuButton) = paladin_auth_gtk::app::model::wire_app_menu_button_menu_model;
}
...
fn build_app_add_action_disables_in_non_unlocked_states() {
    ...
    use libadwaita::prelude::*;
    use paladin_auth_core::ErrorKind;
    use paladin_auth_gtk::app::model::build_app_add_action;
    use paladin_auth_gtk::app::state::AppState;
    use paladin_auth_gtk::startup_error::{StartupError, StartupErrorSource};
...
fn apply_app_window_action_group_sensitivities_is_a_noop_on_missing_actions() {
    ...
    use paladin_auth_gtk::app::model::apply_app_window_action_group_sensitivities;
    use paladin_auth_gtk::app::state::AppState;
...
fn apply_app_primary_menu_sensitivities_is_a_noop_on_missing_actions() {
    ...
    // noop-on-missing behaviour here keeps the runtime path
    // resilient to a future bundling change.
    use paladin_auth_gtk::app::model::apply_app_primary_menu_sensitivities;
    use paladin_auth_gtk::app::state::AppState;
...
fn format_app_primary_menu_action_sensitivities_disables_mutating_entries_off_unlocked() {
    ...
    use paladin_auth_gtk::app::model::format_app_primary_menu_action_sensitivities;
    use paladin_auth_gtk::app::state::AppState;
    use paladin_auth_gtk::startup_error::{StartupError, StartupErrorSource};
...
fn format_app_add_button_sensitive_disabled_off_unlocked() {
    ...
    use paladin_auth_gtk::app::model::format_app_add_button_sensitive;
    use paladin_auth_gtk::app::state::AppState;
    use paladin_auth_gtk::startup_error::{StartupError, StartupErrorSource};
...
#[test]
fn format_app_about_dialog_program_name_returns_paladin_auth() {
    // Per §"libadwaita usage": the application menu's "About
    // Paladin Auth" entry opens an `AdwAboutDialog` that pulls program
    // metadata from a single source of truth. This helper returns
    // the human-readable program name `"Paladin Auth"` shown in the
    // dialog header, matching the §11.3 desktop entry's
    // `Name=Paladin Auth` field so the launcher caption and the about
    ...
    // (`"org.tamx.PaladinAuth.Gui"`), which is the reverse-DNS
    ...
    use paladin_auth_gtk::app::model::format_app_about_dialog_program_name;

    assert_eq!(
        format_app_about_dialog_program_name(),
        "Paladin Auth",
        "AdwAboutDialog program name must be the canonical display name `Paladin Auth`, matching the §11.3 desktop entry `Name=Paladin Auth`",
    );
}
...
fn format_app_about_dialog_version_matches_cargo_pkg_version() {
    // Per §"libadwaita usage": the application menu's "About
    // Paladin Auth" entry's `AdwAboutDialog` pulls program metadata
    ...
    use paladin_auth_gtk::app::model::format_app_about_dialog_version;
...
fn format_app_about_dialog_application_icon_name_matches_app_id() {
    ...
    // `/usr/share/icons/hicolor/<size>/apps/org.tamx.PaladinAuth.Gui.*`
    // and the desktop entry sets `Icon=org.tamx.PaladinAuth.Gui`, so
    ...
    // resolve identically.
    use paladin_auth_gtk::app::model::format_app_about_dialog_application_icon_name;
    use paladin_auth_gtk::APP_ID;
...
#[test]
fn format_app_about_dialog_developer_name_returns_the_paladin_auth_contributors() {
    // Per §"libadwaita usage" and §"About / help": the
    ...
    use paladin_auth_gtk::app::model::format_app_about_dialog_developer_name;

    assert_eq!(
        format_app_about_dialog_developer_name(),
        "The Paladin Auth contributors",
...
fn format_app_about_dialog_developer_name_is_non_empty_and_distinct_from_program_name() {
    ...
    // the bare `Paladin Auth` display string) or with
    // `format_app_about_dialog_application_icon_name` (which
    // returns the reverse-DNS `org.tamx.PaladinAuth.Gui` icon key).
    use paladin_auth_gtk::app::model::{
...
#[test]
fn format_app_about_dialog_copyright_returns_paladin_auth_copyright_line() {
    // Per §"libadwaita usage" and §"About / help": the
    // `AdwAboutDialog` copyright slot displays the project's
    // copyright notice. Paladin Auth is AGPL-3.0-or-later (docs/DESIGN.md
    ...
    use paladin_auth_gtk::app::model::format_app_about_dialog_copyright;
...
fn format_app_about_dialog_license_type_returns_custom() {
    ...
    // root.
    use paladin_auth_gtk::app::model::format_app_about_dialog_license_type;
    use relm4::gtk;
...
fn format_app_about_dialog_license_text_matches_repository_license_file() {
    ...
    use paladin_auth_gtk::app::model::format_app_about_dialog_license_text;
...
fn format_app_about_dialog_license_markup_parses_as_valid_pango_markup() {
    ...
    use paladin_auth_gtk::app::model::format_app_about_dialog_license_markup;
    use relm4::gtk::pango;
...
#[test]
fn format_app_about_dialog_license_resource_path_returns_paladin_auth_gui_license_path() {
    // Per docs/IMPLEMENTATION_PLAN_04_GTK.md §"Milestone 7 checklist"
    // → "About dialog" — the AGPL-3.0-or-later license text is
    // shipped in the gresource bundle under the
    // `/org/tamx/PaladinAuth/Gui` prefix (matching the
    // `APP_ID`-derived prefix used by `style.css` and the
    // bundled icon theme). Pinning the exact gresource path
    // here keeps `data/paladin-auth-gtk.gresource.xml`'s
    // `<file alias="LICENSE">` entry and any consumer that
    // looks up the bundled text by path in lockstep without a
    // duplicated literal.
    use paladin_auth_gtk::app::model::format_app_about_dialog_license_resource_path;

    assert_eq!(
        format_app_about_dialog_license_resource_path(),
        "/org/tamx/PaladinAuth/Gui/LICENSE",
...
fn format_app_about_dialog_license_resource_path_uses_app_id_prefix() {
    ...
    // same `/org/tamx/PaladinAuth/Gui` prefix as `style.css`
    ...
    assert!(
        path.starts_with("/org/tamx/PaladinAuth/Gui/"),
...
fn format_app_about_dialog_license_resource_path_does_not_end_with_a_trailing_slash() {
    ...
    // changes the helper to `"/org/tamx/PaladinAuth/Gui/LICENSE/"`
...
fn format_app_about_dialog_website_matches_cargo_pkg_homepage() {
    ...
    // `homepage = "https://paladin-auth.tamx.org"` so a workspace-
    ...
    // manual update.
    use paladin_auth_gtk::app::model::format_app_about_dialog_website;
...
fn format_app_about_dialog_website_is_non_empty_https_url() {
    ...
    // (Paladin Auth is a secrets-handling tool; an HTTP `about`
    ...
    assert!(
        ...
        "AdwAboutDialog website must be an HTTPS URL (Paladin Auth handles secrets — never link the about dialog to an http:// page); got {website:?}",
    );
    assert!(
        !website.contains(' '),
...
fn format_app_about_dialog_issue_url_appends_issues_to_cargo_pkg_repository() {
    ...
    // table sets `repository = "https://github.com/FreedomBen/paladin-auth"`
    // and `crates/paladin-auth-gtk` inherits via `repository.workspace
    ...
    // duplicate constant.
    use paladin_auth_gtk::app::model::format_app_about_dialog_issue_url;
...
fn format_app_about_dialog_issue_url_is_non_empty_https_url_distinct_from_website() {
    ...
    // (Paladin Auth handles secrets — an HTTP issue tracker link would
    ...
    assert!(
        ...
        "AdwAboutDialog issue-url must be an HTTPS URL (Paladin Auth handles secrets — never link the about dialog to an http:// page); got {issue_url:?}",
    );
    assert!(
        !issue_url.contains(' '),
...
fn format_app_about_dialog_support_url_appends_discussions_to_cargo_pkg_repository() {
    ...
    use paladin_auth_gtk::app::model::format_app_about_dialog_support_url;
...
fn format_app_about_dialog_support_url_is_non_empty_https_url_distinct_from_issue_and_website() {
    ...
    assert!(
        ...
        "AdwAboutDialog support-url must be an HTTPS URL (Paladin Auth handles secrets — never link the about dialog to an http:// page); got {support_url:?}",
    );
    assert!(
...
fn format_app_about_dialog_comments_matches_cargo_pkg_description() {
    ...
    // `description = "Paladin Auth: Rust OTP authenticator (TOTP +
    // HOTP) with CLI, TUI, and GTK front-ends"` and
    // `crates/paladin-auth-gtk` inherits via `description.workspace =
    ...
    use paladin_auth_gtk::app::model::format_app_about_dialog_comments;
...
fn format_app_about_dialog_developers_lists_benjamin_porter() {
    ...
    use paladin_auth_gtk::app::model::format_app_about_dialog_developers;
...
fn format_app_about_dialog_documenters_is_empty_until_a_documenter_joins() {
    ...
    // credits-page "Documentation" section. Paladin Auth does not
    ...
    use paladin_auth_gtk::app::model::format_app_about_dialog_documenters;
...
fn format_app_about_dialog_artists_is_empty_until_an_artist_joins() {
    ...
    // credits-page "Artists" section. Paladin Auth does not yet have
    ...
    // `format_app_about_dialog_developers` owns the Paladin Auth-
    ...
    use paladin_auth_gtk::app::model::format_app_about_dialog_artists;
...
fn format_app_about_dialog_designers_is_empty_until_a_designer_joins() {
    ...
    // credits-page "Designers" section. Paladin Auth does not yet
    ...
    // designer.
    use paladin_auth_gtk::app::model::format_app_about_dialog_designers;
...
fn format_app_about_dialog_translator_credits_is_empty_until_translations_land() {
    ...
    use paladin_auth_gtk::app::model::format_app_about_dialog_translator_credits;
...
fn format_app_about_dialog_release_notes_is_empty_until_v0_2_ships() {
    ...
    // `format_app_about_dialog_release_notes_version`.
    use paladin_auth_gtk::app::model::format_app_about_dialog_release_notes;
...
fn format_app_about_dialog_release_notes_version_matches_cargo_pkg_version() {
    ...
    // field on a release.
    use paladin_auth_gtk::app::model::format_app_about_dialog_release_notes_version;
...
#[test]
fn format_app_about_dialog_debug_info_filename_returns_paladin_auth_debug_info_txt() {
    // Per §"libadwaita usage" and §"About / help": the
    ...
    // slug `paladin-auth` here keeps the suggested filename stable
    // even if a future `application-name` change drifts away
    // from the `paladin-auth` slug used by the CLI / executable name.
    // The `.txt` extension matches the plain-text debug-info
    // payload built by `format_app_about_dialog_debug_info`.
    use paladin_auth_gtk::app::model::format_app_about_dialog_debug_info_filename;

    assert_eq!(
        format_app_about_dialog_debug_info_filename(),
        "paladin-auth-debug-info.txt",
        "AdwAboutDialog debug-info filename pins the paladin-auth slug + .txt extension matching the plain-text payload",
    );
}
...
fn format_app_about_dialog_debug_info_is_non_empty_text_with_no_trailing_whitespace() {
    ...
    use paladin_auth_gtk::app::model::format_app_about_dialog_debug_info;
...
fn format_app_about_dialog_program_name_matches_format_app_window_title() {
    ...
    use paladin_auth_gtk::app::model::{
        format_app_about_dialog_program_name, format_app_window_title,
    };
...
fn format_app_primary_menu_entries_labels_are_distinct() {
    ...
    // not collapse multiple UI elements to identical strings without a failing test.
    use paladin_auth_gtk::app::model::format_app_primary_menu_entries;
...
fn format_app_window_action_names_are_distinct() {
    ...
    // not collapse multiple UI elements to identical strings without a failing test.
    use paladin_auth_gtk::app::model::format_app_window_action_names;
...
fn format_app_about_dialog_program_name_is_segment_of_application_icon_name() {
    ...
    // The two-word display brand ("Paladin Auth") collapses to the single
    // CamelCase app-id segment ("PaladinAuth") once whitespace is removed,
    // keeping the human display name and the reverse-DNS APP_ID tied to the
    // same brand token; a future rename must move both together.
    let program_token = program.replace(' ', "");
    assert!(
        segments.contains(&program_token.as_str()),
        "AdwAboutDialog program-name {program:?} (whitespace-stripped: {program_token:?}) must appear as a `.`-separated segment of the application-icon-name {icon:?} so the human display name and the reverse-DNS APP_ID identifier stay tied to the same brand string; if a future rename moves one, both must move together",
    );
}
...
fn format_app_about_dialog_application_icon_name_ends_with_gui_segment() {
    ...
    assert!(
        icon.ends_with(".Gui"),
        "AdwAboutDialog application-icon must end with `.Gui` to distinguish this crate's reverse-DNS Flatpak identity from a future CLI / daemon front-end sharing the `org.tamx.PaladinAuth.*` namespace; got {icon:?}",
    );
}
...
fn format_app_primary_menu_action_names_are_distinct() {
    ...
    // invariant.
    use paladin_auth_gtk::app::model::format_app_primary_menu_action_names;
...
fn format_app_about_dialog_debug_info_program_name_line_ends_with_the_version() {
    ...
    // silent expansion of the bug-report payload past its minimal `"Paladin Auth <version>"` shape.
    use paladin_auth_gtk::app::model::{
        format_app_about_dialog_debug_info, format_app_about_dialog_program_name,
        format_app_about_dialog_version,
    };
...
fn format_app_about_dialog_debug_info_filename_is_ascii_only() {
    ...
    for (idx, ch) in filename.char_indices() {
        assert!(
            ch.is_ascii(),
            "AdwAboutDialog debug_info_filename must use ASCII characters only so the file-save dialog's suggested filename round-trips through the user's filesystem byte-identically — a Unicode lookalike like `paladin-auth-debug-infо.txt` (Cyrillic `о` U+043E) would render identically but produce a silently-different on-disk filename the user cannot easily find later; got non-ASCII char {ch:?} (U+{:04X}) at byte offset {idx} in {filename:?}",
            ch as u32,
        );
    }
}
...
fn format_app_about_dialog_program_name_has_no_embedded_whitespace() {
    ...
    let program_name = format_app_about_dialog_program_name();
    assert!(
        !program_name.starts_with(' ')
            && !program_name.ends_with(' ')
            && !program_name.contains("  "),
        "AdwAboutDialog program_name must have no leading, trailing, or doubled spaces so the two-word `Paladin Auth` brand renders cleanly in the bold dialog header; got {program_name:?}",
    );
    for (idx, ch) in program_name.char_indices() {
        assert!(
            !ch.is_whitespace() || ch == ' ',
            "AdwAboutDialog program_name may use only the ASCII space (U+0020) as a word separator — guards against tabs, newlines, non-breaking spaces, and other Unicode whitespace lookalikes that would corrupt the rendered brand or mis-split downstream consumers; got whitespace char {ch:?} (U+{:04X}) at byte offset {idx} in {program_name:?}",
            ch as u32,
        );
    }
}
...
fn format_app_about_dialog_url_helpers_are_ascii_only() {
    ...
    for (label, url) in [
        ...
        for (idx, ch) in url.char_indices() {
            assert!(
                ch.is_ascii(),
                "AdwAboutDialog {label} must use ASCII characters only so the URL stays valid per RFC 3986 §2.1 without any raw non-ASCII bytes that some downstream URL-handlers (gtk_show_uri / xdg-open) may not Punycode-encode at the click site — and so the canonical Paladin Auth host cannot drift into a Unicode-lookalike homograph that resolves to a different domain than the user expects to visit; got non-ASCII char {ch:?} (U+{:04X}) at byte offset {idx} in {url:?}",
                ch as u32,
            );
        }
    }
...
fn format_app_window_title_has_no_embedded_whitespace() {
    ...
    assert!(
        ...
        "ApplicationWindow title must have no leading, trailing, or doubled spaces so the desktop window-list renders the `Paladin Auth` brand cleanly; got {title:?}",
    );
    for (idx, ch) in title.char_indices() {
        assert!(
            !ch.is_whitespace() || ch == ' ',
            "ApplicationWindow title may use only the ASCII space (U+0020) as a word separator, staying byte-consistent with the `format_app_about_dialog_program_name` slot — guards against tabs, newlines, non-breaking spaces, and Unicode whitespace lookalikes that window-switcher overlays / screenshot exporters / automation scripts splitting on whitespace would mis-route; got whitespace char {ch:?} (U+{:04X}) at byte offset {idx} in {title:?}",
            ch as u32,
        );
    }
}
...
fn format_app_primary_menu_entries_labels_start_with_an_uppercase_letter() {
    ...
    // `_returns_about_paladin_auth`, `_returns_quit`), the per-entry
    ...
    // `"Passphrase\u{2026}"`, `"Preferences"`, `"About Paladin Auth"`,
...
fn format_app_about_dialog_debug_info_filename_does_not_contain_a_backspace_byte() {
    ...
    assert!(
        ...
        "AdwAboutDialog debug_info_filename must not contain the `\\x08` backspace byte (0x08); unlike form-feed, `_has_no_embedded_whitespace` does NOT catch `\\x08` because `char::is_whitespace()` returns false for U+0008 BS (strictly weaker coverage than form-feed which the whitespace companion does catch transitively); a stray `\\x08` slips past `_is_ascii_only` / `_returns_paladin_auth_debug_info_txt` / `_does_not_contain_path_separators` / `_does_not_start_with_a_dot` / `_contains_exactly_one_period` / `_extension_is_lowercase_txt` / `_is_non_empty_single_line_with_txt_extension` (`str::lines().count() == 1` does not split on `\\x08`) and the per-byte siblings, would mis-render as a literal control glyph in the GtkFileDialog filename entry pre-fill, enable terminal-erase display-spoofing in `ls` / `find` / `tar` output piped to a terminal (the rendered listing diverges from the on-disk filename because `\\x08` erases the preceding glyph — a historical primitive for hiding malicious files behind visually-similar legitimate filenames), and confuse maintainer triage with control-glyph artifacts in chat-attachment column renders; got {debug_info_filename:?}",
    );
}
...
fn format_app_about_dialog_debug_info_filename_does_not_contain_a_bell_byte() {
    ...
    assert!(
        ...
        "AdwAboutDialog debug_info_filename must not contain the `\\x07` bell byte (0x07); like BS, BEL is NOT matched by `char::is_whitespace()` (Unicode returns false for U+0007 BEL), so `_has_no_embedded_whitespace` does NOT catch `\\x07` — strictly as dangerous as backspace here, neither caught by the whitespace companion; a stray `\\x07` slips past `_is_ascii_only` / `_returns_paladin_auth_debug_info_txt` / `_does_not_contain_path_separators` / `_does_not_start_with_a_dot` / `_contains_exactly_one_period` / `_extension_is_lowercase_txt` / `_is_non_empty_single_line_with_txt_extension` (`str::lines().count() == 1` does not split on `\\x07`) and the prior per-byte siblings, would mis-render as a literal control glyph in the GtkFileDialog filename entry pre-fill, ring the terminal bell on `ls` / `find` / `tar` output piped to a terminal (an audible-alert injection / covert side-channel primitive in shared CI environments that process the saved debug-info payload), and confuse maintainer triage with control-glyph / bell artifacts in chat-attachment column renders; got {debug_info_filename:?}",
    );
}
...
fn format_app_about_dialog_debug_info_filename_does_not_contain_an_acknowledge_byte() {
    ...
    assert!(
        ...
        "AdwAboutDialog debug_info_filename must not contain the `\\x06` acknowledge byte (0x06); like BEL and BS, ACK is NOT matched by `char::is_whitespace()` (Unicode returns false for U+0006 ACK), so `_has_no_embedded_whitespace` does NOT catch `\\x06` — strictly as dangerous as backspace and bell here, neither caught by the whitespace companion; a stray `\\x06` slips past `_is_ascii_only` / `_returns_paladin_auth_debug_info_txt` / `_does_not_contain_path_separators` / `_does_not_start_with_a_dot` / `_contains_exactly_one_period` / `_extension_is_lowercase_txt` / `_is_non_empty_single_line_with_txt_extension` (`str::lines().count() == 1` does not split on `\\x06`) and the prior per-byte siblings, would mis-render as a literal control glyph in the GtkFileDialog filename entry pre-fill, confuse serial-protocol-bridging tooling that treats `\\x06` as an ACK-frame indicator on `ls` / `find` / `tar` output piped to a serial-bridged TTY in a shared CI environment that processes the saved debug-info payload, and confuse maintainer triage with control-glyph / ACK-frame artifacts in chat-attachment column renders; got {debug_info_filename:?}",
    );
}
...
fn format_app_about_dialog_debug_info_filename_does_not_contain_an_enquiry_byte() {
    ...
    assert!(
        ...
        "AdwAboutDialog debug_info_filename must not contain the `\\x05` enquiry byte (0x05); like ACK and BEL, ENQ is NOT matched by `char::is_whitespace()` (Unicode returns false for U+0005 ENQ), so `_has_no_embedded_whitespace` does NOT catch `\\x05` — strictly as dangerous as acknowledge, backspace, and bell here, neither caught by the whitespace companion; a stray `\\x05` slips past `_is_ascii_only` / `_returns_paladin_auth_debug_info_txt` / `_does_not_contain_path_separators` / `_does_not_start_with_a_dot` / `_contains_exactly_one_period` / `_extension_is_lowercase_txt` / `_is_non_empty_single_line_with_txt_extension` (`str::lines().count() == 1` does not split on `\\x05`) and the prior per-byte siblings, would mis-render as a literal control glyph in the GtkFileDialog filename entry pre-fill, confuse serial-protocol-bridging tooling that treats `\\x05` as an ENQ-frame inquiry on `ls` / `find` / `tar` output piped to a serial-bridged TTY in a shared CI environment that processes the saved debug-info payload (eliciting an unsolicited ACK response from the receiving end), and confuse maintainer triage with control-glyph / ENQ-frame artifacts in chat-attachment column renders; got {debug_info_filename:?}",
    );
}
...
#[test]
fn format_app_style_css_resource_path_returns_org_tamx_paladin_auth_gui_style_css() {
    // Per `docs/IMPLEMENTATION_PLAN_04_GTK.md` §"Window shell and toast
    // surface", Paladin Auth-specific CSS layers on Adwaita defaults via
    ...
    use paladin_auth_gtk::app::model::format_app_style_css_resource_path;

    assert_eq!(
        format_app_style_css_resource_path(),
        "/org/tamx/PaladinAuth/Gui/style.css",
        "gresource path for the Paladin Auth-specific CSS provider stays in lockstep with `data/paladin-auth-gtk.gresource.xml` and the `wire_app_css_provider` runtime call site",
    );
}
...
fn format_app_style_css_resource_path_carries_app_id_segments() {
    // The path prefix mirrors `crate::APP_ID` (`org.tamx.PaladinAuth.Gui`)
    ...
    let path = format_app_style_css_resource_path();
    for segment in ["/org/", "/tamx/", "/PaladinAuth/", "/Gui/"] {
        assert!(
...
fn data_style_css_file_is_shipped_in_the_crate() {
    ...
    assert!(
        path.is_file(),
        "expected the bundled CSS payload at {}; ensure `crates/paladin-auth-gtk/data/style.css` is committed alongside the gresource XML per docs/IMPLEMENTATION_PLAN_04_GTK.md §\"Crate layout\"",
        path.display(),
    );
}
...
fn data_gresource_xml_is_shipped_in_the_crate() {
    // `data/paladin-auth-gtk.gresource.xml` is the source-of-truth
    ...
    let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("data/paladin-auth-gtk.gresource.xml");
    assert!(
        path.is_file(),
        "expected the gresource manifest at {}; ensure `crates/paladin-auth-gtk/data/paladin-auth-gtk.gresource.xml` is committed per docs/IMPLEMENTATION_PLAN_04_GTK.md §\"Crate layout\"",
        path.display(),
    );
}
...
fn data_gresource_xml_references_style_css_under_app_prefix() {
    ...
    // payload and the `/org/tamx/PaladinAuth/Gui` prefix consumed by
    ...
    assert!(
        xml.contains("/org/tamx/PaladinAuth/Gui"),
        "gresource manifest at {} must declare the `/org/tamx/PaladinAuth/Gui` prefix matching `crate::APP_ID`",
        path.display(),
    );
}
...
fn register_app_gresource_bundle_signature_is_zero_argument_unit() {
    ...
    // `/org/tamx/PaladinAuth/Gui/style.css` payload. Pinning the
    // signature here keeps the call site in `lib.rs::run` stable
    // (`paladin_auth_gtk::app::model::register_app_gresource_bundle()`).
    let _: fn() = paladin_auth_gtk::app::model::register_app_gresource_bundle;
}

#[test]
fn wire_app_css_provider_signature_takes_display_reference() {
    // Per `docs/IMPLEMENTATION_PLAN_04_GTK.md` §"Window shell and toast
    // surface", `wire_app_css_provider` attaches the Paladin Auth-
    ...
    let _: fn(&relm4::gtk::gdk::Display) = paladin_auth_gtk::app::model::wire_app_css_provider;
}
...
#[test]
fn format_app_icon_theme_resource_path_returns_org_tamx_paladin_auth_gui_icons() {
    ...
    use paladin_auth_gtk::app::model::format_app_icon_theme_resource_path;

    assert_eq!(
        format_app_icon_theme_resource_path(),
        "/org/tamx/PaladinAuth/Gui/icons",
        "gresource path for the Paladin Auth-bundled icon theme stays in lockstep with `data/paladin-auth-gtk.gresource.xml` and the `wire_app_icon_theme_resource_path` runtime call site",
    );
}
...
fn format_app_placeholder_icon_resource_path_ends_with_dialog_password_symbolic_svg() {
    ...
    use paladin_auth_gtk::app::model::format_app_placeholder_icon_resource_path;
...
fn data_gresource_xml_references_placeholder_icon_under_scalable_actions() {
    ...
    // and the `/org/tamx/PaladinAuth/Gui` prefix consumed by
...
fn wire_app_icon_theme_resource_path_signature_takes_display_reference() {
    ...
    let _: fn(&relm4::gtk::gdk::Display) =
        paladin_auth_gtk::app::model::wire_app_icon_theme_resource_path;
}
...
fn format_app_toast_overlay_widget_name_returns_toast_overlay() {
    ...
    // truth.
    use paladin_auth_gtk::app::model::format_app_toast_overlay_widget_name;
