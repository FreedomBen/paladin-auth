...
//! Pure-logic top-level `AppState` tests for `paladin-auth-gtk`.
//!
...
//! `paladin_auth_gtk::app::state` shadows this lifecycle:
//!
//! * Startup runs `paladin_auth_core::default_vault_path()` then
//!   `paladin_auth_core::inspect(path)`; the resolution / inspect
//!   outcome routes through [`paladin_auth_gtk::app::state::decide_state_from_path_resolution`]
//!   and [`paladin_auth_gtk::app::state::decide_state_from_inspect`].
...
//!   [`paladin_auth_gtk::app::state::decide_state_from_open_error`] and
...
use paladin_auth_core::{
    format_unsafe_permissions, AccountEdit, AccountId, ErrorKind, PaladinAuthError,
    PermissionSubject, Store, Vault, VaultLock, VaultMode, VaultStatus,
};

use paladin_auth_gtk::add_account::{AddWorkerInput, QrWorkerInput};
use paladin_auth_gtk::app::state::{
...
use paladin_auth_gtk::edit_dialog::{EditWorkerEffect, EditWorkerInput};
use paladin_auth_gtk::startup_error::StartupErrorSource;
use paladin_auth_gtk::unlock_dialog::{
...
fn vault_path() -> PathBuf {
    PathBuf::from("/home/test/.local/share/paladin-auth/vault.bin")
}

fn unsafe_perms_err() -> PaladinAuthError {
...
fn invalid_header_err() -> PaladinAuthError {
    PaladinAuthError::InvalidHeader
}

fn wrong_vault_lock_err() -> PaladinAuthError {
...
fn invalid_payload_err() -> PaladinAuthError {
    PaladinAuthError::InvalidPayload {
        reason: "trailing_bytes",
    }
}

fn unsupported_format_version_err() -> PaladinAuthError {
    PaladinAuthError::UnsupportedFormatVersion { format_ver: 99 }
}

fn kdf_oob_err() -> PaladinAuthError {
...
fn path_resolution_io_err() -> PaladinAuthError {
    PaladinAuthError::IoError {
...
fn decrypt_failed_err() -> PaladinAuthError {
    PaladinAuthError::DecryptFailed
}

fn invalid_passphrase_empty_err() -> PaladinAuthError {
...
fn locked_enters_unlocking_busy_and_preserves_path() {
    // The `gio::spawn_blocking paladin_auth_core::open` worker takes the
...
fn unlocked_busy_leaves_unlocking_busy_to_locked_and_preserves_path() {
    // The `gio::spawn_blocking paladin_auth_core::open` worker can return
...
// `paladin_auth_core::open` failure by attaching the resolved vault path that
...
fn decide_unlock_failure_action_attaches_caller_provided_path() {
    ...
    let alt = PathBuf::from("/var/lib/paladin-auth/alt-vault.bin");
...
fn apply_unlock_failure_action_send_inline_to_dialog_translates_to_send_unlock_dialog_msg() {
    ...
    let inline = InlineError::from_error(&PaladinAuthError::DecryptFailed);
...
fn apply_unlock_failure_action_send_inline_invalid_passphrase_preserves_kind_and_text() {
    ...
    // pre-KDF `invalid_passphrase` rejection from `paladin_auth_core::open`
    // routes through with its stable §5 `reason` discriminator and
    // `Display` text preserved.
    let err = PaladinAuthError::InvalidPassphrase {
        reason: "zero_length",
    };
...
fn apply_unlock_failure_action_round_trip_decrypt_failed_dispatches_inline() {
    ...
    // discriminator and matches `PaladinAuthError::DecryptFailed`'s
...
// `PaladinAuthError` to the concrete `UnlockFailureEffect` in one call
...
fn decide_unlock_success_state_returns_unlocked_variant() {
    ...
    // `AppModel`'s future `gio::spawn_blocking paladin_auth_core::open`
...
// paladin_auth_core::open` worker callback to fan out into the success or
...
fn route_unlock_worker_outcome_ok_returns_success_set_app_state_unlocked() {
    // `Ok(())` represents the `gio::spawn_blocking paladin_auth_core::open`
...
fn route_unlock_worker_outcome_invalid_passphrase_returns_failure_send_unlock_dialog_msg() {
    ...
    // `paladin_auth_core::open`. Same inline-routing rule as
...
fn route_unlock_worker_outcome_unsafe_permissions_returns_failure_set_app_state_startup() {
    // Non-passphrase `paladin_auth_core::open` failures drop the live
    ...
    // the formatter-built text per `paladin_auth_core::format_unsafe_permissions`.
    let path = vault_path();
    let err = unsafe_perms_err();
    let effect = route_unlock_worker_outcome(&path, Err(&err));
    match effect {
...
//
// The `gio::spawn_blocking paladin_auth_core::open` worker returns
// `Result<(Vault, Store), PaladinAuthError>`. Bundling that outcome into a
...
fn route_unlock_open_completion_ok_carries_pair_and_success_effect() {
    ...
    let completion = paladin_auth_gtk::app::state::route_unlock_open_completion(&path, Ok(pair));
...
fn route_unlock_open_completion_err_decrypt_failed_carries_inline_effect_no_pair() {
    ...
    let completion = paladin_auth_gtk::app::state::route_unlock_open_completion(&path, Err(err));
...
fn route_unlock_open_completion_err_routing_matches_route_unlock_worker_outcome() {
    ...
    for err in [
        ...
        let completion =
            paladin_auth_gtk::app::state::route_unlock_open_completion(&path, Err(err));
        assert_eq!(
            format!("{direct:?}"),
...
fn route_unlock_open_completion_attaches_caller_provided_path_on_success() {
    ...
    let completion = paladin_auth_gtk::app::state::route_unlock_open_completion(&alt, Ok(pair));
...
fn route_unlock_open_completion_attaches_caller_provided_path_on_failure() {
    ...
    let completion = paladin_auth_gtk::app::state::route_unlock_open_completion(&alt, Err(err));
...
fn fresh_plaintext_vault_pair() -> (
    ...
    let pair = paladin_auth_core::Store::create(&path, paladin_auth_core::VaultInit::Plaintext)
...
fn should_drop_unlock_dialog_after_success_returns_true() {
    ...
    let effect = route_unlock_worker_outcome(&path, Ok(()));
    assert!(
        paladin_auth_gtk::app::state::should_drop_unlock_dialog_after(&effect),
...
fn should_drop_unlock_dialog_after_inspects_effect_only_not_path() {
    ...
    let path_b = PathBuf::from("/var/lib/paladin-auth/alt-vault.bin");
    let err = decrypt_failed_err();
    let effect_a = route_unlock_worker_outcome(&path_a, Err(&err));
    let effect_b = route_unlock_worker_outcome(&path_b, Err(&err));
    assert_eq!(
        paladin_auth_gtk::app::state::should_drop_unlock_dialog_after(&effect_a),
        paladin_auth_gtk::app::state::should_drop_unlock_dialog_after(&effect_b),
...
fn should_drop_unlock_dialog_after_partitions_inline_vs_non_inline() {
    ...
    let drops: Vec<bool> = [&success, &inline_a, &inline_b, &startup_a, &startup_b]
        .iter()
        .map(|effect| paladin_auth_gtk::app::state::should_drop_unlock_dialog_after(effect))
        .collect();
...
fn unlock_dialog_msg_after_decrypt_failed_returns_open_failed_inline() {
    ...
    // `kind` / `rendered` fields match the typed `PaladinAuthError`
    ...
    match unlock_dialog_msg_after(&effect) {
        Some(UnlockDialogMsg::OpenFailedInline(inline)) => {
            assert_eq!(
                inline.kind,
                err.kind(),
                "inline error kind should match the typed PaladinAuthError"
            );
            assert_eq!(
                inline.rendered,
                err.to_string(),
...
fn unlock_app_state_after_unsafe_permissions_returns_startup_error_with_formatter_text() {
    ...
    // matches `paladin_auth_core::format_unsafe_permissions`. Pin both the
...
fn submit_unlock_app_state_from_locked_returns_unlocked_busy_preserving_path() {
    ...
    // busy gate for the `gio::spawn_blocking paladin_auth_core::open`
...
// the `gio::spawn_blocking paladin_auth_core::open` worker,
...
fn fresh_plaintext_pair() -> (tempfile::TempDir, PathBuf, Vault, Store) {
    ...
    let (vault, store) =
        paladin_auth_core::Store::open(&path, paladin_auth_core::VaultLock::Plaintext)
...
/// pipeline.
fn fresh_add_account() -> paladin_auth_core::Account {
    use paladin_auth_core::{
        validate_manual, AccountInput, AccountKindInput, Algorithm, IconHintInput,
    };
...
// `paladin_auth_core::import::qr_image_bytes` and merges them under
...
/// `validate_manual(AccountInput { ... })` shape the
/// `paladin_auth_core::import::qr_image_bytes` decoded-payload pipeline ends in
...
/// ever changes.
fn fresh_qr_validated_accounts() -> Vec<paladin_auth_core::ValidatedAccount> {
    use paladin_auth_core::{
        validate_manual, AccountInput, AccountKindInput, Algorithm, IconHintInput,
    };
    use secrecy::SecretString;
...
fn qr_final_app_state_success_rolls_back_to_unlocked_preserving_path() {
    use paladin_auth_core::ImportReport;
    use paladin_auth_gtk::add_account::QrWorkerEffect;
    use paladin_auth_gtk::app::state::qr_final_app_state;
...
fn qr_final_app_state_failure_inline_rolls_back_to_unlocked_preserving_path() {
    use paladin_auth_gtk::add_account::{
    ...
    let path = vault_path();
    let busy = AppState::UnlockedBusy { path: path.clone() };
    let err = PaladinAuthError::SaveNotCommitted {
        committed: false,
        backup_path: None,
    };
...
fn qr_final_app_state_failure_defensive_inline_rolls_back_to_unlocked_preserving_path() {
    ...
    let path = vault_path();
    let busy = AppState::UnlockedBusy { path: path.clone() };
    let err = PaladinAuthError::InvalidState {
        operation: "import",
...
fn qr_final_app_state_from_non_unlocked_busy_returns_none() {
    ...
    use paladin_auth_core::ImportReport;
    use paladin_auth_gtk::add_account::{classify_add_post_effect_error, QrWorkerEffect};
    use paladin_auth_gtk::app::state::qr_final_app_state;
...
fn qr_final_app_state_agrees_with_add_final_app_state_for_failure_branches() {
    ...
    use paladin_auth_gtk::app::state::{add_final_app_state, qr_final_app_state};
...
fn should_drop_add_dialog_after_qr_returns_false_on_success() {
    use paladin_auth_core::ImportReport;
    use paladin_auth_gtk::add_account::QrWorkerEffect;
    use paladin_auth_gtk::app::state::should_drop_add_dialog_after_qr;
...
fn should_drop_add_dialog_after_qr_returns_false_on_failure_inline() {
    ...
    let outcome = classify_add_post_effect_error(&PaladinAuthError::SaveNotCommitted {
        committed: false,
        backup_path: None,
    });
    assert!(
...
fn should_drop_add_dialog_after_qr_returns_false_on_failure_keep_with_warning() {
    ...
    let outcome = classify_add_post_effect_error(&PaladinAuthError::SaveDurabilityUnconfirmed);
    assert!(
        matches!(outcome, AddPostEffectOutcome::KeepWithWarning(_)),
        "save_durability_unconfirmed routes to KeepWithWarning",
    );
...
fn should_drop_add_dialog_after_qr_returns_false_for_defensive_inline() {
    ...
    let outcome = classify_add_post_effect_error(&PaladinAuthError::InvalidState {
        operation: "import",
        state: "account_not_found",
    });
    assert!(
...
fn should_drop_add_dialog_after_qr_diverges_from_add_on_success() {
    ...
    // panel.
    use paladin_auth_core::{AccountId, ImportReport};
    use paladin_auth_gtk::add_account::{AddWorkerEffect, QrWorkerEffect};
    use paladin_auth_gtk::app::state::{
        should_drop_add_dialog_after, should_drop_add_dialog_after_qr,
    };
...
fn should_refresh_list_after_qr_returns_true_on_success() {
    use paladin_auth_core::ImportReport;
    use paladin_auth_gtk::add_account::QrWorkerEffect;
    use paladin_auth_gtk::app::state::should_refresh_list_after_qr;
...
fn should_refresh_list_after_qr_mirrors_add_for_every_shared_branch() {
    ...
    use paladin_auth_gtk::app::state::{
        should_refresh_list_after_add, should_refresh_list_after_qr,
    };
...
fn qr_dialog_msg_after_success_returns_qr_success_with_summary_from_report() {
    use paladin_auth_core::{AccountId, ImportReport};
    use paladin_auth_gtk::add_account::{AddAccountMsg, QrWorkerEffect};
    use paladin_auth_gtk::app::state::qr_dialog_msg_after;
    use paladin_auth_gtk::qr_clipboard::QrImportSummary;
...
fn qr_dialog_msg_after_diverges_from_add_on_success() {
    ...
    use paladin_auth_core::{AccountId, ImportReport};
    use paladin_auth_gtk::add_account::{AddAccountMsg, AddWorkerEffect, QrWorkerEffect};
    use paladin_auth_gtk::app::state::{add_dialog_msg_after, qr_dialog_msg_after};
...
fn qr_dialog_msg_after_drop_dialog_partition_inverts_add_on_success() {
    ...
    use paladin_auth_core::ImportReport;
    use paladin_auth_gtk::add_account::QrWorkerEffect;
    use paladin_auth_gtk::app::state::{qr_dialog_msg_after, should_drop_add_dialog_after_qr};
...
fn compose_qr_dispatch_success_carries_qr_success_keep_mounted_and_refresh() {
    use paladin_auth_core::ImportReport;
    use paladin_auth_gtk::add_account::{AddAccountMsg, QrWorkerEffect};
    use paladin_auth_gtk::app::state::compose_qr_dispatch;
    use paladin_auth_gtk::qr_clipboard::QrImportSummary;
    ...
    let report = ImportReport {
        ...
        appended: 0,
        accounts: vec![
            paladin_auth_core::AccountId::new(),
            paladin_auth_core::AccountId::new(),
        ],
        warnings: Vec::new(),
    };
...
fn apply_submit_unlock_inplace_from_locked_mutates_to_unlocked_busy_and_returns_true() {
    ...
    // `gio::spawn_blocking paladin_auth_core::open` worker — the `false`
...
fn apply_add_vault_install_inplace_writes_pair_into_empty_slot() {
    ...
    let mut slot: Option<(paladin_auth_core::Vault, paladin_auth_core::Store)> = None;
...
fn apply_add_vault_install_inplace_consumes_run_add_worker_completion_pair() {
    ...
    let completion = paladin_auth_gtk::add_account::run_add_worker(worker_input);
...
fn add_final_app_state_success_rolls_back_to_unlocked_preserving_path() {
    ...
    use paladin_auth_gtk::add_account::AddWorkerEffect;
    use paladin_auth_gtk::app::state::add_final_app_state;
...
fn add_final_app_state_from_non_unlocked_busy_returns_none() {
    ...
    // the defensive arm cannot drift with the effect routing.
    use paladin_auth_core::AccountId;
    use paladin_auth_gtk::add_account::{classify_add_post_effect_error, AddWorkerEffect};
    use paladin_auth_gtk::app::state::add_final_app_state;
...
// paladin_auth_core::open` worker closure. Both composers inspect `current`
...
fn compose_unlock_worker_input_mirrors_submit_unlock_app_state_gating() {
    ...
    // `gio::spawn_blocking paladin_auth_core::open` worker spawn: if one
...
// ---------------------------------------------------------------------------
//
// `route_unlock_open_completion(&Path, Result<(Vault, Store), PaladinAuthError>)
...
fn apply_unlock_vault_install_inplace_mirrors_route_unlock_open_completion_pair_partition() {
    ...
    let completion = paladin_auth_gtk::app::state::route_unlock_open_completion(&ok_path, Ok(pair));
    let mut slot = None;
...
// paladin_auth_core::open` worker fired by `AppModel::update` from
...
/// same shape is used by `tests/gtk_smoke.rs` and the paladin-auth-tui
/// test fixtures.
fn light_argon2_params() -> paladin_auth_core::Argon2Params {
...
fn persist_fresh_encrypted_vault(passphrase: &str) -> (tempfile::TempDir, PathBuf) {
    ...
    let path = tempdir.path().join("vault.bin");
    {
        ...
        let opts = paladin_auth_core::EncryptionOptions::with_params(pp, light_argon2_params())
            .expect("encryption options accept light Argon2 params");
        let (vault, store) =
            paladin_auth_core::Store::create(&path, paladin_auth_core::VaultInit::Encrypted(opts))
...
fn submit_remove_app_state_from_unlocked_returns_unlocked_busy_preserving_path() {
    use paladin_auth_gtk::app::state::submit_remove_app_state;
    let path = vault_path();
    let unlocked = AppState::Unlocked { path: path.clone() };
...
fn compose_remove_worker_input_from_unlocked_bundles_pair_and_account_id() {
    use paladin_auth_gtk::app::state::compose_remove_worker_input;
    use paladin_auth_gtk::remove_dialog::RemoveWorkerInput;
...
fn apply_submit_remove_inplace_from_unlocked_mutates_to_unlocked_busy_and_returns_true() {
    use paladin_auth_gtk::app::state::apply_submit_remove_inplace;
    let path = vault_path();
    let mut state = AppState::Unlocked { path: path.clone() };
...
fn remove_final_app_state_success_rolls_back_to_unlocked_preserving_path() {
    use paladin_auth_gtk::app::state::remove_final_app_state;
    use paladin_auth_gtk::remove_dialog::RemoveWorkerEffect;
    let path = vault_path();
    let busy = AppState::UnlockedBusy { path: path.clone() };
...
fn remove_final_app_state_failure_rolls_back_to_unlocked_for_every_outcome() {
    use paladin_auth_gtk::app::state::remove_final_app_state;
    use paladin_auth_gtk::remove_dialog::{
        account_not_found_error, classify_remove_error, RemoveWorkerEffect,
    };
    let path = vault_path();
    let busy = AppState::UnlockedBusy { path: path.clone() };
    let effects = [
        RemoveWorkerEffect::Failure(classify_remove_error(&PaladinAuthError::SaveNotCommitted {
            committed: false,
            backup_path: None,
        })),
...
fn should_drop_remove_dialog_after_success_returns_true() {
    use paladin_auth_gtk::app::state::should_drop_remove_dialog_after;
    use paladin_auth_gtk::remove_dialog::RemoveWorkerEffect;
...
fn remove_dialog_msg_after_success_returns_none() {
    use paladin_auth_gtk::app::state::remove_dialog_msg_after;
    use paladin_auth_gtk::remove_dialog::RemoveWorkerEffect;
...
fn remove_dialog_msg_after_failure_forwards_worker_failed() {
    ...
    let outcome = classify_remove_error(&PaladinAuthError::SaveNotCommitted {
        committed: false,
        backup_path: None,
    });
...
fn remove_dialog_msg_after_is_mutually_exclusive_with_should_drop() {
    use paladin_auth_gtk::app::state::{remove_dialog_msg_after, should_drop_remove_dialog_after};
    use paladin_auth_gtk::remove_dialog::{
        account_not_found_error, classify_remove_error, RemoveWorkerEffect,
    };
    let effects = [
...
fn should_refresh_list_after_remove_success_returns_true() {
    use paladin_auth_gtk::app::state::should_refresh_list_after_remove;
    use paladin_auth_gtk::remove_dialog::RemoveWorkerEffect;
...
fn should_refresh_list_after_remove_failure_keep_removed_with_warning_returns_true() {
    ...
    let outcome = classify_remove_error(&PaladinAuthError::SaveDurabilityUnconfirmed);
    assert!(matches!(
        outcome,
...
fn compose_remove_dispatch_success_bundles_drop_and_unlocked_rollback() {
    use paladin_auth_gtk::app::state::compose_remove_dispatch;
    use paladin_auth_gtk::remove_dialog::RemoveWorkerEffect;
    let path = vault_path();
    let busy = AppState::UnlockedBusy { path: path.clone() };
...
fn remove_success_toast_after_success_returns_body() {
    use paladin_auth_gtk::app::state::remove_success_toast_after;
    use paladin_auth_gtk::remove_dialog::{format_remove_dialog_success_toast, RemoveWorkerEffect};
...
fn compose_remove_dispatch_populates_success_toast_only_on_success() {
    ...
    use paladin_auth_gtk::app::state::{compose_remove_dispatch, remove_success_toast_after};
    use paladin_auth_gtk::remove_dialog::{
        account_not_found_error, classify_remove_error, RemoveWorkerEffect,
    };
...
fn apply_remove_dispatch_inplace_success_rolls_back_to_unlocked_and_returns_true() {
    use paladin_auth_gtk::app::state::{apply_remove_dispatch_inplace, compose_remove_dispatch};
    use paladin_auth_gtk::remove_dialog::RemoveWorkerEffect;
    let path = vault_path();
    let busy = AppState::UnlockedBusy { path: path.clone() };
    let dispatch = compose_remove_dispatch(&busy, &RemoveWorkerEffect::Success);
...
fn apply_remove_vault_install_inplace_writes_pair_into_empty_slot() {
    use paladin_auth_gtk::app::state::apply_remove_vault_install_inplace;
...
fn should_drop_add_dialog_after_success_returns_true() {
    use paladin_auth_core::AccountId;
    use paladin_auth_gtk::add_account::AddWorkerEffect;
    use paladin_auth_gtk::app::state::should_drop_add_dialog_after;
...
fn should_refresh_list_after_add_success_returns_true() {
    use paladin_auth_core::AccountId;
    use paladin_auth_gtk::add_account::AddWorkerEffect;
    use paladin_auth_gtk::app::state::should_refresh_list_after_add;
...
//   routing off the `PaladinAuthError`.
//
...
fn add_dialog_msg_after_success_returns_none() {
    use paladin_auth_core::AccountId;
    use paladin_auth_gtk::add_account::AddWorkerEffect;
    use paladin_auth_gtk::app::state::add_dialog_msg_after;
...
fn add_dialog_msg_after_is_mutually_exclusive_with_should_drop() {
    ...
    use paladin_auth_core::AccountId;
    use paladin_auth_gtk::add_account::{classify_add_post_effect_error, AddWorkerEffect};
    use paladin_auth_gtk::app::state::{add_dialog_msg_after, should_drop_add_dialog_after};
...
fn add_success_toast_after_success_returns_body() {
    use paladin_auth_gtk::add_account::{format_add_dialog_success_toast, AddWorkerEffect};
    use paladin_auth_gtk::app::state::add_success_toast_after;
...
fn compose_add_dispatch_populates_success_toast_only_on_success() {
    ...
    use paladin_auth_gtk::add_account::{classify_add_post_effect_error, AddWorkerEffect};
    use paladin_auth_gtk::app::state::{add_success_toast_after, compose_add_dispatch};
...
fn compose_add_dispatch_success_bundles_drop_and_unlocked_rollback() {
    use paladin_auth_gtk::add_account::AddWorkerEffect;
    use paladin_auth_gtk::app::state::compose_add_dispatch;
...
fn apply_add_dispatch_inplace_success_rolls_back_to_unlocked_and_returns_true() {
    ...
    use paladin_auth_gtk::add_account::AddWorkerEffect;
    use paladin_auth_gtk::app::state::{apply_add_dispatch_inplace, compose_add_dispatch};
...
fn add_pipeline_success_returns_to_unlocked_with_new_account_and_drops_dialog() {
    ...
    // no inline message.
    use paladin_auth_gtk::add_account::{run_add_worker, AddWorkerCompletion, AddWorkerEffect};
    use paladin_auth_gtk::app::state::{
...
fn apply_qr_dispatch_inplace_success_rolls_back_to_unlocked_and_returns_true() {
    ...
    use paladin_auth_core::ImportReport;
    use paladin_auth_gtk::add_account::QrWorkerEffect;
    use paladin_auth_gtk::app::state::{apply_qr_dispatch_inplace, compose_qr_dispatch};
...
fn fresh_qr_batch() -> Vec<paladin_auth_core::ValidatedAccount> {
    use paladin_auth_core::{
        validate_manual, AccountInput, AccountKindInput, Algorithm, IconHintInput,
    };
    use secrecy::SecretString;
...
fn submit_import_app_state_from_unlocked_returns_unlocked_busy_preserving_path() {
    use paladin_auth_gtk::app::state::submit_import_app_state;
    let path = vault_path();
    let unlocked = AppState::Unlocked { path: path.clone() };
    let next =
...
fn apply_submit_import_inplace_from_unlocked_mutates_to_unlocked_busy_and_returns_true() {
    use paladin_auth_gtk::app::state::apply_submit_import_inplace;
    let path = vault_path();
    let mut state = AppState::Unlocked { path: path.clone() };
...
fn compose_import_worker_input_from_unlocked_bundles_pair_payload_and_import_time() {
    use paladin_auth_core::ImportFormat;
    use paladin_auth_gtk::app::state::compose_import_worker_input;
    ...
    let payload = ImportSubmitPayload {
        ...
        options: build_import_options(paladin_auth_gtk::import_dialog::FormatChoice::Otpauth, None),
        conflict: paladin_auth_core::ImportConflict::Skip,
    };
    ...
    assert_eq!(bundled.conflict, paladin_auth_core::ImportConflict::Skip);
    assert_eq!(
...
fn compose_import_worker_input_from_non_unlocked_returns_pair_back() {
    use paladin_auth_gtk::app::state::compose_import_worker_input;
    use paladin_auth_gtk::import_dialog::{build_import_options, ImportSubmitPayload};
    ...
    for source in &sources {
        ...
        let payload = ImportSubmitPayload {
            ...
            options: build_import_options(
                paladin_auth_gtk::import_dialog::FormatChoice::AutoDetect,
                None,
            ),
            conflict: paladin_auth_core::ImportConflict::Skip,
        };
...
fn import_report_success(imported: usize) -> paladin_auth_core::ImportReport {
    paladin_auth_core::ImportReport {
        imported,
        ..paladin_auth_core::ImportReport::default()
    }
}

#[test]
fn import_final_app_state_success_rolls_back_to_unlocked_preserving_path() {
    use paladin_auth_gtk::app::state::import_final_app_state;
    use paladin_auth_gtk::import_dialog::classify_merge_result;
    let path = vault_path();
    let busy = AppState::UnlockedBusy { path: path.clone() };
...
fn import_final_app_state_failure_rolls_back_to_unlocked_for_every_outcome() {
    ...
    let busy = AppState::UnlockedBusy { path: path.clone() };
    let outcomes = [
        classify_merge_result(Err(PaladinAuthError::SaveNotCommitted {
            committed: false,
            backup_path: None,
        })),
        classify_merge_result(Err(PaladinAuthError::SaveDurabilityUnconfirmed)),
        classify_merge_result(Err(PaladinAuthError::NoEntriesToImport)),
    ];
...
fn should_drop_import_dialog_after_is_always_false() {
    use paladin_auth_gtk::app::state::should_drop_import_dialog_after;
    use paladin_auth_gtk::import_dialog::classify_merge_result;
    let outcomes = [
        ...
        classify_merge_result(Err(PaladinAuthError::SaveDurabilityUnconfirmed)),
        classify_merge_result(Err(PaladinAuthError::NoEntriesToImport)),
        classify_merge_result(Err(PaladinAuthError::InvalidHeader)),
    ];
    for outcome in &outcomes {
        assert!(
...
fn import_dialog_msg_after_always_forwards_worker_completed() {
    use paladin_auth_gtk::app::state::import_dialog_msg_after;
    use paladin_auth_gtk::import_dialog::{classify_merge_result, ImportDialogMsg, MergeOutcome};
    let outcomes = [
...
fn should_refresh_list_after_import_partitions_on_committed_outcomes() {
    use paladin_auth_gtk::app::state::should_refresh_list_after_import;
    use paladin_auth_gtk::import_dialog::classify_merge_result;
    let refresh = [
    ...
    let skip = [
        ...
        classify_merge_result(Err(PaladinAuthError::NoEntriesToImport)),
        classify_merge_result(Err(PaladinAuthError::InvalidHeader)),
        classify_merge_result(Err(PaladinAuthError::DecryptFailed)),
    ];
...
fn compose_import_dispatch_success_bundles_dialog_msg_and_unlocked_rollback() {
    use paladin_auth_gtk::app::state::compose_import_dispatch;
    use paladin_auth_gtk::import_dialog::{classify_merge_result, ImportDialogMsg, MergeOutcome};
    let path = vault_path();
    let busy = AppState::UnlockedBusy { path: path.clone() };
    let outcome = classify_merge_result(Ok(import_report_success(3)));
...
fn compose_import_dispatch_failure_keeps_dialog_with_msg_and_unlocked_rollback() {
    ...
    let path = vault_path();
    let busy = AppState::UnlockedBusy { path: path.clone() };
    let outcome = classify_merge_result(Err(PaladinAuthError::SaveNotCommitted {
        committed: false,
        backup_path: None,
    }));
    let dispatch = compose_import_dispatch(&busy, &outcome);
...
fn apply_import_dispatch_inplace_success_rolls_back_to_unlocked_and_returns_true() {
    use paladin_auth_gtk::app::state::{apply_import_dispatch_inplace, compose_import_dispatch};
    use paladin_auth_gtk::import_dialog::classify_merge_result;
    let path = vault_path();
    let busy = AppState::UnlockedBusy { path: path.clone() };
...
fn apply_import_vault_install_inplace_writes_pair_into_empty_slot() {
    use paladin_auth_gtk::app::state::apply_import_vault_install_inplace;
    let (_tempdir, _path, vault, store) = fresh_plaintext_pair();
    let mut slot: Option<(Vault, Store)> = None;
...
fn submit_passphrase_app_state_from_unlocked_returns_unlocked_busy_preserving_path() {
    use paladin_auth_gtk::app::state::submit_passphrase_app_state;
    let path = vault_path();
    let unlocked = AppState::Unlocked { path: path.clone() };
...
fn apply_submit_passphrase_inplace_from_unlocked_mutates_to_unlocked_busy_and_returns_true() {
    use paladin_auth_gtk::app::state::apply_submit_passphrase_inplace;
    let path = vault_path();
    let mut state = AppState::Unlocked { path: path.clone() };
...
fn passphrase_remove_payload() -> paladin_auth_gtk::passphrase_dialog::SubmitPayload {
    // `Remove` carries no secret payload — the simplest fixture for
    // the composer tests since it does not need a real
    // `EncryptionOptions`.
    paladin_auth_gtk::passphrase_dialog::SubmitPayload::Remove
}

#[test]
fn compose_passphrase_worker_input_from_unlocked_bundles_pair_and_payload() {
    use paladin_auth_gtk::app::state::compose_passphrase_worker_input;
    let (_tempdir, path, vault, store) = fresh_plaintext_pair();
    let unlocked = AppState::Unlocked { path: path.clone() };
...
fn compose_passphrase_dispatch_success_drops_dialog_and_rolls_busy_back() {
    use paladin_auth_gtk::app::state::compose_passphrase_dispatch;
    use paladin_auth_gtk::passphrase_dialog::{PassphraseWorkerEffect, SubFlow};
...
fn compose_passphrase_dispatch_failure_keeps_dialog_with_worker_failed_msg() {
    use paladin_auth_gtk::app::state::compose_passphrase_dispatch;
    use paladin_auth_gtk::passphrase_dialog::{
        classify_passphrase_error, PassphraseDialogMsg, PassphraseWorkerEffect,
    };

    let path = vault_path();
    let busy = AppState::UnlockedBusy { path: path.clone() };
    let err = paladin_auth_core::PaladinAuthError::SaveNotCommitted {
        committed: false,
        backup_path: None,
    };
...
fn apply_passphrase_dispatch_inplace_applies_unlocked_rollback() {
    use paladin_auth_gtk::app::state::{
        apply_passphrase_dispatch_inplace, compose_passphrase_dispatch,
    };
    use paladin_auth_gtk::passphrase_dialog::{PassphraseWorkerEffect, SubFlow};
...
fn apply_passphrase_vault_install_inplace_writes_pair_into_empty_slot() {
    use paladin_auth_gtk::app::state::apply_passphrase_vault_install_inplace;
    let (_tempdir, _path, vault, store) = fresh_plaintext_pair();
    let mut slot: Option<(Vault, Store)> = None;
...
fn passphrase_new_is_encrypted_after_success_set_returns_some_true() {
    use paladin_auth_gtk::app::state::passphrase_new_is_encrypted_after;
    use paladin_auth_gtk::passphrase_dialog::{PassphraseWorkerEffect, SubFlow};
...
fn passphrase_new_is_encrypted_after_failure_returns_none() {
    use paladin_auth_gtk::app::state::passphrase_new_is_encrypted_after;
    use paladin_auth_gtk::passphrase_dialog::{classify_passphrase_error, PassphraseWorkerEffect};
...
fn compose_passphrase_dispatch_failure_projects_no_new_is_encrypted() {
    ...
    let path = vault_path();
    let busy = AppState::UnlockedBusy { path };
    let err = paladin_auth_core::PaladinAuthError::SaveDurabilityUnconfirmed;
    let outcome = classify_passphrase_error(&err);
    let effect = PassphraseWorkerEffect::Failure(outcome);
    let dispatch = compose_passphrase_dispatch(&busy, &effect);
    assert_eq!(
...
// `paladin_auth_core::policy::auto_lock::IdlePolicy::should_arm` after a
...
fn passphrase_should_arm_idle_after_success_encrypted_consults_idle_policy() {
    use paladin_auth_core::policy::auto_lock::IdlePolicy;
    use paladin_auth_gtk::app::state::passphrase_should_arm_idle_after;
    use paladin_auth_gtk::passphrase_dialog::{PassphraseWorkerEffect, SubFlow};
...
fn submit_settings_app_state_from_unlocked_returns_unlocked_busy_preserving_path() {
    use paladin_auth_gtk::app::state::submit_settings_app_state;
    let path = vault_path();
    let unlocked = AppState::Unlocked { path: path.clone() };
...
fn apply_submit_settings_inplace_from_unlocked_mutates_to_unlocked_busy_and_returns_true() {
    use paladin_auth_gtk::app::state::apply_submit_settings_inplace;
    let path = vault_path();
    let mut state = AppState::Unlocked { path: path.clone() };
...
fn compose_settings_worker_input_from_unlocked_bundles_pair_and_patch() {
    use paladin_auth_gtk::app::state::compose_settings_worker_input;
    let (_tempdir, path, vault, store) = fresh_plaintext_pair();
    let unlocked = AppState::Unlocked { path: path.clone() };
    let patch = paladin_auth_core::SettingPatch::AutoLockEnabled(true);
    ...
    assert_eq!(
        input.patch,
        paladin_auth_core::SettingPatch::AutoLockEnabled(true)
    );
    assert_eq!(input.vault.summaries().count(), 0);
}
...
fn apply_settings_vault_install_inplace_writes_pair_into_empty_slot() {
    use paladin_auth_gtk::app::state::apply_settings_vault_install_inplace;
    let (_tempdir, _path, vault, store) = fresh_plaintext_pair();
    let mut slot: Option<(Vault, Store)> = None;
...
fn settings_effect_success_auto_lock_enabled(
    value: bool,
) -> paladin_auth_gtk::settings::SettingsWorkerEffect {
    paladin_auth_gtk::settings::SettingsWorkerEffect {
        change: paladin_auth_gtk::settings::AcceptedChange::AutoLockEnabled(value),
        outcome: paladin_auth_gtk::settings::SaveOutcome::Success,
    }
}
...
fn settings_effect_success_clipboard_clear_enabled(
    ...
    paladin_auth_gtk::settings::SettingsWorkerEffect {
        change: paladin_auth_gtk::settings::AcceptedChange::ClipboardClearEnabled(value),
        outcome: paladin_auth_gtk::settings::SaveOutcome::Success,
    }
}
...
fn settings_effect_rollback_auto_lock_secs(
    ...
    use paladin_auth_gtk::settings::InlineError;
    let err = paladin_auth_core::PaladinAuthError::SaveNotCommitted {
        committed: false,
        backup_path: None,
    };
    paladin_auth_gtk::settings::SettingsWorkerEffect {
        change: paladin_auth_gtk::settings::AcceptedChange::AutoLockSecs(value),
        outcome: paladin_auth_gtk::settings::SaveOutcome::Rollback {
            error: InlineError::from_error(&err),
            field: paladin_auth_gtk::settings::SettingsField::AutoLockSecs,
        },
    }
}
...
fn settings_effect_durability_warning_auto_lock_enabled(
    ...
    use paladin_auth_gtk::settings::InlineWarning;
    let err = paladin_auth_core::PaladinAuthError::SaveDurabilityUnconfirmed;
    paladin_auth_gtk::settings::SettingsWorkerEffect {
        change: paladin_auth_gtk::settings::AcceptedChange::AutoLockEnabled(value),
        outcome: paladin_auth_gtk::settings::SaveOutcome::DurabilityWarning {
            warning: InlineWarning::from_error(&err),
            field: paladin_auth_gtk::settings::SettingsField::AutoLockEnabled,
        },
    }
}
...
fn settings_effect_inline_clipboard_clear_secs(
    ...
    use paladin_auth_gtk::settings::InlineError;
    let err = paladin_auth_core::PaladinAuthError::IoError {
        operation: "rename",
        source: io::Error::other("synthetic"),
    };
    paladin_auth_gtk::settings::SettingsWorkerEffect {
        change: paladin_auth_gtk::settings::AcceptedChange::ClipboardClearSecs(value),
        outcome: paladin_auth_gtk::settings::SaveOutcome::Inline {
            error: InlineError::from_error(&err),
            field: paladin_auth_gtk::settings::SettingsField::ClipboardClearSecs,
        },
    }
}
...
fn compose_settings_dispatch_success_rolls_busy_back_and_forwards_worker_completed() {
    use paladin_auth_gtk::app::state::compose_settings_dispatch;
    let path = vault_path();
    let busy = AppState::UnlockedBusy { path: path.clone() };
    let dispatch =
    ...
    assert!(
        matches!(
            dispatch.dialog_msg,
            Some(paladin_auth_gtk::settings::SettingsDialogMsg::WorkerCompleted(_))
        ),
...
fn apply_settings_dispatch_inplace_applies_unlocked_rollback() {
    use paladin_auth_gtk::app::state::{
        apply_settings_dispatch_inplace, compose_settings_dispatch,
    };
    let path = vault_path();
    let mut state = AppState::UnlockedBusy { path: path.clone() };
    let dispatch =
...
fn run_settings_worker_success_persists_auto_lock_enabled_change() {
    use paladin_auth_gtk::settings::{run_settings_worker, SaveOutcome, SettingsWorkerInput};

    let (_tempdir, vault_file, vault, store) = fresh_plaintext_pair();
    let prior_enabled = vault.settings().auto_lock_enabled();
    let patch = paladin_auth_core::SettingPatch::AutoLockEnabled(!prior_enabled);
    ...
    let (reopened, _store) =
        paladin_auth_core::Store::open(&vault_file, paladin_auth_core::VaultLock::Plaintext)
...
fn run_settings_worker_success_persists_clipboard_clear_secs_change() {
    ...
    let (_tempdir, _path, vault, store) = fresh_plaintext_pair();
    let target = paladin_auth_core::CLIPBOARD_CLEAR_SECS_MAX;
    let patch = paladin_auth_core::SettingPatch::ClipboardClearSecs(target);
...
fn initial_effects_for_unlocked_returns_some_idle_ownership() {
    use paladin_auth_gtk::app::state::initial_effects_for;
    use paladin_auth_gtk::effect_ownership::AppState as OwnershipAppState;
...
fn handle_quit_request_with_no_effects_slot_fires_now() {
    use paladin_auth_gtk::app::state::handle_quit_request;
    use paladin_auth_gtk::effect_ownership::QuitDecision;
...
fn handle_quit_request_with_idle_effects_fires_now() {
    use paladin_auth_gtk::app::state::handle_quit_request;
    use paladin_auth_gtk::effect_ownership::{EffectOwnership, QuitDecision};
...
fn handle_quit_request_with_busy_effects_is_deferred_and_records_pending_quit() {
    use paladin_auth_gtk::app::state::handle_quit_request;
    use paladin_auth_gtk::effect_ownership::{EffectKind, EffectOwnership, QuitDecision};
...
fn handle_effect_request_with_no_effects_slot_is_rejected() {
    use paladin_auth_gtk::app::state::handle_effect_request;
    use paladin_auth_gtk::effect_ownership::{EffectKind, EffectStart};
...
fn handle_effect_request_with_idle_effects_is_accepted_and_opens_busy_gate() {
    use paladin_auth_gtk::app::state::handle_effect_request;
    use paladin_auth_gtk::effect_ownership::{
...
fn handle_effect_request_while_already_busy_is_rejected_and_does_not_swap_in_flight() {
    use paladin_auth_gtk::app::state::handle_effect_request;
    use paladin_auth_gtk::effect_ownership::{EffectKind, EffectOwnership, EffectStart};
...
fn handle_effect_completion_with_no_effects_slot_is_ready() {
    use paladin_auth_gtk::app::state::handle_effect_completion;
    use paladin_auth_gtk::effect_ownership::CompleteOutcome;
...
fn handle_effect_completion_with_pending_quit_fires_quit_now() {
    use paladin_auth_gtk::app::state::{handle_effect_completion, handle_quit_request};
    use paladin_auth_gtk::effect_ownership::{
...
fn handle_effect_completion_with_pending_lock_on_still_encrypted_vault_fires_lock_now() {
    use paladin_auth_gtk::app::state::{handle_auto_lock_expiry, handle_effect_completion};
    use paladin_auth_gtk::effect_ownership::{
...
fn handle_auto_lock_expiry_with_no_effects_slot_is_ignored() {
    use paladin_auth_gtk::app::state::handle_auto_lock_expiry;
    use paladin_auth_gtk::effect_ownership::LockDecision;
...
fn handle_auto_lock_expiry_while_idle_and_encrypted_fires_now() {
    use paladin_auth_gtk::app::state::handle_auto_lock_expiry;
    use paladin_auth_gtk::effect_ownership::{EffectOwnership, LockDecision};
...
fn handle_auto_lock_expiry_while_busy_is_deferred_and_records_pending_lock() {
    use paladin_auth_gtk::app::state::handle_auto_lock_expiry;
    use paladin_auth_gtk::effect_ownership::{EffectKind, EffectOwnership, LockDecision};
...
fn handle_worker_lost_with_no_effects_slot_is_noop() {
    use paladin_auth_gtk::app::state::handle_worker_lost;
...
fn handle_worker_lost_clears_pending_lock_and_quit_flags() {
    ...
    use paladin_auth_gtk::effect_ownership::{EffectKind, EffectOwnership};
...
fn handle_worker_lost_followed_by_effect_completion_does_not_resurrect_unlocked() {
    use paladin_auth_gtk::app::state::{handle_effect_completion, handle_worker_lost};
    use paladin_auth_gtk::effect_ownership::{
        AppState as OwnershipAppState, CompleteOutcome, EffectKind, EffectOwnership,
    };
...
fn format_app_busy_spinner_visible_none_returns_false() {
    ...
    use paladin_auth_gtk::app::model::format_app_busy_spinner_visible;
...
fn format_app_busy_spinner_visible_missing_returns_false() {
    ...
    let state = AppState::Missing {
        path: PathBuf::from("/tmp/vault.paladin-auth"),
    };
...
fn format_app_busy_spinner_visible_startup_error_returns_false() {
    ...
    use paladin_auth_gtk::app::model::format_app_busy_spinner_visible;
    use paladin_auth_gtk::app::state::AppState;
    use paladin_auth_gtk::startup_error::StartupError;

    let err = PaladinAuthError::InvalidHeader;
...
fn apply_edit_vault_install_inplace_consumes_run_edit_worker_completion_pair() {
    ...
    use paladin_auth_gtk::edit_dialog::run_edit_worker;
    use secrecy::SecretString;
...
fn edit_final_app_state_failure_rolls_back_to_unlocked_preserving_path() {
    use paladin_auth_gtk::edit_dialog::classify_post_effect_error;
