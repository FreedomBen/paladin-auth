...
//! Pure-logic startup-error tests for `paladin-auth-gtk`.
//!
...
use paladin_auth_core::{
    format_unsafe_permissions, ErrorKind, PaladinAuthError, PermissionSubject, VaultMode,
    VaultStatus,
};

use paladin_auth_gtk::startup_error::{
...
fn unsafe_perms_err() -> PaladinAuthError {
    PaladinAuthError::UnsafePermissions {
        path: PathBuf::from("/home/test/.local/share/paladin-auth/vault.bin"),
...
fn invalid_passphrase_err() -> PaladinAuthError {
...
fn retry_resolve_failure_does_not_call_inspect() {
    ...
    let inspect = |_: &Path| -> Result<VaultStatus, PaladinAuthError> {
...
fn retry_inspect_failure_routes_to_inspect_source() {
    ...
    let inspect = |_: &Path| -> Result<VaultStatus, PaladinAuthError> { Err(invalid_header_err()) };
...
fn retry_inspect_unsafe_permissions_routes_with_formatter_text() {
    ...
    let resolve = || Ok(PathBuf::from("/tmp/x"));
    let inspect = |_: &Path| -> Result<VaultStatus, PaladinAuthError> { Err(unsafe_perms_err()) };
...
#[test]
fn startup_error_marker_prefix_is_paladin_auth_gtk_namespaced() {
    // Every smoke-test marker `paladin-auth-gtk` emits under
    // `--exit-after-startup` shares the `paladin-auth-gtk: ` prefix so
    // tests can grep for the namespace and tell our lines apart from
    // GTK / libadwaita warnings on the same stream.
    assert!(
        STARTUP_ERROR_MARKER_PREFIX.starts_with("paladin-auth-gtk: "),
        "STARTUP_ERROR_MARKER_PREFIX must share the paladin-auth-gtk namespace: {STARTUP_ERROR_MARKER_PREFIX:?}",
    );
    assert!(
...
fn format_startup_error_icon_name_returns_dialog_error_symbolic() {
    ...
    use paladin_auth_gtk::startup_error::format_startup_error_icon_name;
...
fn format_startup_error_title_returns_startup_error() {
    ...
    use paladin_auth_gtk::startup_error::format_startup_error_title;
...
fn format_startup_error_retry_label_returns_retry() {
    ...
    use paladin_auth_gtk::startup_error::format_startup_error_retry_label;
...
fn format_startup_error_quit_label_returns_quit() {
    ...
    use paladin_auth_gtk::startup_error::format_startup_error_quit_label;
...
fn startup_error_output_retry_variant_exists() {
    ...
    use paladin_auth_gtk::startup_error::StartupErrorOutput;
...
fn startup_error_output_is_exhaustively_retry_quit_and_delete_vault() {
    ...
    // `paladin_auth_core::destroy_vault`, not here). Exhaustive match arms lock
...
fn dispatch_startup_error_output_retry_routes_to_startup_error_retry_app_msg() {
    ...
    use paladin_auth_gtk::app::model::{dispatch_startup_error_output, AppMsg};
    use paladin_auth_gtk::startup_error::StartupErrorOutput;
...
fn startup_error_from_worker_panic_carries_effect_kind_in_source() {
    use paladin_auth_gtk::effect_ownership::EffectKind;
...
fn startup_error_from_worker_panic_kind_is_io_error() {
    ...
    // palette without leaking a GUI-specific kind into paladin_auth_core.
...
fn startup_error_from_worker_panic_rendered_body_names_the_effect() {
    use paladin_auth_gtk::effect_ownership::EffectKind;
    use paladin_auth_gtk::startup_error::{format_worker_panic_message, StartupError};
    ...
    assert!(
        err.rendered.contains("Restart Paladin Auth"),
...
fn format_worker_panic_message_is_single_line_for_marker_compatibility() {
    use paladin_auth_gtk::effect_ownership::EffectKind;
    use paladin_auth_gtk::startup_error::format_worker_panic_message;
