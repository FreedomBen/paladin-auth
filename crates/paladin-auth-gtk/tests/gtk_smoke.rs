...
//! `xvfb-run` headless smoke test for `paladin-auth-gtk`.
//!
...
//! `CARGO_BIN_EXE_paladin-auth-gtk`, which Cargo provides to integration
...
/// Path to the built `paladin-auth-gtk` binary. Cargo populates this at
/// compile time per the §"Crate layout" `[[bin]]` declaration.
const PALADIN_AUTH_GTK_BIN: &str = env!("CARGO_BIN_EXE_paladin-auth-gtk");

/// Wall-clock ceiling for an `xvfb-run paladin-auth-gtk` invocation that
...
fn run_under_xvfb(args: &[&str]) -> std::process::Output {
    assert!(
        Path::new(PALADIN_AUTH_GTK_BIN).exists(),
        "CARGO_BIN_EXE_paladin-auth-gtk does not point at an existing file: {PALADIN_AUTH_GTK_BIN}",
    );

    let mut child = Command::new("xvfb-run")
        .arg("-a")
        .arg(PALADIN_AUTH_GTK_BIN)
        .args(args)
        ...
        match child.try_wait() {
            ...
            Ok(None) => {
                if std::time::Instant::now() >= deadline {
                    ...
                    let _ = child.wait();
                    panic!(
                        "xvfb-run {PALADIN_AUTH_GTK_BIN} did not exit within \
...
/// Plan bullet: "`xvfb-run` launches `paladin-auth-gtk` and the process
...
#[test]
fn xvfb_run_launches_paladin_auth_gtk_and_process_exits() {
    ...
    assert!(
        ...
        "xvfb-run paladin-auth-gtk exited with status {:?}\n--- stdout ---\n{}\n--- stderr ---\n{}",
        output.status,
...
/// so the file handle is closed before `paladin-auth-gtk` re-opens it.
fn prepare_empty_plaintext_vault() -> (tempfile::TempDir, PathBuf) {
    ...
    {
        let (vault, store) =
            paladin_auth_core::Store::create(&vault_path, paladin_auth_core::VaultInit::Plaintext)
...
/// `paladin_auth_core::Store::create`, then launches `paladin-auth-gtk` with
/// `--vault <path> --exit-after-startup` under `xvfb-run`. The binary
/// runs the §"Vault interaction" startup sequence — resolve path,
/// `paladin_auth_core::inspect`, and `paladin_auth_core::Store::open` with
...
fn app_opens_prepared_plaintext_vault() {
    ...
    assert!(
        output.status.success(),
        "xvfb-run paladin-auth-gtk --vault {path_str} --exit-after-startup exited with status {:?}\n\
         --- stdout ---\n{}\n--- stderr ---\n{}",
        output.status,
        stdout,
        stderr,
    ...
    let expected = format!("paladin-auth-gtk: startup_state=Unlocked path={path_str}");
    assert!(
...
/// one HOTP account, then launches `paladin-auth-gtk` with
/// `--vault <path> --exit-after-startup` under `xvfb-run`. The
/// `AccountListComponent` binds rows from `paladin_auth_core::Vault::summaries()`
...
fn app_renders_prepared_accounts() {
    ...
    // stay aligned.
    let expected = "paladin-auth-gtk: account_list_rows=GitHub:ben|GitLab:alice|solo";
    assert!(
        stdout.contains(expected),
        "expected stdout to contain `{expected}`\n--- stdout ---\n{stdout}\n--- stderr ---\n{stderr}",
    );
    ...
    let widget_states_expected = "paladin-auth-gtk: account_list_widget_states=copy:on,next:off,kebab:on|copy:on,next:off,kebab:on|copy:off,next:on,kebab:on";
    assert!(
...
/// `paladin_auth_core::inspect` therefore routes to
...
fn app_renders_startup_error_for_corrupt_vault() {
    ...
    let vault_path = tempdir.path().join("vault.bin");
    // Non-magic prefix → `paladin_auth_core::inspect` returns
    // `PaladinAuthError::InvalidHeader`, which routes to
    // `StartupErrorSource::Inspect` per §"Vault interaction".
    std::fs::write(&vault_path, b"not a paladin-auth vault").expect("write corrupted vault");
    ...
    let state_expected = format!("paladin-auth-gtk: startup_state=StartupError path={path_str}");
    assert!(
    ...
    // from `paladin_auth_core::PaladinAuthError::InvalidHeader`'s `Display`
    // impl (no `format_unsafe_permissions` projection for this
    // variant), which is the literal "invalid vault header".
    let body_expected = "paladin-auth-gtk: startup_error_body=invalid vault header";
    assert!(
...
/// `paladin_auth_core::inspect` → `VaultStatus::Missing` by pointing
...
fn app_renders_init_dialog_for_missing_vault() {
    ...
    // never created — `paladin_auth_core::inspect` returns
    ...
    assert!(
        !vault_path.exists(),
        "tempdir entry must be absent before launching paladin-auth-gtk: {}",
        vault_path.display()
    );
    ...
    let state_expected = format!("paladin-auth-gtk: startup_state=Missing path={path_str}");
    assert!(
        stdout.contains(&state_expected),
        "expected stdout to contain `{state_expected}`\n--- stdout ---\n{stdout}\n--- stderr ---\n{stderr}",
    );
    ...
    let init_expected = format!("paladin-auth-gtk: init_dialog_path={path_str}");
    assert!(
    ...
    // is caught here.
    assert!(
        !stdout.contains("paladin-auth-gtk: account_list_rows="),
        "expected the Missing branch to skip the account list marker\n--- stdout ---\n{stdout}",
    );
    assert!(
        !stdout.contains("paladin-auth-gtk: account_list_widget_states="),
        "expected the Missing branch to skip the widget-states marker\n--- stdout ---\n{stdout}",
    );
    assert!(
        !stdout.contains("paladin-auth-gtk: startup_error_body="),
...
/// shape is used by the paladin-auth-tui test fixtures.
...
fn prepare_empty_encrypted_vault(passphrase: &str) -> (tempfile::TempDir, PathBuf) {
    ...
    let vault_path = tempdir.path().join("vault.bin");
    {
        ...
        let (vault, store) = paladin_auth_core::Store::create(
            &vault_path,
            paladin_auth_core::VaultInit::Encrypted(opts),
        )
...
///
/// Routes the §"Vault interaction" startup sequence through
/// `paladin_auth_core::inspect` → `VaultStatus::Encrypted` by pre-
...
fn app_renders_unlock_dialog_for_encrypted_vault() {
    ...
    let state_expected = format!("paladin-auth-gtk: startup_state=Locked path={path_str}");
    assert!(
        stdout.contains(&state_expected),
        "expected stdout to contain `{state_expected}`\n--- stdout ---\n{stdout}\n--- stderr ---\n{stderr}",
    );
    ...
    let unlock_expected = format!("paladin-auth-gtk: unlock_dialog_path={path_str}");
    assert!(
    ...
    assert!(
        !stdout.contains("paladin-auth-gtk: init_dialog_path="),
...
/// 2. `paladin_auth_core::destroy_vault` (the exact worker the dialog
...
fn app_destroy_flow_removes_vault_and_routes_to_init_dialog() {
    ...
    let unlocked_marker = format!("paladin-auth-gtk: startup_state=Unlocked path={path_str}");
    assert!(
    ...
    // primary is unlinked.
    let report = paladin_auth_core::destroy_vault(&vault_path).expect("destroy_vault must succeed");
    assert!(
    ...
    let missing_marker = format!("paladin-auth-gtk: startup_state=Missing path={path_str}");
    assert!(
        after_stdout.contains(&missing_marker),
        "expected `{missing_marker}` after destroy\n--- stdout ---\n{after_stdout}",
    );
    let init_marker = paladin_auth_gtk::init_dialog::format_init_dialog_marker(&vault_path);
    assert!(
...
fn add_validated_account(
    vault: &mut paladin_auth_core::Vault,
    store: &paladin_auth_core::Store,
    issuer: Option<&str>,
    label: &str,
    hotp: bool,
) {
    let kind = if hotp {
        paladin_auth_core::AccountKindInput::Hotp
    } else {
        paladin_auth_core::AccountKindInput::Totp
    };
    let input = paladin_auth_core::AccountInput {
        label: label.to_string(),
        issuer: issuer.map(str::to_string),
        secret: SecretString::from("JBSWY3DPEHPK3PXP".to_string()),
        algorithm: paladin_auth_core::Algorithm::Sha1,
        digits: 6,
        kind,
        period_secs: None,
        counter: if hotp { Some(0) } else { None },
        icon_hint: paladin_auth_core::IconHintInput::Default,
    };
    let validated =
        paladin_auth_core::validate_manual(input, SystemTime::now()).expect("valid manual input");
