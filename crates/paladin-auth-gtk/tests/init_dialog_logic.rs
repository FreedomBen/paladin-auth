...
//! Pure-logic init-dialog tests for `paladin-auth-gtk`.
//!
...
//! * `paladin_auth_core::classify_init_precheck` routing:
...
//!
//! The module under test (`paladin_auth_gtk::init_dialog`) is the pure-
//! logic state machine the GTK `InitDialog` shadows. It owns no
//! widgets; the `InitSecretState` from
//! [`paladin_auth_gtk::secret_fields`] holds the secret-bearing
//! passphrase buffers and the pending [`paladin_auth_core::VaultInit`]
...
use paladin_auth_core::{
    ...
    ErrorKind, PaladinAuthError, PermissionSubject, Store, VaultInit, VaultLock, VaultStatus,
};
use secrecy::SecretString;

use paladin_auth_gtk::init_dialog::{
...
use paladin_auth_gtk::secret_fields::{ClearReason, InitSecretState};
...
fn unsafe_permissions_err() -> PaladinAuthError {
...
#[test]
fn submit_rejection_plaintext_warning_required_has_no_paladin_auth_error_kind() {
    // The plaintext-warning gate is a UI-only precondition — it
    // never surfaces as a §5 PaladinAuthError.
...
fn classify_precheck_invalid_header_opens_destructive_gate() {
    ...
    let outcome = classify_precheck(Err(PaladinAuthError::InvalidHeader));
    assert!(matches!(outcome, PrecheckOutcome::DestructiveGate));
}
...
fn classify_precheck_vault_missing_propagates_inline_error() {
    ...
    let outcome = classify_precheck(Err(PaladinAuthError::VaultMissing));
    let PrecheckOutcome::InlineError(inline) = outcome else {
        panic!("expected InlineError, got {outcome:?}");
    };
...
fn classify_create_error_vault_exists_opens_destructive_gate() {
    let outcome = classify_create_error(&PaladinAuthError::VaultExists, attempted_dir());
...
fn classify_create_error_create_vault_dir_renders_friendly_message_with_path() {
    ...
    // `paladin_auth_core::format_create_vault_dir_error`, naming the
    // directory paladin-auth tried to `mkdir -p`.
    let err = PaladinAuthError::IoError {
        operation: "create_vault_dir",
        source: std::io::Error::from(std::io::ErrorKind::PermissionDenied),
    };
    let outcome = classify_create_error(&err, Path::new("/home/u/.local/share/paladin-auth"));
    let CreateOutcome::InlineError(inline) = outcome else {
        panic!("expected InlineError, got {outcome:?}");
    };
    ...
    assert!(
        inline
            .rendered
            .contains("/home/u/.local/share/paladin-auth"),
...
fn classify_create_force_error_create_vault_dir_renders_friendly_message_with_path() {
    ...
    let inline = classify_create_force_error(&err, Path::new("/home/u/.local/share/paladin-auth"));
    assert_eq!(inline.kind, ErrorKind::IoError);
    assert!(
        inline
            .rendered
...
fn init_dialog_marker_prefix_is_stable() {
    assert_eq!(
        paladin_auth_gtk::init_dialog::INIT_DIALOG_MARKER_PREFIX,
        "paladin-auth-gtk: init_dialog_path=",
    );
}
...
fn format_init_dialog_marker_renders_resolved_path() {
    ...
    assert_eq!(
        paladin_auth_gtk::init_dialog::format_init_dialog_marker(path),
        "paladin-auth-gtk: init_dialog_path=/tmp/example/vault.bin",
    );
}
...
fn format_init_dialog_marker_starts_with_prefix() {
    ...
    let marker = paladin_auth_gtk::init_dialog::format_init_dialog_marker(Path::new("/x"));
    assert!(marker.starts_with(paladin_auth_gtk::init_dialog::INIT_DIALOG_MARKER_PREFIX));
}
...
// `PaladinAuthError` through `classify_create_error` /
...
fn run_init_worker_create_existing_vault_routes_destructive_gate() {
    ...
    // `paladin_auth_core::format_init_force_warning`. The pending
...
fn format_init_dialog_description_renders_resolved_path_then_plaintext_warning() {
    ...
    // through `paladin_auth_core::format_plaintext_storage_warning()`.
    ...
    // routes the warning text through the shared paladin-auth-core
    // projection so the GUI cannot drift from the CLI / TUI copy.
    //
    // Sibling of
    // `paladin_auth_gtk::unlock_dialog::format_unlock_dialog_description`
    ...
    use paladin_auth_gtk::init_dialog::format_init_dialog_description;
    ...
    assert_eq!(
        ...
        "description leads with the resolved path and routes the warning through paladin-auth-core",
    );
}
...
#[test]
fn format_init_dialog_description_contains_paladin_auth_core_plaintext_warning_verbatim() {
    // The plaintext-storage warning body is sourced through
    // `paladin_auth_core::format_plaintext_storage_warning()` so the
    ...
    assert!(
        ...
        "description must embed the paladin-auth-core plaintext warning verbatim; got {rendered:?}",
    );
}
...
fn format_init_dialog_icon_name_returns_document_new_symbolic() {
    ...
    // mirror. Sibling of
    // `paladin_auth_gtk::unlock_dialog::format_unlock_dialog_icon_name`
    // on the dialog-status-icon side; together they pin every
    // first-mount dialog's freedesktop glyph against a single
    // source of truth.
    use paladin_auth_gtk::init_dialog::format_init_dialog_icon_name;
...
fn format_init_dialog_title_returns_create_a_new_vault() {
    ...
    // and `paladin_auth_gtk::add_account::format_add_dialog_title` on
    // the dialog-header-title side; together they pin every
    // dialog's titled surface against a single source of truth.
    use paladin_auth_gtk::init_dialog::format_init_dialog_title;
...
fn format_init_dialog_create_label_returns_create_vault() {
    ...
    use paladin_auth_gtk::init_dialog::format_init_dialog_create_label;
...
fn format_init_dialog_create_label_is_non_empty_single_line_distinct_from_title() {
    ...
    // twice.
    use paladin_auth_gtk::init_dialog::{
        format_init_dialog_create_label, format_init_dialog_title,
    };
...
fn format_init_dialog_force_confirm_label_returns_replace() {
    ...
    use paladin_auth_gtk::init_dialog::format_init_dialog_force_confirm_label;
...
fn format_init_dialog_force_cancel_label_returns_cancel() {
    ...
    use paladin_auth_gtk::init_dialog::format_init_dialog_force_cancel_label;
...
fn format_init_dialog_force_cancel_label_matches_other_dialog_cancel_labels() {
    ...
    use paladin_auth_gtk::edit_dialog::format_edit_dialog_cancel_label;
    use paladin_auth_gtk::init_dialog::format_init_dialog_force_cancel_label;
    use paladin_auth_gtk::remove_dialog::format_remove_dialog_cancel_label;
...
fn format_init_dialog_passphrase_title_returns_passphrase() {
    ...
    // silently.
    use paladin_auth_gtk::init_dialog::format_init_dialog_passphrase_title;
...
fn format_init_dialog_force_heading_returns_replace_existing_vault_question() {
    ...
    // one place shared by the widget binding and the pure-logic
    // tests.
    use paladin_auth_gtk::init_dialog::format_init_dialog_force_heading;
...
fn format_init_dialog_plaintext_warning_label_returns_accept_risk() {
    ...
    // shared by the widget binding and the pure-logic tests in
    // `tests/init_dialog_logic.rs`.
    use paladin_auth_gtk::init_dialog::format_init_dialog_plaintext_warning_label;
...
fn format_init_dialog_plaintext_warning_label_is_non_empty_single_line() {
    ...
    // `paladin_auth_core::format_plaintext_storage_warning()` and is
...
fn format_init_dialog_plaintext_warning_label_is_distinct_from_warning_body() {
    ...
    // `paladin_auth_core::format_plaintext_storage_warning()`; the
...
fn format_init_dialog_confirm_passphrase_title_returns_confirm_passphrase() {
    ...
    // `crates/paladin-auth-cli/src/commands/init.rs`) — the CLI's
    ...
    use paladin_auth_gtk::init_dialog::format_init_dialog_confirm_passphrase_title;
...
fn inline_error_from_rejection_confirmation_mismatch_renders_invalid_passphrase_reason() {
    // Rendered body uses the typed `PaladinAuthError::Display` so the
