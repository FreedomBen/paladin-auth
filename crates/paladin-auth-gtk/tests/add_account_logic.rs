...
//! Pure-logic Add Account manual-path tests for `paladin-auth-gtk`.
//!
...
//!   [`paladin_auth_core::parse_icon_hint_token`] so `""`,
...
//!
//! The module under test (`paladin_auth_gtk::add_account`) is the
...
//! [`classify_duplicate`] on the [`paladin_auth_core::Vault::find_duplicate`]
...
use paladin_auth_core::{
    ...
    PaladinAuthError, Store, ValidationWarning, Vault, VaultInit, VaultLock,
};
use secrecy::SecretString;

fn validation_error(field: &'static str, reason: &str) -> PaladinAuthError {
...
use paladin_auth_gtk::secret_fields::AddSecretState;
...
/// with any [`PaladinAuthError`] Display string.
...
fn inline_warning_clones_freely_for_reactive_state() {
    let AddPostEffectOutcome::KeepWithWarning(warning) =
        classify_add_post_effect_error(&PaladinAuthError::SaveDurabilityUnconfirmed)
    else {
...
fn validate_manual_totp(label: &str, issuer: Option<&str>) -> paladin_auth_core::ValidatedAccount {
    let mut fields = manual_totp_defaults();
...
fn add_dialog_marker_prefix_is_stable_grep_anchor() {
    use paladin_auth_gtk::add_account::ADD_DIALOG_MARKER_PREFIX;

    assert_eq!(
        ADD_DIALOG_MARKER_PREFIX,
        "paladin-auth-gtk: add_dialog_path="
    );
}

#[test]
fn format_add_dialog_marker_renders_vault_path() {
    use paladin_auth_gtk::add_account::{format_add_dialog_marker, ADD_DIALOG_MARKER_PREFIX};

    let path = PathBuf::from("/home/test/.local/share/paladin-auth/vault.bin");
    ...
    assert!(
        marker.contains("/home/test/.local/share/paladin-auth/vault.bin"),
...
fn add_account_init_clones_for_reactive_state() {
    use paladin_auth_gtk::add_account::AddAccountInit;

    let init = AddAccountInit {
        vault_path: PathBuf::from("/home/test/.local/share/paladin-auth/vault.bin"),
    };
...
fn apply_msg_cancel_routes_to_cancel_output() {
    use paladin_auth_gtk::add_account::{
        apply_msg, AddAccountMsg, AddAccountOutput, AddDialogState,
    };
...
fn apply_msg_cancel_wipes_secret_state_buffers() {
    ...
    use paladin_auth_gtk::add_account::{apply_msg, AddAccountMsg, AddDialogState};
...
fn add_dialog_state_new_initializes_worker_outcome_to_none() {
    ...
    use paladin_auth_gtk::add_account::AddDialogState;
...
fn apply_msg_submit_proceed_routes_to_submit_output() {
    ...
    use paladin_auth_core::{validate_manual, AccountInput, IconHintInput};
    use paladin_auth_gtk::add_account::{
        apply_msg, AddAccountMsg, AddAccountOutput, AddDialogState,
    };
...
fn add_dialog_state_new_initializes_secret_state_to_manual_path_with_empty_buffers() {
    ...
    use paladin_auth_gtk::add_account::AddDialogState;
    use paladin_auth_gtk::secret_fields::AddPath;
...
fn apply_msg_manual_secret_changed_shadows_into_secret_state() {
    ...
    // arrive as `String`s; the dialog shadows them into the Paladin Auth-
    ...
    assert_eq!(
        ...
        "JBSWY3DPEHPK3PXP",
        "manual Base32 keystrokes shadow into the Paladin Auth-owned buffer",
    );
}
...
fn apply_msg_uri_text_changed_shadows_into_secret_state() {
    ...
    // Paladin Auth-owned `Zeroizing<String>` and emit no output until the
    // user submits.
    use paladin_auth_gtk::add_account::{apply_msg, AddAccountMsg, AddDialogState};
    ...
    assert_eq!(
        ...
        "URI keystrokes shadow into the Paladin Auth-owned buffer",
    );
}
...
fn apply_msg_stage_pending_duplicate_replaces_prior_pending() {
    ...
    // bytes zero out via `paladin_auth_core::Secret`'s `ZeroizeOnDrop`
...
fn apply_msg_manual_label_changed_preserves_other_draft_fields() {
    ...
    use paladin_auth_core::{AccountKindInput, Algorithm};
    use paladin_auth_gtk::add_account::{apply_msg, AddAccountMsg, AddDialogState};
...
fn apply_msg_manual_algorithm_changed_shadows_into_manual_draft() {
    ...
    use paladin_auth_core::Algorithm;
    use paladin_auth_gtk::add_account::{apply_msg, AddAccountMsg, AddDialogState};
...
fn apply_msg_manual_kind_changed_shadows_into_manual_draft() {
    ...
    use paladin_auth_core::AccountKindInput;
    use paladin_auth_gtk::add_account::{apply_msg, AddAccountMsg, AddDialogState};
...
fn manual_draft_state_default_matches_cli_manual_add_defaults() {
    ...
    // §5 / `paladin-auth-cli/src/commands/add.rs`): TOTP, SHA1, 6 digits,
    // 30 s period, HOTP counter 0, and empty label / issuer /
    // icon-hint text (which collapses to `IconHintInput::Default`
    // through `paladin_auth_core::parse_icon_hint_token`).
    use paladin_auth_core::{AccountKindInput, Algorithm};
    use paladin_auth_gtk::add_account::ManualDraftState;
...
fn add_dialog_state_new_initializes_manual_draft_to_defaults() {
    ...
    use paladin_auth_gtk::add_account::{AddDialogState, ManualDraftState};
...
fn compose_manual_fields_maps_every_draft_field_and_secret_text_verbatim() {
    ...
    // Paladin Auth-owned `crate::secret_fields::SecretEntry` text. The
    ...
    use paladin_auth_core::{AccountKindInput, Algorithm};
    use paladin_auth_gtk::add_account::{compose_manual_fields, ManualDraftState};
...
fn compose_uri_submit_outcome_with_valid_uri_proceeds() {
    ...
    // as `Proceed` so the widget can hand the validated account to
    // `Vault::find_duplicate` next.
    use paladin_auth_gtk::add_account::{apply_msg, compose_uri_submit_outcome, AddAccountMsg};
    use paladin_auth_gtk::otpauth_uri_paste::UriSubmitOutcome;

    let mut state = paladin_auth_gtk::add_account::AddDialogState::new();
...
fn compose_uri_submit_outcome_with_default_state_rejects_inline() {
    ...
    // without mutating the vault.
    use paladin_auth_gtk::add_account::{compose_uri_submit_outcome, AddDialogState};
    use paladin_auth_gtk::otpauth_uri_paste::UriSubmitOutcome;
...
fn compose_submit_outcome_manual_path_default_state_rejects_inline() {
    ...
    use paladin_auth_gtk::add_account::{compose_submit_outcome, AddDialogState, SubmitOutcome};
...
fn compose_save_click_outcome_inline_error_short_circuits_before_duplicate_check() {
    ...
    use paladin_auth_gtk::add_account::{
        compose_save_click_outcome, AddDialogState, SaveClickOutcome,
    };
...
fn save_click_outcome_to_msg_proceed_returns_submit_proceed() {
    ...
    // `paladin_auth_core::format_validation_warning` off the
    // `AddAccountOutput::Submit` boundary instead.
    use paladin_auth_gtk::add_account::{
        save_click_outcome_to_msg, AddAccountMsg, SaveClickOutcome,
    };
...
fn format_duplicate_confirm_body_renders_issuer_colon_label() {
    ...
    use paladin_auth_gtk::add_account::format_duplicate_confirm_body;
...
fn format_pending_warnings_body_returns_empty_string_when_no_warnings() {
    ...
    use paladin_auth_gtk::add_account::format_pending_warnings_body;
...
fn format_pending_warnings_body_renders_single_short_secret_warning() {
    ...
    // `paladin_auth_core::format_validation_warning` so the wording stays
    ...
    // `paladin-auth-tui/src/app/reducer.rs` — the "warning: " prefix
    // tracks the same wording.
    use paladin_auth_core::{format_validation_warning, ValidationWarning};
    use paladin_auth_gtk::add_account::format_pending_warnings_body;
...
fn format_pending_warnings_body_threads_through_format_validation_warning() {
    ...
    // through `paladin_auth_core::format_validation_warning` so the
    ...
    assert!(
        ...
        "body must route through paladin_auth_core::format_validation_warning verbatim: \
...
fn format_duplicate_alert_body_returns_confirm_body_when_no_warnings() {
    ...
    use paladin_auth_gtk::add_account::{
        format_duplicate_alert_body, format_duplicate_confirm_body,
    };
...
fn format_duplicate_alert_body_joins_confirm_and_warnings_with_blank_line() {
    ...
    use paladin_auth_core::ValidationWarning;
    use paladin_auth_gtk::add_account::{
...
fn format_duplicate_alert_body_threads_through_existing_summary_projection() {
    ...
    use paladin_auth_gtk::add_account::format_duplicate_alert_body;
...
fn compose_pending_duplicate_alert_body_with_no_pending_returns_none() {
    ...
    use paladin_auth_gtk::add_account::{compose_pending_duplicate_alert_body, AddDialogState};
...
fn compose_post_effect_warning_body_with_no_outcome_returns_none() {
    ...
    use paladin_auth_gtk::add_account::{compose_post_effect_warning_body, AddDialogState};
...
fn compose_post_effect_inline_error_body_with_no_outcome_returns_none() {
    ...
    use paladin_auth_gtk::add_account::{compose_post_effect_inline_error_body, AddDialogState};
...
fn compose_post_effect_inline_error_body_with_inline_returns_rendered() {
    ...
    // the underlying `PaladinAuthError`.
    use paladin_auth_gtk::add_account::{
        apply_msg, classify_add_post_effect_error, compose_post_effect_inline_error_body,
        AddAccountMsg, AddDialogState, AddPostEffectOutcome,
    };
...
fn compose_inline_error_body_with_no_inline_error_returns_none() {
    ...
    use paladin_auth_gtk::add_account::{compose_inline_error_body, AddDialogState};
...
fn compose_inline_error_body_with_inline_error_returns_rendered() {
    ...
    // impl on the underlying `PaladinAuthError`. Symmetric partner of
...
fn compose_inline_error_revealed_with_no_inline_error_is_false() {
    ...
    use paladin_auth_gtk::add_account::{compose_inline_error_revealed, AddDialogState};
...
fn compose_active_path_fresh_dialog_returns_manual() {
    ...
    // accessors inline.
    use paladin_auth_gtk::add_account::{compose_active_path, AddDialogState};
    use paladin_auth_gtk::secret_fields::AddPath;
...
fn compose_active_path_after_switch_to_uri_returns_uri() {
    ...
    use paladin_auth_gtk::add_account::{
        apply_msg, compose_active_path, AddAccountMsg, AddDialogState,
    };
    use paladin_auth_gtk::secret_fields::AddPath;
...
fn format_duplicate_alert_heading_returns_add_anyway_question() {
    ...
    use paladin_auth_gtk::add_account::format_duplicate_alert_heading;
...
fn compose_pending_duplicate_alert_heading_with_no_pending_returns_none() {
    ...
    use paladin_auth_gtk::add_account::{compose_pending_duplicate_alert_heading, AddDialogState};
...
fn format_duplicate_alert_confirm_label_returns_add_anyway() {
    ...
    // [`paladin_auth_core::ValidatedAccount`] and forwards
    ...
    use paladin_auth_gtk::add_account::format_duplicate_alert_confirm_label;
...
fn compose_pending_duplicate_alert_confirm_label_with_no_pending_returns_none() {
    ...
    use paladin_auth_gtk::add_account::{
        compose_pending_duplicate_alert_confirm_label, AddDialogState,
    };
...
fn format_duplicate_alert_cancel_label_returns_cancel() {
    ...
    // [`paladin_auth_core::ValidatedAccount`]. The label wording is the
    ...
    use paladin_auth_gtk::add_account::format_duplicate_alert_cancel_label;
...
fn compose_pending_duplicate_alert_cancel_label_with_no_pending_returns_none() {
    ...
    use paladin_auth_gtk::add_account::{
        compose_pending_duplicate_alert_cancel_label, AddDialogState,
    };
...
fn compose_manual_period_secs_visible_default_state_is_true() {
    ...
    use paladin_auth_gtk::add_account::{compose_manual_period_secs_visible, AddDialogState};
...
fn compose_manual_counter_visible_default_state_is_false() {
    ...
    use paladin_auth_gtk::add_account::{compose_manual_counter_visible, AddDialogState};
...
fn compose_save_button_sensitive_fresh_dialog_is_false() {
    ...
    // the add path.
    use paladin_auth_gtk::add_account::{compose_save_button_sensitive, AddDialogState};
...
fn add_account_output_close_is_distinct_variant() {
    use paladin_auth_gtk::add_account::AddAccountOutput;
...
fn format_add_path_label_manual_returns_manual() {
    ...
    use paladin_auth_gtk::add_account::format_add_path_label;
    use paladin_auth_gtk::secret_fields::AddPath;
...
fn format_add_path_name_qr_returns_qr_slug() {
    ...
    use paladin_auth_gtk::add_account::format_add_path_name;
    use paladin_auth_gtk::secret_fields::AddPath;
...
fn format_add_path_name_qr_is_distinct_from_label() {
    ...
    use paladin_auth_gtk::add_account::{format_add_path_label, format_add_path_name};
    use paladin_auth_gtk::secret_fields::AddPath;
...
fn compose_pending_duplicate_alert_visible_fresh_dialog_is_false() {
    ...
    use paladin_auth_gtk::add_account::{compose_pending_duplicate_alert_visible, AddDialogState};
...
fn compose_post_effect_warning_revealed_with_no_outcome_is_false() {
    ...
    use paladin_auth_gtk::add_account::{compose_post_effect_warning_revealed, AddDialogState};
...
fn compose_post_effect_inline_error_revealed_with_no_outcome_is_false() {
    ...
    use paladin_auth_gtk::add_account::{
        compose_post_effect_inline_error_revealed, AddDialogState,
    };
...
fn compose_manual_period_secs_value_fresh_dialog_returns_default() {
    ...
    use paladin_auth_gtk::add_account::{compose_manual_period_secs_value, AddDialogState};
...
fn compose_manual_counter_value_fresh_dialog_returns_default() {
    ...
    use paladin_auth_gtk::add_account::{compose_manual_counter_value, AddDialogState};
...
fn compose_manual_digits_value_fresh_dialog_returns_default() {
    ...
    use paladin_auth_gtk::add_account::{compose_manual_digits_value, AddDialogState};
...
fn format_manual_kind_selected_totp_returns_zero() {
    ...
    // dropdown side.
    use paladin_auth_core::AccountKindInput;
    use paladin_auth_gtk::add_account::format_manual_kind_selected;
...
fn format_manual_algorithm_selected_sha1_returns_zero() {
    ...
    use paladin_auth_core::Algorithm;
    use paladin_auth_gtk::add_account::format_manual_algorithm_selected;
...
fn compose_manual_label_text_fresh_dialog_returns_empty() {
    ...
    use paladin_auth_gtk::add_account::{compose_manual_label_text, AddDialogState};
...
fn compose_manual_issuer_text_fresh_dialog_returns_empty() {
    ...
    use paladin_auth_gtk::add_account::{compose_manual_issuer_text, AddDialogState};
...
fn compose_manual_icon_hint_text_fresh_dialog_returns_empty() {
    ...
    // is valid input (`paladin_auth_core::parse_icon_hint_token` treats
    // empty / `default` as "use the inferred icon" and `none` as
    // "force the placeholder"), so the Save-button gate ignores it.
    use paladin_auth_gtk::add_account::{compose_manual_icon_hint_text, AddDialogState};
...
fn format_manual_kind_labels_matches_tui_wording() {
    ...
    // "HOTP"`) — see `crates/paladin-auth-tui/src/view/add.rs`. Pinning
    ...
    use paladin_auth_gtk::add_account::format_manual_kind_labels;
...
fn format_manual_kind_labels_index_aligns_with_format_manual_kind_selected() {
    ...
    // stale.
    use paladin_auth_core::AccountKindInput;
    use paladin_auth_gtk::add_account::{format_manual_kind_labels, format_manual_kind_selected};
...
fn format_manual_algorithm_labels_matches_tui_wording() {
    ...
    // `crates/paladin-auth-tui/src/view/add.rs`. Pinning the labels
    ...
    // algorithm-dropdown side.
    use paladin_auth_gtk::add_account::format_manual_algorithm_labels;
...
fn format_add_path_order_matches_view_switcher_page_order() {
    ...
    // sub-path-order side.
    use paladin_auth_gtk::add_account::format_add_path_order;
    use paladin_auth_gtk::secret_fields::AddPath;
...
fn format_add_path_order_labels_align_with_format_add_path_label() {
    ...
    use paladin_auth_gtk::add_account::{format_add_path_label, format_add_path_order};
...
fn format_add_path_order_names_align_with_format_add_path_name() {
    ...
    use paladin_auth_gtk::add_account::{format_add_path_name, format_add_path_order};
...
// f64)` tuple sourced from `paladin_auth_core::{DIGITS_MIN, DIGITS_MAX}`
// (both `u8`, lossless under `f64::from`) and the `1.0` integer
// step, so `clippy::float_cmp` does not apply.
#[test]
#[allow(clippy::float_cmp)]
fn format_manual_digits_adjustment_returns_paladin_auth_core_range() {
    // The digits AdwSpinRow's `gtk::Adjustment` (lower, upper,
    // step_increment) tuple must mirror the
    // `paladin_auth_core::{DIGITS_MIN, DIGITS_MAX}` validated range so a
    ...
    // displays, which `paladin_auth_core::DIGITS_MIN` / `DIGITS_MAX` cast
    ...
    // source of truth in `paladin_auth_core::domain::validation`.
    use paladin_auth_gtk::add_account::format_manual_digits_adjustment;

    let (lower, upper, step) = format_manual_digits_adjustment();
    assert_eq!(
        lower,
        f64::from(paladin_auth_core::DIGITS_MIN),
        "lower bound mirrors paladin_auth_core::DIGITS_MIN",
    );
    assert_eq!(
        upper,
        f64::from(paladin_auth_core::DIGITS_MAX),
        "upper bound mirrors paladin_auth_core::DIGITS_MAX",
    );
    assert_eq!(
        step, 1.0,
...
// f64)` tuple sourced from `paladin_auth_core::{TOTP_PERIOD_MIN,
// TOTP_PERIOD_MAX}` (both `u32`, lossless under the same cast that
// `compose_manual_period_secs_value` uses) and the `1.0` integer
// step, so `clippy::float_cmp` does not apply.
#[test]
#[allow(clippy::float_cmp)]
fn format_manual_period_adjustment_returns_paladin_auth_core_range() {
    // The TOTP period AdwSpinRow's `gtk::Adjustment` (lower, upper,
    // step_increment) tuple must mirror the
    // `paladin_auth_core::{TOTP_PERIOD_MIN, TOTP_PERIOD_MAX}` validated
    ...
    // `paladin_auth_core::domain::validation`. Sibling of
    // `format_manual_digits_adjustment_returns_paladin_auth_core_range`
    // on the TOTP-period side.
    use paladin_auth_gtk::add_account::format_manual_period_adjustment;

    let (lower, upper, step) = format_manual_period_adjustment();
    assert_eq!(
        lower,
        f64::from(paladin_auth_core::TOTP_PERIOD_MIN),
        "lower bound mirrors paladin_auth_core::TOTP_PERIOD_MIN",
    );
    assert_eq!(
        upper,
        f64::from(paladin_auth_core::TOTP_PERIOD_MAX),
        "upper bound mirrors paladin_auth_core::TOTP_PERIOD_MAX",
    );
    assert_eq!(
        step, 1.0,
...
fn format_manual_counter_adjustment_returns_lossless_u64_range() {
    ...
    // `format_manual_digits_adjustment_returns_paladin_auth_core_range`
    // and `format_manual_period_adjustment_returns_paladin_auth_core_range`
    // on the HOTP-counter side.
    use paladin_auth_gtk::add_account::format_manual_counter_adjustment;
...
fn format_manual_label_title_matches_tui_wording() {
    ...
    // shows (the `"Label:"` row in `crates/paladin-auth-tui/src/view/add.rs`
    ...
    use paladin_auth_gtk::add_account::format_manual_label_title;
...
fn format_manual_issuer_title_matches_tui_wording() {
    ...
    // this helper, so the wording must match what the TUI's add view
    // shows (the `"Issuer:"` row in `crates/paladin-auth-tui/src/view/add.rs`
    // — the colon is the TUI's field-name separator and drops out
    // because AdwEntryRow renders its title as a floating label above
    // the entry rather than as a prefix). Sibling of
    // `format_manual_label_title_matches_tui_wording` on the
    // issuer-entry-title side.
    use paladin_auth_gtk::add_account::format_manual_issuer_title;
...
fn format_manual_secret_title_matches_tui_wording() {
    ...
    // `crates/paladin-auth-tui/src/view/add.rs` — the colon is the TUI's
    ...
    // routes through this helper; it stays inside the Paladin Auth-owned
    ...
    use paladin_auth_gtk::add_account::format_manual_secret_title;
...
fn format_manual_icon_hint_title_matches_tui_wording() {
    ...
    // `paladin_auth_core::parse_icon_hint_token` at submit time; this
    ...
    use paladin_auth_gtk::add_account::format_manual_icon_hint_title;
...
fn format_manual_kind_title_matches_tui_wording() {
    ...
    // this helper, so the wording must match what the TUI's add view
    // shows (the `"Kind:"` row in `crates/paladin-auth-tui/src/view/add.rs`
    // — the colon is the TUI's field-name separator and drops out
    ...
    use paladin_auth_gtk::add_account::format_manual_kind_title;
...
fn format_manual_algorithm_title_matches_tui_wording() {
    ...
    use paladin_auth_gtk::add_account::format_manual_algorithm_title;
...
fn format_manual_digits_title_matches_tui_wording() {
    ...
    use paladin_auth_gtk::add_account::format_manual_digits_title;
...
fn format_manual_period_title_matches_tui_wording() {
    ...
    use paladin_auth_gtk::add_account::format_manual_period_title;
...
fn format_manual_counter_title_matches_tui_wording() {
    ...
    use paladin_auth_gtk::add_account::format_manual_counter_title;
...
fn format_add_dialog_title_matches_tui_wording() {
    ...
    // built by `render` in `crates/paladin-auth-tui/src/view/add.rs` —
    ...
    use paladin_auth_gtk::add_account::format_add_dialog_title;
...
fn format_add_dialog_cancel_label_returns_cancel() {
    ...
    // button.
    use paladin_auth_gtk::add_account::format_add_dialog_cancel_label;
...
fn format_add_dialog_save_label_returns_save() {
    ...
    use paladin_auth_gtk::add_account::format_add_dialog_save_label;
...
fn format_uri_text_title_returns_otpauth_uri() {
    ...
    use paladin_auth_gtk::add_account::format_uri_text_title;
...
// Reads `state.secret_state().uri_text.text()` so the same Paladin Auth-owned
...
fn compose_uri_text_value_fresh_dialog_returns_empty() {
    ...
    // `compose_manual_label_text_fresh_dialog_returns_empty` on the
    // URI-buffer side.
    use paladin_auth_gtk::add_account::{compose_uri_text_value, AddDialogState};
...
fn format_add_dialog_success_toast_returns_added() {
    use paladin_auth_gtk::add_account::format_add_dialog_success_toast;
...
fn parse_add_path_name_manual_slug_returns_manual_path() {
    use paladin_auth_gtk::add_account::parse_add_path_name;
    use paladin_auth_gtk::secret_fields::AddPath;
...
fn apply_msg_save_clicked_does_not_clear_inline_error() {
    ...
    // see flicker.
    use paladin_auth_gtk::add_account::{apply_msg, AddAccountMsg, AddDialogState, InlineError};
...
fn parse_manual_kind_from_selected_zero_returns_totp() {
    ...
    // the correct variant.
    use paladin_auth_core::AccountKindInput;
    use paladin_auth_gtk::add_account::parse_manual_kind_from_selected;
...
fn parse_manual_kind_from_selected_round_trips_format_manual_kind_selected() {
    ...
    use paladin_auth_core::AccountKindInput;
    use paladin_auth_gtk::add_account::{
        format_manual_kind_selected, parse_manual_kind_from_selected,
    };
...
fn parse_manual_algorithm_from_selected_zero_returns_sha1() {
    ...
    // algorithm dropdown side.
    use paladin_auth_core::Algorithm;
    use paladin_auth_gtk::add_account::parse_manual_algorithm_from_selected;
...
fn compose_qr_counts_panel_visible_returns_false_for_default_state() {
    ...
    // parked.
    use paladin_auth_gtk::add_account::{compose_qr_counts_panel_visible, AddDialogState};
...
fn format_qr_counts_panel_imported_label_renders_count() {
    use paladin_auth_gtk::add_account::format_qr_counts_panel_imported_label;
...
fn format_qr_counts_panel_skipped_label_renders_count() {
    use paladin_auth_gtk::add_account::format_qr_counts_panel_skipped_label;
...
fn format_qr_counts_panel_warnings_label_renders_count() {
    use paladin_auth_gtk::add_account::format_qr_counts_panel_warnings_label;
...
fn format_qr_counts_panel_heading_is_non_empty() {
    use paladin_auth_gtk::add_account::format_qr_counts_panel_heading;
...
fn format_qr_counts_panel_dismiss_label_is_non_empty() {
    use paladin_auth_gtk::add_account::format_qr_counts_panel_dismiss_label;
...
fn fresh_add_dialog_seeds_manual_form_to_design_section_5_cli_defaults() {
    ...
    // `paladin-auth add` manual-input defaults without any user input.
...
fn fresh_add_dialog_icon_hint_default_resolves_to_default_from_issuer_mode() {
    ...
    // `paladin_auth_core::parse_icon_hint_token` to map widget text →
    ...
    use paladin_auth_core::{parse_icon_hint_token, IconHintInput};
    use paladin_auth_gtk::add_account::{compose_manual_icon_hint_text, AddDialogState};
...
// `format_pending_warnings_body` → `paladin_auth_core::format_validation_warning`)
...
fn apply_msg_dismiss_duplicate_alert_preserves_manual_draft_state() {
    ...
    assert_eq!(
        paladin_auth_gtk::add_account::compose_manual_label_text(&state),
        "alice",
        "DismissDuplicateAlert preserves the manual label buffer",
    );
    assert_eq!(
        paladin_auth_gtk::add_account::compose_manual_issuer_text(&state),
        "Acme",
...
fn should_present_duplicate_alert_does_not_fire_when_state_has_no_pending() {
    ...
    // state.
    use paladin_auth_gtk::add_account::{should_present_duplicate_alert, AddDialogState};
...
fn format_scan_clipboard_button_label_is_non_empty() {
    ...
    use paladin_auth_gtk::add_account::format_scan_clipboard_button_label;
...
fn compose_scan_clipboard_button_sensitive_inactive_on_manual_path() {
    ...
    // outside the QR page.
    use paladin_auth_gtk::add_account::{compose_scan_clipboard_button_sensitive, AddDialogState};
    use paladin_auth_gtk::secret_fields::AddPath;
...
fn run_qr_worker_plaintext_import_succeeds_and_returns_live_pair_with_report() {
    ...
    use paladin_auth_core::ImportConflict;
    use paladin_auth_gtk::add_account::{
        run_qr_worker, QrWorkerCompletion, QrWorkerEffect, QrWorkerInput,
    };
    use paladin_auth_gtk::qr_clipboard::CLIPBOARD_QR_CONFLICT_POLICY;
...
fn run_qr_worker_persists_imported_accounts_to_disk() {
    ...
    use paladin_auth_gtk::add_account::{run_qr_worker, QrWorkerEffect, QrWorkerInput};
...
fn inline_error_from_qr_preflight_no_clipboard_image_uses_invalid_state_kind() {
    use paladin_auth_gtk::qr_clipboard::QrPreflightError;
...
fn inline_error_from_qr_preflight_layout_rejected_uses_invalid_payload_kind() {
    use paladin_auth_gtk::qr_clipboard::{QrLayoutError, QrPreflightError};
...
fn inline_error_from_qr_preflight_download_mismatch_uses_invalid_payload_kind() {
    use paladin_auth_gtk::qr_clipboard::{DownloadMismatch, QrPreflightError};
...
#[test]
fn inline_error_from_qr_preflight_decode_uses_underlying_paladin_auth_error_display_body() {
    use paladin_auth_gtk::qr_clipboard::QrPreflightError;

    // Decode-variant inline bodies thread through `PaladinAuthError`'s
    // `Display`, identical to `InlineError::from_error`, so the QR
    // sub-path's invalid-payload rendering stays in lockstep with the
    // CLI / TUI verbatim.
    let underlying = PaladinAuthError::NoEntriesToImport;
    ...
    assert_eq!(
        ...
        "decode-variant body must equal the underlying PaladinAuthError Display verbatim",
    );
}
...
fn route_qr_clipboard_loaded_err_no_clipboard_image_routes_to_inline_error() {
    use paladin_auth_gtk::add_account::{route_qr_clipboard_loaded, QrClipboardLoadedDispatch};
    use paladin_auth_gtk::qr_clipboard::QrPreflightError;
...
fn route_qr_clipboard_loaded_err_decode_routes_to_inline_error_with_underlying_kind() {
    ...
    let err = QrPreflightError::Decode(PaladinAuthError::NoEntriesToImport);
    let routing = route_qr_clipboard_loaded(Err(err));
    match routing {
        QrClipboardLoadedDispatch::InlineError(inline) => {
            assert_eq!(
                inline.kind,
                ErrorKind::NoEntriesToImport,
                "Decode wraps the underlying PaladinAuthError so the §5 kind passes through",
            );
        }
        QrClipboardLoadedDispatch::SpawnWorker(_) => {
            panic!("Err preflight must not spawn the worker")
...
fn route_qr_clipboard_loaded_signature_takes_qr_preflight_result() {
    ...
    fn assert_signature(
        _: fn(
            std::result::Result<Vec<paladin_auth_core::ValidatedAccount>, QrPreflightError>,
...
fn dispatch_root_dismiss_bare_escape_returns_true() {
    use paladin_auth_gtk::add_account::dispatch_root_dismiss_key;
    use relm4::gtk::gdk;
