...
//! Desktop-entry contract tests for `paladin-auth-gtk`.
//!
//! Per `docs/IMPLEMENTATION_PLAN_04_GTK.md` §"Linux desktop integration" and
//! the Milestone 7 checklist entry "Write
//! `data/org.tamx.PaladinAuth.Gui.desktop`":
//!
//! * `Name=Paladin Auth`
//! * `Icon=org.tamx.PaladinAuth.Gui` (the §11.4 app ID — same string
//!   `RelmApp::new(APP_ID)` consumes and the hicolor install layout
//!   resolves)
//! * `StartupWMClass=org.tamx.PaladinAuth.Gui` (so window-to-launcher
...
//! * `Exec=paladin-auth-gtk` with **no** `%F` / `%U` / `%f` / `%u` file or
...
const DESKTOP_FILE_RELPATH: &str = "data/org.tamx.PaladinAuth.Gui.desktop";
...
#[test]
fn desktop_entry_name_is_paladin_auth() {
    let entry = parse_desktop_entry_section(&read_desktop_file());
    assert_eq!(entry.get("Name").map(String::as_str), Some("Paladin Auth"));
}
...
#[test]
fn desktop_entry_exec_is_paladin_auth_gtk_binary_name() {
    let entry = parse_desktop_entry_section(&read_desktop_file());
    assert_eq!(
...
fn desktop_entry_keywords_covers_security_authenticator_vocabulary() {
    ...
    // The launcher's search index uses these terms to surface Paladin Auth
    ...
    // "otp", "totp", "hotp", or "authenticator" finds Paladin Auth.
