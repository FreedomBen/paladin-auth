...
//! Pure-logic tests for `paladin_auth_gtk::column_view`.
//!
...
use paladin_auth_core::{AccountId, AccountKindSummary};
use paladin_auth_gtk::account_list::AccountRowModel;
use paladin_auth_gtk::column_view::{
...
fn apply_interleaved_inserts_section_rows_at_predicate_positions() {
    ...
    let expected_account_ids: Vec<Option<paladin_auth_core::AccountId>> = vec![
        None,
        Some(rows[0].id),
        Some(rows[1].id),
        None,
    ...
    let actual_ids: Vec<Option<paladin_auth_core::AccountId>> =
...
fn apply_interleaved_account_row_identity_survives_show_section_headers_toggle() {
    ...
    let store = apply_rows(&rows, false);
    let account_ptrs_before: Vec<(paladin_auth_core::AccountId, usize)> = collect_row_items(&store)
        .iter()
    ...
    let account_ptrs_after_enable: Vec<(paladin_auth_core::AccountId, usize)> =
    ...
    let account_ptrs_after_disable: Vec<(paladin_auth_core::AccountId, usize)> =
        collect_row_items(&store)
            .iter()
            .filter_map(|item| item.account_id().map(|id| (id, item.as_ptr() as usize)))
            .collect();
...
fn apply_interleaved_section_rows_match_row_section_header_predicate() {
    use paladin_auth_gtk::account_list::row_section_header;

    // Verifies the structural invariant directly against the underlying
    // predicate `paladin_auth_gtk::account_list::row_section_header`, which is
...
fn apply_interleaved_repeated_toggle_is_idempotent_for_account_rows() {
    ...
    let store = apply_rows(&rows, true);
    let snapshot: Vec<(paladin_auth_core::AccountId, usize)> = collect_row_items(&store)
        .iter()
        .filter_map(|item| item.account_id().map(|id| (id, item.as_ptr() as usize)))
        .collect();
    ...
    for show in [false, true, false, true, false, true] {
        ...
        let ptrs: Vec<(paladin_auth_core::AccountId, usize)> = collect_row_items(&store)
            .iter()
            .filter_map(|item| item.account_id().map(|id| (id, item.as_ptr() as usize)))
            .collect();
        assert_eq!(
