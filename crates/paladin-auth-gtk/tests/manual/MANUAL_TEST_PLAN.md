...
# `paladin-auth-gtk` Manual Test Plan

This document is the human-driven QA checklist for `paladin-auth-gtk`. It
complements the pure-logic unit tests under
`crates/paladin-auth-gtk/tests/*_logic.rs` and the headless `xvfb-run` smoke
test at `crates/paladin-auth-gtk/tests/gtk_smoke.rs` by exercising paths the
...
`crates/paladin-auth-gtk/tests/manual_test_plan_doc.rs` guards drift between
...
1. Build the binary in release mode: `cargo build --release -p paladin-auth-gtk`.
2. Pick a scratch `XDG_DATA_HOME` so the QA run never touches real data:
   `export XDG_DATA_HOME="$(mktemp -d)/paladin-auth-qa"`.
...
4. For the §10 fault-injection items, build with the fault-injection
   feature flag exposed by `paladin-auth-core` (see `docs/DESIGN.md` §10) and
...
5. For the clipboard-image items, prepare both a valid `otpauth://` QR
   ...
   `paladin_auth_core::import::QR_IMAGE_MAX_BYTES`).
...
- [ ] Init plaintext vault: both passphrase fields empty + warning
  ...
  * Expected: the `format_plaintext_storage_warning()` text from
    `paladin-auth-core` is rendered verbatim, the Submit button stays
...
- [ ] Init when a vault already exists at the path opens the
  ...
  * Tied to: `paladin_auth_core::classify_init_precheck`,
    `paladin_auth_core::create_force`.
...
- [ ] HOTP reveal window expires and the row returns to hidden.
  * Expected: after `paladin_auth_core::HOTP_REVEAL_SECS`, the row
...
- [ ] Add via `otpauth://` URI paste — success path.
  * Expected: pasting a valid URI parses through
    `paladin_auth_core::parse_otpauth`, fills the preview, and submits
...
- [ ] Add via clipboard image — success path.
  * Expected: a valid `otpauth://` QR image on the clipboard is
    decoded via `paladin_auth_core::import::qr_image_bytes` (no `image`
    or `rqrr` reaches `paladin-auth-gtk` directly), the resulting
...
- [ ] Add via clipboard image — oversized-image rejection before
  ...
  * Expected: a clipboard image whose byte size exceeds
    `paladin_auth_core::import::QR_IMAGE_MAX_BYTES` is rejected inline
...
- [ ] Import encrypted Paladin Auth bundle with each on-conflict policy;
  ...
  * Expected: the bundle-passphrase row is prompted only when
    `paladin_auth_core::classify_paladin_auth_import_precheck` returns
...
- [ ] Auto-lock fires while the Import dialog is open — the dialog's
  ...
  * Expected: with an encrypted vault, drop the auto-lock interval
    ...
    landing after lock.
  * Tied to: `crates/paladin-auth-gtk/src/import_dialog.rs`
    `connect_clicked` closures, `crates/paladin-auth-gtk/src/app/model.rs`
...
- [ ] Cancel button dismisses the Import / Export / Settings
  ...
  * Tied to: `crates/paladin-auth-gtk/src/app/model.rs`
...
- [ ] Settings persist across restart.
  * Expected: changes to auto-lock interval, clipboard-clear
    ...
    `paladin_auth_core::AUTO_LOCK_SECS_MIN..=AUTO_LOCK_SECS_MAX` and
...
- [ ] Click the Next cell on a TOTP row → clipboard holds the
  ...
  * Tied to: `tests/account_list_logic.rs` Next-column routing,
    `paladin_auth_core::Vault::totp_next_code`, `docs/DESIGN.md` §7.
...
- [ ] Open the kebab menu on a TOTP row → `Show QR…` is the
  ...
  * Expected: the kebab menu lists three rows in the pinned order
    ...
    `paladin_auth_core::format_plaintext_qr_export_warning()` body as
...
- [ ] With the QR visible, wait for auto-lock to fire → the
  ...
  * Tied to: `tests/export_qr_dialog_logic.rs`
    ...
    `crates/paladin-auth-gtk/src/app/model.rs` `lock_on_auto_lock_expiry`
...
verbatim from `paladin_auth_core::format_destroy_warning`; the destructive
response is gated until the confirmation entry reads `yes`. The
deletion runs `paladin_auth_core::destroy_vault` on a `gio::spawn_blocking`
...
- [ ] Destroy vault while another dialog (Add, Edit, Passphrase) is
  ...
  * Tied to: `tests/destroy_dialog_logic.rs`
    ...
    `crates/paladin-auth-gtk/src/app/model.rs` `complete_destroy_transition`.
...
* `paladin-auth-gtk --version` and the workspace commit hash.
