...
//! Pure-logic settings tests for `paladin-auth-gtk`.
//!
...
//!   [`paladin_auth_core::SettingPatch`] per accepted transition).
//! * Spinners clamp to
...
//!
//! The module under test (`paladin_auth_gtk::settings`) is the pure-logic
...
use paladin_auth_core::{
    ErrorKind, PaladinAuthError, SettingPatch, AUTO_LOCK_SECS_MAX, AUTO_LOCK_SECS_MIN,
    CLIPBOARD_CLEAR_SECS_MAX, CLIPBOARD_CLEAR_SECS_MIN,
};

use paladin_auth_gtk::settings::{
...
fn format_settings_dialog_auto_lock_group_title_returns_auto_lock() {
    ...
    // `paladin_auth_core::VaultSettings::auto_lock_enabled` /
    // `paladin_auth_core::VaultSettings::auto_lock_secs` fields
    ...
    use paladin_auth_gtk::settings::format_settings_dialog_auto_lock_group_title;
...
fn format_settings_dialog_clipboard_clear_enabled_row_title_returns_clear_after_copy() {
    ...
    // that controls
    // `paladin_auth_core::VaultSettings::clipboard_clear_enabled`.
    ...
    // GNOME HIG.
    use paladin_auth_gtk::settings::format_settings_dialog_clipboard_clear_enabled_row_title;
...
fn format_settings_dialog_auto_lock_secs_row_title_returns_inactivity_timeout() {
    ...
    // controls `paladin_auth_core::VaultSettings::auto_lock_secs`.
    ...
    use paladin_auth_gtk::settings::format_settings_dialog_auto_lock_secs_row_title;
...
fn format_settings_dialog_auto_lock_enabled_row_title_returns_lock_after_inactivity() {
    ...
    // controls `paladin_auth_core::VaultSettings::auto_lock_enabled`.
    // Per `docs/IMPLEMENTATION_PLAN_04_GTK.md` §"libadwaita usage" >
    ...
    // already names that concept). Verb-led wording per the
    // GNOME HIG.
    use paladin_auth_gtk::settings::format_settings_dialog_auto_lock_enabled_row_title;
...
fn format_settings_dialog_clipboard_clear_group_title_returns_clipboard() {
    ...
    // `paladin_auth_core::VaultSettings::clipboard_clear_enabled` /
    // `paladin_auth_core::VaultSettings::clipboard_clear_secs` fields
    ...
    use paladin_auth_gtk::settings::format_settings_dialog_clipboard_clear_group_title;
...
fn compose_settings_dialog_auto_lock_secs_value_casts_visible_auto_lock_secs_to_f64() {
    ...
    // inline.
    //
    // Sibling of `paladin_auth_gtk::add_account::compose_manual_period_secs_value`,
...
#[test]
fn format_settings_dialog_auto_lock_secs_adjustment_returns_paladin_auth_core_bounds() {
    ...
    //
    // Sibling of `paladin_auth_gtk::add_account::format_manual_period_adjustment`,
    ...
    // domain.
    use paladin_auth_gtk::settings::format_settings_dialog_auto_lock_secs_adjustment;

    let (lower, upper, step) = format_settings_dialog_auto_lock_secs_adjustment();
    assert!(
        (lower - f64::from(paladin_auth_core::AUTO_LOCK_SECS_MIN)).abs() < f64::EPSILON,
        "AdwSpinRow lower bound mirrors paladin_auth_core::AUTO_LOCK_SECS_MIN",
    );
    assert!(
        (upper - f64::from(paladin_auth_core::AUTO_LOCK_SECS_MAX)).abs() < f64::EPSILON,
        "AdwSpinRow upper bound mirrors paladin_auth_core::AUTO_LOCK_SECS_MAX",
    );
    assert!(
...
#[test]
fn format_settings_dialog_clipboard_clear_secs_adjustment_returns_paladin_auth_core_bounds() {
    ...
    use paladin_auth_gtk::settings::format_settings_dialog_clipboard_clear_secs_adjustment;

    let (lower, upper, step) = format_settings_dialog_clipboard_clear_secs_adjustment();
    assert!(
        (lower - f64::from(paladin_auth_core::CLIPBOARD_CLEAR_SECS_MIN)).abs() < f64::EPSILON,
        "AdwSpinRow lower bound mirrors paladin_auth_core::CLIPBOARD_CLEAR_SECS_MIN",
    );
    assert!(
        (upper - f64::from(paladin_auth_core::CLIPBOARD_CLEAR_SECS_MAX)).abs() < f64::EPSILON,
        "AdwSpinRow upper bound mirrors paladin_auth_core::CLIPBOARD_CLEAR_SECS_MAX",
    );
    assert!(
        (step - 1.0).abs() < f64::EPSILON,
        "AdwSpinRow step is 1 second per click for the integer-only seconds domain",
...
fn format_settings_dialog_clipboard_clear_secs_row_title_returns_clear_delay() {
    ...
    // controls `paladin_auth_core::VaultSettings::clipboard_clear_secs`.
    // Per `docs/IMPLEMENTATION_PLAN_04_GTK.md` §"libadwaita usage" >
    // "Preferences" and §"Component tree" > `SettingsComponent`,
    // the spinner clamps to
    // `paladin_auth_core::CLIPBOARD_CLEAR_SECS_MIN..=paladin_auth_core::CLIPBOARD_CLEAR_SECS_MAX`
    ...
    use paladin_auth_gtk::settings::format_settings_dialog_clipboard_clear_secs_row_title;
...
fn format_settings_dialog_saved_toast_returns_settings_saved() {
    ...
    // toast (see `crates/paladin-auth-tui/src/view`), so the wording
    // is GTK-specific.
    use paladin_auth_gtk::settings::format_settings_dialog_saved_toast;
...
fn format_settings_dialog_title_returns_preferences() {
    ...
    // (see `crates/paladin-auth-tui/src/view`), so the wording is
    // GTK-specific. Sibling of
    // `paladin_auth_gtk::unlock_dialog::format_unlock_dialog_title`,
    // `paladin_auth_gtk::init_dialog::format_init_dialog_title`,
    // `paladin_auth_gtk::edit_dialog::format_edit_dialog_title`,
    // `paladin_auth_gtk::remove_dialog::format_remove_dialog_title`,
    // `paladin_auth_gtk::add_account::format_add_dialog_title`, and
    // `paladin_auth_gtk::startup_error::format_startup_error_title`
    // on the dialog-header-title side; together they pin every
    // dialog's titled surface against a single source of truth.
    use paladin_auth_gtk::settings::format_settings_dialog_title;
...
fn compose_settings_dialog_inline_subtitle_for_field_returns_none_without_outcome() {
    ...
    // inline.
    use paladin_auth_gtk::settings::compose_settings_dialog_inline_subtitle_for_field;
...
fn compose_settings_dialog_inline_subtitle_revealed_for_field_mirrors_subtitle_text_for_field() {
    ...
    let io_err = PaladinAuthError::IoError {
        operation: "write",
        source: std::io::Error::other("disk full"),
    };
    let scenarios: [(Option<SaveOutcome>, &'static str); 5] = [
        ...
        (
            Some(SaveOutcome::DurabilityWarning {
                warning: InlineWarning::from_error(&PaladinAuthError::SaveDurabilityUnconfirmed),
                field: SettingsField::AutoLockSecs,
            }),
...
fn compose_settings_dialog_inline_subtitle_css_class_for_field_routes_error_and_warning_by_variant()
{
    ...
    use paladin_auth_gtk::settings::compose_settings_dialog_inline_subtitle_css_class_for_field;
...
fn accepted_change_from_setting_patch_mirrors_setting_patch_enum_field_for_field() {
    ...
    // touches: `paladin_auth_core::SettingPatch` (returned in
    ...
    use paladin_auth_gtk::settings::accepted_change_from_setting_patch;
...
fn format_settings_dialog_spinner_page_increment_returns_ten() {
    ...
    use paladin_auth_gtk::settings::format_settings_dialog_spinner_page_increment;
...
fn format_settings_dialog_spinner_page_size_returns_zero() {
    ...
    use paladin_auth_gtk::settings::format_settings_dialog_spinner_page_size;
...
fn format_settings_dialog_spinner_climb_rate_returns_one() {
    ...
    // already cover).
    use paladin_auth_gtk::settings::format_settings_dialog_spinner_climb_rate;
...
fn format_settings_dialog_spinner_digits_returns_zero() {
    ...
    // receives.
    use paladin_auth_gtk::settings::format_settings_dialog_spinner_digits;
...
fn format_settings_dialog_spinner_wrap_returns_false() {
    ...
    use paladin_auth_gtk::settings::format_settings_dialog_spinner_wrap;
...
fn format_settings_dialog_spinner_numeric_returns_true() {
    ...
    //
    // Pure — returns a `bool` without allocating.
    use paladin_auth_gtk::settings::format_settings_dialog_spinner_numeric;
...
fn format_settings_dialog_spinner_snap_to_ticks_returns_true() {
    ...
    // spinner-property literal the `SettingsComponent` sets beyond
    // the `gtk::Adjustment` bounds.
    use paladin_auth_gtk::settings::format_settings_dialog_spinner_snap_to_ticks;
...
fn format_settings_dialog_spinner_debounce_returns_500_ms() {
    ...
    use paladin_auth_gtk::settings::format_settings_dialog_spinner_debounce;
...
fn format_settings_dialog_search_enabled_returns_false() {
    ...
    //
    // Pure — returns a `bool` without allocating.
    use paladin_auth_gtk::settings::format_settings_dialog_search_enabled;
...
fn format_settings_dialog_saved_toast_use_markup_returns_false() {
    ...
    //
    // Pure — returns a `bool` without allocating.
    use paladin_auth_gtk::settings::format_settings_dialog_saved_toast_use_markup;
...
fn format_settings_dialog_saved_toast_timeout_returns_five_seconds() {
    ...
    use paladin_auth_gtk::settings::format_settings_dialog_saved_toast_timeout;
...
fn settings_dialog_display_group_title_is_pinned() {
    use paladin_auth_gtk::settings::format_settings_dialog_display_group_title;
...
fn settings_dialog_section_headers_row_title_is_pinned() {
    use paladin_auth_gtk::settings::format_settings_dialog_section_headers_row_title;
...
fn settings_dialog_section_headers_row_subtitle_calls_out_default_off() {
    use paladin_auth_gtk::settings::format_settings_dialog_section_headers_row_subtitle;
...
fn settings_dialog_column_headers_row_title_is_pinned() {
    use paladin_auth_gtk::settings::format_settings_dialog_column_headers_row_title;
...
fn settings_dialog_column_headers_row_subtitle_calls_out_default_on() {
    use paladin_auth_gtk::settings::format_settings_dialog_column_headers_row_subtitle;
...
fn settings_dialog_next_code_column_row_title_is_pinned() {
    ...
    use paladin_auth_gtk::settings::format_settings_dialog_next_code_column_row_title;
...
fn settings_dialog_next_code_column_row_subtitle_calls_out_default_on() {
    ...
    use paladin_auth_gtk::settings::format_settings_dialog_next_code_column_row_subtitle;
...
fn settings_dialog_next_code_column_toggle_round_trips_via_helper() {
    ...
    use paladin_auth_gtk::gsettings::{
        set_show_next_code_column, show_next_code_column, SCHEMA_ID,
    };
    use relm4::gtk::gio;

    let schema_dir = env!("PALADIN_AUTH_GTK_SCHEMA_DIR");
    let source = gio::SettingsSchemaSource::from_directory(schema_dir, None, false)
        .expect("build.rs compiled the gschema into PALADIN_AUTH_GTK_SCHEMA_DIR");
...
fn settings_dialog_next_code_column_toggle_fires_changed_signal() {
    ...
    // closure verbatim.
    use paladin_auth_gtk::gsettings::{
        set_show_next_code_column, SCHEMA_ID, SHOW_NEXT_CODE_COLUMN_KEY,
    };
    use relm4::gtk::gio;
...
fn settings_dialog_init_round_trips_committed_settings() {
    use paladin_auth_gtk::gsettings::SCHEMA_ID;
    use paladin_auth_gtk::settings::SettingsDialogInit;
    use relm4::gtk::gio;
...
fn settings_dialog_output_close_is_constructible() {
    use paladin_auth_gtk::settings::SettingsDialogOutput;
...
fn settings_component_input_and_output_match_dispatch_edges() {
    use paladin_auth_gtk::settings::{SettingsComponent, SettingsDialogMsg, SettingsDialogOutput};
...
fn classify_settings_save_result_success_maps_to_save_outcome_success() {
    use paladin_auth_gtk::settings::{classify_settings_save_result, AcceptedChange, SaveOutcome};
...
fn apply_save_outcome_success_promotes_attempted_value_to_committed() {
    use paladin_auth_gtk::settings::{AcceptedChange, SaveOutcome, SettingsState};
    let mut state = SettingsState::new(defaults());
...
fn dispatch_settings_dialog_msg_auto_lock_toggled_value_change_returns_submit() {
    use paladin_auth_core::SettingPatch;
    use paladin_auth_gtk::settings::{
...
fn dispatch_settings_dialog_msg_auto_lock_secs_spinner_change_returns_stage_debounce() {
    ...
    assert_eq!(
        ...
        raw.min(paladin_auth_core::AUTO_LOCK_SECS_MAX)
    );
}
...
fn dispatch_settings_dialog_msg_set_busy_idempotent_same_value() {
    use paladin_auth_gtk::settings::{
        dispatch_settings_dialog_msg, SettingsDialogMsg, SettingsState,
    };
    let mut state = SettingsState::new(defaults());
    dispatch_settings_dialog_msg(&mut state, SettingsDialogMsg::SetBusy(true));
    dispatch_settings_dialog_msg(&mut state, SettingsDialogMsg::SetBusy(true));
