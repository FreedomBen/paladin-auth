...
//! Pure-logic coverage for `paladin_auth_gtk::export_qr_dialog`.
//!
...
use paladin_auth_core::{
    ...
    PaladinAuthError, QrRenderOptions, Store, Vault, VaultInit, VaultLock,
};
use paladin_auth_gtk::export_qr_dialog::{
...
#[test]
fn format_export_qr_dialog_warning_body_matches_paladin_auth_core_verbatim() {
    // The warning-footer body must be the verbatim
    // `paladin_auth_core::format_plaintext_qr_export_warning()` output so
    // the CLI / TUI / GTK warnings flow through one helper — a future
    // warning reword lands in `paladin-auth-core` once and every front-end
...
fn compose_export_qr_caption_text_reads_summary_display_label() {
    ...
    // `paladin_auth_core::summary_display_label(&state.account_summary)`
...
fn run_export_qr_save_worker_png_missing_parent_surfaces_save_not_committed_inline() {
    ...
    assert_eq!(error.kind, paladin_auth_core::ErrorKind::SaveNotCommitted);
}
...
fn classify_export_qr_save_error_io_error_renders_inline() {
    ...
    assert_eq!(error.kind, paladin_auth_core::ErrorKind::IoError);
}
...
fn classify_export_qr_save_error_validation_error_renders_inline() {
    ...
    assert_eq!(error.kind, paladin_auth_core::ErrorKind::ValidationError);
}
