...
//! Pure-logic `ticker` tests for `paladin-auth-gtk`.
//!
//! Tracks `docs/IMPLEMENTATION_PLAN_04_GTK.md` §"Milestone 7 checklist"
//! TOTP ticker section:
//!
//! * `tick_interval()` mirrors `paladin_auth_core::TICK_INTERVAL_MS`.
...
//!
//! The module under test (`paladin_auth_gtk::ticker`) is widget-free and
...
use paladin_auth_core::{
    ...
    IconHintInput, PaladinAuthError, Store, Vault, VaultInit, VaultLock, TICK_INTERVAL_MS,
};

use paladin_auth_gtk::account_list::AccountRowModel;
use paladin_auth_gtk::account_row::{CodeDisplay, ProgressDisplay, RowDisplay};
use paladin_auth_gtk::app::state::AppState;
use paladin_auth_gtk::clipboard_clear::PendingClipboardClear;
use paladin_auth_gtk::startup_error::StartupError;
use paladin_auth_gtk::ticker::{
...
fn vault_path() -> PathBuf {
    PathBuf::from("/tmp/paladin-auth-ticker-fixture.bin")
}
...
#[test]
fn tick_interval_matches_paladin_auth_core_constant() {
    // Routing the per-tick interval through `paladin_auth_core` keeps the
...
fn tick_interval_is_nonzero() {
    ...
    // ticker pins. `paladin_auth_core::TICK_INTERVAL_MS` is 250 ms today;
...
// visible code from `paladin_auth_core::totp_code(account, now)` for every
...
fn pending_with_deadline(
    ...
    let mut pending = paladin_auth_gtk::clipboard_clear::schedule_copy(
        Instant::now(),
        vault.settings(),
