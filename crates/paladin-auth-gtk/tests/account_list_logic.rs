...
//! built from `paladin_auth_core::AccountSummary` projections — the
...
//! the stdout marker `paladin-auth-gtk` emits under `--exit-after-startup`
...
use paladin_auth_gtk::account_list::{
...
use paladin_auth_gtk::column_view::apply_interleaved_splice_plan;
use paladin_auth_gtk::row_item::RowItem;
...
fn row_models_after_sequence_of_add_rename_remove_preserves_surviving_order() {
    ...
    // `paladin_auth_gtk::app::state`). The single-mutation tests above pin
...
fn marker_empty_list_emits_empty_suffix() {
    ...
    assert_eq!(
        ...
        "paladin-auth-gtk: account_list_rows="
    );
}
...
fn marker_pipe_joins_display_labels_in_order() {
    ...
    assert_eq!(
        ...
        "paladin-auth-gtk: account_list_rows=GitHub:ben|GitLab:alice|solo",
    );
}
...
fn widget_states_marker_prefix_is_pinned() {
    assert_eq!(
        ...
        "paladin-auth-gtk: account_list_widget_states=",
    );
}
...
fn widget_states_marker_renders_copy_on_next_off_kebab_on_for_totp() {
    ...
    assert_eq!(
        format_widget_states_marker(&displays),
        "paladin-auth-gtk: account_list_widget_states=copy:on,next:off,kebab:on",
    );
}
...
fn widget_states_marker_renders_copy_off_next_on_kebab_on_for_hidden_hotp() {
    ...
    assert_eq!(
        format_widget_states_marker(&displays),
        "paladin-auth-gtk: account_list_widget_states=copy:off,next:on,kebab:on",
    );
}
...
fn widget_states_marker_renders_kebab_off_when_projection_disables_it() {
    ...
    assert_eq!(
        format_widget_states_marker(&displays),
        "paladin-auth-gtk: account_list_widget_states=copy:on,next:off,kebab:off",
    );
}
...
fn widget_states_marker_pipe_joins_in_order() {
    ...
    assert_eq!(
        format_widget_states_marker(&displays),
        "paladin-auth-gtk: account_list_widget_states=copy:on,next:off,kebab:on|copy:off,next:on,kebab:on|copy:on,next:off,kebab:on",
    );
}
...
#[test]
fn dispatch_row_action_routes_remove_to_request_remove() {
    let id = AccountId::new();
    assert_eq!(
        dispatch_row_action(ROW_REMOVE_ACTION_NAME, id),
        Some(AccountRowOutput::RequestRemove(id)),
    );
}

#[test]
fn dispatch_row_action_routes_next_to_request_advance() {
    // The HOTP "next" button's `row.next` activation resolves to
    // `AccountRowOutput::RequestAdvance(id)`. The parent
    ...
    assert_eq!(
        ...
        Some(AccountRowOutput::RequestAdvance(id)),
    );
}

#[test]
fn dispatch_row_action_routes_copy_to_request_copy() {
    // The per-row copy button's `row.copy` activation resolves to
    // `AccountRowOutput::RequestCopy(id)`; the parent
    // `AccountListComponent` forwards it via [`forward_row_output`]
    ...
    assert_eq!(
        ...
        Some(AccountRowOutput::RequestCopy(id)),
    );
}
...
fn compute_next_code_column_visibility_pref_off_collapses_to_false() {
    ...
    use paladin_auth_gtk::account_list::compute_next_code_column_visibility;
...
fn tick_routes_only_to_changed_rows() {
    ...
    use paladin_auth_core::AccountKindSummary;
    ...
    let display_a = hidden_row_display(&AccountRowModel {
        id: a,
        display_label: "issuer:a".to_string(),
        kind: AccountKindSummary::Totp,
        counter: None,
        icon_hint: None,
        issuer: None,
    ...
    let display_c = hidden_row_display(&AccountRowModel {
        id: c,
        display_label: "issuer:c".to_string(),
        kind: AccountKindSummary::Totp,
        counter: None,
        icon_hint: None,
        issuer: None,
...
#[test]
fn tick_dispatch_plan_drops_ids_no_longer_in_factory() {
    // Edge case: a tick that races a search-filter refresh can
    // carry an `AccountId` that has just been removed from the
    ...
    let display_visible = hidden_row_display(&AccountRowModel {
        id: visible,
        display_label: "issuer:v".to_string(),
        kind: AccountKindSummary::Totp,
        counter: None,
        icon_hint: None,
        issuer: None,
    ...
    let display_removed = hidden_row_display(&AccountRowModel {
        id: removed,
        display_label: "issuer:r".to_string(),
        kind: AccountKindSummary::Totp,
        counter: None,
        icon_hint: None,
        issuer: None,
...
// Composes [`row_models_from_vault`] with `paladin_auth_core::
...
fn filtered_row_models_match_key_keeps_colon_for_empty_issuer() {
    ...
    // `paladin_auth_core::account_matches_search` and the parallel
...
// ---------------------------------------------------------------------------
//
// Wraps `paladin_auth_core::select_after_filter` against
...
fn tick_dispatch_fans_change_signal_through_set_display() {
    ...
    use paladin_auth_gtk::row_item::ROW_ITEM_DISPLAY_CHANGED_SIGNAL;
    use std::cell::Cell;
    use std::rc::Rc;
