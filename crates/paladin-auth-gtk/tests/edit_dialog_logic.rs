...
//! Pure-logic edit-dialog tests for `paladin-auth-gtk`.
//!
...
use paladin_auth_core::{
    ...
    AccountKindSummary, AccountSummary, Algorithm, ErrorKind, IconHintInput, PaladinAuthError,
    Store, Vault, VaultInit, VaultLock,
};

use paladin_auth_gtk::edit_dialog::{
...
fn marker_prefix_carries_edit_dialog_namespace() {
    assert_eq!(
        ...
        "paladin-auth-gtk: edit_dialog_account="
    );
}
...
#[test]
fn inline_error_carries_kind_and_renders_paladin_auth_error_body() {
    let err = PaladinAuthError::SaveNotCommitted {
        committed: false,
        backup_path: None,
    };
...
#[test]
fn inline_warning_carries_kind_and_renders_paladin_auth_error_body() {
    let err = PaladinAuthError::SaveDurabilityUnconfirmed;
