...
//! packaging checklist entry "Add `packaging/flatpak/paladin-auth-gtk.yml`":
//!
//! * Declares the app ID `org.tamx.PaladinAuth.Gui` — the same string
//!   passed to `RelmApp::new(APP_ID)` and set as `StartupWMClass`
//!   in `data/org.tamx.PaladinAuth.Gui.desktop` so window-to-launcher
...
//! * Sets `command: paladin-auth-gtk` so `flatpak run org.tamx.PaladinAuth.Gui`
...
//!   `--filesystem=xdg-config/paladin-auth:create`) and **does not**
...
//! * Installs the binary to `/app/bin/paladin-auth-gtk` (mode `0755`),
...
const FLATPAK_MANIFEST_RELPATH: &str = "packaging/flatpak/paladin-auth-gtk.yml";

/// The Flatpak app ID. Must match `paladin_auth_gtk::APP_ID` byte-for-byte
...
const APP_ID: &str = "org.tamx.PaladinAuth.Gui";
...
const REQUIRED_INSTALL_DESTINATIONS: &[&str] = &[
    "/app/bin/paladin-auth-gtk",
    "/app/share/applications/org.tamx.PaladinAuth.Gui.desktop",
    "/app/share/metainfo/org.tamx.PaladinAuth.Gui.metainfo.xml",
    "/app/share/icons/hicolor/scalable/apps/org.tamx.PaladinAuth.Gui.svg",
    "/app/share/icons/hicolor/symbolic/apps/org.tamx.PaladinAuth.Gui-symbolic.svg",
    "/app/share/icons/hicolor/16x16/apps/org.tamx.PaladinAuth.Gui.png",
    "/app/share/icons/hicolor/24x24/apps/org.tamx.PaladinAuth.Gui.png",
    "/app/share/icons/hicolor/32x32/apps/org.tamx.PaladinAuth.Gui.png",
    "/app/share/icons/hicolor/48x48/apps/org.tamx.PaladinAuth.Gui.png",
    "/app/share/icons/hicolor/64x64/apps/org.tamx.PaladinAuth.Gui.png",
    "/app/share/icons/hicolor/128x128/apps/org.tamx.PaladinAuth.Gui.png",
    "/app/share/icons/hicolor/256x256/apps/org.tamx.PaladinAuth.Gui.png",
    "/app/share/icons/hicolor/512x512/apps/org.tamx.PaladinAuth.Gui.png",
];
...
fn flatpak_manifest_exists_at_expected_path() {
    ...
    assert!(
        ...
        "expected Flatpak manifest at {} — Milestone 7 packaging \
         checklist requires `packaging/flatpak/paladin-auth-gtk.yml`",
        path.display(),
    );
}
...
fn flatpak_manifest_declares_app_id_matching_app_constant() {
    ...
    assert_eq!(
        app_id, APP_ID,
        "Flatpak `app-id:` must equal `paladin_auth_gtk::APP_ID` so the desktop entry's \
...
#[test]
fn flatpak_manifest_declares_command_paladin_auth_gtk() {
    let manifest = read_flatpak_manifest();
    let command = top_level_scalar(&manifest, "command")
        .expect("Flatpak manifest has a top-level `command:` key");
    assert_eq!(
        command, "paladin-auth-gtk",
        "Flatpak `command:` must be `paladin-auth-gtk` so `flatpak run {APP_ID}` invokes the \
         installed binary at /app/bin/paladin-auth-gtk; got {command:?}",
    );
}
...
fn flatpak_manifest_does_not_declare_any_forbidden_finish_arg() {
    ...
    // file system surface is also scoped strictly to the paladin-auth
    ...
    assert!(
        ...
        "Flatpak `finish-args:` must NOT declare any of {FORBIDDEN_FINISH_ARGS:?} — \
         ...
         strictly scoped to the paladin-auth XDG namespace; found: {found:?}",
    );
}
...
fn flatpak_manifest_binary_install_uses_executable_mode_0755() {
    // The /app/bin/paladin-auth-gtk install MUST be 0755 — anything less
    ...
    let landed = cmds.iter().any(|cmd| {
        cmd.contains("/app/bin/paladin-auth-gtk")
            && (cmd.contains("-Dm755") || cmd.contains("--mode=755"))
    });
    assert!(
        landed,
        "Flatpak `build-commands:` must install /app/bin/paladin-auth-gtk with mode 0755 \
         (e.g. `install -Dm755 target/release/paladin-auth-gtk /app/bin/paladin-auth-gtk`); \
         got commands: {cmds:?}",
    );
}
...
fn flatpak_manifest_metainfo_install_lands_under_app_share_metainfo() {
    ...
    let cmds = all_build_commands(&manifest);
    let landed = cmds.iter().any(|cmd| {
        cmd.contains("crates/paladin-auth-gtk/data/metainfo/org.tamx.PaladinAuth.Gui.metainfo.xml")
            && cmd.contains("/app/share/metainfo/org.tamx.PaladinAuth.Gui.metainfo.xml")
    });
    assert!(
        landed,
        "Flatpak `build-commands:` must install the AppStream metainfo from \
         crates/paladin-auth-gtk/data/metainfo/... to /app/share/metainfo/... — that path \
...
fn flatpak_manifest_uses_locked_offline_cargo_build() {
    ...
    assert!(
        !cargo_cmds.is_empty(),
        "Flatpak `build-commands:` must invoke `cargo` to build paladin-auth-gtk; got commands: \
         {cmds:?}",
    );
...
fn flatpak_manifest_module_name_matches_app_id_basename() {
    ...
    for raw in lines {
        ...
        if trimmed == "- name: paladin-auth-gtk" || trimmed == "- name: \"paladin-auth-gtk\"" {
            found = true;
            break;
        }
    }
    assert!(
        found,
        "Flatpak `modules:` list must contain an entry whose `name` is `paladin-auth-gtk`",
    );
}
...
";
    ...
    assert_eq!(
        cmds,
        vec![
            "cargo build --release --locked --offline -p paladin-auth-gtk".to_string(),
            "install -Dm755 target/release/paladin-auth-gtk /app/bin/paladin-auth-gtk".to_string(),
        ],
    );
}
