...
//! `AppStream` metainfo contract tests for `paladin-auth-gtk`.
//!
...
//! `data/metainfo/org.tamx.PaladinAuth.Gui.metainfo.xml`":
//!
//! * `<component type="desktop-application">` so the freedesktop /
//!   GNOME Software UI categorizes Paladin Auth correctly.
//! * `<id>` matches the `paladin_auth_gtk::APP_ID` reverse-DNS app ID
//!   (`org.tamx.PaladinAuth.Gui`).
//! * `<launchable type="desktop-id">` matches the §11.3 desktop file
//!   basename (`org.tamx.PaladinAuth.Gui.desktop`) so the launcher entry
...
//! * `<name>` is `Paladin Auth` — matches the §11.3 desktop entry.
...
const METAINFO_FILE_RELPATH: &str = "data/metainfo/org.tamx.PaladinAuth.Gui.metainfo.xml";
