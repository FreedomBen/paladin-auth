...
//! Pure-logic unlock-dialog tests for `paladin-auth-gtk`.
//!
//! `UnlockComponent` is the passphrase-entry view that `AppModel`
//! presents whenever `paladin_auth_core::inspect` reports
//! [`paladin_auth_core::VaultStatus::Encrypted`]. Per
...
//! [`paladin_auth_core::VaultLock::Encrypted`] from the typed passphrase
//! and hands it to `paladin_auth_core::open` on `gio::spawn_blocking`,
...
//!   [`paladin_auth_gtk::startup_error::classify_open_error`].
//!
//! The pure-logic module under test (`paladin_auth_gtk::unlock_dialog`)
//! owns the gating, pre-submit rejection (empty passphrase), and
//! [`paladin_auth_core::VaultLock`] construction so the GTK widget layer
...
use paladin_auth_core::{
    ErrorKind, PaladinAuthError, PermissionSubject, VaultLock, VaultMode, VaultStatus,
};

use paladin_auth_gtk::secret_fields::SecretEntry;
use paladin_auth_gtk::startup_error::{OpenErrorRouting, StartupErrorSource};
...
fn prepare_unlock_lock_accepts_whitespace_only_passphrase() {
    ...
    // rejected pre-flight. paladin_auth_core decides on whitespace content.
...
fn submit_rejection_empty_passphrase_reason_is_zero_length() {
    ...
    // matches `paladin_auth_core::PaladinAuthError::InvalidPassphrase { reason:
...
fn classify_unlock_error_unsafe_permissions_routes_to_startup() {
    ...
    match classify_unlock_error(&err) {
        OpenErrorRouting::Startup(startup) => {
            ...
            // `paladin_auth_core::format_unsafe_permissions` verbatim so
...
fn classify_unlock_error_wrong_vault_lock_routes_to_startup() {
    let err = PaladinAuthError::WrongVaultLock {
...
// so the future `gio::spawn_blocking paladin_auth_core::open` worker call
...
fn passphrase_entry_set_then_clear_round_trips() {
    ...
    // layer pushes every keystroke into the Paladin Auth-owned
...
// live in Paladin Auth-owned memory rather than escaping into AppMsg /
...
// `gio::spawn_blocking paladin_auth_core::open` worker (deferred); this
...
fn inline_error_from_decrypt_failed_renders_display_text() {
    ...
    // `PaladinAuthError::Display` text unchanged so the wording matches
...
fn inline_error_from_invalid_passphrase_renders_display_text() {
    ...
    // `paladin_auth_core::open` directly with a zero-length passphrase).
    let err = PaladinAuthError::InvalidPassphrase {
        reason: "zero_length",
    };
    let inline = InlineError::from_error(&err);
...
// must match `paladin_auth_core::PaladinAuthError::InvalidPassphrase { reason:
...
fn inline_error_from_rejection_empty_passphrase_renders_zero_length_display() {
    ...
    // `PaladinAuthError::InvalidPassphrase { reason: "zero_length" }`
...
fn inline_error_from_rejection_matches_from_error_for_zero_length() {
    ...
    // identical to `from_error(&PaladinAuthError::InvalidPassphrase {
    ...
    let from_error = InlineError::from_error(&PaladinAuthError::InvalidPassphrase {
        reason: "zero_length",
    });
...
// `VaultLock::Encrypted` for the (deferred) `paladin_auth_core::open`
...
fn unlock_dialog_state_submit_non_empty_returns_encrypted_lock() {
    ...
    // (deferred) `gio::spawn_blocking paladin_auth_core::open` worker.
...
// `gio::spawn_blocking paladin_auth_core::open` worker.
// ---------------------------------------------------------------------------
...
// `gio::spawn_blocking paladin_auth_core::open` worker can consume it.
// ---------------------------------------------------------------------------
...
fn unlock_dialog_delete_vault_link_label_is_pinned() {
    use paladin_auth_gtk::unlock_dialog::format_unlock_dialog_delete_vault_link_label;
    assert_eq!(
...
fn unlock_dialog_output_submit_lock_carries_encrypted_variant() {
    ...
    // `gio::spawn_blocking paladin_auth_core::open` worker is wired up.
    let mut state = UnlockDialogState::new();
    state.set_passphrase("hunter2");
    let out = apply_msg(&mut state, UnlockDialogMsg::SubmitClicked)
...
// paladin_auth_core::open` worker returns a `PaladinAuthError` that
...
// `PaladinAuthError` itself; the call site (`AppModel`) routes once and
...
fn apply_msg_open_failed_inline_overwrites_prior_inline_error() {
    ...
    let next = InlineError::from_error(&PaladinAuthError::DecryptFailed);
...
fn apply_msg_open_failed_inline_with_routed_decrypt_failed_round_trip() {
    ...
    // the typed `PaladinAuthError` once, then send the `InlineError` it
...
fn apply_msg_full_submit_failed_retype_flow_button_sensitivity_round_trip() {
    ...
    let inline = match route_unlock_open_error(&PaladinAuthError::DecryptFailed) {
        UnlockOpenRouting::Inline(inline) => inline,
...
fn unlock_dialog_marker_prefix_is_stable() {
    assert_eq!(
        paladin_auth_gtk::unlock_dialog::UNLOCK_DIALOG_MARKER_PREFIX,
        "paladin-auth-gtk: unlock_dialog_path=",
    );
}
...
fn format_unlock_dialog_marker_renders_resolved_path() {
    ...
    assert_eq!(
        paladin_auth_gtk::unlock_dialog::format_unlock_dialog_marker(path),
        "paladin-auth-gtk: unlock_dialog_path=/tmp/example/vault.bin",
    );
}
...
fn format_unlock_dialog_marker_starts_with_prefix() {
    ...
    let marker = paladin_auth_gtk::unlock_dialog::format_unlock_dialog_marker(Path::new("/x"));
    assert!(marker.starts_with(paladin_auth_gtk::unlock_dialog::UNLOCK_DIALOG_MARKER_PREFIX));
}
...
fn format_unlock_dialog_title_returns_unlock_vault() {
    ...
    // from the TUI unlock view's `" Paladin Auth — unlock "` block
    // title (see `crates/paladin-auth-tui/src/view/unlock.rs`), which
    // names the application instead of the action because the TUI
    // has no other window-chrome to identify the app, while the
    // GTK dialog inherits the `org.tamx.PaladinAuth.Gui` window title
    ...
    //
    // Sibling of `paladin_auth_gtk::add_account::format_add_dialog_title`
    // and `paladin_auth_gtk::edit_dialog::format_edit_dialog_title`
    // on the dialog-header-title side; together they pin every
    // dialog's titled surface against a single source of truth.
    use paladin_auth_gtk::unlock_dialog::format_unlock_dialog_title;
...
fn format_unlock_dialog_passphrase_title_matches_tui_wording() {
    ...
    // by `passphrase_line` in `crates/paladin-auth-tui/src/view/unlock.rs`
    ...
    // side.
    use paladin_auth_gtk::unlock_dialog::format_unlock_dialog_passphrase_title;
...
fn format_unlock_button_label_returns_unlock() {
    ...
    // the unlock surface.
    use paladin_auth_gtk::unlock_dialog::format_unlock_button_label;
...
fn format_unlock_dialog_description_names_the_resolved_vault_path() {
    ...
    // through `paladin_auth_core::default_vault_path` (or the CLI
    ...
    // unlock surface.
    use paladin_auth_gtk::unlock_dialog::format_unlock_dialog_description;
...
fn format_unlock_dialog_icon_name_returns_dialog_password_symbolic() {
    ...
    use paladin_auth_gtk::unlock_dialog::format_unlock_dialog_icon_name;
