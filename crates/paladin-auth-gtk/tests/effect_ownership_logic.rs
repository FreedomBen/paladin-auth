...
//! Pure-logic in-flight-effect ownership tests for `paladin-auth-gtk`.
//!
...
//!
//! The module under test (`paladin_auth_gtk::effect_ownership`) is the
...
use paladin_auth_gtk::settings::{AcceptedChange, DebounceOutcome, SettingsField, SettingsState};
...
use paladin_auth_gtk::settings::CommittedSettings;
...
fn complete_effect_transitions_to_unlocked_before_caller_handles_typed_result() {
    ...
    // (Ok / Err PaladinAuthError) — that's handled by the dialog-
