...
//! Flathub submission contract tests for `paladin-auth-gtk`.
//!
...
//! `org.tamx.PaladinAuth.Gui` app and the subsequent commits to the
//! resulting `flathub/org.tamx.PaladinAuth.Gui` app repo:
//!
//! * `packaging/flathub/org.tamx.PaladinAuth.Gui.yml` — the Flathub
//!   manifest, named with the app-id basename per Flathub
//!   convention. Distinct from `packaging/flatpak/paladin-auth-gtk.yml`
...
/// `flathub/org.tamx.PaladinAuth.Gui`, the file at the repo root is the
/// same `org.tamx.PaladinAuth.Gui.yml` shipped here.
const FLATHUB_MANIFEST_RELPATH: &str = "packaging/flathub/org.tamx.PaladinAuth.Gui.yml";
...
/// The Flatpak / Flathub app ID. Must match `paladin_auth_gtk::APP_ID`
...
const UPSTREAM_REPOSITORY_URL: &str = "https://github.com/FreedomBen/paladin-auth";

/// Required `finish-args:` sandbox permissions — identical to the
/// `packaging/flatpak/paladin-auth-gtk.yml` baseline so Flathub builds
...
fn flathub_manifest_declares_app_id_matching_app_constant() {
    ...
    assert_eq!(
        app_id, APP_ID,
        "Flathub `app-id:` must equal `paladin_auth_gtk::APP_ID` so Flathub's \
...
#[test]
fn flathub_manifest_declares_command_paladin_auth_gtk() {
    let manifest = read_flathub_manifest();
    let command = top_level_scalar(&manifest, "command")
        .expect("Flathub manifest has a top-level `command:` key");
    assert_eq!(
        command, "paladin-auth-gtk",
        "Flathub `command:` must be `paladin-auth-gtk` so `flatpak run {APP_ID}` \
         invokes the installed binary at /app/bin/paladin-auth-gtk; got {command:?}",
    );
}
...
fn flathub_manifest_does_not_declare_any_forbidden_finish_arg() {
    ...
    assert!(
        ...
        "Flathub `finish-args:` must NOT declare any of {FORBIDDEN_FINISH_ARGS:?} \
         ...
         the filesystem surface strictly to the paladin-auth XDG namespace; \
         found: {found:?}",
    );
}
...
fn flathub_manifest_binary_install_uses_executable_mode_0755() {
    ...
    assert!(
        landed,
        "Flathub `build-commands:` must install /app/bin/paladin-auth-gtk with mode \
...
fn flathub_manifest_uses_locked_offline_cargo_build() {
    ...
    assert!(
        ...
        "Flathub `build-commands:` must invoke `cargo build` to compile \
         paladin-auth-gtk; got commands: {cmds:?}",
    );
...
fn flathub_manifest_source_is_upstream_not_local_dir() {
    ...
    // packaging/flatpak/paladin-auth-gtk.yml (which uses `type: dir,
    ...
    assert!(
        ...
        "Flathub manifest must NOT use `type: dir` (that pattern is reserved \
         for `packaging/flatpak/paladin-auth-gtk.yml`'s local packaging dry-run). \
...
#[test]
fn flathub_manifest_source_references_paladin_auth_github_repository() {
    let manifest = read_flathub_manifest();
    assert!(
