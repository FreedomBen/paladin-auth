...
//! `AppStream` validator against the paladin-auth-gtk desktop + metainfo
//! files.
//!
...
fn workspace_root() -> PathBuf {
    // tests run in `crates/paladin-auth-gtk/`; the workspace root is two
...
fn ci_workflow_runs_desktop_file_validate_on_the_desktop_entry() {
    ...
    assert!(
        workflow.contains("crates/paladin-auth-gtk/data/org.tamx.PaladinAuth.Gui.desktop"),
