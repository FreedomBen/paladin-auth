...
//!   `packaging/deb/paladin-auth-gtk.yaml` (wiring `postinstall` and
//!   `postremove` to the shared
//!   `packaging/scripts/paladin-auth-gtk-postinstall.sh` /
//!   `packaging/scripts/paladin-auth-gtk-postremove.sh` files) so a
...
//!   variables (e.g. `${PALADIN_AUTH_VERSION}`).
//!
...
//! convention (no shared `tests/common/` for paladin-auth-gtk).

use std::fs;
use std::path::PathBuf;

/// Path to the `nfpm` `.rpm` manifest, relative to the workspace root.
const RPM_MANIFEST_RELPATH: &str = "packaging/rpm/paladin-auth-gtk.yaml";
...
fn rpm_manifest_exists_at_expected_path() {
    ...
    assert!(
        ...
        "expected nfpm .rpm manifest at {} — Milestone 7 packaging \
         checklist requires `packaging/rpm/paladin-auth-gtk.yaml`",
        path.display(),
    );
}
...
#[test]
fn rpm_manifest_declares_package_name_paladin_auth_gtk() {
    let manifest = read_rpm_manifest();
    let name =
        top_level_scalar(&manifest, "name").expect("rpm nfpm manifest has a top-level `name:` key");
    assert_eq!(
        name, "paladin-auth-gtk",
        "rpm package name must be `paladin-auth-gtk` so the published artifact matches the binary \
    ...
    for (expected_key, expected_path) in REQUIRED_SCRIPT_KEYS_AND_PATHS {
        ...
        assert_eq!(
            ...
            "rpm nfpm manifest `scripts:` must map `{expected_key}: {expected_path}`; got \
             {actual:?}. The same path ships in packaging/deb/paladin-auth-gtk.yaml so a fix \
...
fn rpm_manifest_scripts_match_deb_manifest_scripts() {
    ...
    let deb_path = workspace_root().join("packaging/deb/paladin-auth-gtk.yaml");
...
fn rpm_manifest_binary_install_uses_executable_mode() {
    ...
    let start = binary_block_start.expect(
        "rpm nfpm manifest is missing the /usr/bin/paladin-auth-gtk dst entry — covered by \
    ...
    assert!(
        found_mode_0755,
        "rpm nfpm manifest must set `mode: 0755` on the /usr/bin/paladin-auth-gtk entry so the \
