...
const REQUIRED_ITEMS: &[&str] = &[
    ...
    "Import encrypted Paladin Auth bundle with each on-conflict policy; reported counts match.",
    "Import QR image file with each on-conflict policy; reported counts match.",
    "Export plaintext: warning + confirmation, `0600` output.",
    "Export encrypted Paladin Auth bundle: twice-confirm, round-trip via Import.",
