...
fn workspace_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .and_then(|p| p.parent())
        .expect("crates/paladin-auth-gtk lives two levels below the workspace root")
        .to_owned()
}
...
fn ci_packaging_dry_run_job_runs_in_a_fedora_container() {
    ...
    // `cargo build -p paladin-auth-gtk` (which pulls those system
...
fn ci_packaging_dry_run_builds_release_binary_with_locked_lockfile() {
    ...
    assert!(
        job.contains("paladin-auth-gtk"),
        "packaging-dry-run job must explicitly target `paladin-auth-gtk` (the workspace ships \
         multiple binaries); got:\n{job}",
    );
}

#[test]
fn ci_packaging_dry_run_exports_paladin_auth_version() {
    // Both nfpm manifests reference `${PALADIN_AUTH_VERSION}` directly:
    //   packaging/deb/paladin-auth-gtk.yaml: version: ${PALADIN_AUTH_VERSION}
    //   packaging/rpm/paladin-auth-gtk.yaml: version: ${PALADIN_AUTH_VERSION}
    ...
    let job = extract_packaging_dry_run_job(&workflow).expect("packaging-dry-run job must exist");
    assert!(
        job.contains("PALADIN_AUTH_VERSION"),
        "packaging-dry-run job must export PALADIN_AUTH_VERSION so the nfpm manifests' \
         `version: ${{PALADIN_AUTH_VERSION}}` substitution resolves to a concrete value; got:\n{job}",
    );
}
...
fn ci_packaging_dry_run_builds_deb_package_via_nfpm() {
    let workflow = read_ci_workflow();
    let job = extract_packaging_dry_run_job(&workflow).expect("packaging-dry-run job must exist");
    // The deb manifest lives at packaging/deb/paladin-auth-gtk.yaml.
    // Pin that the workflow references that exact path so a future
    // rename has to land an explicit workflow edit too.
    assert!(
        job.contains("packaging/deb/paladin-auth-gtk.yaml"),
...
fn ci_packaging_dry_run_builds_rpm_package_via_nfpm() {
    ...
    let job = extract_packaging_dry_run_job(&workflow).expect("packaging-dry-run job must exist");
    assert!(
        job.contains("packaging/rpm/paladin-auth-gtk.yaml"),
...
fn ci_packaging_dry_run_runs_desktop_file_validate_on_extracted_payload() {
    ...
    // `usr/share/applications/org.tamx.PaladinAuth.Gui.desktop` per
    ...
    assert!(
        job.contains("usr/share/applications/org.tamx.PaladinAuth.Gui.desktop"),
        "packaging-dry-run job must reference the installed .desktop path \
         `usr/share/applications/org.tamx.PaladinAuth.Gui.desktop` so the validator runs on the \
...
fn ci_packaging_dry_run_runs_appstreamcli_validate_on_extracted_payload() {
    ...
    assert!(
        job.contains("usr/share/metainfo/org.tamx.PaladinAuth.Gui.metainfo.xml"),
        "packaging-dry-run job must reference the installed metainfo path \
         `usr/share/metainfo/org.tamx.PaladinAuth.Gui.metainfo.xml` so the validator runs on the \
         extracted payload, not the source-tree original; got:\n{job}",
    );
}
