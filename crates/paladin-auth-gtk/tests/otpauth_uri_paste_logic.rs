...
//! Pure-logic `otpauth://`-paste tests for `paladin-auth-gtk`.
//!
...
//!   [`paladin_auth_core::ValidatedAccount`] on the duplicate-allowed
//!   path.
...
//!
//! The module under test (`paladin_auth_gtk::otpauth_uri_paste`) is the
...
use paladin_auth_core::{
    AccountKindSummary, Algorithm, ErrorKind, PaladinAuthError, ValidatedAccount, ValidationWarning,
};

use paladin_auth_gtk::otpauth_uri_paste::{
...
/// any [`paladin_auth_core::PaladinAuthError`] Display string.
...
fn classify_uri_submit_unsupported_scheme_rejects_inline() {
    // `mailto:`, `https://`, `paladin-auth://` and friends are not
...
fn inline_error_does_not_echo_uri_secret_text() {
    ...
    // `paladin_auth_core::PaladinAuthError` JSON serialization layer.
...
fn uri_submit_outcome_carries_only_validated_account_or_inline_error() {
    ...
    // pre-rendered body produced by `PaladinAuthError::Display`. Neither
