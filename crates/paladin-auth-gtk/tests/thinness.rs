...
//! Thinness contract test for `paladin-auth-gtk`.
//!
...
//! `paladin-auth-core` (DESIGN §3). This test enforces that contract by
//! scanning:
//!
//! 1. Every `.rs` file under `crates/paladin-auth-gtk/src/` for direct
//!    references to forbidden crate-name spellings, and
//! 2. The crate manifest at `crates/paladin-auth-gtk/Cargo.toml`'s
...
/// Forbidden direct dependencies / source references for `paladin-auth-gtk`.
///
...
fn source_files_have_no_forbidden_crate_references() {
    ...
    assert!(
        offenses.is_empty(),
        "paladin-auth-gtk must not reach into crypto / storage / import / \
         OTP primitives directly — move offending logic into paladin-auth-core \
...
fn manifest_declares_no_forbidden_direct_dependencies() {
    ...
    let manifest =
        fs::read_to_string(&manifest_path).expect("read crates/paladin-auth-gtk/Cargo.toml");
    ...
    assert!(
        offenders.is_empty(),
        "paladin-auth-gtk Cargo.toml [dependencies] must not declare any of: \
         {FORBIDDEN:?}\nfound: {offenders:?}",
    );
}
...
/// comments (`/* … */`) are left in place — `paladin-auth-gtk` does not use
...
/// byte strings are not used in `paladin-auth-gtk` as of this commit; if a
...
[dependencies]
paladin-auth-core = { path = \"../paladin-auth-core\" }
clap = \"4.5\"
...
";
    ...
    assert!(names.iter().any(|n| n == "paladin-auth-core"));
