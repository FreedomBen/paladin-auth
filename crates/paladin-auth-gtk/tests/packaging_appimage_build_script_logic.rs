...
//! `AppImage` assembly script contract tests for `paladin-auth-gtk`.
//!
...
//!   `paladin-auth-gtk-${PALADIN_AUTH_VERSION}-x86_64.AppImage` (the
//!   §11.5 artifact name shape).
//! * Carries the `UPDATE_INFORMATION` env var with the
//!   `gh-releases-zsync|FreedomBen|paladin-auth|latest|paladin-auth-gtk-*-x86_64.AppImage.zsync`
//!   value so the resulting `AppImage` embeds the `GitHub` Releases
//!   `zsync` pointer per §11.5.
//! * Reads `PALADIN_AUTH_VERSION` from the environment (the release
//!   pipeline injects it from the tag; the script never invents
//!   a version, so a stray local run that forgets to set it
//!   fails loudly instead of writing `paladin-auth-gtk--x86_64.AppImage`).
//!
...
/// `${PALADIN_AUTH_VERSION}` is substituted by the script at run time).
const EXPECTED_OUTPUT_NAME: &str = "paladin-auth-gtk-${PALADIN_AUTH_VERSION}-x86_64.AppImage";
...
#[test]
fn appimage_script_reads_paladin_auth_version_from_environment() {
    let script = read_appimage_script();
    // The script may either reference `${PALADIN_AUTH_VERSION}` directly
    // or use the parameter-expansion guard form `${PALADIN_AUTH_VERSION:?msg}`
    // (which is the stronger, fail-loud variant); either is acceptable.
    let landed =
        script.contains("${PALADIN_AUTH_VERSION}") || script.contains("${PALADIN_AUTH_VERSION:?");
    assert!(
        landed,
        "AppImage build script must read PALADIN_AUTH_VERSION from the environment so the release \
         pipeline can inject the tag-derived version; the script body referenced neither \
         `${{PALADIN_AUTH_VERSION}}` nor `${{PALADIN_AUTH_VERSION:?...}}`",
    );
}
...
fn appimage_script_does_not_hardcode_a_version_string() {
    ...
    // release pipeline injects PALADIN_AUTH_VERSION; a hard-coded
    ...
    assert!(
        found.is_empty(),
        "AppImage build script must not hard-code a semver — PALADIN_AUTH_VERSION is the only \
