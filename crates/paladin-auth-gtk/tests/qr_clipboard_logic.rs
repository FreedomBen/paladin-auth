...
//! Pure-logic clipboard-QR tests for `paladin-auth-gtk`.
//!
...
//! * Sizes above `paladin_auth_core::QR_RGBA_MAX_BYTES` reject before
//!   allocation / download.
//! * Decoded buffer is passed to `paladin_auth_core::import::qr_image_bytes`
...
use paladin_auth_gtk::qr_clipboard::{
...
fn prepare_rgba_layout_uses_checked_multiplication_for_bytes_step() {
    ...
    // same multiplication that paladin-auth-core itself guards. Either result
...
//    above paladin_auth_core::QR_RGBA_MAX_BYTES before allocation /
//    download."
//
...
// `paladin_auth_core::import::qr_image_bytes` consumes pixel values
...
fn decode_clipboard_qr_propagates_buffer_length_mismatch_from_core() {
    ...
    // the wrapper handed control over to paladin-auth-core after sizing.
    ...
    // The error must come from paladin-auth-core's QR path, not the wrapper.
...
// ---------------------------------------------------------------------------
// Sanity: import-time is forwarded verbatim to paladin-auth-core
// ---------------------------------------------------------------------------
...
fn decode_clipboard_qr_uses_supplied_import_time() {
    ...
    fn assert_signature(
        _: fn(
            ...
            &[u8],
            SystemTime,
        ) -> paladin_auth_core::Result<Vec<paladin_auth_core::ValidatedAccount>>,
    ) {
    }
    assert_signature(decode_clipboard_qr);

    // And a small smoke run with two different import_time values
    // should both surface a paladin-auth-core decode error, not a wrapper
    // pre-check failure.
...
// reaching `paladin_auth_core::import::qr_image_bytes`.
// ---------------------------------------------------------------------------
...
//   * zero-decoded QRs  (`paladin_auth_core::import::qr_image_bytes` returned
...
use paladin_auth_gtk::qr_clipboard::{classify_qr_outcome, QrPreflightError};
...
fn qr_preflight_error_no_clipboard_image_kind_is_invalid_state() {
    ...
    // matching `PaladinAuthError` variant for this GUI-only failure.
    assert_eq!(
...
fn qr_preflight_error_decode_validation_error_kind_is_validation_error() {
    let err = QrPreflightError::Decode(PaladinAuthError::ValidationError {
        field: "secret",
...
#[test]
fn qr_preflight_error_decode_display_includes_underlying_paladin_auth_error_body() {
    let underlying = PaladinAuthError::NoEntriesToImport.to_string();
    let body = QrPreflightError::Decode(PaladinAuthError::NoEntriesToImport).to_string();
    assert!(
        body.contains(&underlying),
        "decode body {body:?} should include the underlying paladin-auth error {underlying:?}",
    );
}
...
fn qr_preflight_error_display_does_not_echo_secret_bytes() {
    ...
    // `PaladinAuthError::NoEntriesToImport`) include the raw RGBA buffer
    ...
    let bodies = [
        ...
        .to_string(),
        QrPreflightError::Decode(PaladinAuthError::NoEntriesToImport).to_string(),
    ];
    for body in &bodies {
        // ASCII-printable letters / digits / spaces / punctuation
        // only — the underlying paladin-auth / GDK / core wording never
...
fn classify_qr_outcome_decode_error_returns_preflight_decode_variant() {
    let outcome = QrDecodeOutcome::DecodeError(PaladinAuthError::NoEntriesToImport);
    match classify_qr_outcome(outcome) {
        Err(QrPreflightError::Decode(err)) => {
            assert_eq!(err.kind(), ErrorKind::NoEntriesToImport);
        }
...
fn classify_qr_outcome_routes_validation_error_through_decode_variant() {
    ...
    // core's parser. The classifier wraps the `PaladinAuthError` in
    ...
    let validation_err = PaladinAuthError::ValidationError {
        field: "secret",
        reason: "invalid_base32".to_string(),
        source_index: None,
        decoded_len: None,
