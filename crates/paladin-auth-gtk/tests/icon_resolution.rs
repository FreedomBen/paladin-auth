...
//! Pure-logic icon-resolution tests for `paladin-auth-gtk`.
//!
...
use paladin_auth_gtk::icon_resolution::{resolve_display_icon, PLACEHOLDER_ICON_NAME};
...
#[test]
fn add_modal_icon_hint_input_uses_paladin_auth_core_parse_icon_hint_token() {
    // Documents the contract the GTK add modal will follow when
    // it lands: icon-hint text-field input routes through
    // `paladin_auth_core::parse_icon_hint_token` so the slug /
    ...
    // Malformed slugs surface a `paladin_auth_core::PaladinAuthError` rather
