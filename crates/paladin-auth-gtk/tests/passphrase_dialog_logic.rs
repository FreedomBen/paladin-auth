...
//! Pure-logic passphrase-dialog tests for `paladin-auth-gtk`.
//!
...
//!   close.
//!
//! The module under test (`paladin_auth_gtk::passphrase_dialog`) is the
...
use paladin_auth_core::{format_plaintext_storage_warning, ErrorKind};
...
use paladin_auth_gtk::secret_fields::{ClearReason, PassphraseSecretState};
...
#[test]
fn remove_warning_body_matches_paladin_auth_core_verbatim() {
...
fn passphrase_dialog_init_round_trips_vault_path_and_encryption_state() {
    use paladin_auth_gtk::passphrase_dialog::PassphraseDialogInit;
...
fn passphrase_dialog_output_close_is_constructible() {
    use paladin_auth_gtk::passphrase_dialog::PassphraseDialogOutput;
...
fn passphrase_dialog_component_input_and_output_match_dispatch_edges() {
    use paladin_auth_gtk::passphrase_dialog::PassphraseDialogComponent;
...
fn fresh_plaintext_vault() -> (
    paladin_auth_core::Vault,
    paladin_auth_core::Store,
    tempfile::TempDir,
) {
    ...
    let (vault, store) =
        paladin_auth_core::Store::create(path.as_path(), paladin_auth_core::VaultInit::Plaintext)
            .expect("store create");
    (vault, store, dir)
}
...
fn run_passphrase_worker_set_encrypts_plaintext_vault() {
    ...
    let options =
        paladin_auth_core::EncryptionOptions::new(SecretString::from("hunter2".to_string()))
...
fn run_passphrase_worker_remove_decrypts_encrypted_vault() {
    ...
    let (vault, store, _dir) = fresh_plaintext_vault();
    let opts = paladin_auth_core::EncryptionOptions::new(SecretString::from("hunter2".to_string()))
        .expect("options");
...
fn run_passphrase_worker_set_on_already_encrypted_returns_failure_with_pair() {
    ...
    let opts2 =
        paladin_auth_core::EncryptionOptions::new(SecretString::from("hunter3".to_string()))
            .expect("options");
    let completion = run_passphrase_worker(PassphraseWorkerInput {
        vault,
        store,
...
fn apply_msg_worker_failed_clears_dispatching() {
    ...
    let outcome = classify_passphrase_error(&PaladinAuthError::SaveNotCommitted {
        committed: false,
        backup_path: None,
    });
    apply_msg(&mut state, PassphraseDialogMsg::WorkerFailed(outcome));
