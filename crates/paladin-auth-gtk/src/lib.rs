...
//! `paladin-auth-gtk` library surface.
//!
...
//!
//! The crate intentionally re-exports nothing from `paladin_auth_core`;
//! callers go through `paladin_auth_core::*` directly. The §"Thinness
...
pub const APP_ID: &str = "org.tamx.PaladinAuth.Gui";

/// Run the `paladin-auth-gtk` binary.
///
...
pub fn run() -> ExitCode {
    ...
    if let Err(err) = libadwaita::init() {
        eprintln!("paladin-auth-gtk: failed to initialize libadwaita: {err}");
    ...
    // `/org/tamx/PaladinAuth/Gui/...` per `docs/IMPLEMENTATION_PLAN_04_GTK.md`
    // §"Window shell and toast surface".
    app::model::register_app_gresource_bundle();

    // Layer Paladin Auth's `data/style.css` on top of the Adwaita
    ...
    // widget styles; the CSS layer only adds the Paladin Auth-specific
    ...
    if let Some(display) = relm4::gtk::gdk::Display::default() {
        ...
        eprintln!(
            "paladin-auth-gtk: no default GDK display available; skipping Paladin Auth CSS layer and bundled icon-theme attach (Adwaita defaults still apply)",
        );
    }
    ...
    // Clap owns the argument grammar for paladin-auth-gtk; GApplication's
