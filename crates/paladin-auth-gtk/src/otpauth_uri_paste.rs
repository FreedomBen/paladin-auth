...
//! `otpauth://`-paste pure-logic state machine for `paladin-auth-gtk`.
//!
...
//! # URI parse
//!
//! [`classify_uri_submit`] runs [`paladin_auth_core::parse_otpauth`] on
//! the typed text and surfaces either:
//!
//! * [`UriSubmitOutcome::Proceed`] carrying the validated account —
//!   the widget calls [`paladin_auth_core::Vault::find_duplicate`] before
...
//! # Post-effect routing
//!
//! [`classify_uri_add_error`] maps the [`PaladinAuthError`] from a failed
...
use paladin_auth_core::{parse_otpauth, ErrorKind, PaladinAuthError, ValidatedAccount};
...
#[must_use]
pub fn classify_uri_add_error(err: &PaladinAuthError) -> UriAddErrorOutcome {
    match err.kind() {
