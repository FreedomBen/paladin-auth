...
//! Widget-bearing `AppModel` for `paladin-auth-gtk`.
//!
...
//! vault path, call `paladin_auth_core::inspect`, and (for plaintext
//! vaults) `paladin_auth_core::Store::open` on the GTK main loop, then
...
//! `gio::spawn_blocking paladin_auth_core::open` worker that consumes the
//! forwarded `VaultLock` and posts `AppMsg::UnlockWorkerCompleted` on
//! completion lands in a follow-up commit.
//!
//! `AppMsg::AccountListAction(OpenEditDialog(id))` mounts an
//! [`EditDialogComponent`] seeded from
//! [`crate::edit_dialog::decide_edit_target`] plus the pre-edit
//! `Account` clone so the kebab `Edit…` action is reachable
//! end-to-end (kebab activation → `AccountListOutput` → `AppMsg` →
//! live dialog widget). The dialog's Cancel button bubbles back here
//! as `AppMsg::EditDialogAction(EditDialogOutput::Cancel)`, which
//! drops the controller and removes its widget from the content
//! tree. The Save button bubbles
//! `AppMsg::EditDialogAction(EditDialogOutput::Submit { .. })`,
//! which runs the §4.7 duplicate pre-flight against the live vault
//! and, on no collision, spawns the `gio::spawn_blocking
//! Vault::mutate_and_save(|v| v.edit_account_metadata(...))` worker
//! whose `AppMsg::EditWorkerCompleted` outcome drops the dialog +
//! toasts on success or re-renders the inline error on failure.
//! `OpenRemoveDialog(id)` mirrors the same
...
use crate::app::state::{
    apply_add_dispatch_inplace, apply_add_vault_install_inplace, apply_edit_vault_install_inplace,
    apply_export_dispatch_inplace, apply_export_vault_install_inplace,
    apply_import_dispatch_inplace, apply_import_vault_install_inplace,
    apply_passphrase_dispatch_inplace, apply_passphrase_vault_install_inplace,
    apply_qr_dispatch_inplace, apply_remove_dispatch_inplace, apply_remove_vault_install_inplace,
    apply_settings_dispatch_inplace, apply_settings_vault_install_inplace,
    apply_submit_add_inplace, apply_submit_edit_inplace, apply_submit_export_inplace,
...
use crate::edit_dialog::{
    ...
    run_edit_worker, EditDialogComponent, EditDialogMsg, EditDialogOutput, EditWorkerCompletion,
    EditWorkerEffect,
};
...
use crate::export_qr_dialog::{
    decide_export_qr_target, run_export_qr_save_worker, ExportQrDialogComponent, ExportQrDialogMsg,
    ExportQrDialogOutput, ExportQrSaveCompletion, ExportQrSaveRequest,
    ExportQrSaveWorkerCompletion, ExportQrSaveWorkerInput, SaveKind,
};
...
pub struct AppInit {
    ...
    /// app falls back to `paladin_auth_core::default_vault_path()` on
    /// startup.
...
pub struct StartupOutcome {
    ...
    pub vault: Option<(paladin_auth_core::Vault, paladin_auth_core::Store)>,
}

/// Top-level relm4 component for `paladin-auth-gtk`.
///
...
pub struct AppModel {
    ...
    #[allow(dead_code)]
    vault: Option<(paladin_auth_core::Vault, paladin_auth_core::Store)>,
    ...
    /// Live [`EditDialogComponent`] controller when the user has
    ...
    /// handler.
    #[allow(dead_code)]
    edit_dialog: Option<Controller<EditDialogComponent>>,
    ...
    /// vault through `paladin_auth_core::account_matches_search` without
    ...
    /// Open HOTP reveal windows keyed by [`paladin_auth_core::AccountId`].
    ///
    ...
    reveal_windows: HashMap<paladin_auth_core::AccountId, RevealWindow>,
    ...
    pending_copy_after_advance: Option<paladin_auth_core::AccountId>,
    ...
    /// Last `AppState::is_busy()` value dispatched to the live
    /// [`crate::edit_dialog::EditDialogComponent`] via
    /// [`crate::edit_dialog::EditDialogMsg::SetBusy`].
    ///
    /// Tracked here so [`AppModel::sync_edit_dialog_busy`] can
    /// debounce, same pattern as [`Self::last_add_dialog_busy`].
    /// Initialized to `false` for the same reason as
    /// [`Self::last_account_list_busy`].
    last_edit_dialog_busy: bool,
    /// Last `AppState::is_busy()` value dispatched to the live
    ...
    /// [`paladin_auth_core::policy::auto_lock::IdlePolicy`] so the
    ...
    /// `paladin_auth_gtk::gsettings::SCHEMA_ID`. Held on `self` so the
...
impl std::fmt::Debug for AppModel {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AppModel")
            ...
            .field(
                "edit_dialog",
                &self.edit_dialog.as_ref().map(|_| "<mounted>"),
            )
            .field(
                "remove_dialog",
            ...
            .field("last_edit_dialog_busy", &self.last_edit_dialog_busy)
...
pub enum AppMsg {
    ...
    /// per-row actions bubble the row's [`paladin_auth_core::AccountId`]
    ...
    /// signal handler installed in `init`. `AppModel`'s handler
    /// dispatches `AccountListMsg::SetShowColumnHeaders` to the
    /// live [`AccountListComponent`] when it is mounted, and is a
    /// benign no-op otherwise (the next mount reads the freshly
    ...
    /// Forwarded from the live [`EditDialogComponent`] when the
    /// user interacts with the dialog. [`EditDialogOutput::Cancel`]
    /// drops the controller and removes the dialog widget from the
    /// content tree; [`EditDialogOutput::Submit`] runs the duplicate
    /// pre-flight against the live vault and, on no collision, spawns
    /// the `gio::spawn_blocking Vault::mutate_and_save(|v|
    /// v.edit_account_metadata(...))` worker through the
    /// `UnlockedBusy` busy gate.
    EditDialogAction(EditDialogOutput),
    ...
    OpenAddDialog,
    /// Posted by the application menu's "About Paladin Auth" entry's
    ...
    /// [`paladin_auth_core::VaultSettings`] toggles + spinners with
    ...
    /// [`paladin_auth_core::Vault::mutate_and_save`] live-apply
    ...
    /// [`paladin_auth_core::Vault::mutate_and_save`] merge worker,
    ...
    /// editable form widgets, the
    /// [`paladin_auth_core::Vault::export`] worker, and the inline
    ...
    /// targeted [`paladin_auth_core::AccountSummary`] resolved from the
    /// live vault through
    ...
    /// is a benign no-op.
    OpenExportQrDialog(paladin_auth_core::AccountId),
    /// Forwarded from the live [`ExportQrDialogComponent`] when the
    /// user dismisses the dialog. Both
    /// [`ExportQrDialogOutput::Cancel`] (a bare Escape press) and
    /// [`ExportQrDialogOutput::Close`] (the `Done` button /
    /// window-manager close) drop the live controller so the
    /// rendered `adw::Dialog` is released; the variants stay distinct
    /// in [`crate::export_qr_dialog`] so a future telemetry / undo
    ...
    /// the [`paladin_auth_core::Vault::change_passphrase`] worker,
    /// and the inline error / warning surfaces) lands in
    /// follow-up commits; this commit only wires the dispatch
    /// edge so the menu activation reaches `AppModel`.
    ...
    /// accepted [`paladin_auth_core::SettingPatch`] (from a toggle click
    ...
    /// post-merge [`paladin_auth_core::ImportReport`] (or typed failure
    ...
    /// `gio::spawn_blocking
    /// paladin_auth_core::write_secret_file_atomic(.., otpauth_list /
    ...
    /// [`paladin_auth_core::Vault::remove_passphrase`] outcomes to
    ...
    /// `gio::spawn_blocking paladin_auth_core::open` worker that consumes
    /// the forwarded [`paladin_auth_core::VaultLock`] and transitions
    ...
    /// Posted by the `gio::spawn_blocking paladin_auth_core::open` worker
    /// after it consumes the forwarded [`paladin_auth_core::VaultLock`]
    ...
    /// `paladin_auth_core::Store::open` on the success branch.
    ///
    ...
    /// §"Effect errors".
    ///
    /// The `gio::spawn_blocking paladin_auth_core::open` worker that
    ...
    /// worker after it consumes a
    /// [`crate::edit_dialog::EditWorkerInput`] and reports its routed
    /// outcome as an [`EditWorkerCompletion`] — the typed
    /// [`EditWorkerEffect`] bundled with the live `(Vault, Store)`
    /// pair returned by `mutate_and_save` regardless of typed outcome
    /// (the edit worker always returns the pair per
    /// `docs/IMPLEMENTATION_PLAN_04_GTK.md` §"Vault interaction").
    ///
    /// The carried `(Vault, Store)` pair is reinstalled into
    /// [`AppModel::vault`] via
    /// [`crate::app::state::apply_edit_vault_install_inplace`]
    /// unconditionally — `mutate_and_save` is authoritative for the
    /// post-edit / rollback state. The state machine rolls
    /// `UnlockedBusy → Unlocked` via
    /// [`crate::app::state::edit_final_app_state`] on every typed
    /// effect. On [`EditWorkerEffect::Success`] the handler drops the
    /// dialog, refreshes the list, and raises the
    /// [`crate::edit_dialog::format_edit_dialog_success_toast`]
    /// confirmation; on [`EditWorkerEffect::Failure`] it forwards
    /// [`crate::edit_dialog::EditDialogMsg::WorkerCompleted`] so the
    /// mounted dialog re-renders the inline warning / error.
    EditWorkerCompleted(EditWorkerCompletion),
    /// Posted by the `gio::spawn_blocking
    ...
    /// interaction").
    ///
    /// Mirrors the [`Self::EditWorkerCompleted`] dispatch path
    /// closely — `compose_remove_dispatch` bundles the typed
    ...
    /// paladin_auth_core::destroy_vault(path)` worker targets the same
    ...
    /// Posted by the `gio::spawn_blocking paladin_auth_core::destroy_vault`
    /// worker. [`crate::app::state::compose_destroy_dispatch`] routes
    /// the typed `Result<DestroyReport, PaladinAuthError>` (mapped to a
    ...
    /// interaction").
    ///
    /// Mirrors the [`Self::EditWorkerCompleted`] and
    /// [`Self::RemoveWorkerCompleted`] dispatch paths closely —
    ...
    ///    [`paladin_auth_core::QR_RGBA_MAX_BYTES`] gate rejects oversized
    ...
    QrClipboardLoaded(
        std::result::Result<
            Vec<paladin_auth_core::ValidatedAccount>,
    ...
    /// the [`paladin_auth_core::VaultInit`] plus the resolved vault path
    ...
    /// Posted by the `gio::spawn_blocking
    /// paladin_auth_core::Store::create` / `Store::create_force` init
    ...
    /// [`paladin_auth_core::policy::auto_lock::IdlePolicy::should_arm`] on
    ...
    /// is itself read-only — `paladin_auth_core::default_vault_path`,
    ...
    /// shared `paladin_auth_core::TICK_INTERVAL_MS` cadence (parity with
    ...
    Tick {
        ...
        /// generation (`paladin_auth_core::totp_code` is
    ...
    ClipboardWakeRead {
        ...
        token: paladin_auth_core::ClipboardClearToken,
...
impl SimpleComponent for AppModel {
    ...
    fn init(
        ...
        // model so the live `AccountListComponent` (if any)
        // toggles the `gtk::ColumnView` header strip.
        let app_settings_input = sender.input_sender().clone();
        model.app_settings.connect_changed(
    ...
    fn update(&mut self, msg: Self::Input, sender: ComponentSender<Self>) {
        match msg {
            ...
            AppMsg::AccountListAction(AccountListOutput::OpenEditDialog(id)) => {
                ...
                // mount the edit dialog. A `None` projection means the
                // account was removed between the kebab activation and
                // this dispatch — treat that as a benign race and drop
                // the action. `EditDialogComponent::Init` is
                // `(EditDialogInit, Account)`: the summary-derived
                // `init` seeds the row buffers and the pre-edit
                // `Account` clone backs the `validate_account_edit`
                // cross-field check on submit, so both must resolve
                // off the same live vault before the dialog mounts.
                if let Some((vault, _store)) = self.vault.as_ref() {
                    if let (Some(init), Some(account)) = (
                        decide_edit_target(vault, id),
                        vault.accounts().iter().find(|a| a.id() == id).cloned(),
                    ) {
                        let controller = EditDialogComponent::builder()
                            .launch((init, account))
                            .forward(sender.input_sender(), AppMsg::EditDialogAction);
                        self.content.append(controller.widget());
                        self.edit_dialog = Some(controller);
                    }
                }
            }
            AppMsg::AccountListAction(AccountListOutput::OpenExportQrDialog(id)) => {
            ...
            AppMsg::AccountListAction(AccountListOutput::QueryChanged(query)) => {
                ...
                // `paladin_auth_core::account_matches_search` (via
            ...
            AppMsg::ShowColumnHeadersChanged(enabled) => {
                ...
                // toggle, but any external write hits the same
                // path). Drive the live `AccountListComponent` to
                // toggle the `gtk::ColumnView` header strip;
                // benign no-op when the list is not mounted
                // (Locked / Missing / StartupError) because the
                // next mount reads the freshly committed value
            ...
            AppMsg::EditDialogAction(EditDialogOutput::Cancel) => {
                ...
                if let Some(controller) = self.edit_dialog.take() {
                    self.content.remove(controller.widget());
                }
            }
            AppMsg::EditDialogAction(EditDialogOutput::Submit { account_id, edit }) => {
                // Save-button entry side of the `gio::spawn_blocking
                // Vault::mutate_and_save(|v|
                // v.edit_account_metadata(account_id, edit, now))`
                // worker. The dialog already ran the client-side
                // per-row + cross-field validators, so the steps here
                // run in order per `docs/IMPLEMENTATION_PLAN_04_GTK.md`
                // §"Vault interaction" / §4.7:
                //
                // 0. Duplicate pre-flight against the LIVE vault
                //    BEFORE `self.vault.take()`. While holding
                //    `&Vault`, `Vault::find_duplicate_after_edit`
                //    resolves the §4.7 collision. On `Some(other)`,
                //    forward `EditDialogMsg::DuplicateDetected` to the
                //    mounted dialog and bail — the vault is never
                //    taken, no effect starts, and the busy gate stays
                //    idle. This must read the vault before the take
                //    so the borrow does not fight the move.
                // 1. Take the live `(Vault, Store)` pair and bundle it
                //    with the payload into an `EditWorkerInput` via
                //    `compose_edit_worker_input` (only `Unlocked`
                //    returns `Ok`; every other variant hands the pair
                //    back so `apply_edit_vault_install_inplace`
                //    reinstalls it). A `None` state or a `None` vault
                //    slot is the defensive no-op.
                // 2. Apply the `Unlocked → UnlockedBusy` busy-gate
                //    transition via `apply_submit_edit_inplace` and
                //    flip the dialog busy latch through
                //    `sync_edit_dialog_busy` so the Save button dims
                //    for the worker's lifetime. The dialog stays
                //    mounted; the worker's success dispatch drops it
                //    once it returns.
                // 3. Capture `SystemTime::now()` at the dispatch site
                //    so the worker thread does not race later
                //    wall-clock drift; `Vault::edit_account_metadata`
                //    uses it for the new `updated_at`.
                // 4. Spawn `run_edit_worker` on
                //    `gtk::gio::spawn_blocking` so the durability
                //    fsync hop does not block the GTK main loop; the
                //    wrapping `gtk::glib::spawn_future_local` posts the
                //    bundled `EditWorkerCompletion` back via
                //    `AppMsg::EditWorkerCompleted`.
                let is_unlocked = self.state.as_ref().is_some_and(AppState::is_unlocked);
                if let (true, Some((vault, _store))) = (is_unlocked, self.vault.as_ref()) {
                    if let Some(other) = vault.find_duplicate_after_edit(account_id, &edit) {
                        let marker = duplicate_marker_from_account(other);
                        if let Some(controller) = self.edit_dialog.as_ref() {
                            controller.emit(EditDialogMsg::DuplicateDetected(marker));
                        }
                        return;
                    }
                }
                ...
                let worker_input = match (self.state.as_ref(), self.vault.take()) {
                    (Some(state), Some(pair)) => {
                        match compose_edit_worker_input(state, pair, account_id, edit, now) {
                            Ok(input) => {
                            ...
                            Err(pair) => {
                                apply_edit_vault_install_inplace(&mut self.vault, pair);
                                None
                            }
                        }
                    }
                ...
                if let Some(state) = self.state.as_mut() {
                    apply_submit_edit_inplace(state);
                }
                if let Some(input) = worker_input {
                    let sender = sender.clone();
                    gtk::glib::spawn_future_local(async move {
                        match gtk::gio::spawn_blocking(move || run_edit_worker(input)).await {
                            Ok(completion) => {
                                sender.input(AppMsg::EditWorkerCompleted(completion));
                            }
                            Err(_) => {
                                sender.input(AppMsg::WorkerPanic(EffectKind::EditAccountMetadata));
                            }
                        }
                    });
                }
            ...
            AppMsg::OpenAboutDialog => {
                // Application menu "About Paladin Auth" activation.
            ...
            AppMsg::OpenPreferencesDialog => {
                ...
                // the live vault's `paladin_auth_core::VaultSettings` and
            ...
            AppMsg::ImportDialogAction(ImportDialogOutput::Submit(payload)) => {
                ...
                //    Paladin Auth variant's §4.4 Argon2id KDF and the
            ...
            AppMsg::PassphraseWorkerCompleted(completion) => {
                ...
                if let Some(dispatch) = dispatch {
                    ...
                    // `paladin_auth_core::inspect`. The arm/disarm decision
            ...
            AppMsg::DestroyVault { path } => {
                // Entry side of the `gio::spawn_blocking
                // paladin_auth_core::destroy_vault(path)` worker. When the
            ...
            AppMsg::UnlockDialogAction(UnlockDialogOutput::SubmitLock(lock)) => {
                // Entry side of the `gio::spawn_blocking
                // paladin_auth_core::open` worker. Three steps run in
                ...
                if let Some(input) = worker_input {
                    ...
                    gtk::glib::spawn_future_local(async move {
                        let completion = gtk::gio::spawn_blocking(move || run_unlock_worker(input))
                            .await
                            .expect("paladin_auth_core::Store::open unlock worker panicked");
            ...
            AppMsg::InitDialogAction(output) => {
                // Spawn the `paladin_auth_core::Store::create` /
                ...
                if let Some(vault_path) = vault_path {
                    ...
                    gtk::glib::spawn_future_local(async move {
                        let completion = gtk::gio::spawn_blocking(move || run_init_worker(input))
                            .await
                            .expect("paladin_auth_core::Store::create init worker panicked");
            ...
            AppMsg::EditWorkerCompleted(completion) => {
                // Worker-outcome dispatch for the `gio::spawn_blocking
                // Vault::mutate_and_save(|v|
                // v.edit_account_metadata(...))` worker per
                // `docs/IMPLEMENTATION_PLAN_04_GTK.md` §"Vault
                // interaction":
                //
                // * The carried `(vault, store)` pair is reinstalled
                //   into `AppModel::vault` via
                //   `apply_edit_vault_install_inplace`
                //   unconditionally — `mutate_and_save` is
                //   authoritative for the post-edit / rollback state.
                // * The state machine rolls `UnlockedBusy → Unlocked`
                //   via `edit_final_app_state` on every typed effect
                //   (the `None` defensive case — a stray completion
                //   from a non-`UnlockedBusy` state — leaves
                //   `AppModel::state` byte-for-byte intact).
                // * `EditWorkerEffect::Success { post_summary }` drops
                //   the dialog (detach widget + take controller),
                //   refreshes the list so the edited row re-renders,
                //   and raises the `Edited {label}.` toast against the
                //   post-edit summary.
                // * `EditWorkerEffect::Failure(outcome)` keeps the
                //   dialog mounted and forwards
                //   `EditDialogMsg::WorkerCompleted(outcome)` so the
                //   body re-renders the inline warning / error.
                //
                // The dialog busy latch releases through the
                // end-of-`update` `sync_edit_dialog_busy` once the
                // state rolls back to `Unlocked` (only meaningful on
                // the failure branch, since success drops the dialog).
                let EditWorkerCompletion {
                    effect,
                    vault,
                    store,
                } = completion;
                apply_edit_vault_install_inplace(&mut self.vault, (vault, store));
                if let Some(state) = self.state.as_mut() {
                    if let Some(new_state) = edit_final_app_state(state, &effect) {
                        *state = new_state;
                    }
                }
                match effect {
                    EditWorkerEffect::Success { post_summary } => {
                        ...
                        self.refresh_account_list();
                        let body = format_edit_dialog_success_toast(post_summary.as_ref());
                        self.show_toast(&sender, body);
                    }
                    EditWorkerEffect::Failure(outcome) => {
                        if let Some(controller) = self.edit_dialog.as_ref() {
                            controller.emit(EditDialogMsg::WorkerCompleted(outcome));
                        }
                    }
                }
                let vault_still_encrypted =
...
impl AppModel {
    ...
    /// Reconcile the live
    /// [`crate::edit_dialog::EditDialogComponent`]'s busy flag
    /// against the current [`AppState::is_busy()`] reading. Peer of
    /// [`Self::sync_add_dialog_busy`] on the Edit dialog side.
    ///
    ...
    /// v.edit_account_metadata(...))` worker owns the live
    /// `(Vault, Store)` pair. Debounced through
    /// [`Self::last_edit_dialog_busy`].
    fn sync_edit_dialog_busy(&mut self) {
        let busy = self.state.as_ref().is_some_and(AppState::is_busy);
        if busy == self.last_edit_dialog_busy {
            return;
        }
        self.last_edit_dialog_busy = busy;
        if let Some(controller) = self.edit_dialog.as_ref() {
            controller.emit(EditDialogMsg::SetBusy(busy));
        }
    }
...
impl AppModel {
    ...
    fn lock_on_auto_lock_expiry(&mut self, sender: &ComponentSender<Self>) {
        ...
        // `EditDialogComponent` and `AddAccountComponent` use a
        ...
        //
        // Edit dialog: explicit `clear_for_lock` wipes every row
        // buffer + cached inline error + duplicate marker + pending
        // worker outcome on the controller's state BEFORE the
        // controller is dropped (mirroring the QR-export dialog), so
        // the draft never outlives the `(Vault, Store)` pair at the
        // lock boundary.
        if let Some(controller) = self.edit_dialog.as_ref() {
            crate::edit_dialog::clear_for_lock(controller.state().get_mut().model.state_mut());
            self.content.remove(controller.widget());
        }
        let edit_dialog = self.edit_dialog.take();
        if let Some(controller) = self.remove_dialog.as_ref() {
            controller.widget().force_close();
        }
    ...
    fn handle_tick(
        ...
        wall_clock: SystemTime,
        monotonic: Instant,
    ) -> Option<paladin_auth_core::ClipboardClearToken> {
    ...
    fn publish_reveal_for(
        &mut self,
        account_id: paladin_auth_core::AccountId,
        effect: RevealEffect,
        sender: &ComponentSender<Self>,
    ) {
        match effect {
...
///    if `Some`, otherwise `paladin_auth_core::default_vault_path()`. A
...
/// 2. `paladin_auth_core::inspect(path)` resolves the mode. Missing
...
/// 3. Plaintext only: `paladin_auth_core::Store::open(path,
...
/// `StartupError` so a future divergence in `paladin_auth_core` cannot
...
pub fn run_startup_probes(vault_path_override: Option<PathBuf>) -> StartupOutcome {
    let path = match vault_path_override {
        Some(p) => p,
        None => match paladin_auth_core::default_vault_path() {
            Ok(p) => p,
            Err(err) => {
    ...
    if let Some(state) = decide_state_from_inspect(&path, paladin_auth_core::inspect(&path)) {
        return StartupOutcome { state, vault: None };
    }

    match paladin_auth_core::Store::open(&path, paladin_auth_core::VaultLock::Plaintext) {
...
///
/// Format: `paladin-auth-gtk: startup_state=<Variant> path=<path>`. For
...
pub fn startup_state_marker(state: &AppState) -> String {
    ...
    format!("paladin-auth-gtk: startup_state={variant} path={path_repr}")
}
...
/// renders for the running binary. The wording (`"Paladin Auth"`) names
...
/// Preferences, About Paladin Auth, Quit) and matches the GNOME-HIG
...
/// Preferences, About Paladin Auth, Quit) which will land in follow-up
...
/// Fixed label the widget hands to the primary `gio::Menu`'s
/// "About Paladin Auth" entry.
///
/// Returns the static label `"About Paladin Auth"` — the wording for
...
/// GNOME app's primary-menu About entry. The trailing "Paladin Auth"
...
/// Fully-qualified `detailed_action_name` the widget hands to the
/// primary `gio::Menu`'s "About Paladin Auth" entry.
///
...
/// than `"about_paladin_auth"` so the action name does not need to
...
/// Paladin Auth" entry binds via [`format_app_menu_about_action`].
///
...
/// Preferences, About Paladin Auth, Quit) and `menu.append(...)`s
...
/// Build the application menu's "About Paladin Auth" entry's
...
/// "About Paladin Auth" dialog content against a single source of
/// truth.
///
...
/// Paladin Auth" entry's `AdwAboutDialog` displays in its header.
///
/// Returns the static program name `"Paladin Auth"` — the canonical
...
///
/// Distinct from [`crate::APP_ID`] (`"org.tamx.PaladinAuth.Gui"`),
...
/// Version string the application menu's "About Paladin Auth"
...
/// lockstep without manual updates — the `crates/paladin-auth-gtk`
...
/// Icon-theme key the application menu's "About Paladin Auth" entry's
/// `AdwAboutDialog` hands to `set_application_icon`.
///
/// Returns the reverse-DNS [`crate::APP_ID`]
/// (`"org.tamx.PaladinAuth.Gui"`) — the same key consumed by
/// `RelmApp::new(APP_ID)`, the §11.3 desktop entry's
/// `Icon=org.tamx.PaladinAuth.Gui` field, and the §11 hicolor icon
/// install layout (`/usr/share/icons/hicolor/<size>/apps/org.tamx.PaladinAuth.Gui.*`).
...
/// human "Paladin Auth" display name); the icon key is the
...
/// Attribution string the application menu's "About Paladin Auth"
/// entry's `AdwAboutDialog` shows in its developer-name slot.
///
/// Returns the canonical collective attribution
/// `"The Paladin Auth contributors"`. The workspace `Cargo.toml`
/// deliberately omits the `authors` field — Paladin Auth is an
...
pub fn format_app_about_dialog_copyright() -> &'static str {
    "\u{00A9} The Paladin Auth contributors"
}

/// Typed GTK license enum the application menu's "About
/// Paladin Auth" entry's `AdwAboutDialog` hands to
...
/// menu's "About Paladin Auth" entry's `AdwAboutDialog` hands to
...
/// `data/paladin-auth-gtk.gresource.xml` with the workspace root as
...
/// Paladin Auth" entry's bundled AGPL-3.0-or-later `LICENSE` body
/// mounts at inside the application's gresource pool.
///
/// Returns the absolute path
/// `"/org/tamx/PaladinAuth/Gui/LICENSE"`. The prefix segments
/// `/org/tamx/PaladinAuth/Gui` mirror [`crate::APP_ID`]
/// (`"org.tamx.PaladinAuth.Gui"`) reformatted as a slash-delimited
/// path so the gresource pool namespaces by reverse-DNS app ID,
/// matching the other Paladin Auth-bundled paths
/// ([`format_app_style_css_resource_path`] →
/// `"/org/tamx/PaladinAuth/Gui/style.css"`,
/// [`format_app_icon_theme_resource_path`] →
/// `"/org/tamx/PaladinAuth/Gui/icons"`). The terminal `LICENSE`
/// segment matches the alias declared in
/// `data/paladin-auth-gtk.gresource.xml`, so the path the helper
...
/// gresource side of the Paladin Auth bundled-license layer against
/// a single source of truth.
#[must_use]
...
/// Website URL the application menu's "About Paladin Auth" entry's
...
/// `crates/paladin-auth-gtk` inherits its `homepage` from the
...
/// Issue-tracker URL the application menu's "About Paladin Auth"
...
/// `crates/paladin-auth-gtk` inherits its `repository` from
...
/// Support URL the application menu's "About Paladin Auth" entry's
...
/// Short description the application menu's "About Paladin Auth"
...
/// `crates/paladin-auth-gtk` inherits its `description` from
...
/// Pure — returns a `'static str` resolved at compile time.
/// Distinct from [`format_app_about_dialog_program_name`] (the
/// bare `"Paladin Auth"` display string) so the dialog header
...
/// string (`"The Paladin Auth contributors"`) used in the dialog's
...
///
/// Returns the empty array for the v0.2 release. Paladin Auth does
...
/// Paladin Auth" entry's `AdwAboutDialog` hands to `set_artists` for
...
/// Paladin Auth" entry's `AdwAboutDialog` hands to `set_documenters`
...
/// Paladin Auth does not yet ship a gettext catalog (no `LINGUAS` /
...
/// Paladin Auth" entry's `AdwAboutDialog` release-notes-version slot
...
/// Markup body the application menu's "About Paladin Auth" entry's
...
/// because Paladin Auth has not yet shipped a tagged release.
/// `AdwAboutDialog` follows the libadwaita convention of
...
/// Plain-text payload the application menu's "About Paladin Auth"
...
pub fn format_app_about_dialog_debug_info() -> &'static str {
    concat!(
        ...
        env!("CARGO_PKG_VERSION"),
        "\nApp ID: ",
        "org.tamx.PaladinAuth.Gui",
    )
}

/// Suggested filename the application menu's "About Paladin Auth"
/// entry's `AdwAboutDialog` hands to `set_debug_info_filename`
/// for the "Save debug info" file-save dialog.
///
/// Returns the static filename `"paladin-auth-debug-info.txt"` —
...
/// `application-name` change drifts away from the `paladin-auth`
...
/// `"/org/tamx/PaladinAuth/Gui/style.css"`. The prefix segments
/// (`org` / `tamx` / `PaladinAuth` / `Gui`) mirror [`crate::APP_ID`]
/// (`"org.tamx.PaladinAuth.Gui"`) split on `.` so the resource pool
...
/// `data/paladin-auth-gtk.gresource.xml`, so the bundle compiled by
...
/// together they pin the gresource side of the Paladin Auth CSS layer
/// against a single source of truth.
#[must_use]
...
/// `data/paladin-auth-gtk.gresource.xml` (which references
/// `data/style.css` and, in subsequent commits, the placeholder
/// icon and any `*.ui` templates) into
/// `OUT_DIR/paladin-auth-gtk.gresource`. The `include_bytes!` macro
...
const APP_GRESOURCE_BUNDLE_BYTES: &[u8] =
    include_bytes!(concat!(env!("OUT_DIR"), "/paladin-auth-gtk.gresource"));
...
/// side; together they pin the three halves of the Paladin Auth CSS
...
pub fn register_app_gresource_bundle() {
    ...
    let resource = gio::Resource::from_data(&bytes)
        .expect("paladin-auth-gtk gresource bundle bytes must be valid GResource format");
    gio::resources_register(&resource);
}

/// Layer Paladin Auth's CSS on top of the Adwaita stylesheet for
/// `display`.
///
...
/// Absolute gresource path to the Paladin Auth-bundled icon-theme root.
///
/// Returns the static absolute resource path
/// `"/org/tamx/PaladinAuth/Gui/icons"`. The prefix segments
/// (`org` / `tamx` / `PaladinAuth` / `Gui`) mirror [`crate::APP_ID`]
/// (`"org.tamx.PaladinAuth.Gui"`) split on `.` so the resource pool
/// namespaces by reverse-DNS app ID and never collides with other
...
/// the gresource side of the Paladin Auth icon-theme layer against a
/// single source of truth.
#[must_use]
...
///
/// Returns the static absolute resource path
/// `"/org/tamx/PaladinAuth/Gui/icons/scalable/actions/dialog-password-symbolic.svg"`.
...
/// side; together they pin the three halves of the Paladin Auth
...
///    gates against the §5 [`paladin_auth_core::QR_RGBA_MAX_BYTES`]
...
///    buffer to [`paladin_auth_core::import::qr_image_bytes`];
