...
//! Incremental search filter for the `paladin-auth-gtk` account list.
//!
...
//! [`paladin_auth_core::account_matches_search`] so the search-bar
//! predicate stays byte-for-byte identical to the CLI query
//! resolution path in `docs/DESIGN.md` §5 and the parallel
//! `paladin_auth_tui::search` module. Insertion order is preserved because
...
/// [`paladin_auth_core::select_after_filter`] so the GUI's list-view
