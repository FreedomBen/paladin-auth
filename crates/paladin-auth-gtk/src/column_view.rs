...
/// glyph in `paladin-auth-tui/src/view/list.rs`).
...
/// onto the matching [`AccountRowOutput`], then onto an
/// [`AccountListOutput`] for `AppModel`.
...
pub fn any_totp(rows: &[AccountRowModel]) -> bool {
    rows.iter()
        .any(|row| matches!(row.kind, paladin_auth_core::AccountKindSummary::Totp))
}
