...
//! Clipboard auto-clear pure-logic glue for `paladin-auth-gtk`.
//!
...
//! [`paladin_auth_core::ClipboardClearPolicy`]:
//!
...
//! and that rule lives in [`paladin_auth_core::IdlePolicy`]).
//!
...
///
/// Routes through
/// [`ClipboardClearPolicy::schedule`][paladin_auth_core::ClipboardClearPolicy::schedule].
...
/// Duplicated against `paladin_auth_tui::app::state::format_next_code_copied`
/// because the two binary crates can't depend on each other and
/// `paladin-auth-core` shouldn't grow a text helper for a single
