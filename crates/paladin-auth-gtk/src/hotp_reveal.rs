...
//! HOTP reveal-window pure-logic glue for `paladin-auth-gtk`.
//!
...
//! [`paladin_auth_core::hotp_reveal_deadline`], which sources
//! [`paladin_auth_core::HOTP_REVEAL_SECS`] from the shared `ui_contract`.
//!
...
use paladin_auth_core::{
    hotp_reveal_deadline, AccountId, AccountSummary, Code, PaladinAuthError, Store, Vault,
};
...
///
/// Routes through [`paladin_auth_core::hotp_reveal_deadline`]; equivalent
/// to `now + Duration::from_secs(paladin_auth_core::HOTP_REVEAL_SECS)`.
#[must_use]
...
pub struct AdvanceOutcome {
    ...
    pub result: Result<Code, PaladinAuthError>,
...
pub fn apply_advance_outcome(outcome: AdvanceOutcome) -> AdvanceDecision {
    ...
    match result {
        ...
        Err(PaladinAuthError::SaveDurabilityUnconfirmed) => match staged_code {
...
// their secrets via their own `Debug` impls in `paladin_auth_core`, but
