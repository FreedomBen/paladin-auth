...
//! Init-dialog pure-logic state machine for `paladin-auth-gtk`.
//!
...
//! [`paladin_auth_core::VaultInit`] the caller hands to a worker calling
//! [`paladin_auth_core::Store::create`] (or
//! [`paladin_auth_core::Store::create_force`] after the destructive gate).
//!
//! # Precheck routing
//!
//! Before the `create` worker spawns, the dialog runs
//! [`paladin_auth_core::classify_init_precheck`] against
//! [`paladin_auth_core::inspect`]. [`classify_precheck`] maps the
//! [`paladin_auth_core::InitPrecheck`] truth table onto the dialog's three
...
//! [`paladin_auth_core::format_init_force_warning`] (see
...
//! [`paladin_auth_core::format_unsafe_permissions`] so wording matches
...
//! [`paladin_auth_core::EncryptionOptions`] passphrase wipes on drop via
...
use paladin_auth_core::{
    ...
    InitPrecheck, PaladinAuthError, Store, Vault, VaultInit, VaultStatus,
};
...
impl SubmitRejection {
    ...
    /// [`PaladinAuthError`] kind; `None` for the UI-only plaintext
    /// warning gate.
    #[must_use]
...
/// [`paladin_auth_core::format_plaintext_storage_warning`] verbatim so it
...
/// Wording matches [`paladin_auth_core::format_init_force_warning`]
...
pub enum PrecheckOutcome {
    ...
    /// [`paladin_auth_core::Store::create`].
    Proceed,
    /// `InitPrecheck::Existing` — open the destructive-confirmation
    /// gate; on confirm, call [`paladin_auth_core::Store::create_force`].
    DestructiveGate,
...
/// Map a [`paladin_auth_core::inspect`] result onto the dialog's
/// three-way routing decision via
/// [`paladin_auth_core::classify_init_precheck`].
#[must_use]
pub fn classify_precheck(probe: Result<VaultStatus, PaladinAuthError>) -> PrecheckOutcome {
...
/// Routing decision for a [`paladin_auth_core::Store::create`] failure.
///
...
/// Classify a [`paladin_auth_core::Store::create`] failure into a
/// [`CreateOutcome`].
///
...
/// [`paladin_auth_core::format_create_vault_dir_error`] wording naming the
/// directory paladin-auth tried to `mkdir -p`.
#[must_use]
pub fn classify_create_error(err: &PaladinAuthError, attempted_dir: &Path) -> CreateOutcome {
    match err.kind() {
...
/// Classify a [`paladin_auth_core::Store::create_force`] failure into an
/// [`InlineError`].
///
...
/// [`paladin_auth_core::format_create_vault_dir_error`] wording using
/// `attempted_dir`.
#[must_use]
pub fn classify_create_force_error(err: &PaladinAuthError, attempted_dir: &Path) -> InlineError {
    InlineError::from_create_error(err, attempted_dir)
}

/// Whether [`run_init_worker`] should route through
/// [`paladin_auth_core::Store::create`] or
/// [`paladin_auth_core::Store::create_force`].
///
...
pub struct InitWorkerInput {
    ...
    /// [`paladin_auth_core::Store::create`] or
    /// [`paladin_auth_core::Store::create_force`]. Threaded through to
    /// [`classify_create_error`] / [`classify_create_force_error`]
    /// so a `create_vault_dir` `IoError` renders the friendly
    /// [`paladin_auth_core::format_create_vault_dir_error`] wording
    /// naming the directory paladin-auth tried to `mkdir -p`.
    pub vault_path: PathBuf,
    /// Toggle between [`paladin_auth_core::Store::create`] and
    /// [`paladin_auth_core::Store::create_force`].
...
/// [`paladin_auth_core::Store::create`] or
/// [`paladin_auth_core::Store::create_force`] per [`InitWorkerMode`],
...
/// `paladin-auth init` flow (`Store::create` + `vault.save(&store)`) so
/// the on-disk vault survives an app restart even when the user
/// never adds an account. [`InitWorkerMode::CreateForce`] does not
/// need this hand-off because [`paladin_auth_core::Store::create_force`]
...
pub fn run_init_worker(input: InitWorkerInput) -> InitWorkerCompletion {
    ...
    // resolves vaults under `$XDG_DATA_HOME/paladin-auth/`). Falling back
...
pub struct InlineError {
    ...
    /// [`paladin_auth_core::format_unsafe_permissions`]; other variants
    /// fall back to the typed [`std::fmt::Display`].
    pub rendered: String,
    /// Optional rotated-`.bak` path threaded through from
    /// [`PaladinAuthError::SaveNotCommitted::backup_path`]. Always
    /// `None` for non-`save_not_committed` variants.
    pub backup_path: Option<PathBuf>,
}

impl InlineError {
    /// Build an [`InlineError`] from a [`PaladinAuthError`]. Renders
    ...
    /// [`paladin_auth_core::format_create_vault_dir_error`] helper, so the
    /// dialog body names the directory paladin-auth tried to `mkdir -p`.
    /// `attempted_dir` is typically the dialog's
    /// `InitDialogInit::vault_path.parent()`.
    #[must_use]
    pub fn from_create_error(err: &PaladinAuthError, attempted_dir: &Path) -> Self {
        Self {
            kind: err.kind(),
    ...
    /// [`PaladinAuthError`] kind; the dialog surfaces the unticked warning
    ...
    /// [`PaladinAuthError`] variant so the GUI surfaces the same stable §5
    ...
    pub fn from_rejection(rejection: SubmitRejection) -> Option<Self> {
        match rejection {
            SubmitRejection::ConfirmationMismatch => {
                Some(Self::from_error(&PaladinAuthError::InvalidPassphrase {
...
fn render_inline(err: &PaladinAuthError) -> String {
    format_unsafe_permissions(err).unwrap_or_else(|| err.to_string())
}

fn render_create_inline(err: &PaladinAuthError, attempted_dir: &Path) -> String {
    format_create_vault_dir_error(err, attempted_dir).unwrap_or_else(|| render_inline(err))
}

fn backup_path_of(err: &PaladinAuthError) -> Option<PathBuf> {
    match err {
        PaladinAuthError::SaveNotCommitted { backup_path, .. } => backup_path.clone(),
        _ => None,
    }
}
...
pub const INIT_DIALOG_MARKER_PREFIX: &str = "paladin-auth-gtk: init_dialog_path=";

/// Format the smoke-test stdout marker line for a mounted
/// [`InitDialogComponent`].
///
/// The marker is `paladin-auth-gtk: init_dialog_path=<path>` where
/// `<path>` is the resolved vault path the dialog will pass to
/// `paladin_auth_core::Store::create` on submit.
#[must_use]
...
/// `paladin_auth_core::format_plaintext_storage_warning()` body.
...
/// body lives in [`paladin_auth_core::format_plaintext_storage_warning`]
...
/// [`paladin_auth_core::format_init_force_warning`]); this helper
...
/// Paladin Auth-owned memory rather than escaping through `AppMsg` /
...
/// [`paladin_auth_core::EncryptionOptions`] whose `SecretString` passphrase
        ...
        gtk::Box {
            ...
            adw::PreferencesGroup {
                ...
                add = &adw::PasswordEntryRow {
                    ...
                    // entry and Paladin Auth-owned `Zeroizing<String>` is
