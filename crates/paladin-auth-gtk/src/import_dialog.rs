...
//! Import-dialog pure-logic state machine for `paladin-auth-gtk`.
//!
...
//! Paladin Auth-header probe returns
//! [`paladin_auth_core::PaladinAuthImportPrecheck::PromptForPassphrase`]. The
//! widget layer drives this module's helpers to:
//!
//! * Translate the format selector into the
//!   [`paladin_auth_core::ImportFormat`] that
//!   [`paladin_auth_core::ImportOptions::format`] consumes
//!   ([`FormatChoice::forced_format`] /
//!   [`build_import_options`]).
//! * Translate the conflict selector into the
//!   [`paladin_auth_core::ImportConflict`] passed to
//!   [`paladin_auth_core::Vault::import_accounts`]
//!   ([`ConflictChoice::into_policy`]).
//! * Route the Paladin Auth-header probe via
//!   [`paladin_auth_core::classify_paladin_auth_import_precheck`] →
//!   [`classify_precheck`] into one of three decisions:
//!     - [`PrecheckOutcome::Proceed`] continues into
//!       [`paladin_auth_core::import::from_file`] so the importer facade
...
//! * Classify the [`paladin_auth_core::Vault::mutate_and_save`] result
...
//!       [`paladin_auth_core::Vault::import_accounts`] errors. All stay
...
//! [`paladin_auth_core::ImportOptions::paladin_auth_passphrase`] zeroizes on
//! drop in turn. Inline-error / inline-warning bodies are rendered
//! through [`PaladinAuthError::Display`], so wording stays in lock-step
...
use paladin_auth_core::{
    ErrorKind, ImportConflict, ImportFormat, ImportOptions, ImportReport, PaladinAuthError,
    PaladinAuthImportPrecheck, Store, Vault,
};
...
pub enum FormatChoice {
    ...
    Aegis,
    /// Force the [`ImportFormat::PaladinAuth`] (encrypted bundle) path.
    PaladinAuth,
...
impl FormatChoice {
    ...
    pub fn forced_format(self) -> Option<ImportFormat> {
        match self {
            ...
            Self::PaladinAuth => Some(ImportFormat::PaladinAuth),
...
///
/// Maps to [`paladin_auth_core::ImportConflict`] via
/// [`ConflictChoice::into_policy`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum ConflictChoice {
    ...
    /// `paladin-auth import --on-conflict=skip` default.
    #[default]
    Skip,
...
impl ConflictChoice {
    ...
    /// [`paladin_auth_core::Vault::import_accounts`] consumes.
    #[must_use]
...
///
/// `paladin_auth_passphrase` is taken verbatim; the helper does not
/// pre-filter on `format` because the importer facade itself ignores
/// the field for non-Paladin Auth formats. Passing the
...
/// Routing decision after the Paladin Auth-header probe.
///
/// See [`classify_precheck`].
#[derive(Debug)]
pub enum PrecheckOutcome {
    /// `NoPrompt` — no Paladin Auth-bundle passphrase needed. The dialog
    /// continues into [`paladin_auth_core::import::from_file`] (the
    /// importer facade owns the typed format / I/O errors).
    Proceed,
    /// `PromptForPassphrase` — encrypted Paladin Auth header detected.
...
/// Map a [`paladin_auth_core::classify_paladin_auth_import_precheck`] result
/// onto the dialog's three-way routing decision.
#[must_use]
pub fn classify_precheck(probe: PaladinAuthImportPrecheck) -> PrecheckOutcome {
    match probe {
        PaladinAuthImportPrecheck::NoPrompt => PrecheckOutcome::Proceed,
        PaladinAuthImportPrecheck::PromptForPassphrase => PrecheckOutcome::PromptForPassphrase,
...
/// in its counts panel. Each [`paladin_auth_core::ImportWarning`] is
...
pub struct MergeSummary {
    ...
    pub appended: usize,
    /// Count of [`paladin_auth_core::ImportWarning`] entries collected
...
/// Outcome of a [`paladin_auth_core::Vault::mutate_and_save`] call wrapping
/// the importer + [`paladin_auth_core::Vault::import_accounts`] closure.
///
...
pub enum MergeOutcome {
    ...
    /// [`paladin_auth_core::Vault::import_accounts`] errors. Vault state is
    /// unchanged because the error fired before the save path; the
    /// dialog stays inline.
    Inline(InlineError),
}

/// Classify the [`paladin_auth_core::Vault::mutate_and_save`] result into
/// a [`MergeOutcome`].
///
...
#[must_use]
pub fn classify_merge_result(result: Result<ImportReport, PaladinAuthError>) -> MergeOutcome {
    match result {
...
pub enum ImportDialogMsg {
    ...
    /// [`paladin_auth_core::classify_paladin_auth_import_precheck`] inline (cheap,
...
pub enum ImportDialogOutput {
    ...
    /// worker that runs
    /// `Vault::mutate_and_save(|v| { from_file(...) -> v.import_accounts(...) })`
    /// (the encrypted-Paladin Auth variant runs Argon2id; keep it off the
    /// main loop).
...
/// `[AutoDetect, Otpauth, Aegis, Paladin Auth, Qr]`.
#[must_use]
...
pub fn format_choice_from_index(selected: u32) -> Option<FormatChoice> {
    match selected {
        ...
        3 => Some(FormatChoice::PaladinAuth),
...
impl SimpleComponent for ImportDialogComponent {
                            ...
                            connect_selected_notify[sender] => move |row| {
                                    ...
                                    format_choice_from_index(row.selected())
                                {
                                    ...
                                    let _ = sender.input_sender().send(ImportDialogMsg::FormatChanged {
                                        ...
                                        precheck:
                                            paladin_auth_core::PaladinAuthImportPrecheck::NoPrompt,
                                    });
                                }
                            },
                        },
    ...
    fn init(
        ...
        // `paladin_auth_core::classify_paladin_auth_import_precheck` inline
                ...
                move |result| {
                    if let Ok(file) = result {
                        if let Some(path) = file.path() {
                            let precheck = paladin_auth_core::classify_paladin_auth_import_precheck(
                                &path,
                                forced_format,
                            );
...
/// worker runs.
///
/// `Clone` is deliberately not derived: [`ImportOptions::paladin_auth_passphrase`]
...
pub struct ImportSubmitPayload {
    ...
    /// [`paladin_auth_core::import::from_file`] which handles format
    /// detection via [`paladin_auth_core::import::detect`] when
    /// [`ImportOptions::format`] is `None`.
    pub source_path: PathBuf,
    /// Format + bundle-passphrase bundle ready for
    /// [`paladin_auth_core::import::from_file`]. The widget builds it
    /// through [`build_import_options`] so the format-selector
    /// routing stays in one helper.
    pub options: ImportOptions,
    /// On-conflict policy threaded into
    /// [`paladin_auth_core::Vault::import_accounts`]. Built via
    /// [`ConflictChoice::into_policy`].
...
pub enum SubmitOutcome {
    ...
    /// completed the Paladin Auth-header probe for the current
    ...
    /// The precheck staged an inline error (e.g. malformed Paladin Auth
    /// header under explicit `format = Paladin Auth`). The widget should
...
/// [`paladin_auth_core::classify_paladin_auth_import_precheck`], the busy
...
impl ImportDialogState {
    ...
    pub fn set_source_path(&mut self, path: PathBuf, precheck: PaladinAuthImportPrecheck) {
    ...
    pub fn set_format(&mut self, format: FormatChoice, precheck: PaladinAuthImportPrecheck) {
    ...
    /// [`paladin_auth_core::Vault::import_accounts`] at submit time.
...
/// holds non-`Clone` filesystem state, [`ImportOptions::paladin_auth_passphrase`]
...
pub struct ImportWorkerInput {
    ...
    /// to both [`paladin_auth_core::import::from_file`] (for
    /// `created_at` / `updated_at` on the per-row
    /// [`paladin_auth_core::ValidatedAccount`]) and
    /// [`paladin_auth_core::Vault::import_accounts`] (for the merge-time
...
/// outcome — [`paladin_auth_core::Vault::mutate_and_save`] already
...
/// merge inside [`paladin_auth_core::Vault::mutate_and_save`], and
...
pub fn run_import_worker(input: ImportWorkerInput) -> ImportWorkerCompletion {
    ...
    let result: Result<ImportReport, PaladinAuthError> = vault.mutate_and_save(&store, move |v| {
        let accounts = paladin_auth_core::import::from_file(&source_path, options, import_time)?;
