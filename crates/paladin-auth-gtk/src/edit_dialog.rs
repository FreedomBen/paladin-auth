...
//! Edit-dialog pure-logic state machine for `paladin-auth-gtk`.
//!
...
//! and icon-hint — over a [`paladin_auth_core::AccountEdit`] value and
//! calls [`paladin_auth_core::Vault::edit_account_metadata`] inside
...
//!   [`paladin_auth_core::validate_label`]. Whitespace touches that
...
//!   `paladin-auth edit --no-issuer`: an empty buffer over a `Some(_)`
...
//!   [`paladin_auth_core::parse_icon_hint_token`]. Byte-equal to the
//!   pre-fill ⇒ leave-untouched; empty over `Some(_)` ⇒
//!   re-derive from post-edit issuer
//!   ([`paladin_auth_core::IconHintInput::Default`]); literal `none`
//!   (case-insensitive) ⇒ explicit clear
//!   ([`paladin_auth_core::IconHintInput::Clear`]); anything else ⇒
...
//! [`paladin_auth_core::validate_account_edit`] runs first; only on
...
//! # Post-effect routing
//!
//! [`classify_post_effect_error`] maps the [`PaladinAuthError`] from
...
use paladin_auth_core::{
    ...
    AccountSummary, ErrorKind, IconHintInput, PaladinAuthError, Store, Vault,
};

/// §4.1 issuer length cap. Mirrors the constant `paladin_auth_core`
...
/// * `Validated` — per-field
///   [`paladin_auth_core::validate_account_edit`] passed; the carried
...
/// Map a [`PaladinAuthError`] from the
/// `Vault::edit_account_metadata` worker into a typed
/// [`PostEffectOutcome`].
#[must_use]
pub fn classify_post_effect_error(err: &PaladinAuthError) -> PostEffectOutcome {
    match err.kind() {
...
/// `Edit…` activation.
pub const EDIT_DIALOG_MARKER_PREFIX: &str = "paladin-auth-gtk: edit_dialog_account=";
