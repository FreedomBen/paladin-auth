...
//! `AccountListComponent` for `paladin-auth-gtk`.
//!
...
use paladin_auth_core::{select_after_filter, AccountId, AccountKindSummary, Vault};
...
pub const ACCOUNT_LIST_RENDERED_MARKER_PREFIX: &str = "paladin-auth-gtk: account_list_rows=";
...
/// keeps `paladin_auth_core::account_matches_search` the single source of
...
///
/// Built from `paladin_auth_core::AccountSummary` (which itself carries
...
pub struct AccountRowModel {
    ...
    /// menu, …) back to `paladin_auth_core::Vault`.
    pub id: AccountId,
    ...
    /// `paladin_auth_core::Vault::summaries` always supplies one for
    /// HOTP).
...
/// `AccountSummary`-driven, so no secret bytes leave `paladin_auth_core`
...
/// `paladin_auth_core::account_matches_search` (and therefore by the CLI
...
///
/// Wraps [`paladin_auth_core::select_after_filter`] against the row
...
/// rendered row set.
///
/// The shape is `paladin-auth-gtk: account_list_rows=<labels>` where
...
///
/// The shape is
/// `paladin-auth-gtk: account_list_widget_states=<entries>` where
...
pub enum AccountListMsg {
    ...
    /// `crates/paladin-auth-gtk/data/style.css`.
    ///
    /// Idempotent — sending the same value twice is a benign
    /// no-op.
...
///
/// The application stylesheet (`crates/paladin-auth-gtk/data/style.css`)
