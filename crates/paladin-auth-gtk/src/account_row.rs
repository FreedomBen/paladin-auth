...
//! Per-row pure-logic projection for `paladin-auth-gtk`.
//!
...
use paladin_auth_core::{AccountId, AccountKindSummary, AccountSummary, Code};
...
///   `paladin_auth_core::Vault` validation today).
...
pub struct ProgressDisplay {
    ...
    /// [`Code::seconds_remaining`]; `paladin_auth_core` pins this to
    /// `1..=period_secs` per `crates/paladin-auth-core/src/otp/totp.rs`).
...
/// `seconds_remaining: None` both yield `None` — `paladin_auth_core` never
...
/// `paladin_auth_core` invariant pins it to `1..=period`) and to `0.0`
/// when `period_secs == 0` (defensive — `paladin_auth_core::validation`
...
/// [`ProgressUrgency::Critical`] defensively — `paladin_auth_core::validation`
