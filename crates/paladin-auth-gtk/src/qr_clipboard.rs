...
//! Clipboard-QR add-image pure-logic glue for `paladin-auth-gtk`.
//!
...
//! buffer, rejects sizes above [`paladin_auth_core::QR_RGBA_MAX_BYTES`]
...
//! are then passed to [`paladin_auth_core::import::qr_image_bytes`], and
//! the resulting [`paladin_auth_core::ValidatedAccount`]s are merged into
...
use paladin_auth_core::{
    import::qr_image_bytes, ErrorKind, ImportConflict, ImportReport, PaladinAuthError, Result,
...
/// [`paladin_auth_core::import::qr_image_bytes`]. The live wiring in
...
/// [`paladin_auth_core::import::qr_image_bytes`] and surface its
/// `Vec<ValidatedAccount>` (or [`PaladinAuthError`]) verbatim.
///
...
/// [`paladin_auth_core::import::qr_image_bytes`] without first running
...
pub enum QrDecodeOutcome {
    ...
    /// [`paladin_auth_core::import::qr_image_bytes`] rejected the
    /// buffer. Covers `NoEntriesToImport` (no QR present),
    /// `validation_error` (decoded payload is not an `otpauth://`
    /// URI or fails [`paladin_auth_core::parse_otpauth`]), and any
    /// other typed [`PaladinAuthError`] the core decoder surfaces.
    DecodeError(PaladinAuthError),
}
...
/// `import_time` into [`paladin_auth_core::import::qr_image_bytes`] via
...
impl QrImportSummary {
    /// Extract the §6 counts panel projection from a paladin-auth-core report.
    #[must_use]
...
///   [`PaladinAuthError::NoEntriesToImport`] (`paladin_auth_core::import::qr_image_bytes`
///   found no QR in the texture).
/// * Invalid payload — [`Self::Decode`] wrapping
///   [`PaladinAuthError::ValidationError`] (decoded payload is not a
...
///
/// Not `Clone` because [`PaladinAuthError`] is not `Clone`; the converter
...
pub enum QrPreflightError {
    ...
    DownloadMismatch(DownloadMismatch),
    /// [`paladin_auth_core::import::qr_image_bytes`] rejected the buffer.
    /// Covers [`PaladinAuthError::NoEntriesToImport`] (zero decoded QRs)
    /// and [`PaladinAuthError::ValidationError`] (invalid payload), plus
...
impl QrPreflightError {
    ...
    ///   stride).
    /// * [`Self::Decode`] → the underlying [`PaladinAuthError::kind`] so
...
impl core::fmt::Display for QrPreflightError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            // Forward the underlying paladin-auth error verbatim so the
            ...
            // had piped a malformed QR through `paladin-auth import`.
...
pub fn classify_qr_outcome(
    ...
    match outcome {
        ...
        QrDecodeOutcome::Decoded(_) => Err(QrPreflightError::Decode(
