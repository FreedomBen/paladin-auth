...
//! Startup-error pure-logic glue for `paladin-auth-gtk`.
//!
//! Per `docs/IMPLEMENTATION_PLAN_04_GTK.md` §"Component tree" and
//! §"Vault interaction", `AppModel` runs `paladin_auth_core::default_vault_path()`
//! and `paladin_auth_core::inspect(path)` at startup, then opens the vault
//! through `paladin_auth_core::open(path, lock)`. Three categories of
...
//! [`paladin_auth_core::format_unsafe_permissions`] so wording matches the
...
use paladin_auth_core::{format_unsafe_permissions, ErrorKind, PaladinAuthError, VaultStatus};
...
pub enum StartupErrorSource {
    /// `paladin_auth_core::default_vault_path()` returned `Err`.
    PathResolution,
    /// `paladin_auth_core::inspect(path)` returned `Err`.
    Inspect,
    /// `paladin_auth_core::open(path, lock)` returned a non-passphrase `Err`.
    Open,
...
///
/// All fields are presentation-side projections of a [`PaladinAuthError`]
...
pub struct StartupError {
    ...
    /// `PaladinAuthError::kind`. Consumers read this to drive
    /// instrumentation / structured logging.
    pub kind: ErrorKind,
    /// Display body for the `AdwStatusPage`. Uses
    /// [`paladin_auth_core::format_unsafe_permissions`] for the
...
impl StartupError {
    ...
    #[must_use]
    pub fn from_path_resolution(err: &PaladinAuthError) -> Self {
        Self {
    ...
    #[must_use]
    pub fn from_inspect(err: &PaladinAuthError) -> Self {
        Self {
    ...
    #[must_use]
    pub fn from_open(err: &PaladinAuthError) -> Self {
        Self {
    ...
    /// a GUI-specific kind into `paladin_auth_core`.
    #[must_use]
...
/// Decision tag for routing a [`PaladinAuthError`] returned by
...
/// Classify a `paladin_auth_core::open` failure into the routing decision
/// described in §"Vault interaction" of the plan.
#[must_use]
pub fn classify_open_error(err: &PaladinAuthError) -> OpenErrorRouting {
    match err.kind() {
...
/// Render a [`PaladinAuthError`] for the `StartupErrorComponent` body.
///
/// `UnsafePermissions` routes through
/// [`paladin_auth_core::format_unsafe_permissions`] so the wording matches
...
#[must_use]
pub fn render_startup_error(err: &PaladinAuthError) -> String {
...
/// instructs the user to restart Paladin Auth. The exact wording is
...
pub fn format_worker_panic_message(effect: EffectKind) -> String {
    format!(
        "A background task ({}) failed unexpectedly. Restart Paladin Auth to continue.",
        effect.user_name(),
    )
}
...
pub const STARTUP_ERROR_MARKER_PREFIX: &str = "paladin-auth-gtk: startup_error_body=";

/// Format the smoke-test stdout marker line for a mounted
/// [`StartupError`].
///
/// The marker is `paladin-auth-gtk: startup_error_body=<rendered>` where
...
/// `paladin_auth_core::format_unsafe_permissions` or `PaladinAuthError`
...
/// body, sourced from the typed [`paladin_auth_core::PaladinAuthError`]
...
/// `paladin_auth_core::destroy_vault` in `AppModel`, not here, so this
...
/// sees matches `paladin_auth_core::format_unsafe_permissions` /
/// `PaladinAuthError::Display` exactly (same text the CLI and TUI use).
...
pub struct StartupErrorComponent {
    /// Cloned projection of the typed [`PaladinAuthError`] that routed
...
where
    R: FnOnce() -> Result<PathBuf, PaladinAuthError>,
    I: FnOnce(&Path) -> Result<VaultStatus, PaladinAuthError>,
{
