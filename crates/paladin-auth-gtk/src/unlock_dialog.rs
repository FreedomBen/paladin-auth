...
//! Unlock-dialog pure-logic state machine for `paladin-auth-gtk`.
//!
...
//! [`paladin_auth_core::inspect`] reports
//! [`paladin_auth_core::VaultStatus::Encrypted`]. Plaintext vaults skip
...
//! [`paladin_auth_core::VaultLock::Encrypted`] handed to
//! [`paladin_auth_core::open`] inside a `gio::spawn_blocking` worker so
...
//! * `Err(PaladinAuthError)` routes through [`classify_unlock_error`],
...
//! libadwaita. The `gio::spawn_blocking paladin_auth_core::open` worker
...
use paladin_auth_core::{ErrorKind, PaladinAuthError, VaultLock, VaultStatus};
...
/// to receive [`PaladinAuthError::InvalidPassphrase`] with
...
/// Build the [`VaultLock`] passed to [`paladin_auth_core::open`] from the
...
///   policy lives in `paladin_auth_core::open`).
...
/// Route a [`paladin_auth_core::open`] failure returned by the unlock
...
/// every `paladin_auth_core::open` call.
#[must_use]
pub fn classify_unlock_error(err: &PaladinAuthError) -> OpenErrorRouting {
...
pub const UNLOCK_DIALOG_MARKER_PREFIX: &str = "paladin-auth-gtk: unlock_dialog_path=";

/// Format the smoke-test stdout marker line for a mounted
/// [`UnlockDialogComponent`].
///
/// The marker is `paladin-auth-gtk: unlock_dialog_path=<path>` where
/// `<path>` is the resolved vault path the dialog will pass to
/// `paladin_auth_core::open` (inside `gio::spawn_blocking`) on submit.
#[must_use]
...
/// TUI unlock view's `" Paladin Auth — unlock "` block title (see
/// `crates/paladin-auth-tui/src/view/unlock.rs`), which names the
/// application instead of the action because the TUI has no other
/// window-chrome to identify the app, while the GTK dialog
/// inherits the `org.tamx.PaladinAuth.Gui` window title and only needs
...
/// `crates/paladin-auth-tui/src/view/unlock.rs`) — the TUI's trailing
...
/// `paladin_auth_core::open` on submit. The path is the `vault_path`
/// `AppModel` resolved through `paladin_auth_core::default_vault_path`
...
/// `gio::spawn_blocking` [`paladin_auth_core::open`] worker (deferred —
...
impl InlineError {
    ...
    /// [`PaladinAuthError::DecryptFailed`] and
    /// [`PaladinAuthError::InvalidPassphrase`] — the §5 errors
    ...
    /// [`PaladinAuthError`] through without re-routing here.
    #[must_use]
    ...
    pub fn from_rejection(rejection: SubmitRejection) -> Self {
        match rejection {
            SubmitRejection::EmptyPassphrase => {
                Self::from_error(&PaladinAuthError::InvalidPassphrase {
...
pub enum UnlockOpenRouting {
    ...
    /// verbatim through the [`PaladinAuthError::Display`] impl so the
...
/// Route a [`paladin_auth_core::open`] failure into the corresponding
/// [`UnlockOpenRouting`] decision.
///
/// Wrong-passphrase failures
/// ([`PaladinAuthError::DecryptFailed`], [`PaladinAuthError::InvalidPassphrase`])
...
#[must_use]
pub fn route_unlock_open_error(err: &PaladinAuthError) -> UnlockOpenRouting {
...
/// bytes into a Paladin Auth-owned [`SecretEntry`]
...
/// [`paladin_auth_core::VaultLock::Encrypted`] passed to
/// `paladin_auth_core::open` inside `gio::spawn_blocking`. On submit /
...
/// `gio::spawn_blocking paladin_auth_core::open` worker. Typing,
...
impl UnlockDialogState {
    ...
    /// [`paladin_auth_core::VaultLock::Encrypted`], dropping the wrapper
    /// once `paladin_auth_core::open` returns so the bytes zeroize. Any
    ...
    /// `gio::spawn_blocking paladin_auth_core::open` worker fired from
    /// `AppModel::update` reports a [`PaladinAuthError::DecryptFailed`]
    /// or [`PaladinAuthError::InvalidPassphrase`] result (per
    ...
    ///
    /// Called by the `gio::spawn_blocking paladin_auth_core::open` worker
...
pub enum UnlockDialogMsg {
    ...
    /// `AppModel`. The `gio::spawn_blocking paladin_auth_core::open`
    ...
    /// `gio::spawn_blocking paladin_auth_core::open` worker reports a
    /// `decrypt_failed` / `invalid_passphrase` failure.
    ///
    /// The variant carries the already-projected [`InlineError`]
    /// (built once by `AppModel` from the typed `PaladinAuthError`) so
...
/// paladin_auth_core::open`, and never observes the cleartext again.
#[derive(Debug)]
pub enum UnlockDialogOutput {
    ...
    /// `gio::spawn_blocking paladin_auth_core::open` worker that
...
pub struct UnlockDialogComponent {
    ...
    /// `paladin_auth_core::Store::open` call site reads the path from
        ...
        gtk::Box {
            ...
            adw::PreferencesGroup {
                ...
                add = &adw::PasswordEntryRow {
                    ...
                    // and Paladin Auth-owned `Zeroizing<String>` is the only
            ...
            // `gio::spawn_blocking paladin_auth_core::open` worker fired by
