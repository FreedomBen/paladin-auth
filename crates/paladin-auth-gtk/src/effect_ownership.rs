...
//! In-flight vault-effect ownership state machine for `paladin-auth-gtk`.
//!
...
pub enum EffectKind {
    ...
    PassphraseRemove,
    /// `DestroyDialog` → `paladin_auth_core::destroy_vault` (Milestone 10 —
...
impl EffectOwnership {
    ...
    /// `paladin_auth_core::open` (plaintext path) or a successful
