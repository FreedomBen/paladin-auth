...
//! Export-dialog pure-logic state machine for `paladin-auth-gtk`.
//!
...
//! [`ExportFormatChoice::EncryptedPaladinAuth`]), an overwrite-confirm
...
//!   [`paladin_auth_core::format_plaintext_export_warning`]
//!   ([`plaintext_warning_body`]).
//! * Build a validated [`paladin_auth_core::EncryptionOptions`] from the
...
//!   [`paladin_auth_core::write_secret_file_atomic`] wrapping either
//!   [`paladin_auth_core::export::otpauth_list`] or
//!   [`paladin_auth_core::export::encrypted`] ([`classify_export_result`])
...
//! on drop / clear; the [`paladin_auth_core::EncryptionOptions`] returned
//! by [`prepare_encrypted_export`] wraps the passphrase in a
//! `secrecy::SecretString` that zeroizes on drop. Inline-error /
//! inline-warning bodies render through [`PaladinAuthError::Display`] so
...
/// [`paladin_auth_core::export::otpauth_list`] (plaintext, newline-separated
/// list of `otpauth://` URIs — the same shape Gnome Authenticator's
/// "Backup → Save in plain text" produces) and
/// [`paladin_auth_core::export::encrypted`] (encrypted Paladin Auth bundle).
...
/// CLI parity: `paladin-auth export <DEST>` with no `--format` flag writes
...
pub enum ExportFormatChoice {
    ...
    #[default]
    PlaintextOtpauth,
    /// Encrypted Paladin Auth bundle. Requires the twice-confirm
    /// passphrase row to be filled with a matching, non-empty pair.
    EncryptedPaladinAuth,
}
...
impl ExportFormatChoice {
    ...
    pub fn requires_passphrase(self) -> bool {
        matches!(self, Self::EncryptedPaladinAuth)
    }
    ...
    /// `[PlaintextOtpauth, EncryptedPaladinAuth]`.
    #[must_use]
    pub fn index(self) -> u32 {
        match self {
            Self::PlaintextOtpauth => 0,
            Self::EncryptedPaladinAuth => 1,
        }
    }
}
...
pub fn format_choice_from_index(selected: u32) -> Option<ExportFormatChoice> {
    match selected {
        ...
        1 => Some(ExportFormatChoice::EncryptedPaladinAuth),
        _ => None,
    }
}
...
pub fn format_export_dialog_format_labels() -> &'static [&'static str] {
    &[
        "Plaintext otpauth:// URI list",
        "Encrypted Paladin Auth bundle",
    ]
}
...
/// [`paladin_auth_core::format_plaintext_export_warning`] verbatim so it
...
/// [`ExportFormatChoice::EncryptedPaladinAuth`] both reset the row: any
...
/// ([`paladin_auth_core::write_secret_file_atomic`] wrapping the chosen
/// payload).
///
...
#[must_use]
pub fn classify_export_result(result: Result<(), PaladinAuthError>) -> ExportOutcome {
    match result {
...
impl InlineError {
    ...
    /// from the matching [`PaladinAuthError::InvalidPassphrase`] variant so
...
/// either [`paladin_auth_core::export::otpauth_list`] (plaintext) or
/// [`paladin_auth_core::export::encrypted`] (encrypted) and writes the
/// resulting bytes via [`paladin_auth_core::write_secret_file_atomic`].
...
pub struct ExportSubmitPayload {
    ...
    /// [`paladin_auth_core::export::otpauth_list`] and
...
///      [`paladin_auth_core::export::otpauth_list`] (a newline-separated
...
///      [`paladin_auth_core::export::encrypted`] with the worker-supplied
///      [`EncryptionOptions`], which runs the §4.4 Argon2id KDF and
///      a fresh AEAD key derivation.
/// 2. Hand the bytes to
///    [`paladin_auth_core::write_secret_file_atomic`], which writes
...
pub fn run_export_worker(input: ExportWorkerInput) -> ExportWorkerCompletion {
    ...
    let bytes_result: Result<Vec<u8>, PaladinAuthError> = match format {
        ExportFormatChoice::PlaintextOtpauth => {
            Ok(paladin_auth_core::export::otpauth_list(&vault).into_bytes())
        }
        ExportFormatChoice::EncryptedPaladinAuth => {
            ...
            let opts = encryption_options
                .expect("EncryptedPaladinAuth format requires EncryptionOptions from the dispatch");
            paladin_auth_core::export::encrypted(&vault, opts)
        }
    };
...
/// [`paladin_auth_core::export::encrypted`] against the same in-memory
...
pub enum ExportDialogMsg {
    ...
    /// fires this so the Paladin Auth-owned [`SecretEntry`] shadow tracks
...
pub enum ExportDialogOutput {
    ...
    /// [`paladin_auth_core::export::otpauth_list`] or
    /// [`paladin_auth_core::export::encrypted`] then
    /// [`paladin_auth_core::write_secret_file_atomic`] (the encrypted
...
/// [`ExportFormatChoice::EncryptedPaladinAuth`] path. Subsequent sub-items
...
impl ExportDialogState {
    ...
    /// per keystroke so the Paladin Auth-owned shadow tracks the GTK
...
/// [`paladin_auth_core::format_plaintext_export_warning`] so the GUI,
...
///
/// Returns `true` iff the active format is the encrypted Paladin Auth
