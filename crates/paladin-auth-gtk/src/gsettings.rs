...
//! Per-user `GSettings` access for `paladin-auth-gtk`.
//!
//! The reverse-DNS schema id ([`SCHEMA_ID`]) and the per-key
//! constants below mirror `data/org.tamx.PaladinAuth.Gui.gschema.xml`.
//! That schema is compiled at build time by `build.rs` into
//! `OUT_DIR/schemas/gschemas.compiled` and the directory path is
//! exposed through the `PALADIN_AUTH_GTK_SCHEMA_DIR` cargo env var so
...
//!
//! 1. The build-time `PALADIN_AUTH_GTK_SCHEMA_DIR` directory (always
...
//! `paladin_auth_core::VaultSettings` and are persisted inside the
...
/// `data/org.tamx.PaladinAuth.Gui.gschema.xml`.
pub const SCHEMA_ID: &str = "org.tamx.PaladinAuth.Gui";
...
/// group.  See `crates/paladin-auth-gtk/src/account_list.rs` for the
...
/// `data/org.tamx.PaladinAuth.Gui.gschema.xml` was not compiled into
...
pub fn app_settings() -> gio::Settings {
    ...
    panic!(
        "GSettings schema `{SCHEMA_ID}` not found.  Build-time \
         ...
         `data/org.tamx.PaladinAuth.Gui.gschema.xml` or that \
