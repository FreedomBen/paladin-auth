...
//! Destroy-dialog pure-logic state machine and widget for `paladin-auth-gtk`.
//!
...
//! before calling [`paladin_auth_core::destroy_vault`]. The widget layer
//! hosts the destructive `AdwAlertDialog` chrome; the pure-logic
//! helpers here own the warning-body sourcing (a single call to
//! [`paladin_auth_core::format_destroy_warning`] so the wording matches
...
//! [`PaladinAuthError`] from a failed [`paladin_auth_core::destroy_vault`]
...
use paladin_auth_core::{
    destroy_vault, format_destroy_warning, DestroyReport, ErrorKind, PaladinAuthError,
};
...
/// `paladin_auth_core` storage layer's `.bak` rotation convention so the
...
/// [`BACKUP_SUFFIX`].
///
/// Pure — mirrors the `paladin_auth_core` storage `backup_path_for`
...
/// [`paladin_auth_core::destroy_vault`].
///
...
/// Classify a [`paladin_auth_core::destroy_vault`] failure into a
/// [`DestroyErrorOutcome`].
///
...
#[must_use]
pub fn classify_destroy_error(err: &PaladinAuthError) -> DestroyErrorOutcome {
...
pub enum DestroyWorkerEffect {
    /// [`paladin_auth_core::destroy_vault`] returned `Ok(report)`. The
    ...
    /// [`paladin_auth_core::destroy_vault`] returned
    /// [`PaladinAuthError::VaultMissing`] — the primary was already
    ...
    VaultMissing,
    /// [`paladin_auth_core::destroy_vault`] returned any other typed
...
/// paladin_auth_core::destroy_vault(path)` worker fired by
/// `AppModel::update` from
/// `AppMsg::DestroyVault { path }`.
///
/// Consumes the resolved vault `path` by value, calls
/// [`paladin_auth_core::destroy_vault`], and maps the result into a
/// [`DestroyWorkerEffect`]: `Ok(report)` →
/// [`DestroyWorkerEffect::Success`]; [`PaladinAuthError::VaultMissing`]
...
pub fn run_destroy_worker(path: PathBuf) -> DestroyWorkerEffect {
    match destroy_vault(&path) {
        ...
        Err(PaladinAuthError::VaultMissing) => DestroyWorkerEffect::VaultMissing,
...
pub const DESTROY_DIALOG_MARKER_PREFIX: &str = "paladin-auth-gtk: destroy_dialog_path=";

/// Format the smoke-test stdout marker line for a mounted
/// [`DestroyDialogComponent`].
///
/// The marker is
/// `paladin-auth-gtk: destroy_dialog_path=<path> backup_present=<bool>`.
#[must_use]
...
///
/// Sourced verbatim from [`paladin_auth_core::format_destroy_warning`] —
