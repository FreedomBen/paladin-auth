...
//! Clap argument tree for the `paladin-auth-gtk` binary.
//!
...
//!
//! `paladin-auth-gtk` has no JSON output mode: `--json` is intentionally
...
/// Top-level arguments accepted by `paladin-auth-gtk`.
#[derive(Debug, Parser)]
#[command(
    name = "paladin-auth-gtk",
    version,
    about = "Paladin Auth GTK: Rust OTP authenticator (TOTP + HOTP)"
)]
