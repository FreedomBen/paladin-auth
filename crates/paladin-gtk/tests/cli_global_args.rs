...
use clap::Parser;
use paladin_gtk::cli::GlobalArgs;
...
fn vault_flag_selects_inspected_path() {
    let args = GlobalArgs::try_parse_from(["paladin-gtk", "--vault", "/tmp/v.bin"])
...
fn no_color_flag_is_accepted_for_parity() {
    let args =
        GlobalArgs::try_parse_from(["paladin-gtk", "--no-color"]).expect("--no-color should parse");
...
fn json_flag_is_rejected_at_parse_time() {
    let err =
        GlobalArgs::try_parse_from(["paladin-gtk", "--json"]).expect_err("--json should reject");
...
fn positional_file_argument_is_rejected() {
    let err = GlobalArgs::try_parse_from(["paladin-gtk", "/tmp/some-import.json"])
...
fn version_flag_returns_clap_text_output() {
    let err = GlobalArgs::try_parse_from(["paladin-gtk", "--version"])
