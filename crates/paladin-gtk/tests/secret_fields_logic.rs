...
//!
//! The module under test (`paladin_gtk::secret_fields`) models the
...
//! Paladin-owned `Box<ValidatedAccount>` / `VaultInit` pending slots
...
fn add_state_path_switch_drops_pending_duplicate_add() {
    ...
    // `ZeroizeOnDrop` impl on `paladin_core::Secret`.
    drop(prior);
}
