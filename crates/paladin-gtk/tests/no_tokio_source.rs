...
//!
//! This test mirrors `crates/paladin-core/tests/no_network.rs`'s
...
/// `paladin-gtk/src/`. Only the source-level rule of the broader
/// "no network" guard applies here — the GUI legitimately uses
/// `gio::spawn_blocking` for blocking work, so `std::net` /
/// `TcpStream` / `UdpSocket` patterns from `paladin-core`'s list
...
/// `paladin-gtk/Cargo.toml`. `tokio` is exempted *only* when it
...
#[test]
fn paladin_gtk_source_tree_has_no_tokio_or_network_symbols() {
...
#[test]
fn paladin_gtk_manifest_has_no_direct_tokio_or_network_dependency() {
    ...
    assert!(
        hits.is_empty(),
        "paladin-gtk/Cargo.toml directly depends on forbidden crates — \
