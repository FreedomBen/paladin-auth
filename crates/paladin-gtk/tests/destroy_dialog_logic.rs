...
//! Pure-logic destroy-dialog tests for `paladin-gtk`.
//!
...
use paladin_core::{format_destroy_warning, DestroyReport, ErrorKind, PaladinError};

use paladin_gtk::app::state::{compose_destroy_dispatch, AppState, DestroyDispatch};
use paladin_gtk::destroy_dialog::{
...
use paladin_gtk::secret_fields::{self, SecretSurface};
...
fn vault_path() -> PathBuf {
    PathBuf::from("/home/alice/.local/share/paladin/vault.bin")
}
...
fn io_error(operation: &'static str) -> PaladinError {
...
fn probe_backup_present_false_when_parent_unreadable() {
    ...
    let probe = probe_backup_present(Path::new("/nonexistent-paladin-root/vault.bin"));
...
fn classify_generic_error_renders_inline() {
    let err = PaladinError::VaultMissing;
...
fn auto_lock_pre_dispatch_zeroizes_confirmation() {
    ...
    paladin_gtk::destroy_dialog::clear_for_lock(&mut state);
    assert_eq!(
        state.confirmation(),
        "",
