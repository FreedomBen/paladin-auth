...
//! `data/paladin-gtk.gresource.xml`. The XML manifest declares each
...
const MANIFEST_RELPATH: &str = "data/paladin-gtk.gresource.xml";
...
fn manifest_prefix_matches_app_id_reverse_dns_path() {
    ...
    // `/org/tamx/Paladin/Gui` (APP_ID split on `.`). Anything inside
    // the bundle resolves at runtime under `/org/tamx/Paladin/Gui/…`.
...
fn build_script_invokes_glib_compile_resources_against_manifest() {
    ...
    assert!(
        build_script.contains("data/paladin-gtk.gresource.xml"),
