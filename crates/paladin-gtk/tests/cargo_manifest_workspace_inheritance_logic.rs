...
/// Fields the `paladin-gtk` package MUST inherit from
...
/// Binary-specific facets the `paladin-gtk` package MUST declare
...
fn workspace_root() -> PathBuf {
    // `paladin-gtk` lives at `<workspace>/crates/paladin-gtk`, so two
...
fn crate_manifest_inherits_required_fields_from_workspace_package() {
    ...
    assert!(
        missing.is_empty(),
        "paladin-gtk Cargo.toml [package] must inherit each of {INHERITED_FIELDS:?} \
...
fn crate_manifest_declares_keywords_locally_with_expected_values() {
    ...
    assert!(
        ...
        "paladin-gtk Cargo.toml [package] must declare `keywords` locally — \
    ...
    assert_eq!(
        ...
        LOCAL_KEYWORDS,
        "paladin-gtk keywords must be the GUI-binary set {LOCAL_KEYWORDS:?}; \
         got {parsed:?}",
    );
}
...
fn crate_manifest_declares_categories_locally_with_expected_values() {
    ...
    assert!(
        ...
        "paladin-gtk Cargo.toml [package] must declare `categories` locally — \
         binary-specific facets do not belong on [workspace.package]",
    );
    ...
    assert_eq!(
        parsed.iter().map(String::as_str).collect::<Vec<_>>(),
        LOCAL_CATEGORIES,
        "paladin-gtk categories must be the GUI-binary set {LOCAL_CATEGORIES:?}; \
         got {parsed:?}",
    );
}
...
fn crate_manifest_does_not_inherit_keywords_or_categories_from_workspace() {
    ...
    assert!(
        ...
        "paladin-gtk Cargo.toml [package] must NOT use \
...
fn workspace_manifest_supplies_each_inherited_field() {
    ...
    assert!(
        ...
        "[workspace.package] in the workspace Cargo.toml must declare each of \
         {INHERITED_FIELDS:?} so the paladin-gtk crate's inheritance resolves; \
         missing: {missing:?}",
    );
}
