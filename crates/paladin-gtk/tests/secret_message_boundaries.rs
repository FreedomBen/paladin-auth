...
//! [`paladin_core`] wrappers ([`paladin_core::VaultLock`],
//! [`paladin_core::VaultInit`], [`paladin_core::EncryptionOptions`],
//! [`paladin_core::Account`], …) so the bytes zeroize on drop:
//!
//! * Passphrases
...
fn long_lived_types_carry_no_raw_secret_bearing_strings() {
    ...
    // dialog-`*Output` enums, `paladin_core` types) are the
    // allowed shape.
