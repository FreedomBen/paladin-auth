...
//! # Post-effect routing
//!
//! [`classify_passphrase_error`] maps the [`PaladinError`] from a
...
use paladin_core::{
    format_plaintext_storage_warning, EncryptionOptions, ErrorKind, PaladinError, Store, Vault,
};
...
impl SubmitRejection {
    ...
    /// [`PaladinError::InvalidPassphrase`] for the same `reason`.
    #[must_use]
...
#[must_use]
pub fn classify_passphrase_error(err: &PaladinError) -> PassphraseErrorOutcome {
    match err.kind() {
