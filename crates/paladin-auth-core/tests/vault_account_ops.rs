...
use paladin_auth_core::{parse_otpauth, Account, AccountId, AccountKindSummary, Algorithm, Vault};
...
fn empty_plaintext_vault() -> Vault {
    ...
    std::mem::forget(dir);
    let (vault, _store) =
        paladin_auth_core::Store::create(&path, paladin_auth_core::VaultInit::Plaintext).unwrap();
    vault
}
