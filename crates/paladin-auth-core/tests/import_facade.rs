...
use paladin_auth_core::{
    ...
    PaladinAuthError, Store, VaultInit,
};
use qrcode::QrCode;
...
fn paladin_auth_bundle_bytes(passphrase: &str, accounts: &[&str]) -> Vec<u8> {
...
#[test]
fn from_bytes_auto_detects_paladin_auth_with_passphrase() {
    let bytes = paladin_auth_bundle_bytes("hunter2", &[URI_TOTP_A]);
    let opts = ImportOptions {
        format: None,
        paladin_auth_passphrase: Some(pp("hunter2")),
    };
...
#[test]
fn from_bytes_paladin_auth_without_passphrase_returns_invalid_state_missing_passphrase() {
    let bytes = paladin_auth_bundle_bytes("hunter2", &[URI_TOTP_A]);
    let err = import::from_bytes(&bytes, ImportOptions::default(), import_time()).unwrap_err();
    let PaladinAuthError::InvalidState { operation, state } = err else {
        panic!("expected InvalidState, got {err:?}");
    };
    assert_eq!(operation, "import_paladin_auth");
...
#[test]
fn from_bytes_forced_paladin_auth_on_aegis_returns_unsupported_import_format_paladin_auth() {
    let opts = ImportOptions {
...
#[test]
fn from_file_auto_detects_paladin_auth_bundle_file() {
    let dir = vault_test_dir();
    let path = dir.path().join("vault.bin");
    let (mut vault, store) =
...
#[test]
fn from_file_paladin_auth_without_passphrase_returns_invalid_state() {
    let dir = vault_test_dir();
    let path = dir.path().join("vault.bin");
    let (mut vault, store) =
        Store::create(&path, VaultInit::Encrypted(cheap_options("hunter2"))).unwrap();
