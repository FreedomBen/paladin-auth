...
use paladin_auth_core::{
    ...
    PaladinAuthError, PermissionSubject, Store, TimeRangeKind, VaultInit, VaultLock, VaultMode,
};
...
fn validation_error_manual_add_label_empty_carries_field_and_reason() {
    ...
    let PaladinAuthError::ValidationError {
        field,
        reason,
        source_index,
        decoded_len,
...
fn validation_error_settings_parse_unknown_key() {
    ...
    assert_eq!(err.kind(), ErrorKind::ValidationError);
    let PaladinAuthError::ValidationError { field, .. } = err else {
        panic!("expected ValidationError, got {err:?}");
    };
    assert_eq!(field, "key");
}
...
fn invalid_passphrase_zero_length_carries_reason() {
    ...
    let PaladinAuthError::InvalidPassphrase { reason } = err else {
...
fn assert_invalid_state(err: &PaladinAuthError, expected_op: &str, expected_state: &str) {
...
#[test]
fn invalid_state_import_paladin_auth_auth_missing_passphrase() {
    // Build a real encrypted Paladin Auth bundle, then drop the passphrase
    // when invoking the facade.
    let dir = vault_test_dir();
    let path = dir.path().join("vault.bin");
    let (mut vault, _store) = Store::create(&path, VaultInit::Plaintext).unwrap();
    vault.add(make_account("alice", None));
    let bundle = paladin_auth_core::export::encrypted(&vault, cheap_options("hunter2"))
    ...
    .unwrap_err();
    assert_invalid_state(&err, "import_paladin_auth", "missing_passphrase");
}
...
fn vault_missing_kind_when_opening_absent_path() {
    ...
    matches!(err, PaladinAuthError::VaultMissing);
}
...
fn vault_exists_kind_when_creating_existing_path() {
    ...
    matches!(err, PaladinAuthError::VaultExists);
}
...
fn unsafe_permissions_carries_path_subject_and_modes() {
    ...
    let PaladinAuthError::UnsafePermissions {
        path: bad_path,
        subject,
        actual_mode,
        expected_mode,
...
fn wrong_vault_lock_plaintext_supplied_for_encrypted_file() {
    ...
    let PaladinAuthError::WrongVaultLock { expected, actual } = err else {
...
fn invalid_header_when_file_has_unknown_magic() {
    ...
    let dir = vault_test_dir();
    let path = dir.path().join("vault.bin");
    fs::write(&path, b"NOTAPALAUTH\0".repeat(8)).unwrap();
    fs::set_permissions(&path, fs::Permissions::from_mode(0o600)).unwrap();
    let err = Store::open(&path, VaultLock::Plaintext).unwrap_err();
    assert_eq!(err.kind(), ErrorKind::InvalidHeader);
    matches!(err, PaladinAuthError::InvalidHeader);
}
...
fn assert_invalid_payload(err: PaladinAuthError, expected_reason: &str) {
    assert_eq!(err.kind(), ErrorKind::InvalidPayload);
    match err {
        PaladinAuthError::InvalidPayload { reason } => assert_eq!(reason, expected_reason),
...
fn invalid_payload_reasons_are_constructible_with_stable_strings() {
    ...
    for reason in [
        ...
        assert_invalid_payload(PaladinAuthError::InvalidPayload { reason }, reason);
    }
}
...
fn unsupported_format_version_carries_offending_byte() {
    ...
    let PaladinAuthError::UnsupportedFormatVersion { format_ver } = err else {
...
fn kdf_params_out_of_bounds_via_argon2_validation() {
    ...
    // assertion below.
    match err {
        PaladinAuthError::ValidationError { field, .. } => assert!(field.starts_with("kdf_params")),
        PaladinAuthError::KdfParamsOutOfBounds { .. } => {}
        other => panic!("expected ValidationError or KdfParamsOutOfBounds, got {other:?}"),
    }

    let direct = PaladinAuthError::KdfParamsOutOfBounds {
        m_kib: 1,
        t: 1,
        p: 1,
    };
    assert_eq!(direct.kind(), ErrorKind::KdfParamsOutOfBounds);
    let PaladinAuthError::KdfParamsOutOfBounds { m_kib, t, p } = direct else {
...
fn unsupported_import_format_auto_detect_unknown() {
    ...
    let PaladinAuthError::UnsupportedImportFormat { format } = err else {
...
fn unsupported_plaintext_vault_is_a_bare_variant() {
    ...
    matches!(err, PaladinAuthError::UnsupportedPlaintextVault);
}
...
fn unsupported_encrypted_aegis_via_real_aegis_input() {
    ...
    matches!(err, PaladinAuthError::UnsupportedEncryptedAegis);
}
...
fn unsupported_aegis_entry_type_carries_source_index_and_type() {
    ...
    let PaladinAuthError::UnsupportedAegisEntryType {
        source_index,
        entry_type,
    } = err
    else {
...
fn no_entries_to_import_when_aegis_db_is_empty() {
    ...
    matches!(err, PaladinAuthError::NoEntriesToImport);
}
...
fn counter_overflow_carries_account_summary() {
    ...
    let PaladinAuthError::CounterOverflow { account } = err else {
...
fn time_range_totp_code_pre_epoch() {
    ...
    let PaladinAuthError::TimeRange { operation, kind } = err else {
...
fn save_not_committed_carries_committed_false_and_optional_backup_path() {
    ...
    let PaladinAuthError::SaveNotCommitted {
        committed,
        backup_path,
    } = err
    else {
    ...
    let bare = PaladinAuthError::SaveNotCommitted {
        committed: false,
        backup_path: None,
    };
    let PaladinAuthError::SaveNotCommitted { backup_path, .. } = bare else {
        panic!();
    };
...
fn save_durability_unconfirmed_is_a_bare_variant() {
    ...
    matches!(err, PaladinAuthError::SaveDurabilityUnconfirmed);
}
...
fn io_error_every_stable_operation_is_constructible_and_round_trips() {
    for op in STABLE_IO_OPERATIONS {
        ...
        match err {
            PaladinAuthError::IoError { operation, source } => {
...
fn io_error_read_import_file_via_missing_path() {
    ...
    let PaladinAuthError::IoError { operation, .. } = err else {
