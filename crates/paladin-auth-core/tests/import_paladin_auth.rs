...
//
// Phase I.4 — `import::paladin_auth` (docs/DESIGN.md §4.6 / §4.7).
//
// A Paladin Auth import bundle is the same on-disk format as the encrypted
...
fn encrypted_bundle_round_trips_with_correct_passphrase() {
    ...
    let imported = import::paladin_auth(&bytes, pp("hunter2")).unwrap();
...
#[test]
fn plaintext_paladin_auth_file_rejects_unsupported_plaintext_vault() {
    let dir = vault_test_dir();
    let path = dir.path().join("vault.bin");
...
fn wrong_passphrase_returns_decrypt_failed() {
    let bytes = build_bundle_bytes("hunter2", &[URI_TOTP_A]);
    let err = import::paladin_auth(&bytes, pp("wrong-passphrase")).unwrap_err();
...
fn source_vault_settings_are_discarded() {
    ...
    // import::paladin_auth must surface only accounts (no settings type
    ...
    let _: &paladin_auth_core::ValidatedAccount = &imported[0];
}
...
fn truncated_input_returns_invalid_header() {
    let err = import::paladin_auth(b"PAL", pp("hunter2")).unwrap_err();
    assert_eq!(err.kind(), ErrorKind::InvalidHeader);
}
