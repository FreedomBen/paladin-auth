...
use paladin_auth_core::policy::clipboard_clear::{ClipboardClearPolicy, ClipboardClearToken};
use paladin_auth_core::policy::hotp_reveal;
...
fn clipboard_clear_policy_and_token_reachable_at_crate_root() {
    ...
    // through `paladin_auth_core::ClipboardClearPolicy` and
    // `paladin_auth_core::ClipboardClearToken` so a refactor that moves
    // internal modules cannot silently drop the surface.
    let _: paladin_auth_core::ClipboardClearPolicy;
    let _: Option<(paladin_auth_core::ClipboardClearToken, Instant)> =
...
fn hotp_reveal_deadline_reachable_at_crate_root_re_export() {
    ...
    // root as `paladin_auth_core::hotp_reveal_deadline` so a refactor that
    // moves the internal module cannot silently drop the surface
    // (matches the IdlePolicy / ClipboardClearPolicy precedent).
    let now = Instant::now();
    let got: Instant = paladin_auth_core::hotp_reveal_deadline(now);
    assert_eq!(got, now + Duration::from_secs(HOTP_REVEAL_SECS));
    ...
    assert_eq!(got, paladin_auth_core::policy::hotp_reveal::deadline(now));
}
