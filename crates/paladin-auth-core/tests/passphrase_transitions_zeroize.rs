...
use paladin_auth_core::zeroize_witness::{
...
// `PALADIN_AUTH_FAULT_INJECT` env var. Serialize through a single mutex so
// parallel test runners do not race a fault setting against an
// unrelated zeroize observation.
static SERIAL: Mutex<()> = Mutex::new(());

#[cfg(feature = "test-fault-injection")]
const FAULT_ENV: &str = "PALADIN_AUTH_FAULT_INJECT";
...
fn change_passphrase_pre_commit_does_not_zeroize_active_cache() {
    run_serial(|| {
        ...
        let err = vault
            ...
            .unwrap_err();
        std::env::remove_var(FAULT_ENV);
        assert_eq!(err.kind(), paladin_auth_core::ErrorKind::SaveNotCommitted);
