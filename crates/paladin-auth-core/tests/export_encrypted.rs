...
//
// Pinning:
//   - Round-trip with `import::paladin_auth` recovers the same accounts.
...
fn sample_vault() -> (TempDir, paladin_auth_core::Vault, Store) {
...
#[test]
fn round_trip_via_import_paladin_auth_auth_returns_same_accounts() {
    let (_dir, vault, _store) = sample_vault();
    let bundle = export::encrypted(&vault, cheap_options("hunter2")).unwrap();

    let imported = import::paladin_auth(&bundle, pp("hunter2")).unwrap();
...
#[test]
fn bundle_starts_with_paladin_auth_magic_and_is_encrypted_mode() {
    let (_dir, vault, _store) = sample_vault();
    let bundle = export::encrypted(&vault, cheap_options("hunter2")).unwrap();
    assert!(bundle.len() >= 64);
    assert_eq!(&bundle[0..8], b"PALAUTH\0");
...
fn bundle_written_with_passphrase_a_opened_with_b_returns_decrypt_failed() {
    ...
    let err = import::paladin_auth(&bundle, pp("bbb")).unwrap_err();
...
fn corrupt_ciphertext_byte_returns_decrypt_failed() {
    ...
    bundle[last] ^= 0x42;
    let err = import::paladin_auth(&bundle, pp("hunter2")).unwrap_err();
    assert_eq!(err.kind(), ErrorKind::DecryptFailed);
}
...
fn garbage_plaintext_under_right_key_returns_invalid_payload() {
    use paladin_auth_core::_testing_write_encrypted_with_raw_plaintext;
    ...
    let err = import::paladin_auth(&bytes, pp("hunter2")).unwrap_err();
    assert_eq!(err.kind(), ErrorKind::InvalidPayload);
    if let PaladinAuthError::InvalidPayload { reason } = err {
        assert!(matches!(
            reason,
...
fn source_vault_settings_are_replaced_with_defaults_in_bundle() {
    ...
    let (opened, _store) = Store::open(
        &bundle_path,
        paladin_auth_core::VaultLock::Encrypted(pp("hunter2")),
    )
    .unwrap();
