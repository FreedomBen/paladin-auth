...
use paladin_auth_core::{
    ...
    Account, ErrorKind, InitPrecheck, PaladinAuthError, PermissionSubject, SettingPatch, Store,
...
fn plaintext_header_bytes() -> Vec<u8> {
    let mut v = Vec::new();
    // PALAUTH\0 + format_ver=1 + mode=0
    v.extend_from_slice(b"PALAUTH\0");
    v.push(1);
    v.push(0);
    v
}
...
#[test]
fn default_vault_path_resolves_under_paladin_auth_with_vault_bin_filename() {
    ...
    assert!(path.to_string_lossy().contains("paladin-auth"));
}
...
fn inspect_unrecognized_magic_is_invalid_header() {
    let dir = vault_test_dir();
    let path = write(&dir, "vault.bin", b"NOTPALAUTH\0\x01\x00");
    assert_eq!(inspect(&path).unwrap_err().kind(), ErrorKind::InvalidHeader);
}
...
/// 10-byte plaintext header (`PALAUTH\0` + `format_ver` + `mode`)
...
fn saved_primary_starts_with_plaintext_header_bytes() {
    ...
    assert!(bytes.len() >= 10);
    assert_eq!(&bytes[0..8], b"PALAUTH\0");
...
fn open_cleanup_surfaces_io_error_when_leftover_tmp_is_directory() {
    ...
    let err = Store::open(&path, VaultLock::Plaintext).unwrap_err();
    match err {
        PaladinAuthError::IoError { operation, .. } => assert_eq!(operation, "cleanup_temp_file"),
...
fn open_does_not_read_bak_on_success_path() {
    ...
    // try.
    fs::write(&bak, b"NOTPALAUTH garbage bytes that are not a vault").unwrap();
    fs::set_permissions(&bak, fs::Permissions::from_mode(0o600)).unwrap();
...
fn write_secret_file_atomic_creates_destination_with_zero_six_zero_zero_mode_and_content() {
    ...
    let bytes = b"PALAUTH-EXPORT-PAYLOAD-FIXTURE";
...
fn write_secret_file_atomic_returns_save_not_committed_when_parent_is_read_only() {
    ...
    let probe = dir.path().join(".paladin-auth-root-probe");
...
fn create_plaintext_mkdir_p_missing_single_level_parent_at_0700() {
    ...
    let parent = tmp.path().join("paladin-auth");
...
fn create_plaintext_mkdir_p_missing_multi_level_parent_at_0700() {
    let tmp = vault_test_dir();
    let parent = tmp.path().join("a").join("b").join("paladin-auth");
    let path = parent.join("vault.bin");
...
fn create_plaintext_when_mkdir_eacces_returns_create_vault_dir() {
    ...
    // `create_vault_dir` surface a fresh `paladin-auth init` would hit on a
    // sandboxed data dir.
    let tmp = vault_test_dir();
    ...
    let probe = grandparent.join(".paladin-auth-root-probe");
    ...
    let path = grandparent.join("paladin-auth").join("vault.bin");
