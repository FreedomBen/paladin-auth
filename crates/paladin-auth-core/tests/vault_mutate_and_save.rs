...
// process-wide `PALADIN_AUTH_FAULT_INJECT` env var serializes on the
...
use paladin_auth_core::{
    parse_otpauth, Account, AccountId, ErrorKind, ImportConflict, PaladinAuthError, Store,
...
fn synthetic_error() -> PaladinAuthError {
...
fn mutate_and_save_returns_closure_success_value_on_clean_save() {
    ...
    let returned = vault
        .mutate_and_save(&store, |v| {
            ...
            Ok::<AccountId, PaladinAuthError>(id)
        })
...
fn mutate_and_save_restores_accounts_on_closure_error_after_add() {
    ...
    match err {
        PaladinAuthError::InvalidPayload { reason } => assert_eq!(reason, SYNTHETIC_REASON),
...
fn mutate_and_save_closure_sees_prior_mutations_inside_the_call() {
    ...
    let (mut vault, store, _dir) = vault_with_path();
    let observed = vault
        .mutate_and_save(&store, |v| -> Result<usize, PaladinAuthError> {
            v.add(make_totp_account("alice"));
            v.add(make_totp_account("bob"));
            Ok(v.iter().count())
        })
...
fn mutate_and_save_panic_in_closure_does_not_leave_partial_state() {
    ...
    vault
        .apply_setting_patch(
            paladin_auth_core::parse_setting_patch("auto_lock.timeout_secs", "300").unwrap(),
        )
    ...
    let result = catch_unwind(AssertUnwindSafe(|| {
        let _: PaladinAuthError = vault
            .mutate_and_save(&store, |v| -> Result<PaladinAuthError, PaladinAuthError> {
                v.add(make_totp_account("dave"));
                v.apply_setting_patch(
                    paladin_auth_core::parse_setting_patch("auto_lock.timeout_secs", "600")
                        .unwrap(),
                )
