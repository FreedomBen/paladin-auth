...
//
// J.3 — Send. The `paladin-auth-gtk` front end moves vault state across
// `gio::spawn_blocking` boundaries and `paladin-auth-tui` hands import work
...
// `ValidatedAccount`, `VaultLock`, `VaultInit`, `PaladinAuthError`) are
...
use paladin_auth_core::{
    ...
    PaladinAuthError, PaladinAuthImportPrecheck, QrRenderOptions, SettingKey, SettingPatch, Store,
...
const _: fn() = || {
    ...
    assert_send::<PaladinAuthImportPrecheck>();
    assert_send::<PaladinAuthError>();
...
const _: fn() = || {
    ...
    assert_sync::<PaladinAuthImportPrecheck>();
    assert_sync::<PaladinAuthError>();
