...
// `paladin-auth-core` manifest and `src/` tree for direct references to
...
//
// Subtree scoping: the lockfile check is rooted at `paladin-auth-core`,
// `paladin-auth-cli`, and `paladin-auth-tui` — the no-network surface per
// docs/DESIGN.md §8 and §13. `paladin-auth-gtk` is intentionally excluded
...
fn workspace_root() -> PathBuf {
    // crates/paladin-auth-core -> crates -> workspace root
    crate_dir()
        .parent()
        .and_then(Path::parent)
        .expect("workspace root above crates/paladin-auth-core")
        .to_path_buf()
}
...
#[test]
fn paladin_auth_core_source_tree_has_no_network_symbols() {
    ...
    assert!(
        hits.is_empty(),
        "paladin-auth-core src/ contains forbidden network references:\n  {}",
        hits.join("\n  "),
    );
}

#[test]
fn paladin_auth_core_manifest_has_no_network_dependency() {
    ...
    assert!(
        hits.is_empty(),
        "paladin-auth-core/Cargo.toml directly depends on denied crates: {hits:?}",
    );
}
...
/// [`FORBIDDEN_LOCK_NAMES`]. `paladin-auth-gtk` is intentionally not in
/// this list — see the module-level comment.
const NO_NETWORK_SUBTREE_ROOTS: &[&str] =
    &["paladin-auth-core", "paladin-auth-cli", "paladin-auth-tui"];
...
fn no_network_subtree_resolves_no_network_dependencies() {
    ...
    assert!(
        ...
        "Cargo.lock subtree under {NO_NETWORK_SUBTREE_ROOTS:?} resolves denied network \
         deps: {hits:?}.\n\
         If the offending dep is reachable only from `paladin-auth-gtk` via `relm4`, the \
...
#[test]
fn reachable_packages_excludes_paladin_auth_gtk_subtree() {
    // Models the live workspace: `paladin-auth-core` depends on `argon2`
    // (clean); `paladin-auth-gtk` depends on `relm4` which carries
    // `tokio`. From `paladin-auth-core`'s root only, `tokio` must not
    // appear in the reachable set.
    let mut graph: HashMap<String, Vec<String>> = HashMap::new();
    graph.insert("paladin-auth-core".into(), vec!["argon2".into()]);
    graph.insert("argon2".into(), Vec::new());
    graph.insert("paladin-auth-gtk".into(), vec!["relm4".into()]);
    graph.insert("relm4".into(), vec!["tokio".into()]);
    graph.insert("tokio".into(), Vec::new());

    let reached = reachable_packages(&graph, &["paladin-auth-core"]);
