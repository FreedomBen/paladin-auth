...
impl Argon2Params {
    ...
    /// [`PaladinAuthError::KdfParamsOutOfBounds`]. The error payload
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        {
            return Err(PaladinAuthError::KdfParamsOutOfBounds {
                m_kib: self.m_kib,
                t: self.t,
                p: self.p,
            });
...
/// [`PaladinAuthError::InvalidPassphrase`] (no trimming, no Unicode
/// normalization).
...
impl EncryptionOptions {
    ...
    fn ensure_non_empty(passphrase: &SecretString) -> Result<()> {
        if passphrase.expose_secret().is_empty() {
            return Err(PaladinAuthError::InvalidPassphrase {
                reason: "zero_length",
            });
        }
        Ok(())
...
pub(crate) fn argon2id_derive_key(
    ...
    argon2
        ...
        .map_err(|_| PaladinAuthError::KdfParamsOutOfBounds {
            m_kib: params.m_kib,
            t: params.t,
            p: params.p,
        })?;
...
mod tests {
    ...
    fn boundary_table_for_m_kib() {
        ...
        assert!(matches!(
            ...
            .validate()
            .unwrap_err(),
            PaladinAuthError::KdfParamsOutOfBounds { m_kib: 8_191, .. }
        ));
        Argon2Params {
            m_kib: 8_192,
            t: 3,
    ...
    fn boundary_table_for_t() {
        assert!(matches!(
            ...
            .validate()
            .unwrap_err(),
            PaladinAuthError::KdfParamsOutOfBounds { t: 0, .. }
        ));
        Argon2Params {
            m_kib: 65_536,
            t: 1,
        ...
        assert!(matches!(
            ...
            .validate()
            .unwrap_err(),
            PaladinAuthError::KdfParamsOutOfBounds { t: 11, .. }
        ));
    }
    ...
    fn boundary_table_for_p() {
        assert!(matches!(
            ...
            .validate()
            .unwrap_err(),
            PaladinAuthError::KdfParamsOutOfBounds { p: 0, .. }
        ));
        Argon2Params {
            m_kib: 65_536,
            t: 3,
        ...
        assert!(matches!(
            ...
            .validate()
            .unwrap_err(),
            PaladinAuthError::KdfParamsOutOfBounds { p: 5, .. }
        ));
    }
    ...
    fn encryption_options_new_rejects_empty_passphrase() {
        match EncryptionOptions::new(pp("")) {
            ...
            Err(PaladinAuthError::InvalidPassphrase { reason }) => {
    ...
    fn encryption_options_with_params_rejects_out_of_range() {
        ...
        match EncryptionOptions::with_params(pp("hunter2"), bad) {
            ...
            Err(PaladinAuthError::KdfParamsOutOfBounds { m_kib, t, p }) => {
    ...
    fn encryption_options_with_params_rejects_empty_before_bounds() {
        ...
        match EncryptionOptions::with_params(pp(""), bad) {
            Ok(_) => panic!("expected InvalidPassphrase"),
            Err(PaladinAuthError::InvalidPassphrase { .. }) => {}
            Err(other) => panic!("unexpected variant: {other:?}"),
        }
    }
    ...
    /// Self-pinned KAT — pins the exact crate configuration Paladin Auth
    ...
    fn argon2id_derive_key_known_answer_self_pinned() {
        let passphrase = pp("paladin-auth-test-passphrase");
        let salt = [
        ...
        let expected: [u8; 32] = [
            0x0A, 0xC2, 0xE2, 0xFF, 0x2A, 0x69, 0xDF, 0x5A, 0xE3, 0x51, 0xEA, 0x39, 0x18, 0xFB,
            0x7B, 0xA8, 0x89, 0xAF, 0xB7, 0x21, 0xBB, 0xAB, 0xF8, 0xF7, 0xA5, 0x4D, 0x01, 0xFF,
            0x4A, 0x47, 0x24, 0x6A,
        ];
        assert_eq!(
            *key, expected,
    ...
    fn kat_passphrase() -> SecretString {
        pp("paladin-auth-test-passphrase")
    }
    ...
    const ARGON2ID_KAT_KEY: [u8; 32] = [
        0x0A, 0xC2, 0xE2, 0xFF, 0x2A, 0x69, 0xDF, 0x5A, 0xE3, 0x51, 0xEA, 0x39, 0x18, 0xFB, 0x7B,
        0xA8, 0x89, 0xAF, 0xB7, 0x21, 0xBB, 0xAB, 0xF8, 0xF7, 0xA5, 0x4D, 0x01, 0xFF, 0x4A, 0x47,
        0x24, 0x6A,
    ];
    ...
    fn argon2id_kat_inputs_through_argon2i_produce_distinct_committed_key() {
        ...
        let expected_argon2i: [u8; 32] = [
            0x16, 0xE9, 0x49, 0x82, 0x95, 0xFE, 0x10, 0xB7, 0xBF, 0x9F, 0xDA, 0x2C, 0xBD, 0xAA,
            0x74, 0xF8, 0xE9, 0x77, 0x97, 0xB8, 0x14, 0xD1, 0x56, 0x43, 0xF1, 0xBD, 0x86, 0x57,
            0xC0, 0xCF, 0xC6, 0x24,
        ];
    ...
    fn argon2id_kat_inputs_through_argon2d_produce_distinct_committed_key() {
        ...
        let expected_argon2d: [u8; 32] = [
            0x3F, 0x79, 0xFB, 0xEE, 0x6A, 0xEE, 0xC8, 0x67, 0xFE, 0x60, 0x4F, 0xD7, 0x52, 0xE8,
            0x44, 0x56, 0x20, 0x92, 0x5C, 0x99, 0xEA, 0xB0, 0xCA, 0xC5, 0x32, 0x57, 0xDF, 0xE9,
            0xEF, 0x0A, 0xF9, 0xA2,
        ];
