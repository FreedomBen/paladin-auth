...
pub fn validate_slug(slug: &str) -> Result<String, PaladinAuthError> {
    if slug.is_empty() {
        return Err(PaladinAuthError::validation("icon_hint", "empty"));
    }
    if slug.len() > ICON_HINT_MAX_BYTES {
        return Err(PaladinAuthError::validation("icon_hint", "too_long"));
    }
    if !slug.bytes().all(is_slug_byte) {
        return Err(PaladinAuthError::validation("icon_hint", "invalid_chars"));
    }
    Ok(slug.to_owned())
}
...
pub fn validate_icon_hint_slug(slug: &str) -> Result<IconHintInput, PaladinAuthError> {
