...
use crate::error::{PaladinAuthError, TimeRangeKind};
...
/// `paladin-auth add` flow constructs this from CLI arguments and passes it
...
/// `PaladinAuthError::ValidationError { field: "label", reason: … }` for
...
pub fn validate_label(raw: &str) -> Result<String, PaladinAuthError> {
    let trimmed = raw.trim();
    if trimmed.is_empty() {
        return Err(PaladinAuthError::validation("label", "empty"));
    }
    if trimmed.len() > LABEL_MAX_BYTES {
        return Err(PaladinAuthError::validation("label", "too_long"));
    }
    Ok(trimmed.to_owned())
}

/// Trim Unicode whitespace and reject overlong issuers. Empty becomes `None`.
pub(crate) fn validate_issuer(raw: Option<&str>) -> Result<Option<String>, PaladinAuthError> {
    ...
    if trimmed.len() > ISSUER_MAX_BYTES {
        return Err(PaladinAuthError::validation("issuer", "too_long"));
    }
...
pub(crate) fn validate_digits(digits: u8, _default: u8) -> Result<u8, PaladinAuthError> {
    if !(DIGITS_MIN..=DIGITS_MAX).contains(&digits) {
        return Err(PaladinAuthError::validation("digits", "out_of_range"));
    }
    Ok(digits)
}

pub(crate) fn validate_totp_period(period: u32) -> Result<u32, PaladinAuthError> {
    if !(TOTP_PERIOD_MIN..=TOTP_PERIOD_MAX).contains(&period) {
        return Err(PaladinAuthError::validation("period", "out_of_range"));
    }
    Ok(period)
}
...
pub(crate) fn decode_and_validate_secret(
    text: &str,
) -> Result<(Secret, Option<ValidationWarning>), PaladinAuthError> {
    if text.is_empty() {
        return Err(PaladinAuthError::validation("secret", "empty"));
    }
    if text.bytes().any(|b| b.is_ascii_whitespace()) {
        return Err(PaladinAuthError::validation("secret", "whitespace"));
    }
...
pub(crate) fn decode_secret_bytes(
    ...
    let trimmed_str = std::str::from_utf8(trimmed)
        .map_err(|_| PaladinAuthError::validation("secret", "decode_failed"))?;
    let bytes = base32::decode(base32::Alphabet::Rfc4648 { padding: false }, trimmed_str)
        .ok_or_else(|| PaladinAuthError::validation("secret", "decode_failed"))?;
    let len = bytes.len();
    if len < SECRET_MIN_BYTES {
        return Err(PaladinAuthError::ValidationError {
            field: "secret",
...
pub(crate) fn system_time_to_secs(now: SystemTime) -> Result<u64, PaladinAuthError> {
...
pub(crate) fn system_time_to_secs_for(
    ...
    now: SystemTime,
) -> Result<u64, PaladinAuthError> {
    let secs = now
        .duration_since(UNIX_EPOCH)
        .map_err(|_| PaladinAuthError::TimeRange {
            operation,
            kind: TimeRangeKind::PreEpoch,
        })?
        .as_secs();
    if secs > TIMESTAMP_MAX_INCLUSIVE {
        return Err(PaladinAuthError::TimeRange {
            operation,
...
pub(crate) fn validate_and_normalize_account_edit(
    edit: &AccountEdit,
) -> Result<NormalizedAccountEdit, PaladinAuthError> {
...
mod tests {
    ...
    fn secret_length_boundaries() {
        ...
        assert!(matches!(
            ...
            PaladinAuthError::ValidationError { field, reason, decoded_len, .. }
    ...
    fn icon_hint_explicit_slug_validated() {
        ...
        let err = validate_manual(input, now_for_tests()).unwrap_err();
        match err {
            PaladinAuthError::ValidationError { field, .. } => {
