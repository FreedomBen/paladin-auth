...
pub fn parse_setting_key(key: &str) -> Result<SettingKey> {
    match key {
        ...
        _ => Err(PaladinAuthError::validation("key", "unknown_setting_key")),
    }
}
...
fn parse_bool(field: &'static str, value: &str) -> Result<bool> {
    match value {
        "true" => Ok(true),
        "false" => Ok(false),
        _ => Err(PaladinAuthError::validation(field, "expected_bool")),
    }
}

fn parse_u32(field: &'static str, value: &str) -> Result<u32> {
    if value.is_empty() || !value.bytes().all(|b| b.is_ascii_digit()) {
        return Err(PaladinAuthError::validation(field, "expected_u32"));
    }
    value
        .parse::<u32>()
        .map_err(|_| PaladinAuthError::validation(field, "expected_u32"))
}
...
mod tests {
    ...
    fn assert_validation(err: &PaladinAuthError, field: &str) {
