...
pub fn parse_otpauth(
    uri: &str,
    import_time: SystemTime,
) -> Result<ValidatedAccount, PaladinAuthError> {
    ...
    let issuer = match (normalized_label_issuer, normalized_query_issuer) {
        (Some(a), Some(b)) if a != b => {
            return Err(PaladinAuthError::validation("issuer", "mismatch"));
        }
    ...
    let secret_text = params
        .secret
        .ok_or_else(|| PaladinAuthError::validation("secret", "missing"))?;
    ...
    let digits = match params.digits.as_deref() {
        ...
        Some(s) => s
            .parse::<u8>()
            .map_err(|_| PaladinAuthError::validation("digits", "out_of_range"))?,
    };
...
/// `https://`, `mailto:`, `paladin-auth://`, etc., instead of opaque
/// url-crate parser errors.
fn parse_scheme_and_url(uri: &str) -> Result<Url, PaladinAuthError> {
    let colon_idx = uri
        .find(':')
        .ok_or_else(|| PaladinAuthError::validation("uri", "missing_scheme"))?;
    if !uri[..colon_idx].eq_ignore_ascii_case("otpauth") {
        return Err(PaladinAuthError::validation("uri", "invalid_scheme"));
    }
    Url::parse(uri).map_err(|_| PaladinAuthError::validation("uri", "malformed"))
}

/// Map the URL's host/authority to the OTP kind discriminator.
/// Returns `true` for HOTP, `false` for TOTP. Case-insensitive per §4.4.
fn parse_kind(url: &Url) -> Result<bool, PaladinAuthError> {
    let host = url
        .host_str()
        .ok_or_else(|| PaladinAuthError::validation("type", "missing"))?;
    match host.to_ascii_lowercase().as_str() {
        "totp" => Ok(false),
        "hotp" => Ok(true),
        _ => Err(PaladinAuthError::validation("type", "invalid")),
    }
}

/// Percent-decode the path, reject empty/whitespace-only labels, and
/// split on the first `:` for the optional issuer prefix.
fn parse_label_path(url: &Url) -> Result<(Option<String>, String), PaladinAuthError> {
    let raw_path = url.path();
    let path_inner = raw_path.strip_prefix('/').unwrap_or(raw_path);
    if path_inner.is_empty() {
        return Err(PaladinAuthError::validation("label", "empty"));
    }
    let decoded_label = percent_decode_str(path_inner)
        .decode_utf8()
        .map_err(|_| PaladinAuthError::validation("label", "invalid_utf8"))?
        .into_owned();
...
/// ignore unknown keys.
fn collect_query_params(url: &Url) -> Result<QueryParams, PaladinAuthError> {
    ...
    for (key, value) in url.query_pairs() {
        ...
        if slot.is_some() {
            return Err(PaladinAuthError::validation(field, "duplicate"));
        }
...
) -> Result<OtpKind, PaladinAuthError> {
    if is_hotp {
        if period_param.is_some() {
            return Err(PaladinAuthError::validation("period", "rejected_on_hotp"));
        }
        let counter_text =
            counter_param.ok_or_else(|| PaladinAuthError::validation("counter", "missing"))?;
        let counter: u64 = counter_text
            .parse()
            .map_err(|_| PaladinAuthError::validation("counter", "out_of_range"))?;
        Ok(OtpKind::Hotp { counter })
    } else {
        if counter_param.is_some() {
            return Err(PaladinAuthError::validation("counter", "rejected_on_totp"));
        }
        let period = match period_param {
            None => TOTP_PERIOD_DEFAULT,
            Some(s) => s
                .parse::<u32>()
                .map_err(|_| PaladinAuthError::validation("period", "out_of_range"))?,
        };
...
pub(crate) fn parse_algorithm(text: &str) -> Result<Algorithm, PaladinAuthError> {
    match text.to_ascii_uppercase().as_str() {
        "SHA1" => Ok(Algorithm::Sha1),
        "SHA256" => Ok(Algorithm::Sha256),
        "SHA512" => Ok(Algorithm::Sha512),
        _ => Err(PaladinAuthError::validation("algorithm", "invalid")),
    }
}
...
mod tests {
    ...
    fn assert_validation(err: &PaladinAuthError, want_field: &str, want_reason: &str) {
        match err {
    ...
    #[test]
    fn rejects_non_otpauth_scheme_paladin_auth() {
        let err = parse_otpauth("paladin-auth://totp/alice", now_for_tests()).unwrap_err();
        assert_validation(&err, "uri", "invalid_scheme");
    }
...
mod proptests {
    ...
    //!    `parse_otpauth` either succeed or return a `PaladinAuthError`,
    ...
    proptest! {
        ...
        /// successfully or returns a `PaladinAuthError`. Catches a future
