...
//
// `import::aegis_plaintext` (docs/DESIGN.md §4.6 / §4.7).
//
...
pub fn aegis_plaintext(bytes: &[u8], import_time: SystemTime) -> Result<Vec<ValidatedAccount>> {
    let export: AegisExport = serde_json::from_slice(bytes)
        .map_err(|e| PaladinAuthError::validation("input", format!("invalid_json: {e}")))?;

    let db_value = export.db;
    if db_value.is_null() {
        return Err(PaladinAuthError::validation("db", "missing"));
    }
    if db_value.is_string() {
        return Err(PaladinAuthError::UnsupportedEncryptedAegis);
    }
    if !db_value.is_object() {
        return Err(PaladinAuthError::validation(
            "db",
            "expected_object_or_string",
        ));
    }
    let db: AegisDb = serde_json::from_value(db_value)
        .map_err(|e| PaladinAuthError::validation("db", format!("invalid_db: {e}")))?;
...
fn build_account(
    ...
    let entry_type = entry
        .entry_type
        .ok_or_else(|| PaladinAuthError::validation("type", "missing").tag_source_index(idx))?;
    let kind_token = entry_type.to_ascii_lowercase();
    let is_hotp = match kind_token.as_str() {
        "totp" => false,
        "hotp" => true,
        _ => {
            return Err(PaladinAuthError::UnsupportedAegisEntryType {
                source_index: idx,
                entry_type,
            });
        }
    ...
    let name = entry
        .name
        .ok_or_else(|| PaladinAuthError::validation("name", "missing").tag_source_index(idx))?;
    ...
    let info = entry
        .info
        .ok_or_else(|| PaladinAuthError::validation("info", "missing").tag_source_index(idx))?;

    let secret_text = info
        .secret
        .ok_or_else(|| PaladinAuthError::validation("secret", "missing").tag_source_index(idx))?;
    ...
    let kind = if is_hotp {
        let counter = info.counter.ok_or_else(|| {
            PaladinAuthError::validation("counter", "missing").tag_source_index(idx)
        })?;
