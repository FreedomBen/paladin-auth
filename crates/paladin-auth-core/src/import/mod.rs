...
//     order: Paladin Auth magic → image magic → Aegis JSON shape →
//     otpauth text/JSON → `Unknown`.
//
// Format-specific importers, the `from_file` / `from_bytes` facade,
// and the Paladin Auth import precheck land in subsequent Phase I steps.
//
...
pub use paladin_auth::paladin_auth;
pub use precheck::{classify_paladin_auth_import_precheck, PaladinAuthImportPrecheck};
pub use qr::{qr_image, qr_image_bytes, read_qr_image, read_qr_image_bytes};

use crate::storage::header::MAGIC as PALADIN_AUTH_MAGIC;
...
pub enum ImportFormat {
    ...
    Aegis,
    /// Paladin Auth native vault file or encrypted bundle. Detected by the
    /// `PALAUTH\0` magic prefix, regardless of mode.
    PaladinAuth,
...
impl ImportFormat {
    ...
    /// (`unsupported_import_format` / `import_paladin_auth`).
    #[must_use]
...
///
/// 1. **Paladin Auth magic** — first 8 bytes equal `b"PALAUTH\0"`. Magic
...
pub fn detect(bytes: &[u8]) -> ImportFormat {
    if is_paladin_auth(bytes) {
        return ImportFormat::PaladinAuth;
    }
    if is_image(bytes) {
...
fn is_paladin_auth(bytes: &[u8]) -> bool {
    bytes.starts_with(&PALADIN_AUTH_MAGIC)
}
...
mod tests {
    ...
    fn import_format_as_str_matches_design() {
        ...
        assert_eq!(ImportFormat::PaladinAuth.as_str(), "paladin-auth");
