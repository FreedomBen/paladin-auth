...
pub fn otpauth(bytes: &[u8], import_time: SystemTime) -> Result<Vec<ValidatedAccount>> {
    let text = std::str::from_utf8(bytes)
        .map_err(|_| PaladinAuthError::validation("input", "invalid_utf8"))?;
...
fn collect_json_array(trimmed: &str) -> Result<Vec<String>> {
    ...
    let serde_json::Value::Array(arr) = value else {
        return Err(PaladinAuthError::validation("input", "expected_array"));
    };
    ...
    for (idx, item) in arr.into_iter().enumerate() {
        match item {
            ...
            _ => {
                return Err(
                    PaladinAuthError::validation("uri", "expected_string").tag_source_index(idx)
                )
            }
        }
    }
...
fn collect_line_list(trimmed: &str) -> Result<Vec<String>> {
    ...
    for line in trimmed.split('\n') {
        ...
        if row.as_bytes().contains(&0) {
            return Err(PaladinAuthError::validation("uri", "embedded_nul").tag_source_index(idx));
        }
