...
fn payloads_to_accounts(
    ...
    for (idx, payload) in payloads.into_iter().enumerate() {
        ...
        if !is_otpauth_uri(trimmed) {
            return Err(
                PaladinAuthError::validation("qr_image", "non_otpauth_payload")
                    .tag_source_index(idx),
            );
        }
...
fn validate_rgba_dims(width: u32, height: u32, rgba: &[u8]) -> Result<()> {
    if width == 0 || height == 0 {
        return Err(PaladinAuthError::validation("qr_image", "zero_dimensions"));
    }
    let pixels = (width as usize)
        .checked_mul(height as usize)
        .ok_or_else(|| PaladinAuthError::validation("qr_image", "dimensions_overflow"))?;
    let expected = pixels
        .checked_mul(4)
        .ok_or_else(|| PaladinAuthError::validation("qr_image", "dimensions_overflow"))?;
    if expected > QR_RGBA_MAX_BYTES {
        return Err(PaladinAuthError::validation("qr_image", "image_too_large"));
    }
