...
//
// The `paladin_auth_passphrase` field on [`ImportOptions`] is only
// consulted when dispatching the Paladin Auth format; absence there returns
// `invalid_state { operation: "import_paladin_auth", state: "missing_passphrase" }`.
...
use super::{
    aegis_plaintext, detect, otpauth, paladin_auth, qr_image, qr_image_bytes, ImportFormat,
};
...
pub struct ImportOptions {
    ...
    /// [`ImportFormat::PaladinAuth`]. Otherwise ignored.
    pub paladin_auth_passphrase: Option<SecretString>,
}

/// Import bytes from memory (text, JSON, image bytes, or Paladin Auth
/// bundle bytes).
pub fn from_bytes(
    bytes: &[u8],
    ...
    match format {
        ...
        ImportFormat::PaladinAuth => {
            dispatch_paladin_auth_bytes(bytes, options.paladin_auth_passphrase)
        }
...
/// Import from a file path (text, JSON, image, or Paladin Auth bundle).
pub fn from_file(
    path: &Path,
    options: ImportOptions,
    import_time: SystemTime,
) -> Result<Vec<ValidatedAccount>> {
    let bytes = std::fs::read(path).map_err(|err| PaladinAuthError::IoError {
...
fn dispatch_paladin_auth_bytes(
    bytes: &[u8],
    passphrase: Option<SecretString>,
) -> Result<Vec<ValidatedAccount>> {
    let pp = passphrase.ok_or(PaladinAuthError::InvalidState {
        operation: "import_paladin_auth",
        state: "missing_passphrase",
    })?;
    paladin_auth(bytes, pp)
}

fn dispatch_qr_bytes(bytes: &[u8], import_time: SystemTime) -> Result<Vec<ValidatedAccount>> {
    let img = image::load_from_memory(bytes).map_err(|err| PaladinAuthError::IoError {
...
fn unsupported(format: ImportFormat) -> PaladinAuthError {
