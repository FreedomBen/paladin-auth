...
//
// Presentation crates ask `paladin_auth_core::default_vault_path()` for the
...
// end before reaching `paladin-auth-core`.
...
/// Resolve the default vault path: `ProjectDirs::from("", "", "paladin-auth")`,
...
pub fn default_vault_path() -> Result<PathBuf> {
    let dirs =
        ProjectDirs::from("", "", "paladin-auth").ok_or_else(|| PaladinAuthError::IoError {
...
mod tests {
    ...
    fn default_vault_path_ends_with_vault_bin() {
        // On every supported platform `ProjectDirs::from("", "", "paladin-auth")`
    ...
    #[test]
    fn default_vault_path_uses_paladin_auth_project_dirs() {
        // The path must contain "paladin-auth" somewhere — `ProjectDirs::from`
        ...
        assert!(
            s.contains("paladin-auth"),
            "default_vault_path {s:?} does not contain 'paladin-auth'"
        );
    }
