...
// `PALADIN_AUTH_FAULT_INJECT=pre_commit|post_commit|csprng_read|kdf_allocation`
// env var:
//
...
// `paladin-auth-core`: regular `save_plaintext`, `save_plaintext_clobber`
...
/// `PALADIN_AUTH_FAULT_INJECT=pre_commit` is set in the process environment.
...
/// site — i.e. the `test-fault-injection` cargo feature is enabled and
/// `PALADIN_AUTH_FAULT_INJECT=post_commit` is set in the process environment.
/// Always `false` on production builds.
///
...
/// site — i.e. the `test-fault-injection` cargo feature is enabled and
/// `PALADIN_AUTH_FAULT_INJECT=csprng_read` is set in the process
...
/// enabled and `PALADIN_AUTH_FAULT_INJECT=kdf_allocation` is set in the
...
mod tests {
    ...
    fn unknown_value_fires_neither() {
        ...
        // `PALADIN_AUTH_FAULT_INJECT=1` in the environment cannot
