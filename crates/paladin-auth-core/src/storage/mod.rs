...
use crate::error::{ErrorKind, PaladinAuthError, Result, VaultMode};
...
pub fn inspect(path: &Path) -> Result<VaultStatus> {
    let mut file = match File::open(path) {
        ...
        Err(err) => {
            return Err(PaladinAuthError::IoError {
...
pub fn classify_init_precheck(probe: Result<VaultStatus>) -> InitPrecheck {
    match probe {
        ...
        Ok(VaultStatus::Plaintext | VaultStatus::Encrypted)
        | Err(
            PaladinAuthError::InvalidHeader | PaladinAuthError::UnsupportedFormatVersion { .. },
...
pub fn destroy_vault(path: &Path) -> Result<DestroyReport> {
    ...
    let primary_meta = match fs::symlink_metadata(path) {
        ...
        Err(err) if err.kind() == io::ErrorKind::NotFound => {
            return Err(PaladinAuthError::VaultMissing);
        }
        Err(err) => {
            return Err(PaladinAuthError::IoError {
    ...
    fs::remove_file(path).map_err(|err| PaladinAuthError::IoError {
    ...
    if backup_present {
        match fs::remove_file(&bak) {
            ...
            Err(err) if err.kind() == io::ErrorKind::NotFound => {}
            Err(err) => {
                return Err(PaladinAuthError::DestroyIoError {
...
fn create_plaintext(path: &Path) -> Result<(crate::Vault, Store)> {
    ...
    // never silently tightened — same rule used by `paladin-auth init` on
    ...
    if path.exists() {
        return Err(PaladinAuthError::VaultExists);
    }
    Ok((
        crate::Vault::empty(),
        Store {
...
fn open_plaintext(path: &Path) -> Result<(crate::Vault, Store)> {
    ...
    let bytes = fs::read(path).map_err(|err| PaladinAuthError::IoError {
        operation: "read_vault_file",
        source: err,
    })?;
    ...
    match parse_header(&bytes)? {
        ...
        ParsedHeader::Encrypted(_) => {
            return Err(PaladinAuthError::WrongVaultLock {
...
fn save_plaintext(path: &Path, payload: &VaultPayload) -> Result<()> {
    let parent = path.parent().ok_or_else(|| PaladinAuthError::IoError {
    ...
    // when `PALADIN_AUTH_FAULT_INJECT=pre_commit` so the failure is
    ...
    if fault::pre_commit_should_fail() || fs::rename(&primary_tmp, path).is_err() {
        let _ = fs::remove_file(&primary_tmp);
        return Err(PaladinAuthError::SaveNotCommitted {
            committed: false,
            backup_path: None,
        });
    }
    ...
    if fault::post_commit_should_fail() || fsync_dir(parent).is_err() {
        return Err(PaladinAuthError::SaveDurabilityUnconfirmed);
    }
...
fn stage_temp_file(path: &Path, content: &[u8], write_op: &'static str) -> Result<()> {
    ...
    #[cfg(unix)]
    opts.mode(0o600);
    let mut f = opts.open(path).map_err(|err| PaladinAuthError::IoError {
        operation: write_op,
        source: err,
    })?;
    f.write_all(content)
        .map_err(|err| PaladinAuthError::IoError {
            operation: write_op,
            source: err,
        })?;
    f.sync_all().map_err(|err| PaladinAuthError::IoError {
...
fn cleanup_leftover_temp(path: &Path) -> Result<()> {
    ...
    match fs::remove_file(path) {
        ...
        Err(err) => Err(PaladinAuthError::IoError {
            operation: "cleanup_temp_file",
            source: err,
        }),
    }
...
fn save_plaintext_clobber(path: &Path, payload: &VaultPayload) -> Result<()> {
    ...
    // hook short-circuits the rename for `PALADIN_AUTH_FAULT_INJECT=pre_commit`
...
pub fn write_secret_file_atomic(path: &Path, content: &[u8]) -> Result<()> {
    let parent = path
        ...
        .ok_or_else(|| PaladinAuthError::IoError {
            operation: "resolve_secret_file_parent",
            source: io::Error::new(
                io::ErrorKind::InvalidInput,
    ...
    // for `PALADIN_AUTH_FAULT_INJECT=pre_commit` so the surface is identical
...
/// Build the on-disk bytes for a Paladin Auth export bundle (docs/DESIGN.md
/// §4.6 export path).
///
...
/// `Store::open` / `import::paladin_auth`.
///
...
/// Decrypt an in-memory Paladin Auth bundle (docs/DESIGN.md §4.6 import path).
///
...
/// never inspects permissions. Plaintext-mode Paladin Auth bytes are
/// rejected with [`PaladinAuthError::UnsupportedPlaintextVault`] (the
/// import-semantic mapping; vault-file callers use `WrongVaultLock`).
///
/// Used by `import::paladin_auth` and `classify_paladin_auth_import_precheck` so
...
#[allow(clippy::needless_pass_by_value)]
pub(crate) fn decrypt_paladin_auth_bundle(
    bytes: &[u8],
    ...
    let trailer = match parse_header(bytes)? {
        ParsedHeader::Plaintext => {
            return Err(PaladinAuthError::UnsupportedPlaintextVault);
        }
        ParsedHeader::Encrypted(t) => t,
    };
    let params = Argon2Params {
...
fn csprng_read_error() -> PaladinAuthError {
...
fn kdf_allocation_error() -> PaladinAuthError {
    PaladinAuthError::IoError {
...
/// (`PALADIN_AUTH_FAULT_INJECT=kdf_allocation`) short-circuits the real
...
/// `test-fault-injection` hook (`PALADIN_AUTH_FAULT_INJECT=csprng_read`)
...
mod tests {
    ...
    fn stage_temp_file_surfaces_caller_supplied_write_op_string() {
        ...
        match err {
            PaladinAuthError::IoError { operation, .. } => assert_eq!(operation, "write_vault_tmp"),
        ...
        match err {
            PaladinAuthError::IoError { operation, .. } => {
                assert_eq!(operation, "write_backup_tmp");
            }
            other => panic!("expected IoError, got {other:?}"),
        }
    }
    ...
    fn classify_init_precheck_truth_table() {
        ...
        assert!(matches!(
            classify_init_precheck(Err(PaladinAuthError::InvalidHeader)),
            InitPrecheck::Existing
        ));
        assert!(matches!(
            classify_init_precheck(Err(PaladinAuthError::UnsupportedFormatVersion {
                format_ver: 99
            })),
            InitPrecheck::Existing
        ));
        ...
        match classify_init_precheck(Err(PaladinAuthError::VaultMissing)) {
            InitPrecheck::Propagate(PaladinAuthError::VaultMissing) => {}
            other => panic!("expected Propagate(VaultMissing), got {other:?}"),
        }
        match classify_init_precheck(Err(PaladinAuthError::IoError {
            operation: "read_vault_file",
            source: std::io::Error::new(std::io::ErrorKind::PermissionDenied, "denied"),
        })) {
            InitPrecheck::Propagate(PaladinAuthError::IoError { operation, .. }) => {
                assert_eq!(operation, "read_vault_file");
            }
            other => panic!("expected Propagate(IoError), got {other:?}"),
        }
        match classify_init_precheck(Err(PaladinAuthError::DecryptFailed)) {
            InitPrecheck::Propagate(PaladinAuthError::DecryptFailed) => {}
    ...
    fn enforce_on_disk_encrypted_size_cap_rejects_one_byte_over_threshold() {
        ...
        match err {
            PaladinAuthError::InvalidPayload { reason } => assert_eq!(reason, "exceeds_size_limit"),
