...
/// Magic bytes at the start of every Paladin Auth vault file.
pub(crate) const MAGIC: [u8; 8] = *b"PALAUTH\0";
...
///    `invalid_header`.
/// 2. Magic equals `PALAUTH\0` → otherwise `invalid_header`.
...
pub(crate) fn parse_header(bytes: &[u8]) -> Result<ParsedHeader> {
    if bytes.len() < PLAINTEXT_HEADER_LEN {
        return Err(PaladinAuthError::InvalidHeader);
    }
    if bytes[0..8] != MAGIC {
        return Err(PaladinAuthError::InvalidHeader);
    }
...
mod tests {
    ...
    fn parse_header_rejects_unknown_magic() {
        ...
        let mut bytes = plaintext_header_bytes();
        bytes[..8].copy_from_slice(b"palauth\0");
        assert_eq!(
            parse_header(&bytes).unwrap_err().kind(),
            ErrorKind::InvalidHeader,
        );
