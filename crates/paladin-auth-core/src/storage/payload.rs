...
#[allow(dead_code)] // Wired up by storage::Store (Phase E continuation).
pub(crate) fn encode_vault_payload(payload: &VaultPayload) -> Result<Vec<u8>, PaladinAuthError> {
...
#[allow(dead_code)] // Wired up by storage::Store (Phase E continuation).
pub(crate) fn decode_vault_payload(bytes: &[u8]) -> Result<VaultPayload, PaladinAuthError> {
    if bytes.len() > MAX_PAYLOAD_BYTES {
        return Err(PaladinAuthError::InvalidPayload {
            reason: "exceeds_size_limit",
        });
    ...
    let (payload, consumed) =
        bincode::decode_from_slice::<VaultPayload, _>(bytes, bincode_config()).map_err(|err| {
            match err {
                DecodeError::LimitExceeded => PaladinAuthError::InvalidPayload {
                    reason: "exceeds_size_limit",
                },
                _ => PaladinAuthError::InvalidPayload {
