...
pub(crate) fn ensure_vault_dir(path: &Path) -> Result<()> {
    match std::fs::symlink_metadata(path) {
        ...
        Err(err) if err.kind() == io::ErrorKind::NotFound => {
            std::fs::create_dir_all(path).map_err(|err| PaladinAuthError::IoError {
...
mod tests {
    ...
    fn enforce_dir_perms_is_unconditional_ok_stub() {
        ...
        let nonexistent = PathBuf::from("/definitely/does/not/exist/paladin-auth-perms-other");
    ...
    fn nix_can_write_into_readonly_dir(parent: &std::path::Path) -> bool {
        ...
        let probe = parent.join(".paladin-auth-root-probe");
