...
use crate::error::{PaladinAuthError, PermissionSubject, Result};
...
pub(crate) fn enforce_dir_perms(path: &Path) -> Result<()> {
    let meta = std::fs::symlink_metadata(path).map_err(|err| PaladinAuthError::IoError {
...
fn symlink_io_error(operation: &'static str) -> PaladinAuthError {
    PaladinAuthError::IoError {
        operation,
...
mod tests {
    ...
    fn ensure_vault_dir_creates_missing_single_level_parent_at_0700() {
        let tmp = tempdir().unwrap();
        let dir = tmp.path().join("paladin-auth");
        assert!(!dir.exists());
    ...
    fn ensure_vault_dir_creates_missing_multi_level_parent_at_0700() {
        let tmp = tempdir().unwrap();
        let dir = tmp.path().join("a").join("b").join("paladin-auth");
        assert!(!dir.exists());
    ...
    fn ensure_vault_dir_when_mkdir_eacces_surfaces_create_vault_dir() {
        ...
        let target = grandparent.join("paladin-auth");
    ...
    fn ensure_vault_dir_when_intermediate_is_a_file_surfaces_stat_vault_dir() {
        ...
        let blocking_file = parent.join("paladin-auth");
