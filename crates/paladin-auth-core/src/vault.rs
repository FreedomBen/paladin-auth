...
use crate::error::{ErrorKind, PaladinAuthError, Result};
...
/// Top-level in-memory representation of a Paladin Auth vault.
///
...
impl Vault {
    ...
    pub fn export_qr_png(
        ...
        let account = self.get(id).ok_or(PaladinAuthError::InvalidState {
    ...
    /// time per §4.6, so source IDs from a Paladin Auth bundle never leak
    ...
    fn edit_metadata_with_operation(
        ...
        if edit.label.is_none() && edit.issuer.is_none() && edit.icon_hint.is_none() {
            return Err(PaladinAuthError::validation("edit", "empty"));
        }
        ...
        // Step 4: resolve `id`.
        let pos = self.accounts.iter().position(|a| a.id() == id).ok_or(
    ...
    /// `EditDialog`, TUI Edit modal, and CLI `paladin-auth edit` use this
    ...
    pub fn set_passphrase(&mut self, store: &Store, options: EncryptionOptions) -> Result<()> {
        if self.is_encrypted() {
            return Err(PaladinAuthError::InvalidState {
    ...
    pub fn totp_code(&self, id: AccountId, now: SystemTime) -> Result<Code> {
        let account =
            self.accounts
                .iter()
                .find(|a| a.id() == id)
                .ok_or(PaladinAuthError::InvalidState {
                    operation: "totp_code",
                    state: "account_not_found",
                })?;
    ...
    pub fn totp_next_code(&self, id: AccountId, now: SystemTime) -> Result<Code> {
        ...
        let next_window_start_secs = current_counter
            ...
            .ok_or(PaladinAuthError::TimeRange {
                operation: "totp_next_code",
    ...
    pub fn hotp_advance(&mut self, store: &Store, id: AccountId, now: SystemTime) -> Result<Code> {
        ...
        if counter == u64::MAX {
            return Err(PaladinAuthError::CounterOverflow {
