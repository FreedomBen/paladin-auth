...
//! End-to-end tests for `paladin-auth tui`, the `execvp` wrapper around
//! `paladin-auth-tui`. See `docs/IMPLEMENTATION_PLAN_02_CLI.md` "`paladin-auth tui`
//! exec wrapper". These tests place a stub `paladin-auth-tui` script on
...
/// Build an `assert_cmd::Command` for the `paladin-auth` binary with a
...
/// Write a stub `paladin-auth-tui` script that records its argv (one
...
fn stub_dir_with_argv_recorder() -> TempDir {
    ...
    let script = dir.path().join("paladin-auth-tui");
...
/// Path to the argv-recording log written by the stub `paladin-auth-tui`.
...
#[test]
fn paladin_auth_tui_execs_paladin_auth_tui_with_no_extra_flags_when_globals_are_default() {
...
#[test]
fn paladin_auth_tui_forwards_vault_in_global_position() {
    let stub = stub_dir_with_argv_recorder();
    paladin_auth()
        .env("PATH", stub.path())
...
#[test]
fn paladin_auth_tui_forwards_vault_in_subcommand_position() {
    let stub = stub_dir_with_argv_recorder();
    paladin_auth()
        .env("PATH", stub.path())
...
#[test]
fn paladin_auth_tui_forwards_no_color_in_global_position() {
    let stub = stub_dir_with_argv_recorder();
    paladin_auth()
        .env("PATH", stub.path())
        .args(["--no-color", "tui"])
...
#[test]
fn paladin_auth_tui_forwards_no_color_in_subcommand_position() {
    let stub = stub_dir_with_argv_recorder();
    paladin_auth()
        .env("PATH", stub.path())
        .args(["tui", "--no-color"])
...
#[test]
fn paladin_auth_tui_forwards_both_vault_and_no_color() {
    let stub = stub_dir_with_argv_recorder();
    paladin_auth()
        .env("PATH", stub.path())
...
#[test]
fn paladin_auth_json_tui_rejects_at_parse_time_with_validation_error_envelope() {
    let stub = stub_dir_with_argv_recorder();
    let assert = paladin_auth()
        .env("PATH", stub.path())
        .args(["--json", "tui"])
    ...
    assert!(
        ...
        "exec_tui must not invoke paladin-auth-tui when --json is rejected",
    );
}

#[test]
fn paladin_auth_tui_json_after_subcommand_rejects_with_validation_error_envelope() {
    let stub = stub_dir_with_argv_recorder();
    let assert = paladin_auth()
        .env("PATH", stub.path())
        .args(["tui", "--json"])
...
#[test]
fn paladin_auth_json_tui_help_emits_help_envelope_without_inspecting_path() {
    ...
    assert_eq!(
        value["help"]["command"],
        serde_json::json!("paladin-auth tui")
    );
    ...
    assert!(
        text.contains("paladin-auth-tui"),
        "help text missing tui description that names paladin-auth-tui: {text:?}",
    );
    assert!(
        !argv_log(&stub).exists(),
        "--json tui --help must not exec paladin-auth-tui",
    );
}

#[test]
fn paladin_auth_tui_text_mode_with_missing_paladin_auth_tui_returns_io_error_exec_paladin_auth_tui()
{
    // Empty PATH guarantees `paladin-auth-tui` cannot be resolved; the
    // wrapper surfaces `io_error` with `operation: "exec_paladin_auth_tui"`
    ...
    assert!(
        ...
        "expected exec_paladin_auth_tui operation tag in stderr, got {stderr:?}",
    );
}
