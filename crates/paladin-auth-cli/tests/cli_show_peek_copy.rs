...
//! End-to-end tests for `paladin-auth show`, `paladin-auth peek`, and (gated
//! behind the `paladin-auth-cli/test-hooks` cargo feature) `paladin-auth copy`.
...
//! `PALADIN_AUTH_CLIPBOARD_DRYRUN=1|fail` env var honored only when
//! `paladin-auth-cli/test-hooks` is enabled, so CI can exercise the
//! command end-to-end without a system clipboard server. Run them
//! with `cargo test -p paladin-auth-cli --features test-hooks`.
...
// the `PALADIN_AUTH_CLIPBOARD_DRYRUN` env-var bypass is compiled into the
...
mod copy {
    ...
    /// Build a `paladin-auth` command with `PALADIN_AUTH_CLIPBOARD_DRYRUN` set.
    /// Use `"1"` for the success-path bypass (no actual write) and
    /// `"fail"` to exercise the `clipboard_write_failed` envelope.
    fn paladin_auth_dryrun(mode: &str) -> Command {
        let mut cmd = paladin_auth();
        cmd.env("PALADIN_AUTH_CLIPBOARD_DRYRUN", mode);
        cmd
    }
    ...
    fn json_copy_missing_vault_rejects_with_vault_missing_envelope() {
        let (_dir, path) = fresh_vault_path();
        let assert = paladin_auth_dryrun("1")
            .args([
                "--json",
                "--vault",
                path.to_str().unwrap(),
    ...
    fn json_copy_clipboard_failure_on_totp_emits_clipboard_write_failed_with_null_counter() {
        ...
        let assert = paladin_auth_dryrun("fail")
            .args(["--json", "--vault", path.to_str().unwrap(), "copy", "alice"])
            .assert()
            .failure();
        let stderr = std::str::from_utf8(&assert.get_output().stderr).unwrap();
