...
//! End-to-end tests for `paladin-auth qr <query>`. Covers the no-prompt
...
//! is gated behind the `paladin-auth-cli/test-hooks` cargo feature.
...
/// Mirrors `paladin-auth-core/tests/export_qr.rs::decode_png_to_payload` so
...
/// `paladin_auth_core::export::otpauth_list`, the same emitter the
...
fn expected_otpauth_uri_for_single_account_vault(vault_path: &Path) -> String {
    let (vault, _store) =
        paladin_auth_core::Store::open(vault_path, paladin_auth_core::VaultLock::Plaintext)
            .expect("open");
    let list = paladin_auth_core::export::otpauth_list(&vault);
...
fn qr_png_to_out_decodes_back_to_matching_otpauth_uri() {
    // `paladin-auth qr alice --out <path>.png` writes a PNG whose embedded
    // payload, decoded through `rqrr`, must equal the URI
    // `paladin_auth_core::export::otpauth_list` would emit for the same
    // account (parity with the core round-trip test
    // `paladin-auth-core/tests/export_qr.rs::
...
// ==========================================================================
// `--out` durability — `PALADIN_AUTH_FAULT_INJECT` round-trip
// ==========================================================================
...
mod fault_inject {
    ...
    fn qr_out_pre_commit_fault_surfaces_save_not_committed() {
        ...
        // `committed: false`. `paladin-auth qr` opens its source vault
...
fn pty_qr_against_encrypted_vault_unlocks_once_and_decodes_correctly() {
    use paladin_auth_core::{Argon2Params, EncryptionOptions, VaultInit};
    ...
    let expected = {
        let pp = SecretString::from(passphrase.to_string());
        let (vault, _store) = Store::open(&path, paladin_auth_core::VaultLock::Encrypted(pp))
            .expect("open encrypted");
        let list = paladin_auth_core::export::otpauth_list(&vault);
        list.trim_end_matches('\n').to_string()
    };

    // 2. Drive `paladin-auth qr` through the PTY harness. `--json` keeps
...
// entry of crates/paladin-auth-cli/Cargo.toml. The dev-dependency stays.
// ==========================================================================
...
fn deny_qrcode_in_runtime_deps() {
    ...
    assert!(
        hits.is_empty(),
        "paladin-auth-cli must not declare `qrcode` as a runtime dependency \
