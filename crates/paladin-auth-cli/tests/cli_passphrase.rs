...
//! End-to-end tests for `paladin-auth passphrase {set,change,remove}`.
...
/// `PALAUTH\0` magic prefix on every vault file (docs/DESIGN.md §4.6).
...
fn pty_set_confirmation_mismatch_rejects_with_invalid_passphrase_before_mutation() {
    ...
    assert_eq!(&before[..8], PALADIN_AUTH_MAGIC);
    ...
    exit.assert_exit(1);
    // Text-mode renderer prefixes `paladin-auth: ` to the
    // `Display::fmt` body of `PaladinAuthError::InvalidPassphrase`,
    // which is `invalid passphrase: <reason>`.
    exit.assert_transcript_contains("paladin-auth: invalid passphrase: confirmation_mismatch");
...
fn pty_set_without_dev_tty_surfaces_io_error_passphrase_prompt() {
    ...
    assert!(
        stderr.starts_with("paladin-auth: ") || stderr.contains("\npaladin-auth: "),
        "expected `paladin-auth:` text-mode prefix on the error line, got {stderr:?}",
    );
...
fn pty_change_on_open_encrypted_vault_succeeds_and_rotates_salt_under_requested_kdf_params() {
    ...
    assert_eq!(&after[..8], PALADIN_AUTH_MAGIC);
...
fn pty_set_under_json_keeps_streams_clean_after_prompts_consume_dev_tty() {
    // §5 strict-mode rule: under `--json`, `paladin-auth passphrase set`
...
fn json_remove_without_yes_rejects_at_parse_time_with_yes_required_under_json() {
    ...
    // before any disk I/O. This mirrors the `paladin-auth remove --json`
    // pattern.
    let (_dir, path) = fresh_vault_path();
    let assert = paladin_auth()
        .args([
