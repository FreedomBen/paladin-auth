...
//! End-to-end tests for `paladin-auth init`.
//!
...
//! `PALADIN_AUTH_FAULT_INJECT={pre,post}_commit`) are gated on the
//! `paladin-auth-cli/test-hooks` cargo feature so the hook is compiled
//! into the binary.
...
use common::{fresh_vault_path, paladin_auth, write_existing_plaintext_vault, Pty};

// --- Local fixture helpers --------------------------------------------------

/// Magic bytes whose last byte is `\0` (matching `paladin-auth-core`'s
/// `MAGIC` constant) so `inspect` parses the file as a real Paladin Auth
/// header rather than rejecting the magic.
const PALADIN_AUTH_MAGIC: &[u8; 8] = b"PALAUTH\0";

/// Write a 16-byte file whose magic does **not** match
/// `PALAUTH\0`. `inspect` rejects it as `invalid_header` →
...
fn write_unsupported_format_version_vault(path: &Path) {
    ...
    bytes.extend_from_slice(PALADIN_AUTH_MAGIC);
...
/// Write a real encrypted Paladin Auth vault at `path` with the §4.4
...
#[test]
fn text_existing_vault_without_force_emits_paladin_auth_vault_exists_message() {
    let (_dir, path) = fresh_vault_path();
    write_existing_plaintext_vault(&path);
    let assert = paladin_auth()
    ...
    assert!(
        ...
        "expected paladin-auth: prefix, got {stderr:?}"
    );
    assert!(
...
fn pty_text_empty_passphrase_creates_plaintext_with_warning() {
    ...
    assert_eq!(&header[..8], PALADIN_AUTH_MAGIC);
...
fn pty_confirmation_mismatch_rejects_before_mutation() {
    ...
    let exit = pty.wait_for_exit();
    exit.assert_exit(1);
    exit.assert_transcript_contains("paladin-auth: ");
...
#[test]
fn pty_force_rotates_paladin_auth_existing_into_bak() {
    let (_dir, path) = fresh_vault_path();
    write_existing_plaintext_vault(&path);
    ...
    assert_eq!(&new_primary[..8], PALADIN_AUTH_MAGIC);
    assert_eq!(new_primary[9], 0, "new primary is plaintext");
}

#[test]
fn pty_force_rotates_non_paladin_auth_existing_into_bak() {
    // §5: `create_force` rotates the old file verbatim regardless of
    // its content, including non-Paladin Auth garbage. The clobber warning
    // also fires because `classify_init_precheck` maps
    // `InvalidHeader` to `Existing`.
    let (_dir, path) = fresh_vault_path();
    let original = b"not-a-paladin-auth-vault-at-all-just-bytes" as &[u8];
    std::fs::write(&path, original).expect("write non-paladin-auth file");
    std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o600)).expect("chmod 0600");
    let bak = path.with_file_name("vault.bin.bak");
...
fn pty_init_unsafe_parent_dir_surfaces_unsafe_permissions_with_chmod_hint() {
    ...
    exit.assert_exit(1);
    exit.assert_transcript_contains("paladin-auth: unsafe permissions");
...
mod fault_inject {
    ...
    fn pty_init_force_pre_commit_surfaces_save_not_committed_with_backup_path() {
        // Existing Paladin Auth file → `--force` rotates it to `.bak`,
        ...
        let mut pty = Pty::spawn(
            ...
            &[("PALADIN_AUTH_FAULT_INJECT", "pre_commit")],
        );
    ...
    fn pty_init_force_post_commit_surfaces_save_durability_unconfirmed() {
        ...
        let mut pty = Pty::spawn(
            ...
            &[("PALADIN_AUTH_FAULT_INJECT", "post_commit")],
        );
        pty.expect(PROMPT_NEW_PASSPHRASE);
        pty.send_line("");
        let exit = pty.wait_for_exit();
