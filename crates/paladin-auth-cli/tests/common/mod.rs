...
//! Shared test helpers for the `paladin-auth` CLI integration suite.
//!
//! Provides:
//!
//! * `paladin_auth()` / `paladin_auth_command()` — `assert_cmd::Command` and
//!   `std::process::Command` factories that pin the cargo-built
//!   `paladin-auth` binary and clear `NO_COLOR` so test output is
...
//! * `paladin_auth_command_without_tty()` — the no-controlling-tty
//!   companion to `paladin_auth_command()`. Used by the `[PTY]` bullets
...
/// `assert_cmd::Command` for the cargo-built `paladin-auth` binary with a
/// cleared `NO_COLOR` so tests don't inherit terminal-coloring
/// preferences from the developer's shell.
pub fn paladin_auth() -> assert_cmd::Command {
    let mut cmd = assert_cmd::Command::cargo_bin("paladin-auth").expect("cargo bin");
    cmd.env_remove("NO_COLOR");
    cmd
}

/// `std::process::Command` for the same `paladin-auth` binary used by
/// `paladin_auth()`. Used by the PTY harness, which needs a real
/// `Command` to hand off to `rexpect::session::spawn_command`.
pub fn paladin_auth_command() -> StdCommand {
    let mut cmd = StdCommand::cargo_bin("paladin-auth").expect("cargo bin");
    cmd.env_remove("NO_COLOR");
    cmd
}

/// `std::process::Command` for `paladin-auth` that runs without a
...
/// `setsid(1)` execs into `paladin-auth` directly.
///
...
pub fn paladin_auth_command_without_tty() -> StdCommand {
    let paladin_auth = StdCommand::cargo_bin("paladin-auth").expect("cargo bin");
    let bin = paladin_auth.get_program().to_os_string();
...
/// Write a 16-byte `PALAUTH1` plaintext header to `path` so `inspect`
...
pub fn write_existing_plaintext_vault(path: &Path) {
    ...
    bytes.extend_from_slice(b"PALAUTH1");
...
impl Pty {
    /// Spawn `paladin-auth` with the given args + extra env on a fresh
    ...
    where
        ...
        S: AsRef<OsStr>,
    {
        let mut cmd = paladin_auth_command();
        cmd.args(args);
        for (k, v) in envs {
            cmd.env(k, v);
        }
        let session =
            spawn_command(cmd, Some(DEFAULT_TIMEOUT_MS)).expect("spawn paladin-auth on a PTY");
        Self {
            session,
