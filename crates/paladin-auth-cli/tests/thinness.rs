...
//
// Source-level "thinness contract" guard for `paladin-auth-cli`
...
// imported directly here — they belong in `paladin-auth-core`. This test
...
// the logic can be moved into `paladin-auth-core`.
//
// This mirrors the defense-in-depth pattern used by
// `paladin-auth-core/tests/no_network.rs`. Update the pattern lists below
...
/// `paladin-auth-core`; the CLI consumes its public API instead. Update
...
#[test]
fn paladin_auth_cli_source_tree_does_not_reference_forbidden_crates() {
    ...
    assert!(
        hits.is_empty(),
        "paladin-auth-cli/src/ contains forbidden crate references — move the \
...
#[test]
fn paladin_auth_cli_manifest_does_not_declare_forbidden_dependency() {
    ...
    assert!(
        hits.is_empty(),
        "paladin-auth-cli/Cargo.toml declares forbidden direct dependencies — \
