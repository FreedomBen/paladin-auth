...
//! End-to-end tests for `paladin-auth add`. Covers the no-prompt input
...
/// `paladin-auth add --qr` invocation has to walk every grid that
/// `rqrr::PreparedImage::detect_grids` returns. Mirrors the
/// `make_side_by_side_rgba` shape used by
/// `paladin-auth-core/tests/import_qr.rs`, but writes a real PNG on disk
...
fn pty_interactive_add_reads_manual_fields_once_with_defaults() {
    ...
    // the result through `paladin_auth_core::validate_manual` with the
...
fn pty_interactive_add_without_dev_tty_surfaces_io_error_account_prompt() {
    ...
    let output = common::paladin_auth_command_without_tty()
    ...
    // Text-mode `paladin-auth: <message>` prefix from `output::error::render`.
    assert!(
        stderr.starts_with("paladin-auth: "),
        "expected `paladin-auth:` text-mode prefix, got {stderr:?}",
    );
    // `PaladinAuthError::IoError` Display is
    // `"I/O error during {operation}: {source}"` per
    // `paladin_auth_core::error::PaladinAuthError`. Asserting the operation tag
