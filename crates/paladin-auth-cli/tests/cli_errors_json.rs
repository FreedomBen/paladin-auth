...
fn write_plaintext_header(path: &Path, mode: u32) {
    ...
    bytes.extend_from_slice(b"PALAUTH\0"); // 8-byte magic
...
fn invalid_header_envelope_for_unknown_magic_is_error_kind_only() {
    let (_dir, path) = fresh_vault_dir();
    // Magic that is not PALAUTH\0; inspect() rejects with InvalidHeader.
    std::fs::write(&path, b"NOT_PALAUTH_BYTES_PADDED________").expect("write garbage");
