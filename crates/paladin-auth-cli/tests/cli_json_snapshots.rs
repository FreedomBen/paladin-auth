...
//! `#[test]` here drives one named code path through real `paladin-auth`
...
use common::paladin_auth_command_without_tty;
...
/// Spawn `paladin-auth` via `setsid(1)` so the child runs in a fresh
...
fn run_paladin_auth(args: &[&str]) -> Output {
    paladin_auth_command_without_tty()
        .args(args)
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
...
fn snapshot_list_empty_vault_envelope() {
    ...
    let out = run_paladin_auth(&["--json", "--vault", path.to_str().unwrap(), "list"]);
    assert_success(&out);
...
fn snapshot_show_totp_envelope() {
    let (_dir, path) = fresh_vault_path();
    seed_populated_plaintext_vault(&path);
    let out = run_paladin_auth(&["--json", "--vault", path.to_str().unwrap(), "show", "alice"]);
    assert_success(&out);
    assert_json_snapshot!(parse_stdout_json(&out), {
...
fn snapshot_peek_totp_envelope() {
    let (_dir, path) = fresh_vault_path();
    seed_populated_plaintext_vault(&path);
    let out = run_paladin_auth(&["--json", "--vault", path.to_str().unwrap(), "peek", "alice"]);
    assert_success(&out);
    assert_json_snapshot!(parse_stdout_json(&out), {
        ".codes[].account.id" => "[uuid]",
        ".codes[].account.created_at" => "[timestamp]",
...
fn snapshot_edit_envelope() {
    // `paladin-auth edit` mirrors the `rename` success envelope:
...
fn snapshot_validation_error_unknown_subcommand_envelope() {
    let out = run_paladin_auth(&["--json", "definitely-not-a-subcommand"]);
    assert_failure(&out);
...
fn snapshot_vault_exists_envelope() {
    let (_dir, path) = fresh_vault_path();
    seed_empty_plaintext_vault(&path);
    let out = run_paladin_auth(&["--json", "--vault", path.to_str().unwrap(), "init"]);
    assert_failure(&out);
    assert_json_snapshot!(parse_stderr_json(&out));
}
...
fn snapshot_unsupported_format_version_envelope() {
    ...
    bytes.extend_from_slice(b"PALAUTH\0");
...
fn snapshot_help_envelope_top_level() {
    let out = run_paladin_auth(&["--json", "--help"]);
    assert_success(&out);
    assert_json_snapshot!(parse_stdout_json(&out), {
...
fn snapshot_help_envelope_subcommand() {
    let out = run_paladin_auth(&["--json", "init", "--help"]);
    assert_success(&out);
    assert_json_snapshot!(parse_stdout_json(&out), {
        ".help.text" => "[help-text]",
    });
...
fn snapshot_edit_help_envelope() {
    let out = run_paladin_auth(&["--json", "edit", "--help"]);
    assert_success(&out);
    assert_json_snapshot!(parse_stdout_json(&out), {
        ".help.text" => "[help-text]",
    });
...
fn snapshot_destroy_help_envelope() {
    let out = run_paladin_auth(&["--json", "destroy", "--help"]);
    assert_success(&out);
    assert_json_snapshot!(parse_stdout_json(&out), {
        ".help.text" => "[help-text]",
    });
...
fn snapshot_version_envelope() {
    let out = run_paladin_auth(&["--json", "--version"]);
    assert_success(&out);
    assert_json_snapshot!(parse_stdout_json(&out), {
