...
//! End-to-end tests for `paladin-auth export`. Covers the no-prompt code
...
use paladin_auth_core::{parse_otpauth, Argon2Params, EncryptionOptions, Store, VaultInit};
...
/// §5 prompt label fired by `paladin-auth export --encrypted` for the new
/// bundle passphrase.
...
/// `classify_paladin_auth_import_precheck` returns `PromptForPassphrase` for
/// an encrypted Paladin Auth bundle.
...
fn pty_encrypted_export_round_trips_through_import_with_independent_passphrases() {
    ...
    // passphrase protects only the exported Paladin Auth bundle. It is
...
fn pty_encrypted_export_with_custom_kdf_writes_requested_params_to_bundle_header() {
    ...
    let bundle_path = dir.path().join("alice-custom.paladin-auth");
...
fn pty_encrypted_export_with_default_kdf_writes_section_4_4_defaults_to_bundle_header() {
    ...
    let (dir, vault_path) = fresh_vault_path();
    create_plaintext_vault_with(&vault_path, &[TOTP_URI_ALICE]);
    let bundle_path = dir.path().join("alice-default.paladin-auth");
...
mod fault_inject {
    ...
    fn pty_encrypted_export_pre_commit_surfaces_save_not_committed() {
        ...
        let (dir, vault_path) = fresh_vault_path();
        create_plaintext_vault_with(&vault_path, &[TOTP_URI_ALICE]);
        let bundle_path = dir.path().join("bundle.paladin-auth");
    ...
    fn pty_encrypted_export_post_commit_surfaces_save_durability_unconfirmed() {
        ...
        // rename committed: the bundle now exists with the Paladin Auth
        ...
        assert_eq!(&bytes[..8], PALADIN_AUTH_MAGIC);
