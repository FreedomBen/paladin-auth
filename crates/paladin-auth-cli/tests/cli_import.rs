...
//! End-to-end tests for `paladin-auth import`. Covers the no-prompt code
...
//! `unsupported_format_version` for malformed Paladin Auth headers (which
...
fn open_vault(path: &Path) -> Vault {
    use paladin_auth_core::VaultLock;
...
fn seed_with_alice(path: &Path) {
    ...
    let (mut vault, store) =
        Store::open(path, paladin_auth_core::VaultLock::Plaintext).expect("open");
...
#[test]
fn json_paladin_auth_plaintext_bundle_rejects_without_passphrase_prompt() {
    // A Paladin Auth-format file in plaintext mode is not importable as a
    // bundle source; the precheck rejects it before any prompt. We
    // create a real plaintext Paladin Auth file via the vault Store and
...
fn imported_account_is_persisted_to_disk() {
    ...
    let accounts = vault
        .matching_accounts(&paladin_auth_core::parse_account_query("alice").expect("parse query"));
...
// =========================================================================
// Encrypted Paladin Auth bundle PTY tests
// =========================================================================

/// §5 prompt label fired by `paladin-auth import` after
/// `classify_paladin_auth_import_precheck` returns `PromptForPassphrase`
/// for an encrypted Paladin Auth bundle.
const PROMPT_BUNDLE_PASSPHRASE: &str = "Bundle passphrase: ";

/// Build an encrypted Paladin Auth bundle whose contents are the accounts
/// currently held in `src_vault`. Uses the §4.4 minimum Argon2id
/// parameters so the test stays fast in CI; the bundle still goes
/// through the real `paladin_auth_core::export::encrypted` writer.
...
#[test]
fn pty_encrypted_paladin_auth_bundle_prompts_once_for_bundle_passphrase() {
    // §5: an encrypted Paladin Auth bundle import must prompt exactly
    ...
    // serialize it as an encrypted Paladin Auth bundle.
    ...
    let bundle_path = src_dir.path().join("alice.paladin-auth");
...
#[test]
fn pty_encrypted_paladin_auth_bundle_preserves_timestamps_and_assigns_fresh_uuids() {
    // §4.6 / §5: Paladin Auth encrypted bundles preserve each account's
