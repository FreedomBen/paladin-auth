...
//!
//! `/dev/tty` is unconditionally targeted because Paladin Auth v0.1 is
...
fn io_err(operation: &'static str) -> impl Fn(io::Error) -> PaladinAuthError {
    move |source| PaladinAuthError::IoError { operation, source }
}

fn open_tty_write(op: &'static str) -> Result<File, PaladinAuthError> {
    OpenOptions::new()
        .write(true)
        .open("/dev/tty")
        .map_err(io_err(op))
...
fn open_tty_read(op: &'static str) -> Result<File, PaladinAuthError> {
    OpenOptions::new()
        .read(true)
        .open("/dev/tty")
        .map_err(io_err(op))
...
fn write_prompt(prompt: &str, op: &'static str) -> Result<(), PaladinAuthError> {
...
fn read_visible_line(op: &'static str) -> Result<String, PaladinAuthError> {
...
) -> Result<Option<SecretString>, PaladinAuthError> {
    match policy {
        NewPassphraseEmptyPolicy::AllowAsPlaintext => Ok(None),
        NewPassphraseEmptyPolicy::Reject => Err(PaladinAuthError::InvalidPassphrase {
            reason: "zero_length",
        }),
    }
}
...
fn classify_destructive_response(line: &str) -> Result<(), PaladinAuthError> {
    if line.trim() == "yes" {
        Ok(())
    } else {
        Err(PaladinAuthError::ValidationError {
            field: "confirmation",
...
pub fn prompt_passphrase(prompt: &str) -> Result<SecretString, PaladinAuthError> {
...
pub fn prompt_account_line(prompt: &str) -> Result<String, PaladinAuthError> {
...
pub fn prompt_account_secret(prompt: &str) -> Result<SecretString, PaladinAuthError> {
    write_prompt(prompt, OP_ACCOUNT)?;
...
pub fn prompt_destructive_confirmation(prompt: &str) -> Result<(), PaladinAuthError> {
...
mod tests {
    ...
    fn confirmation_decline_kind_is_validation_error() {
        ...
        assert_eq!(err.kind(), paladin_auth_core::ErrorKind::ValidationError);
    }
    ...
    fn confirmation_mismatch_kind_is_invalid_passphrase() {
        ...
        let b = secret("beta");
        let err = check_confirmation_match(&a, &b).unwrap_err();
        assert_eq!(err.kind(), paladin_auth_core::ErrorKind::InvalidPassphrase);
    }
