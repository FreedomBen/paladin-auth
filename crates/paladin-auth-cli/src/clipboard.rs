...
//! CLI clipboard adapter for `paladin-auth copy`. Production calls
//! [`arboard`]; under the `paladin-auth-cli/test-hooks` cargo feature an
//! env-var-driven dryrun short-circuit lets process-level integration
//! tests exercise `paladin-auth copy` end-to-end without a system clipboard
...
use paladin_auth_core::AccountSummary;
...
/// Test-build backend (gated on `paladin-auth-cli/test-hooks`): honors
/// `PALADIN_AUTH_CLIPBOARD_DRYRUN` so process-level tests can exercise
/// `paladin-auth copy` without a clipboard server.
///
/// - `PALADIN_AUTH_CLIPBOARD_DRYRUN=1` → bypass arboard, return `Ok(())`.
/// - `PALADIN_AUTH_CLIPBOARD_DRYRUN=fail` → bypass arboard, return `Err(())`
