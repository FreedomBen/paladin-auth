...
//! `paladin-auth passphrase {set,change,remove}` — vault passphrase
...
//!    [`paladin_auth_core::Vault`] method (`set_passphrase` /
...
use paladin_auth_core::{
    format_plaintext_storage_warning, inspect, EncryptionOptions, PaladinAuthError, VaultMode,
    VaultStatus,
};
...
/// `paladin-auth passphrase set`: encrypt a plaintext vault under a new
...
pub fn set(global: &GlobalArgs, kdf_args: &KdfArgs) -> Result<(), CliError> {
    ...
    // before unlocking it.
    match inspect(&path)? {
        VaultStatus::Missing => return Err(CliError::PaladinAuth(PaladinAuthError::VaultMissing)),
        VaultStatus::Encrypted => {
            return Err(CliError::PaladinAuth(PaladinAuthError::InvalidState {
...
/// `paladin-auth passphrase change`: re-encrypt an encrypted vault under a
...
/// `paladin-auth passphrase remove`: decrypt an encrypted vault to plaintext.
...
pub fn remove(global: &GlobalArgs, yes: bool) -> Result<(), CliError> {
    ...
    // streams). Mirrors the parse-time pattern used by `paladin-auth remove
    // --json`.
