...
//! `paladin-auth import` — import accounts from a file (docs/DESIGN.md §5).
...
//! 3. Call [`paladin_auth_core::classify_paladin_auth_import_precheck`] with the
...
//!    fallthrough.
//! 4. Run [`paladin_auth_core::import::from_file`] to materialise the
//!    `Vec<ValidatedAccount>` batch.
//! 5. Apply the merge inside [`paladin_auth_core::Vault::mutate_and_save`]
...
use paladin_auth_core::{
    import as core_import, AccountSummary, ImportConflict, ImportFormat, PaladinAuthImportPrecheck,
};
...
pub fn run(global: &GlobalArgs, args: &ImportArgs) -> Result<(), CliError> {
    ...
    let bundle_passphrase: Option<SecretString> =
        match core_import::classify_paladin_auth_import_precheck(&args.path, forced_format) {
            PaladinAuthImportPrecheck::NoPrompt => None,
            PaladinAuthImportPrecheck::PromptForPassphrase => {
                Some(prompt::prompt_passphrase("Bundle passphrase: ")?)
            }
            PaladinAuthImportPrecheck::Reject(err) => return Err(err.into()),
        };

    let now = SystemTime::now();
    let options = core_import::ImportOptions {
        format: forced_format,
        paladin_auth_passphrase: bundle_passphrase,
    };
    ...
    let summaries: Vec<AccountSummary> = report
        ...
        .filter_map(|id| {
            opened
                .vault
                .get(*id)
                .map(paladin_auth_core::Account::summary)
        })
        .collect();
...
fn map_format(arg: ImportFormatArg) -> ImportFormat {
    match arg {
        ...
        ImportFormatArg::PaladinAuth => ImportFormat::PaladinAuth,
...
fn render_success(
    mode: Mode,
    report: &paladin_auth_core::ImportReport,
    ...
    match mode {
        ...
        Mode::Text { .. } => {
            ...
            if report.skipped > 0 {
                let _ = writeln!(
                    std::io::stderr().lock(),
                    "paladin-auth: warning: skipped {} duplicate account(s) (use --on-conflict to change behavior)",
                    report.skipped,
                );
            }
...
fn io_err(source: std::io::Error) -> CliError {
    CliError::PaladinAuth(paladin_auth_core::PaladinAuthError::IoError {
...
mod tests {
    ...
    fn map_format_round_trips() {
        ...
        assert_eq!(
            map_format(ImportFormatArg::PaladinAuth),
