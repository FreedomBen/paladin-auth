...
//! `paladin-auth export` — write the vault to a file (docs/DESIGN.md §5).
//!
...
//! produces) and `--encrypted <path>` (Paladin Auth-format encrypted
//! bundle). Both modes refuse to overwrite without `--force`, both
//! write through [`paladin_auth_core::write_secret_file_atomic`] so the
//! destination ends up `0600`, and both render the §5 stable success
//! envelope (`{ "written": ..., "format": "otpauth"|"paladin-auth" }`) on
//! completion.
//!
...
//!    [`paladin_auth_core::format_plaintext_export_warning`] to stderr.
...
//! 7. Render bytes via [`paladin_auth_core::export::otpauth_list`] /
//!    [`paladin_auth_core::export::encrypted`].
//! 8. Persist via [`paladin_auth_core::write_secret_file_atomic`] (output is
//!    always `0600`).
...
/// Stable §5 `format` value for an encrypted Paladin Auth-bundle export.
const FORMAT_PALADIN_AUTH: &str = "paladin-auth";
...
impl Target<'_> {
    ...
    fn format_label(&self) -> &'static str {
        match self {
            ...
            Self::Encrypted(_) => FORMAT_PALADIN_AUTH,
        }
    }
}
