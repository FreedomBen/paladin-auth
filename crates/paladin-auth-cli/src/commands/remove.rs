...
//! `paladin-auth remove <query>` — remove a single account. See docs/DESIGN.md §5
...
use paladin_auth_core::{AccountId, AccountSummary, PaladinAuthError};
...
pub fn run(global: &GlobalArgs, args: &RemoveArgs) -> Result<(), CliError> {
    ...
    opened.vault.mutate_and_save(&opened.store, |vault| {
        vault.remove(id).ok_or(PaladinAuthError::InvalidState {
            operation: "remove",
