...
//! `paladin-auth add` — add an account interactively, from `--uri`, manual
//! flags, or `--qr`. See docs/DESIGN.md §5 and `docs/IMPLEMENTATION_PLAN_02_CLI.md`
//! "Add modes".
//!
...
//! 2. Build a [`paladin_auth_core::ValidatedAccount`] from the chosen input
//!    mode. `--uri` calls [`paladin_auth_core::parse_otpauth`]; manual flags
//!    and interactive prompts build an
//!    [`paladin_auth_core::AccountInput`] and route it through
//!    [`paladin_auth_core::validate_manual`] so §4.1 validation lives in
//!    core. `--qr` defers to [`paladin_auth_core::import::from_file`] with a
//!    fixed `ImportConflict::Skip` policy and is multi-entry.
//! 3. Open the vault via the shared open pipeline.
//! 4. For single-entry modes, call [`paladin_auth_core::Vault::find_duplicate`]
//!    against the running vault. A collision without `--allow-duplicate`
//!    rejects with [`CliError::DuplicateAccount`] before any save.
//! 5. Mutate via [`paladin_auth_core::Vault::mutate_and_save`] so a
...
use paladin_auth_core::{
    ...
    PaladinAuthError, ValidatedAccount, Vault,
};
...
fn render_single_success(
    ...
    warnings: &[paladin_auth_core::ValidationWarning],
) -> Result<(), CliError> {
    match mode {
        Mode::Json => {
...
fn collect_interactive_input(args: &AddArgs) -> Result<AccountInput, CliError> {
    ...
    let icon_hint = paladin_auth_core::parse_icon_hint_token(&icon_hint_line)?;
...
fn validation_err(field: &'static str, reason: &str) -> CliError {
    CliError::PaladinAuth(PaladinAuthError::ValidationError {
        field,
