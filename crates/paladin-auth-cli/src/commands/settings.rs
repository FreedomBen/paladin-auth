...
//! `paladin-auth settings {get,set}` — read or modify vault settings. See
...
//!    `paladin_auth_core::parse_setting_key` so an unknown key rejects with
...
//!    `paladin_auth_core::parse_setting_patch`, then apply the result through
...
use paladin_auth_core::{parse_setting_key, parse_setting_patch, PaladinAuthError};
