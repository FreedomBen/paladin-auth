...
//! `paladin-auth qr <query>` — render an account's `otpauth://` URI as a QR
...
use paladin_auth_core::{
    format_plaintext_qr_export_warning, write_secret_file_atomic, AccountSummary, PaladinAuthError,
...
/// Same `output_exists` shape that `paladin-auth export` returns. The
