...
//! `paladin-auth edit <query>` — multi-field non-cryptographic metadata
...
//!    flow through `paladin_auth_core::parse_icon_hint_token` so the
//!    `add`-grammar (`empty -> Default`, `none -> Clear`,
//!    `slug -> Slug`) is shared.
//! 5. Run `paladin_auth_core::validate_account_edit(&edit, prior, now)`
...
use paladin_auth_core::{
    ...
    IconHintInput, PaladinAuthError, Vault,
};
...
/// Map argv onto a `paladin_auth_core::AccountEdit`. Issuer / icon-hint
