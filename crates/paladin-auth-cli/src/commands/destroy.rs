...
//! `paladin-auth destroy [--yes]` — permanently delete the vault file (and
...
//!    stdout).
//! 5. Call `paladin_auth_core::destroy_vault(path)` and render the result.

use std::io::Write;

use paladin_auth_core::{format_destroy_warning, PaladinAuthError};
...
pub fn run(global: &GlobalArgs, args: &DestroyArgs) -> Result<(), CliError> {
    ...
    let backup_present = backup_path
        ...
        .map_err(|source| CliError::DestroyIo {
            path: path.clone(),
            source: PaladinAuthError::IoError {
    ...
    let report =
        paladin_auth_core::destroy_vault(&path).map_err(|err| map_destroy_err(&path, err))?;
...
fn map_destroy_err(path: &std::path::Path, err: PaladinAuthError) -> CliError {
    match err {
        PaladinAuthError::VaultMissing => CliError::DestroyVaultMissing {
            path: path.to_path_buf(),
        },
        source @ (PaladinAuthError::IoError { .. } | PaladinAuthError::DestroyIoError { .. }) => {
            CliError::DestroyIo {
                path: path.to_path_buf(),
                source,
            }
...
fn render(
    mode: Mode,
    path: &std::path::Path,
    report: paladin_auth_core::DestroyReport,
) -> Result<(), CliError> {
    match mode {
        Mode::Json => {
