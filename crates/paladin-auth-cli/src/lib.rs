...
//! `paladin-auth-cli` library surface.
//!
...
//!
//! Mirrors the lib+bin layout `paladin-auth-tui` and `paladin-auth-gtk` already
...
/// Return the live clap `Command` for the `paladin-auth` binary.
///
/// Consumed by `xtask::man` (via `clap_mangen`) so `paladin-auth.1`
...
mod tests {
    ...
    #[test]
    fn clap_command_reports_paladin_auth_binary_name() {
        let cmd = clap_command();
        assert_eq!(
            cmd.get_name(),
            "paladin-auth",
            "clap_command() must return the `paladin-auth` binary's Command — \
    ...
    fn clap_command_exposes_top_level_subcommands() {
        ...
        for required in [
            ...
            assert!(
                ...
                "clap_command() must expose `paladin-auth {required}` — got {names:?}",
            );
        }
    }
}
