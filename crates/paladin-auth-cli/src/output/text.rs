...
/// Print the success line for `paladin-auth init` to stdout, e.g.
...
/// One row in the `paladin-auth list` text output. The CLI passes the
...
/// `paladin-auth list` to stdout. An empty `rows` slice writes zero bytes
...
/// One row in the `paladin-auth show` / `paladin-auth peek` text output.
...
/// Print the success line for `paladin-auth add` (single-entry) to stdout,
...
/// Print the `paladin-auth copy` success line to stdout. Mirrors the §5
...
/// Print the success line for `paladin-auth remove` to stdout, e.g.
...
/// Print the success line for `paladin-auth rename` to stdout, e.g.
...
/// Print the success line for `paladin-auth destroy` to stdout. Mirrors the
...
/// Print the success line for `paladin-auth passphrase
...
/// Print the success summary for `paladin-auth add --qr` (multi-entry) to
...
/// Print the success line for `paladin-auth export` to stdout. `format_label`
/// is the §5 stable string (`"otpauth"` for plaintext, `"paladin-auth"` for
...
/// Print the one-line success message for `paladin-auth qr <query> --out
...
/// supplied stream, prefixed with `paladin-auth: warning:`. The CLI calls
...
/// for `paladin-auth settings get` (no key filter) and as the post-mutation
/// summary for `paladin-auth settings set` in text mode.
...
/// `paladin-auth settings get <key>` text-mode output.
...
mod tests {
    ...
    use paladin_auth_core::{parse_otpauth, Account};
    ...
    #[test]
    fn export_success_renders_encrypted_mode_label_for_paladin_auth_format() {
    ...
    #[test]
    fn validation_warning_writes_paladin_auth_warning_prefix_to_stream() {
        ...
        let s = String::from_utf8(buf).expect("utf-8");
        assert!(s.starts_with("paladin-auth: warning: "), "got {s:?}");
        assert!(s.ends_with('\n'));
    }
