...
/// One row in the `paladin-auth list` success envelope. Each row is an
...
/// `paladin-auth list` success envelope: `{ "accounts": [ListAccountRow] }`
...
/// Render the `paladin-auth list` success envelope.
...
/// `paladin-auth add` (single-entry) success envelope:
...
/// Render the `paladin-auth add` (single-entry) success envelope. Used by
...
/// `paladin-auth add --qr` / `paladin-auth import` success envelope:
...
/// Render the `paladin-auth show` / `paladin-auth peek` success envelope:
...
/// `paladin-auth copy` success envelope per the §5 JSON shape table:
...
/// Render the `paladin-auth copy` success envelope.
...
/// `paladin-auth remove` success envelope per the §5 JSON shape table:
...
/// Render the `paladin-auth remove` success envelope.
...
/// Inner object of the `paladin-auth destroy` success envelope. `vault_path`
...
/// `paladin-auth destroy` success envelope per the §5 JSON shape table:
...
/// Render the `paladin-auth destroy` success envelope.
...
/// `paladin-auth rename` success envelope per the §5 JSON shape table:
...
/// Render the `paladin-auth rename` success envelope.
...
/// `paladin-auth edit` success envelope per the §5 JSON shape table:
...
/// Render the `paladin-auth edit` success envelope (committed mutation).
...
/// `paladin-auth edit --dry-run` success envelope per the §5 JSON shape
...
/// Render the `paladin-auth edit --dry-run` envelope.
...
/// Render the `paladin-auth add --qr` / `paladin-auth import` success envelope.
...
/// `paladin-auth export` success envelope per the §5 JSON shape table:
/// `{ "written": "/path/to/out", "format": "otpauth"|"paladin-auth" }`.
/// `format` reflects the exporter that produced the file: `"otpauth"`
/// for plaintext exports (JSON `otpauth://` array) and `"paladin-auth"` for
/// encrypted Paladin Auth bundles.
#[derive(Debug, Serialize)]
...
/// Render the `paladin-auth export` success envelope.
...
/// `paladin-auth qr <query> --out <path>` success envelope per the §5 JSON
...
/// Render the `paladin-auth qr` success envelope (PNG or SVG file write).
...
/// Render the `paladin-auth settings {get,set}` success envelope per the §5
...
mod tests {
    ...
    fn account_list_envelope_flattens_summary_with_code_fields_for_totp() {
        use paladin_auth_core::parse_otpauth;
    ...
    #[test]
    fn export_success_envelope_uses_paladin_auth_format_for_encrypted_bundle() {
        let path = std::path::Path::new("/tmp/bundle.bin");
        let mut buf: Vec<u8> = Vec::new();
        write_export_success(path, "paladin-auth", &mut buf).expect("render");
        let v: serde_json::Value =
            serde_json::from_str(String::from_utf8(buf).unwrap().trim()).unwrap();
        assert_eq!(v["format"], serde_json::json!("paladin-auth"));
    }
