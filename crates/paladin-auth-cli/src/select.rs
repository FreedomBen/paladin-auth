...
//! Thin wrapper around `paladin_auth_core::parse_account_query`,
...
//! `paladin-auth-core` (see `docs/IMPLEMENTATION_PLAN_02_CLI.md` "Query resolution"
...
//! as `CliError::PaladinAuth(PaladinAuthError::ValidationError { field: "query", … })`.
...
use paladin_auth_core::{Account, AccountKindSummary, AccountQuery, Vault};
...
/// `validation_error` returned by [`paladin_auth_core::parse_account_query`].
...
fn matching_accounts<'v>(vault: &'v Vault, query_text: &str) -> Result<Vec<&'v Account>, CliError> {
    let query: AccountQuery = paladin_auth_core::parse_account_query(query_text)?;
...
mod tests {
    ...
    use paladin_auth_core::{parse_otpauth, ErrorKind, PaladinAuthError, Store, Vault, VaultInit};
    ...
    fn make_account(kind: &str, label: &str, issuer: Option<&str>) -> paladin_auth_core::Account {
    ...
    fn malformed_id_prefix_propagates_validation_error_from_core() {
        ...
        match err {
            CliError::PaladinAuth(PaladinAuthError::ValidationError { field, .. }) => {
    ...
    fn malformed_id_prefix_in_resolve_all_propagates_too() {
        ...
        assert!(matches!(err, CliError::PaladinAuth(p) if p.kind() == ErrorKind::ValidationError));
    }
